...
1. **文字列リテラルは拾えない**（`func` などの宣言だけを見るため）。`models released`
   のようなログの文言を取り逃がすのはそのため——**`--literal '<文字列>'` で足す**。
2. **関数の中の `let` / `var` も宣言として拾う**（テキストだけでは型のメンバーと区別できない）。
   ただし「もうコードのどこにも無い名前が Sources のコメントに出ている」ときしか失敗しないので、
   誤検出でも**直す価値のある指摘**にはなる（消えた語を説明したままのコメント）。
3. **`/* */` のブロックコメントは見ない**（このリポジトリは `//` しか使っていない）。
"""
