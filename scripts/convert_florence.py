...
# デコーダは fp16。実機の GPU/ANE fp16 は Mac と食い違うため、ランタイム側で **VLM を CPU_ONLY** に
# 固定して Mac の CPU_ONLY(=検証済みで正しい)と数値を一致させる（fp32 化はメモリ倍増で不採用）。
# encoder_hidden/mask 入力は fp16（VLMVision 出力を直結）。
