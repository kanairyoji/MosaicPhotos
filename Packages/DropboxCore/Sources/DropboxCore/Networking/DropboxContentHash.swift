...
///
/// ⚠️ **置き場所は DropboxCore**（Dropbox のプロトコルの一部であって、バックアップ固有では
/// ないため）。テストの偽 Dropbox（`DropboxTestSupport`）も本物と同じ hash を返す必要があり、
/// BackupKit に置いたままだと共有できなかった。
...
public enum DropboxContentHash {

    public static let blockSize = 4 * 1024 * 1024

    /// content_hash を計算する。`blockSize` はテスト用に注入可能（本番は既定の 4MB）。
    public static func hash(of data: Data, blockSize: Int = DropboxContentHash.blockSize) -> String {
