...
public final class DropboxPhotoStore {
    ...
    private func reflectCachedItems(accountId: String) async -> Int {
        ...
        // それに合流する＝通常は 2 本目を起こさない）。版は増える一方なので、後から始まった反映は
        // 必ず `wanted` 以上を読む＝この輪は実際には 2 周で抜ける。
        // ⚠️ 上限に当たった稀な場合だけ、走っている反映と**重なり得る**（実体化が 1 回余分に
        // 走るだけで、結果は正しい。`HeavyLoad` は同じ札の再入を数で持つので札も壊れない）。
        // 完全に直列化するには鎖を持つ必要があるが、そのための仕掛けを増やす価値は無い
        // ——正しさは版の判定が担保しており、余分な 1 回は間引き（ADR-224）が押さえる。
        var joins = 0
    ...
    /// 合流を試す回数の上限（超えたら自分で反映する）。実際には 2 周で抜けるが、
    /// アクターの割り込み順に頼った終了条件は事故になるので明示的に切る。
