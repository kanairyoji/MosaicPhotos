...
public final class SingleFlightTask {
    ...
    @discardableResult
    public func start(_ body: @escaping () async -> Void) -> Bool {
    ...
    public func restart(_ body: @escaping () async -> Void) {
    ...
    public func coalesce(_ body: @escaping () async -> Void) {
        guard isRunning else { launch(body); return }
        pending = body
    }
    ...
    private func launch(_ body: @escaping () async -> Void) {
        generation &+= 1
        let mine = generation
        isRunning = true
        task = Task { [weak self] in
            ...
            if let next = self.pending {
                self.pending = nil
                self.launch(next)       // 予約を 1 回だけ拾い直す
            } else {
