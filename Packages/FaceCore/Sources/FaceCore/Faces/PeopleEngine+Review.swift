...
extension PeopleEngine {
    ...
    /// 10: ADR-220（平均連結へ入れる品質の線を 0.40 → 0.10。実機では顔の 6 割が入れていなかった）
    /// 11: ADR-221（名前付き人物の重心を作り直す顔の品質の線を 0.40 → 0.20）
    public static let clusterRuleVersion = 11
    ...
    public func rebuildClustersIfNeeded() async {
        ...
        // 結果（0 件も含む）は `FaceStore.absorbFragments` が必ず記録する（ADR-157）。
        await store.absorbFragments()
