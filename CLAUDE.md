...
**MosaicPhotos** は iOS (iPhone) 向けの写真ビューワーアプリ。端末内の写真と Dropbox 上の写真を、ソース別（All / Photos / Cloud）・端末アルバム別・場所（市区町村）別に閲覧できる。外部 SDK・ライブラリは不使用（コードはすべて標準フレームワーク）。AI はオープンソースの学習済みモデル（OpenCLIP / AuraFace）を同梱し、OS 内蔵の Core ML・Vision・Foundation Models で実行する。
...
| パッケージ管理 | Swift Package Manager（ローカルパッケージ 14 個。基盤: `MosaicSupport` / `PhotoSourceKit` / `ImageCacheKit` / **`PerceptionCore`**（ClipMath・PhotoRef・BackgroundTrickle・AnalysisActivity＝CLIP と顔の共通下層）、ローカル写真: `LocalPhotoCore`(ロジック) / `LocalPhotoKit`(UI)、Dropbox: `DropboxCore`(ロジック) / `DropboxKit`(UI)、`BackupKit`、写真機能統合: `PhotosFeatureKit`、**顔認識/ピープル: `FaceCore`**（依存 PerceptionCore・顔クラスタ一式）、自動アルバム/AI: `AutoAlbumCore`（`@_exported import` で PerceptionCore/FaceCore を再エクスポート＝consumer は import AutoAlbumCore のまま）、CLIP ランタイム/AI seam 実装: `MobileCLIPKit`、**ピープルの UI: `PeopleKit`**（人物一覧/人物アルバム/レビュー/顔の付け替え。ロジックは FaceCore）） |
...
| AI | `PeopleKit` | ピープルの UI（人物一覧・人物アルバム・レビュー・顔の整理） |
...
- **AI/自動アルバムは AutoAlbumCore に集約（SwiftUI 非依存）**: 旅行/フォルダ/AI アルバム生成・知覚・検索は `AutoAlbumCore`。`@Model` は `@ModelActor`（`AutoAlbumStore`）の外へ漏らさず Sendable 値（`EnrichedPhoto` 等）に変換して返す。ファサードは `@MainActor @Observable` の `AutoAlbumEngine`。スキーマ変更時に `ModelConfiguration` 名（現行 `"AutoAlbumV10"`）を採番して旧ストアを破棄→再構築するのは**作り直せるキャッシュだけ**。台帳（FacesV1 の人物名・修正、BackupKit）は採番せず optional 列の追加だけで進める（ADR-186）
...
- **モデル更新は索引を少しずつ移す（ADR-186）**: 学習済みモデルを差し替えても DB を丸ごと作り直さない。
  CLIP は `PhotoEmbedding.modelID`（行ごと）で旧行を新しい写真から上書き、顔は同梱モデルの ID
  （face_config.json の `model`）が現行世代と違えば別コンテナ `Faces-<id>` の**影の世代**を育てて
  90% で切り替え（名前は写真の重なりで持ち越し）。台帳（FacesV1・BackupKit）は `.ledger` 方針＋
  `StoreSnapshot`（アプリの版が変わった最初の起動で開く前に控え）。方針の一次記述は
  `PerceptionCore/ModelGeneration.swift`。**モデル更新の手順もそこに書いてある**。
- **ModelContainer は自己修復で構築**: SwiftData の `ModelContainer` 初期化は、ストア破損・スキーマ不整合のとき起動時に trap して実機で原因不明のクラッシュになりやすい。`AutoAlbumStore` / `TagStore` / `FaceStore` / `DropboxCacheStore` / `BackupEngine` は `resilientModelContainer(name:schema:policy:)`（内部は `makeResilientModelContainer`）で「**失敗の理由を見る**→一時的（容量不足・ファイル保護）ならファイルを残しその起動だけインメモリ／作り直せるキャッシュ（`.rebuildable`）ならストアファイル（.store/-wal/-shm）削除して再試行→なお失敗ならインメモリ」とフォールバックし、起動を止めない。台帳（`.ledger`）は開く前に控え（`StoreSnapshot`）を取り、失敗時は控えから戻して 1 回だけ開き直す（ADR-186）。失敗は診断ログへ
...
- **「重い処理を動かしてよいか」は 1 つの表で決める（ADR-196）**: 判定は
  ...
  - ⚠️ **単純化で消してよいのは「状態」であって「選択肢」ではない（ADR-197）**。設定を 1 つ
    足す/戻すコストは**表の行 1 つ**なので、「構造を単純にするために設定を減らす」は成立しない。
    利用者が自分で要望した設定を消すときは、**それだけを取り出して確認する**。
...
- **ピープル（顔クラスタ）＝オンデバイス顔認識**: 写真アプリの「ピープル」は**公開 PhotoKit API でアクセス不可**（旧 subtype-1000 方式は誤りで常に空＝撤去）。代わりに **Vision 顔検出＋同梱顔モデル（AuraFace-v1／ArcFace 系 R100・Apache 2.0・512次元L2正規化。ADR-70 で facenet から換装）で identity 埋め込み→逐次クラスタリング**で自前の「人物」を作る。ロジックは `FaceCore`（`Sources/FaceCore/Faces/`。`FaceClustering`（純・コサイン逐次・テスト）/ `FaceStore`（@ModelActor・**別コンテナ "FacesV1"**）/ `FaceTagger`（背景スキャン）/ `PeopleEngine`（@MainActor @Observable ファサード）/ `PersonInfo`（表示値型）/ 純ロジック各種（下記）/ seam `FacePerceptionProvider`・`DetectedFaceSignal`）。実体（Vision+CoreML）は `MobileCLIPKit`（`FaceModelRuntime`・`FacePerceptionAdapter`）。モデルは `MosaicPhotos/FaceModel/`（**`.gitignore` 対象**・`scripts/build_auraface.sh`＋`convert_auraface.py` で生成・FLOAT16。世代は `face_config.json` の `model`＝現行 `auraface-v1-r100`）。未同梱なら `isFaceModelAvailable==false` でセクション非表示。**端末写真は 1024px（ADR-51・旧640px）・クラウド写真は Dropbox のキャッシュ済みサムネ**で顔検出（追加DL無し）。人物アルバム表示・代表顔アバターもクラウド対応。パイプライン版（`PeopleEngine.faceScanVersion`）を上げると全再スキャンし、**ユーザーの表明（名前・束ね・家族グループの所属）を写真の重なりで持ち越す**（`CarriedAssertion`・ADR-51/232）。精度・アルゴリズムは**データセット計測で決める**（`docs/architecture-note/records/face-accuracy.md`＝台帳・FG-NET/LFW・`FaceAccuracyEvalTests`／`FaceEvalMetrics`）。
  - **検出ゲート（不確かな顔を弾く）**: 検出信頼度<0.8・クロップ再検証（二段検出＝顔中心の切り抜きで再検出できないものを棄却）・ぼけ（ラプラシアン分散）・露出（平均輝度）・絶対80px（ADR-90・旧48px は 2.3 倍拡大で低品質を量産していた）/正規化サイズ比・顔向き（yaw≥30°/roll≥45°）・目閉じで品質を減衰し、品質フロア 0.40 未満はクラスタへ入れない（記録は残す）。しきい値は `FaceQualityGate` に集約（ADR-48/52/53）。偽陽性は計測で 0%。
  - **埋め込みの安定化**: ランドマークで**アライメント**（AuraFace は 5 点整列＝ArcFace の学習形式・`face_config.json` の `alignment`）＋**マルチクロップ平均**（アライメント/水平反転/bbox の3埋め込み）＋EXIF 回転の正規化（ADR-51/54）。
  - **クラスタリング（定数は `FaceTuning` のプロファイル）**: しきい値・マージンは**モデルの類似度分布に張り付く**ので同梱モデルの宣言（`face_config.json` の `tuning`）で選ぶ。現行 `arcface`（AuraFace）は既定しきい値 **0.35**（校正の可動域 0.25〜0.40）＋マージンゲート **0.04**（1位と2位の差が小さい紛らわしい顔は入れない）＋サイズ適応マージン（最大 0.08・小/新クラスタほど合流を厳しく）＋同一写真 cannot-link＋共起 notSame。夜間の再クラスタは**平均連結**（`FaceAgglomeration`・ADR-217：小さな山→近い相手 30 個だけで HAC・種どうし／同じ写真／負例はまとめない。赤ちゃんの顔どうしで撮影日が 3 年超ならまとめない決まり（`BabyRule`）は仕組みだけあり、判別器を同梱していないので無効（ADR-219 撤回）・線は `FaceTuning.agglomeration`）で、命名/確認クラスタは種として固定。クラウドの顔の撮影日は EXIF の撮影日時だけ（ADR-218）。品質の線は 3 つ: 平均連結へ入れる 0.10（ADR-220）・昼の割り当て 0.40（下げない＝名前付き人物に昼の誤りが残る）・名前付き人物の重心の作り直し 0.20（ADR-221）。⚠️ **シミュレータでは OS の顔品質が取れず 1.0 になる**ので、品質が絡む計測は Mac で取った `os-quality.json`（`scripts/os_face_quality.swift`）で実機相当にして行う。FG-NET 全体で B-Cubed F1 **0.790**・分裂 2.8（facenet 最良 0.664/3.7）＝ADR-56〜58・ADR-70。数値の正本は `face-accuracy.md`。
  - **ユーザー修正から学習**（ADR-45/46）: 品質重み＋負例エグゼンプラ（`FaceCorrection` に埋め込みで永続化＝再スキャン・モデル更新を跨いで再発防止）・確認済み顔をアンカー（マルチプロトタイプ）・しきい値のユーザー校正。**レビュー**（「同じ人物ですか？」カード＝`FaceReviewView`）で判断が割れるケースだけ尋ねる。**ピープルに載せるのは 10 枚以上＋名前付き**（ADR-125・表示だけの線で、学習の母数は 3 枚以上のまま）。候補探索の**基準は命名済み人物**に絞る（ADR-123・無名の断片は候補側にだけ出る）＝人物が増えても遅くならない。数秒かかる探索には必ずキャンセルを出す（ADR-124）。
  - **ユーザーが表明したものは種にする**（ADR-130/132/134/231）: 名前・確認顔・代表写真・束ね・
    **家族グループ（`PeopleGroupRecord`）のメンバー**を持つ人物は再クラスタで行を消さない
    （`FaceSeedBuilder`）。⚠️ 無名でもグループに入れていれば守る——グループに入れる行為そのものが
    表明で、名前を付けるのと同じ重み。解決できなかったメンバーは診断ログに出す（無音にしない）。
  - **遷移で表明が消えたら、勝手に記録に残る**（ADR-233/234）: 顔の不具合はほぼ全部
    「利用者が手で作ったものが機械の都合で消える」で、出るのは**遷移のときだけ**
    （夜の再クラスタ・再スキャン・世代切り替え・写真の整理）。定常状態では 1 つも出ないので
    「使っていれば気づく」が成り立たない。`AssertionCensus` が遷移の前後で名前・束ね・
    グループ・確認顔・代表を突き合わせ、**減った／別人に入れ替わった**ら診断ログへ出す
    （何も減っていなくても 1 行は出す＝走ったことが分かる）。
    調べるときは Developer Options で台帳を**控えて／書き出して**、Mac の
    `FaceLedgerReplayTests` で回す（`FACE_LEDGER_DIR`。**写真本体は要らない**——埋め込みは
    台帳の中にある）。書き出しは既定で人物名を仮名・写真のパスをハッシュに置き換える
    （`FaceLedgerRedaction`）。⚠️ ただし**顔の埋め込みは外せない**（再生が使うもの）ので、
    書き出したファイルは名前の有無に関わらず**生体情報**。`.gitignore` が `*-export/` と
    `*.store` を弾く（公開リポジトリなので「気をつける」に頼らない）。
  ...
  - **連写・服装で繋ぐのは撤回済み**（ADR-211/212 → ADR-215）: PIPA（Flickr の個人アルバム・連写と服装を含む）で測ったら、第2パスの後に残る顔はほとんど無く、繋いだ顔の正解率は 0%（第2パスは 92.5%）・B-Cubed F1 も下がった。「埋め込み以外の証拠」を足すなら**先に PIPA で測る**（`scripts/fetch_pipa_eval.py`・ハーネスはコミット 8a7c2d3）。
  - **重心を守る仕掛け**（ADR-210）: 「重心に寄与したか」は**事実の記録**（足した当人だけが書く・品質から推し量らない）。再クラスタの前に `FaceCentroidAudit` で記録と実体を突き合わせる。散らばり（`FaceClusterHealth`）は事後監査の順番に使う。※ 散らばりで重心を凍結する案は FG-NET / LFW で一度も発動せず撤回（ADR-216）。
  - **統合の帯は 1 つの表**（ADR-213）: 自動で寄せる／チェック済みで見せる／尋ねる／無視の目盛りは `MergePolicy` に集約。**人物どうしは、どれだけ似ていても自動で結合しない**（ADR-153）。
