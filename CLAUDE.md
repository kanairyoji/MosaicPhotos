...
MosaicPhotosApp
  └── HomeView  (@State dropboxStore / mergedStore / backupEngine / albumScanner / placeScanner)
        ...
        ├── [People]     ピープル（顔クラスタ）← PeopleEngine（Vision 顔検出＋同梱顔モデルで埋め込み→逐次クラスタリング・端末写真640px＋クラウド256px（サムネAPIサイズ w256h256 に追従））。Time&Place 直下・顔モデル未同梱なら非表示
