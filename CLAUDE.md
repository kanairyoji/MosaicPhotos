...
| オンデバイス AI | 多層構成（ADR-24）: **Vision 画像分類**（OS 内蔵・約1,300クラス・`hasMinimumRecall(forPrecision:)` の校正済み足切り）＋ **OpenCLIP ViT-B-32（DataComp・MIT・INT8量子化＝容量半減/精度ほぼ不変・ADR-31）**（Core ML・画像/テキスト埋め込み。ファイル名 `MobileCLIP*` は互換のため据え置き）＋ **Florence-2-base（Microsoft・MIT・任意同梱）**（写真キャプション＝encoder-decoder・自然文＋OCR も滲む・SmolVLM 比 3〜5倍速・`scripts/build_florence.sh` で生成・ADR-32）。クエリ解釈・翻訳・候補審査は Apple Foundation Models（`FoundationModels`）で、解釈は**作成時 1 回・永続化**（ADR-23）＋防御的サニタイズ＋決定的レキシコン。ロジックは `AutoAlbumCore`、各ランタイム/seam 実装は `MobileCLIPKit` に集約 |
...
MosaicPhotos/                      ← メインアプリターゲット（合成のみの薄い層）
  ...
  VLM/                             Florence-2-base（キャプション・.gitignore 対象・scripts/build_florence.sh で生成）
...
Packages/MobileCLIPKit/            ← CLIP/翻訳ランタイム＋AutoAlbumCore seam のアプリ側実装（AutoAlbumCore / MosaicSupport に依存）
  Sources/MobileCLIPKit/
    ...
    VLMRuntime.swift               Florence-2 実行系（encoder-decoder・エンコーダ1回＋固定長デコーダ貪欲・遅延ロード・ADR-32）
    GPT2Tokenizer.swift            byte-level BPE 復号（vlm_vocab/merges から構築・BART も同一写像）
...
MosaicPhotosApp
  └── HomeView  (@State dropboxStore / mergedStore / backupEngine / albumScanner / placeScanner)
        ...
        ├── [People]     ピープル（顔クラスタ）← PeopleEngine（Vision 顔検出＋同梱顔モデルで埋め込み→逐次クラスタリング・端末写真のみ）。Time&Place 直下・顔モデル未同梱なら非表示
