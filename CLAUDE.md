...
| オンデバイス AI | MobileCLIP-S2（Core ML・画像/テキスト埋め込み）でオープン語彙の意味検索 / 表示タグ。クエリ解釈・翻訳は Apple Foundation Models（`FoundationModels`）。ロジックは `AutoAlbumCore`、CLIP/翻訳ランタイム＋アプリ側 seam 実装は `MobileCLIPKit`（`AutoAlbumCore` に依存）に集約 |
...
- **CLIP 埋め込みのバッチ検索**: 全 `clipVector`（数万件・100MB 超）を一度にロードするとメモリ急増で jetsam の恐れがあるため、AI アルバムの生成/更新は `allEnrichedPhotosLite()`（clipVector なし）＋ `enrichmentVectorPage(offset:limit:)` のページングで `AIAlbumSearcher.search(baseLite:...loadPage:)` にストリームする。純関数版 `search(_ all:)` と選定ロジックは一致させる
...
- **MobileCLIP モデルの扱い**: Core ML モデルと語彙は `MosaicPhotos/MobileCLIP/` に置き **`.gitignore` 対象**（サイズ）。`scripts/build_mobileclip.sh` で生成する。⚠️ 変換は**画像エンコーダを `compute_precision=FLOAT32`**（fp16 はシミュレータで NaN 化）、**mean/std 正規化は入れない**（MobileCLIP の前処理は Resize/CenterCrop/ToTensor のみ）。未同梱でもアプリは動作し、CLIP 機能だけ無効化される。ランタイム（`MobileCLIPRuntime`）は `MobileCLIPKit` にあり `static let shared` で**遅延ロード**（起動を重くしない）。ロード結果は診断ログに残し、`MobileCLIP.modelsBundled` でロードせず同梱判定できる（Developer Options で可視化）。シミュレータは `.cpuOnly`、実機は `.all`
