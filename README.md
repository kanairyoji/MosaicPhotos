...
<p align="center">
  A privacy-first iOS photo viewer that unifies your <b>device library</b> and <b>Dropbox</b> into one experience — built with <b>no third-party SDKs or libraries</b> — all code uses standard Apple frameworks; the AI runs bundled open-source models (OpenCLIP / AuraFace) on Apple's Core ML.
</p>
...
<td align="center" width="50%">
  ...
  <sub>Open any photo for place, date, full EXIF (camera, lens, exposure) and a map — plus detected scene tags, face count, screenshot badge and **text in the photo (OCR)** — all generated on device. Share to other apps from full screen; view and share counts are recorded too.</sub>
</td>
</tr>
<tr>
<td align="center" width="50%">
...
<td align="center" width="50%">
  ...
  <sub>See exactly what the on-device AI has done: per-pass progress (semantic index, scene tags, People), last-run times and an <i>Analyze&nbsp;Now</i> button.</sub>
</td>
<td align="center" width="50%">
...
- **Backup — verified, automatic, family-friendly** — Back up device photos to Dropbox with **content-hash verification**: a photo is only marked “backed up” after Dropbox confirms the uploaded bytes match (name collisions are detected the same way and auto-renamed). Runs **automatically at night** alongside the AI indexing (charging + Wi-Fi + not in use), with a **“Backed up X / Y”** status in Settings and a per-photo badge in the full-screen view. Each device backs up into its **own subfolder** (e.g. `iPhone-3F2A8C`), so a family sharing one Dropbox account never collides. Preserved alongside each photo: albums, favorites, people names, GPS and the local-identifier mapping — everything needed to survive a reinstall or a future device-storage **offload** (a verified, dry-run-first preview screen already ships).
- **Cloud Sharing — share selected photos, analysis included** — Create a shared set from any album, person or people group and the photos are **copied server-side inside Dropbox** into a shared folder (no photo data is re-uploaded, and **your originals and backup are never touched**). If the other side also runs MosaicPhotos, the **AI analysis travels with the photos** (scene tags, CLIP embeddings, faces) so their device never re-analyzes them — shared photos of your child merge straight into the person album they already named (the join key is `content_hash`, which is device-independent). **Receiving, providing and backup are independent settings**: receiving needs nothing but a Dropbox connection (only *providing device photos* requires backup, since sharing copies from the backed-up copy). Who can see the folder is decided in the Dropbox app itself (view-only invites recommended).

- **Background work, battery & data** — Two independent controls keep battery and cellular data in check.
  - **Processing Timing** (Settings → Albums & Search → Album Automation) decides **when the heavy AI work runs** (scene tags, CLIP embeddings, face scanning, album generation). It is not a ladder but a set of independent switches: *Analyze automatically* (on/off) and *Run image analysis gently* (**default on** — nothing runs while you are using the app). Processing **speed** is a separate 4-step setting (Very Gentle / Gentle (default) / Balanced / Fast). Low Power Mode and memory pressure always pause the work.
...
- **Scene tags** — Alongside CLIP, every photo gets **scene tags** from the built-in Vision classifier (~1,300 classes, precision-calibrated with `hasMinimumRecall(forPrecision:)`). Model choices — and removals — are grounded in measured comparisons: VLM captions (SmolVLM-500M) shipped for several versions and were **removed after a coverage ablation measured zero retrieval contribution** (ADR-108, −877 MB). See [`records/model-evaluations.md`](docs/architecture-note/records/model-evaluations.md) and [`records/search-quality.md`](docs/architecture-note/records/search-quality.md).
- **Two-stage album creation** — Creating an AI album shows a **deterministic preview** immediately (lexicon + date + tag matching, no LLM), then the album is **finalized on the spot** (LLM interpretation + paraphrase probes + evidence gate + LLM review) within seconds; vocabulary grounding for broad words is refined in the next background window when the tag-centroid cache is cold. The composer assists input with **library-derived suggestion chips**, a **live grounding preview** and a **hard-condition hit count** — all driven by the same deterministic layers as the real search, so the preview never lies.
...
- **Search** — Hard conditions filter first (`QueryEvaluator`), then three signals are fused with **Reciprocal Rank Fusion**: **tag matches** (discrete, threshold-free), **CLIP contrast** (positive vs. per-exclusion negative embeddings; probes are disabled for exclusion albums so the contrast stays strict), and lexical matches. Exclusion-bearing albums pass an **evidence gate** (scene tags / measured face counts / measured human counts), and finally an **LLM review** (`AlbumVerifier`) keeps/drops each candidate from its evidence line, re-judging unsure cases with majority voting. Re-evaluation is incremental — only newly indexed photos are scored and merged into a persisted score pool. Quality is guarded by a **123-query COCO/Caltech harness** with per-category F1 regression floors (`SearchEvalTests` / `SearchEvalRegressionTests`) that runs the real pipeline.
...
| Minimum OS | iOS 26 |
| Packaging | Swift Package Manager (14 local packages) |

## Privacy & Security

- **No third-party SDKs or libraries** — all code uses standard Apple frameworks. The AI ships bundled open-source model weights (OpenCLIP / AuraFace, licenses below) executed by Apple's Core ML / Vision / Foundation Models.
- **OAuth 2.0 + PKCE** for Dropbox; access/refresh tokens are stored in the **Keychain**, never in plain files.
- **On-device processing** — reverse geocoding, EXIF parsing and all AI (tags, embeddings, faces, LLM) happen locally.
...
bash scripts/build_auraface.sh     # AuraFace-v1 (Apache 2.0) → Core ML (People clustering)
```
...
Third-party assets are listed in-app under **Settings → Licenses** (and in `MosaicPhotos/Settings/Licenses.swift`): the bundled CLIP model is **OpenCLIP ViT-B-32 (DataComp, MIT)**, the CLIP BPE vocabulary / tokenizer (MIT), build tools (coremltools, PyTorch, open_clip, transformers, Pillow, NumPy), and Mermaid (docs). Apple SDKs and SF Symbols are used under Apple's terms.
