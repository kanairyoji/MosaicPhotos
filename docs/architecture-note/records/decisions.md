...
- 決定: 持ち越しの荷物を `CarriedAssertion`（名前・`personGroupID`・所属グループの UUID・
  ...
  - **グループは代表クラスタだけで引かない**（`PersonInfo.clusterIDs`）。グループは
    ...
    **テストが 1 度も走らない**（一度そう書いてコンパイルエラーで気づいた）。
    ...
    （`PeopleGroupSelection.selectable`・レビュー 3 周目）。`people` は「ピープルに載せるか」
    だけの線（ADR-125）なので、メンバーの写真が減ってフロアを割ると一覧から消えて
    **外せなくなる**——見えない・触れないメンバーがグループに居座る。無名のメンバーを
    守るようにしたぶん、この状態は起きやすくなった。「2 人以上」の母数も同じ理由で
    `allPeople`（フロア未満も数える）。
...
- 決定: 「グループの所属も表明として扱う」を、**行が消え得る全ての経路**に通す。
  ...
  - メンバーの集合は**全グループを 1 回読んで**集合で引く（人物ごとに引き直さない・ADR-119）。
