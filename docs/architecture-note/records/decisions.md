...
- 文脈: 版上げ（顔モデル・パイプライン）後の再スキャンでは、旧人物の名前を
  「旧メンバー写真との重なりが最大の新クラスタ」へ戻していた。ここに 2 つの欠陥があった。
  (1) **同名クラスタが既にあるとエントリを `continue` で捨てていた**——「太郎」が 2 人いるのは
  普通で、捨てると 2 人目の名前と旧メンバーの対応が**永久に失われる**（残りにも積まないので
  再試行もされない）。(2) 旧人物ごとの貪欲な最大選択は、**局所的な最良ペアが別の旧人物にとって
  唯一の対応先を奪う**——存在する一対一対応を見逃す。
- 決定: `NameCarryoverMatching`（純ロジック）で**最大マッチング**（Kuhn の増加道法）として解く。
  候補は重なりの多い順に見るので、同数戻せる割り当てが複数あるときは重なりの大きい対を優先する。
  既に名前が付いているクラスタは**候補から外すだけ**にして、エントリ自体は必ず生かす。
  対応先が決まらなかったものは**必ず残りとして返す**（次のスキャンで再評価）。
- 結果: 同名の別人が両方とも命名され、唯一の対応先が奪われなくなる。旧人物は多くて数十なので
...
- 状態: 採用（レビューループの指摘）
- 文脈: ピープルグループ（ADR-113）は **clusterID で人物を指す**。人物を統合すると src の行は
  消えるのに、グループの参照は付け替えていなかったため、`PeopleGroupInfo.resolve` が
  「現存しないメンバー」として黙って落とす＝**家族グループから 1 人消える**。
  さらに取り消し（ADR-136）の控えにグループが入っていないため、付け替えを足すと今度は
  「顔と人物は戻るのにグループは統合後のまま」になる（レビューループが実際にこの退行を作り、
  受け入れ条件の判定で巻き戻された）。
- 決定: 統合時に `remapPeopleGroups(from:to:)` で参照を付け替える（統合先が既にメンバーなら
  重複させず取り除くだけ）。取り消しの控えには**全グループの構成**を入れて書き戻す
  ——グループはユーザーが作る数個〜十数個なので、影響範囲を操作ごとに見極めるより漏れがない。
  控えより後に作られたグループは触らない。
- 結果: 統合しても家族グループから人が消えず、取り消せば構成も戻る。
...
- 文脈: キャッシュが 5 つ（端末サムネ・Dropbox サムネ・Dropbox 本体画像・顔アバター・spool）あり、
  上限がそれぞれ別の場所に別の既定で置かれていた（端末サムネ 500MB→ADR-184 で 10%・Dropbox サムネ
  **50MB**・本体 200MB・アバター無制限）。Dropbox サムネ 50MB は 2,000 枚ぶんで、クラウドの顔スキャンが
  「キャッシュ済みサムネだけ」を使う設計（ADR-88）と噛み合わず、スキャンできる写真もそこに縛られていた。
  「枚数から割り当てを計算する」案は今の枚数のスナップショットで線を引くことになり、写真が増えれば
  引き直し、ユーザーが違えば別の線＝筋が悪い（実フィードバック）。
- 決定: キャッシュごとの上限を**持たない**。予算 B＝端末の総容量 × 10%（設定で 5% / 20% / 固定 GB）を
  全キャッシュが共有し、合計が B を超えたら**価値の低い層から**捨てる:
  第 3 層 本体画像（先読みしただけ → 開いたもの）→ 第 2 層 顔アバター → 第 1 層 サムネ（端末・Dropbox を
  区別せず、最も古く触った方から）。各層に床（本体 10%・派生 2%・サムネ 50%）。安全弁として
  空きが 2GB を切る手前で予算を縮める。spool は一時（転送が終われば消える）なので予算の外。
  実装は `ImageCacheKit` の `CacheBudget`（予算）・`CacheBudgetPlanner`（1 手の決定＝純ロジック）・
  `CacheBudgetCoordinator`（actor・参加者に「いくら捨てるか」だけ配る）。各キャッシュは
  `BudgetedCache`（usage / oldestAccess / evict(bytes:)）で参加し、書き込み後に `noteGrowth()` を
  呼ぶだけ。判定はまとめて（3 秒デバウンス）行う——旧実装は Dropbox が書き込みごとに 6.8 万行を
  fetch して容量判定していた（ADR-119 型）。使用量は起動時に 1 回集計し以後は増減で追う。
  設定は Storage 画面の 1 つに一本化し、各ソースのディスク上限の Picker は撤去
  （旧キーは残置・未使用）。
- 結果: 枚数もユーザーも見ない規則になった。サムネはライブラリの成長に合わせて勝手に増え、
...
## ADR-184 端末写真サムネのディスク上限は「総容量の 10%」を既定にし、設定で変えられるようにする
- 状態: **ADR-185 で置換**（同日。キャッシュごとの上限そのものを廃止）
- 文脈: 実フィードバック「ローカルサムネのディスク上限が少ない。ディスクの 10% はあってよい。変更もできるように。
  メモリの節にあるがメモリではない」。既定は 500MB 固定・選択肢は 2GB まで。1 枚 20〜40KB のサムネで
  500MB は 1.5 万枚ぶんにしかならず、8 万枚のライブラリではグリッドを往復するたびに捨てて作り直していた。
  ディスクは RAM と違って余裕がある。
- 決定: `diskLimitMB` の 0 を Auto＝**端末の総容量の 10%**（500MB〜50GB にクランプ・`ThumbnailDiskBudget`）
  とし既定にする。選択肢は Auto / 1 / 2 / 5 / 10 / 20 GB。旧既定（200/500MB）で保存されていた値は
  起動時に Auto へ引き上げる。設定画面は「サムネイル（メモリ）」「サムネイル（ディスク）」の 2 節に分け、
  ディスクの説明に「使うのは実際に閲覧した分だけ・上限で古いものから削除」を明記。
- 結果: 128GB 端末で約 12.8GB＝30〜60 万枚ぶん＝実質「一度作ったサムネは捨てない」。
  Dropbox 側のサムネ上限（既定 50MB）は据え置き（別途判断）。
- 関連: `ThumbnailDiskBudget.swift` / `ThumbnailCache.swift` / `LocalPhotoSettingsView.swift` /
  `ThumbnailDiskBudgetTests`。
...
- 文脈: 実フィードバック「バックアップは SSD とネットワークしか使わず CPU は限定的。画像解析と
  並行して動かしてよい。現状あまり動いていない」。コードを追うと、動かない理由は 3 つあった。
  (1) 夜間の起動条件が「埋め込みの残作業が無い窓」→「4 回に 1 回」と譲っていた（ADR-72）。
  (2) 「1 回あたり 10 枚」の既定上限が夜間にも当たり、5 分の窓に 10 枚しか上がらない
  ——ADR-175 で 6 万枚を上げ直す状況では何年も終わらない。(3) 窓の期限（約 5 分）で止まる。
  一方、資源の観点では並行して構わない: バックアップは 1 枚読んで上げる trickle で、
  CPU/ANE を使う解析とは重ならない。ADR-72 が譲らせた根拠（メモリのピーク）は、
  当時の一括読み込みの話で、いまの実装には当たらない。
- 決定: (1) 毎窓、解析と**並行して**起こす（見送りを廃止）。(2) 夜間（自動）は**枚数の上限を外す**
  ——夜間には窓の期限という別の上限がある。手動実行の上限は従来どおり（待ち時間を有界にする）。
  (3) 窓の期限での停止は維持（「済み」は hash 検証後にしか付かないので中断は安全＝ADR-40）。
- 結果: 窓のあいだ、解析とバックアップが同時に進む。1 窓で上がる枚数は「窓の長さ ÷ 1 枚の所要」
  （実測 1 枚 1〜2 秒なら 150〜300 枚／窓）。
  ⚠️ **残る限界**: アップロードは前面の `URLSession` なので窓の外には出られない。
  `URLSession` の背景転送に載せれば OS が窓の外でも上げ続けるが、hash 検証つきの台帳更新
  （ADR-40）を fire-and-forget に作り替える必要があり、別途の設計になる。
  並列アップロード（2〜3 本）も、台帳の順序とレート制限の計測をしてから。
- 関連: `HeavyWorkScheduler.swift` / `BackupEngine.swift`（`isNightlyRun`）/
  `NightlyUploadLimitTests` / ADR-40 / ADR-72 / ADR-163 / ADR-175。

## ADR-179 夜間の顔スキャンは「画面が無い」ので UI への譲りをしない／空振りが続いたら畳む
- 状態: 採用
- 文脈: 実フィードバック「ピープルの夜間解析が進まなくなった」。コードを追うと 2 つの欠陥が
  重なっていた。(1) クラウド写真の解析画像取得（`cloudAnalysisImages`）は `BackgroundYield.uiBusy`
  なら**空を返す**。`uiBusy` には `cloudThumbnailBusy`（Dropbox サムネ取得のドレイン中）が含まれ、
  夜間ウィンドウでは画面が無いのに、バックアップ・共有・生成の準備がサムネ取得を走らせて
  これが立ち続ける。残作業はすべてクラウド写真（20,078 枚）なので、**全バッチが空**になる。
  (2) `FaceTagger` は「全件未解析のバッチ」を**進めて次へ**としていたため、画像が一切取れない
  状態が続くと todo 全体を空で歩き切って `finished — scanned=0` で終わり、次の窓でまた同じ。
  ログ上は「動いた」ように見えるのに 1 枚も進まない。
- 決定: (1) UI への譲りは**アプリがアクティブなときだけ**。非アクティブ（夜間）ではメモリ圧迫
  （jetsam の保護）だけを見る。(2) 連続 3 バッチ（48 枚）で 1 枚も解析できなければ**今回は畳む**
  （原因は続いているので窓を無駄にしない）。畳んだことと進捗を診断ログに残す。
- 結果: 夜間の顔スキャンがサムネ取得の有無に左右されなくなる。空振りは最大 48 枚で止まり、
  次の窓に回る。
- 関連: `AutoAlbumAdapters.swift`（`cloudAnalysisImages`）/ `FaceTagger.swift` /
  `FaceTaggerRecordingTests` / ADR-92 / ADR-163。
...
- 文脈: 人物名・アルバム・位置は**実行の最後にまとめて**送っていた。夜間ウィンドウは毎回
  `expired` で終わるので、途中終了すると**それまでの全件**が失われる。しかも写真本体は
  進捗台帳に載って次回の対象から外れるため、**二度と作られない**（端末を消すと復元不能）。
- 決定: 追記ジャーナル（1 行 1 エントリの `.jsonl`）を作り、**アップロード成功のたびに 1 行**
  足す。本体（JSON 全体）に書かないのは、毎回まるごと書き直すため枚数の 2 乗に比例するから。
  順序は「**メタデータを永続化 → 完了記録を保存**」。永続化できなければ完了記録も作らない
  （次回この写真をもう一度通す。実体は Dropbox にあるので再アップロードは起きない）。
  送信が確認できた後にだけジャーナルを消す（本体へ書いてから消す＝順序も逆にしない）。
- 結果: 途中終了しても、それまでに上げた全写真のメタデータが残る。壊れた行は読み飛ばす
  （途中終了はファイル末尾を壊し得るので、1 行の破損で残り全部を失わない）。
- 関連: `PendingMetadataStore.swift` / `BackupRunner.swift` / `PendingMetadataJournalTests`。

## ADR-172 共有の墓標は「不在を確認できるまで」消さない
- 状態: 採用（残課題 P1・ADR-112 の補足）
- 文脈: 発行済みの `copy_batch` は、クライアントが諦めてもサーバー側で走り続ける。単枚の墓標を
  猶予（15 分）で捨てていたため、**その後にジョブが完走すると外したはずの写真が残り続ける**
  ——記録には無いので以後どの反映でも掃除されず、家族には見えたまま。
- 決定: 単枚の墓標は**時間で消さない**。削除を投げたうえで親フォルダを 1 回ずつ見て、
  **実在しないことを確かめられたものだけ**消す。一覧が取れない（通信断・フォルダ消失）ときは
  残して次の反映で再確認する。無限に増えないよう件数の上限（5,000・古い順に破棄）だけ置き、
  そこに達したら記録に残す（確認が延々できていない＝異常事態）。
  フォルダ墓標は従来どおり猶予で消す（フォルダは作り直せば済み、残す害の方が大きい）。
- 結果: 遅れて完走したコピーも次の反映で掃除される。
- 関連: `ShareSyncEngine+Sync.swift` / `ShareSettingsKeys.swift` / `ShareTombstoneRetentionTests`。

## ADR-173 端末写真の削除は、実行の直前にもう一度バイト列で照合する
- 状態: 採用（残課題 P1・wrongDeletion）
- 文脈: オフロード（実削除）は実行前に照合しているが、そこから実際の削除までに
  **台帳の書き込みと OS の確認ダイアログ**が挟まる。ダイアログはユーザーが放置すれば
  何分でも開いたままで、その間に写真が編集・置換されても、クラウド側の同一コピーが消えても、
  最初の照合結果は古いまま。`modificationDate` が変わらない置換もあるので日付では足りない。
- 決定: `deleter.delete` の**直前に**、対象（≤ limit・往復は有界）を**バイト列から**取り直して
  照合し、hash が変わったもの・クラウド側が消えたものを外す。外した分は台帳から戻す
  （消さないものを「オフロード済み」にしない）。
- 結果: 検証後の変更・置換・クラウド側の消失を検出して削除を見送る。
  ⚠️ **残る窓**: 最終照合から `delete` 呼び出しまでの間は原理的に閉じられない
  （PhotoKit に原子的な compare-and-delete が無い）。ただし削除は「最近削除した項目」へ入るので、
  30 日間は復元できる。この 2 点をもって受け入れ可能と判断する。
- 関連: `OffloadService.swift` / `OffloadFinalRecheckTests` / ADR-168。

## ADR-174 画像は「開始時のアカウント」を照合してから保存・配達する
- 状態: 採用（残課題 P2）
- 文脈: 一覧の読み込みには世代＋アカウントの札（`shouldApplyLoad`）があるが、**画像経路には
  無かった**。フル画像・原本の取得は「ダウンロード → デコード」と待ち合わせが複数あり、
  その間にアカウントが切り替わると、旧アカウントの画像が**新しいキャッシュへ書かれる**
  ——同じパスの写真として別人の画像が出て、キャッシュを消すまで直らない。
- 決定: 取得の開始時に札を控え、(1) **キャッシュへ保存する直前**と (2) **返す直前**の両方で
  照合する。合わなければ保存も配達もしない（`shouldDeliverImage`）。
- 結果: 切替を跨いだ画像が混ざらない。⚠️ 同一パスに対する新旧 2 本のダウンロードが並走した場合の
  書き込み順序までは直列化していない（切替時にキャッシュを消しているので窓は狭い）。
  完全な直列化・原子的公開は、実害が観測されたときに検討する。
- 関連: `DropboxPhotoStore.swift` / `DropboxPhotoStore+Images.swift` /
  `DropboxImageDeliveryGuardTests`。
...
- 文脈: 実フィードバック「顔の認識が動いていない気がする」。実機 diagnostics-72 を見ると
  **動いてはいる**が、5 分の BGTask 窓で 280 枚しか進んでいない。内訳は
  「実作業 22 秒（1 枚 80ms）／2.5 秒休止 × 35 バッチ＝87 秒／残りは譲り待ち」。
  譲りの相手は**アルバム生成**で、生成は `isGeneratingAlbums` を立てるため
  `heavyShouldPause()` が真になり解析が止まる。しかも生成は窓内に終わらず毎回
  `generate: aborted`（このログでは中断 3 回・完了 0 回）。窓は「止まった解析＋終わらない生成」で
  空転していた。残りは 20,078 枚（すべてクラウド）で、この速度では数十日かかる。
- 決定: (1) 残作業（顔・埋め込み）があるうちは**アルバム生成を見送る**。ただしバックアップと
  同じく連続見送りの上限（4 回）を置いて順番を回す（生成も飢えさせない）。
  (2) 顔スキャンの既定を「8 枚 / 2.5 秒休止」→「**16 枚 / 0.5 秒休止**」にする。前面では
  スキャンを始めない（`isAppActive` で弾く）ので長い休止の相手は居らず、譲りは
  `shouldPause` が 1 枚ごとに見る。バッチ倍増はクラウドのサムネ往復（1 回 約 870ms）を半減させる。
- 結果: 1 つの窓が「解析を進める窓」か「生成を終わらせる窓」のどちらかになる。
  効果は次の実機ログの `faces: finished — scanned=N` と `bgtask: defer generate n/4` で確認する。
- 関連: `HeavyWorkScheduler.swift` / `FaceTagger.swift` / ADR-72（バックアップの公平性）/
  ADR-107（一枚岩は非アクティブ限定）/ 事例「顔の認識が動いていないように見える」。
...
- 決定: 解析データの `Face` に `n`（名前）を足し、**既定で共有する**。
  ...
  （受信のたびに相手の呼び方へ書き換わるのは事故に近い）、(2) 同じ人物に複数の候補が来たら
  **多数決**（1 枚の外れ値で人物名が決まらないように）、(3) 名前は外部入力なので
  空白を落とし 64 文字で切り詰める。
...
- 決定: (1) 解析データは**内容が同じときだけ実在確認**して、無ければ復元する
  ...
  (3) 共有の反映と受信を**夜間ウィンドウに入れる**（回線ポリシーは各処理の内側が見る）。
- 結果: アプリを開かなくても、共有の内容と Dropbox の実体が一致へ向かう。
- 関連: `ShareSyncEngine+Sync.swift` / `BackupEngine.swift` / `HeavyWorkScheduler.swift` /
  ADR-112 / ADR-163（窓の使い方）。

## ADR-165 「この画面を開いている間、解析する」を正面から用意する
- 状態: 採用
- 文脈: 背景ウィンドウ（`BGProcessingTask`）は **1 日に数回・1 回数分**しか来ない
  （実機 diagnostics-72: 5 分の窓のあと 8 時間半ゼロ）。2 万枚規模の積み残しは、
  自動処理だけでは何日経っても終わらない。従来の逃げ道は Developer Options の
  「重い処理のゲートを常に開く」だったが、**デバッグ用トグルを本来の用途外で使う**形で、
  戻し忘れると通常利用が重くなる。
- 決定: 設定 →「AI Analysis」に **「この画面を開いている間、解析する」** を置く。
  押している間だけ (1) 待機・電源のゲートを開き、(2) 画面を消灯させない
  （`isIdleTimerDisabled`＝消灯すると背景扱いで中断するため）、(3) 顔スキャンと
  背景処理を起こす。**停止・画面を離れる（`onDisappear`）で必ず元に戻す**。
- 結果: 「今すぐ終わらせたい」に正面から応える経路ができ、デバッグトグルの流用が要らなくなる。
  ⚠️ 外すのは**待機・電源のゲートだけ**——発熱（`ThermalGate`）と一括ロード保護（`HeavyLoad`）は
  `heavyShouldPause()` の手前にあるので外れない。「見ているから全力」と「端末を痛めない」を両立させる。
- 関連: `Settings/AIAnalysisStatusView.swift` / ADR-80（4 軸のゲート）/ ADR-163（窓の使い方）。

## ADR-164 取り消しの控えは射影で読む（大きな人物こそ戻したい）
- 状態: 採用（ADR-136 の補足）
- 文脈: 「1 対 1 の確認で間違えることがある。戻す機能がほしい」。機能は既にあるが
  （`FaceReviewView` のツールバー）、実機ログには `undo skipped — too many faces (5017)` が並び、
  **大きな人物の統合だけ控えが取れない＝戻すボタンが出ない**状態だった。
- 決定: 上限の理由は行数ではなく `embedding`（1 枚 約 1KB）の materialize だったので、
  控えの fetch を**射影**（faceID / clusterID / confirmedAt / contributesToCentroid）にし、
  1 手あたりの上限を 5,000 → **50,000** に上げる。無制限にはせず、控え全体の総量にも
  上限（60,000 顔）を置いて古い手から捨てる。
- 結果: 実ライブラリの統合（最大 5 千枚規模）が戻せるようになる。常駐は総量上限で有界。
- 関連: `FaceStore+Undo.swift` / `FaceUndoTests` / ADR-136 / ADR-96（射影で読む）。

## ADR-162 断片吸収のバーは 0.75 のまま。判定不能帯は実機の分布で決める
- 状態: 採用（ADR-154/155 の補足。K=2 の据え置きを再確認）
- 文脈: 実機で自動吸収が 1 件も発火しない（433 断片中 333 がバー 0.75 未満）。
  「バーが厳しすぎるのでは」を確かめるため、FG-NET / LFW でバー×K を掃引した。
- 決定: **バー 0.75・K=2 を据え置く**。根拠は (1) 0.60 未満は FG-NET でコイン投げ（50%）・
  LFW では K>2 の吸収が全滅、(2) 0.65〜0.80 は**両データセットとも発火せず判定不能**。
  判定不能帯を動かすかは、実機の分布（`faces: absorb` の「バー別に寄る数」）で決める。
- 結果: 「機械を大胆にする」方向は、いま手元にあるデータでは支持できない。ご要望
  （1〜2 枚の断片をいちいち確認したくない）に応える筋は**尋ね方を安くする**方——
  まとめて確認の候補下限を下げ、高い帯を既定でチェック済みにする——が次の候補として残る。
- 関連: `face-accuracy.md` 2026-08-31 の項 / `FaceStore+Absorb.swift`（バー別分布のログ）/
  ADR-154 / ADR-155 / ADR-148（ユーザー回答の分布）。
...
- 文脈: 「人物を調べるの似ている人・別の人かもしれない写真で、次の候補も見たい。
  表示数を増やす機能でもページ送りでもよい」。あわせて「まとめて確認でも全体画像を見たい」。
- 決定:
  1. **「さらに表示」で件数を伸ばす**（近傍 +15・間違い候補 +24）。ページ送りにしなかったのは、
     ここが**比べながら見る画面**だから——前の並びが消えず、戻る操作も要らない。
     実装も足し算だけで、次に増やす軸（例: 束ねた人物の候補）にも同じ形で付けられる。
     人物を切り替えたら件数は最初に戻す（前の人の「もっと見る」を持ち越さない）。
  2. **まとめて確認にも「写真全体」トグル**（1 対 1 の確認は ADR-91 で先に入っていた）。
     あわせて全体表示は `.fit` で収める——正方形の枠に `.fill` で入れると左右が切れて、
     「全体を見たい」という目的を果たさない。
- 結果: 候補を掘り下げられる／顔だけでは判断が付かない場面で状況を見て決められる。
  ⚠️ 最初から全件を出さないのは、写真の多い人物で読み込みが重くなるため（`outlierFaces` は
  メンバー全員の埋め込みを触る）。
- 関連: `PersonInspectorView` / `FaceBatchReviewView` / `FaceStore.decisionReport(outlierLimit:)` /
  ADR-91/135/137。

## ADR-155 断片の上限（2 枚）は据え置く——データセットが「上げると誤る」としか言わないため
- 状態: 採用（ADR-154 の上限の根拠づけ）
- 文脈: 「2 枚以下という数字に意味はあるのか。3 枚や 5 枚も自動でよいのでは」。
  当て推量だったので、**バー × 断片の大きさ K** をデータセットで掃引した
  （face-accuracy.md 2026-08-30 3 回目）。
- 測定結果:
  - FG-NET（真の断片が多い）: **どの K でも吸収 0 件**。年齢差が極端でクラスタ対の類似度が
    0.6 を超えず、バー 0.75 では発火しない。
  - LFW（真の断片がほぼ無い）: 発火する帯（0.50〜0.55）で吸収したものは **全部間違い**
    （K≤5 で 5 件・K≤10 で 12 件、正答率 0%）。**K≤2 はどのバーでも一度も誤らなかった**。
- 決定: **上限は 2 枚のまま**。「大きい断片ほど発火し、しかも間違える」という傾向しか
  出ていない以上、推測で上げない。代わりに実機で判断できるよう、
  **「上限だけが理由で見送った数」（tooBig）** を結果と診断ログに出す。
- 結果: 次は実機の数字（tooBig の量と、2 枚以下の吸収が正しかったか）で上限を動かせる。
  ⚠️ **どちらのデータセットも実機を代表していない**——FG-NET は真の断片があるが類似度が低く、
  LFW は類似度は届くが真の断片が無い。実機は「真の断片が高い類似度で存在する」第 3 の状況で、
  だからこそ**手元のデータで測る**（ADR-148）の出番になる。
- 関連: `FaceStore.absorbMaxPhotos` / `FragmentAbsorbResult.skippedTooBig` /
  face-accuracy.md 2026-08-30（3 回目）/ ADR-148/154。

## ADR-154 断片（1〜2 枚）は自動で寄せる。人物どうしは寄せない
- 状態: 採用（`clusterRuleVersion` は据え置き＝夜間の再クラスタ後に走る）
- 文脈: 実フィードバック「1〜2 枚のグループがたくさんあって、4,000 枚の人物と同一人物。
  でも確認画面にはあまり出てこない。1 枚ずつ確認なんてしたくない」。調べると 2 つあった。
  1. **レビューの母数が `minFaces`（3 枚）で切られていた**——断片は候補にすらなっていなかった。
  2. ADR-153 で「自動結合はしない」と決めたが、その判断材料は**人物どうし**の対だった。
- 決定:
  1. **候補側の下限を 1 枚に**する（基準＝アンカー側は従来どおり確立した人物・ADR-123）。
  2. **断片だけは自動で寄せる**（`absorbFragments`）。ADR-153 と矛盾しないのは、
     **失敗の代償が非対称**だから——断片 1〜2 枚が入っても 4,000 枚の重心は動かず、
     間違いは 1 枚外せば直る（ADR-133/137）。確立した人物どうしを機械が混ぜると
     取り返しが付かない（ADR-130 で経験した）。
     条件: 断片は 2 枚以下・無名・アンカーなし・束ねなし／吸収先は確立した人物
     （名前かアンカーがあり 11 枚以上）／近さ `autoAbsorbBar`（arcface 0.75）以上かつ
     **2 位の人物と 0.05 以上の差**（紛らわしければ人に尋ねる）／同一写真・負例・
     「別人」記録があるものは触らない。
  3. 機械の判断は**修正ジャーナルにもアンカーにも残さない**（ADR-152）。
     夜間の再クラスタ後に自動実行し、件数を診断ログへ。手動実行の導線も置く
     （「人物を調べる」→「小さなグループをまとめる」）。
- 結果: 断片が確認画面に出るようになり、明らかな分は機械が寄せる。
  ⚠️ 「自動化するか」は精度ではなく**失敗の代償**で決める（ADR-153 と同じ物差しで、
  結論が逆になった例）。回帰テスト `FragmentAbsorbTests`（寄せる／紛らわしいと寄せない／
  ユーザーが名前を付けた人物は触らない／同一写真は触らない）。
- 関連: `FaceStore+Absorb.swift` / `FaceStore+Review.batchReviewItem` / `FaceTuning.autoAbsorbBar` /
  ADR-123/130/133/152/153。

## ADR-153 人物どうしは自動結合しない。「あらかじめ選んで見せる」までにする
- 状態: 採用（残していた「C: 自動結合バー」の結論）
- 文脈: 「似ている度 80% 以上ならほぼ本人。自動で繋げられないか」を実測で判断した。
  実機 7,710 件の「同じ人」回答と、汚れを分離した「別人」回答（統合拒否の自動記録を
  時間的近接で除いた 10 件）で、バーごとの効果を出した（face-accuracy.md 2026-08-30 2 回目）。
- 決定: **自動結合はしない。** 理由:
  1. ユーザー自身が **0.885 / 0.920 の対を「別人」と答えている**（家族＝似た顔が多いライブラリ
     では当然）。安全なバーが存在しない。
  2. 0.90 まで上げても自動化できるのは yes 対の **2.5%**——得るものが小さい。
  3. 失敗の代償が非対称。人物が丸ごと混ざる事故は、ADR-130 で実際に起きて何日も尾を引いた。
  代わりに **0.85 以上を「あらかじめ選んだ状態」で見せる**（まとめて確認の候補を選択済みにする）。
  この帯の正しさは 98.2%（判断らしい負例だけなら 99.9%）で、ユーザーは**外すだけ**で済む。
- 結果: 一括の作業量は下がり、間違いは**起きる前に見える**。⚠️ 一般則:
  **自動化の可否は「精度」ではなく「失敗の代償」で決める。** 98% は表示の判断には十分でも、
  取り返しの付きにくい変更には足りない。
- 関連: `FaceTuning.autoSuggestBar` / `FaceBatchReviewItem.Candidate.preselected` /
  `FaceBatchReviewView` / face-accuracy.md 2026-08-30（2 回目）/ ADR-130/148/150。

## ADR-152 「写真の事実」と「ユーザーの判断」を同じ引き出しに入れない
- 状態: 採用
- 文脈: ADR-146 で「ユーザーが自分で選んだ統合の拒否では『別人』を学習しない」ようにしたが、
  **まとめて確認からの統合**は対象外だった。実機ログ（2026-08-30）でも、
  1 分間に 10 件の `notSame` が類似度 0.64〜0.79 で入っており（＝一括統合で同一写真の拒否が
  連続した痕跡）、負例と校正の材料が汚れ続けていた。
  同一写真の重なりは**写真の事実**であって、ユーザーが「別人だ」と答えたわけではない。
- 決定: 同一写真で統合できなかった対は `samePhotoBlock` という**別の種類**で記録する。
  - レビューの「もう尋ねない」判定には使う（同じ対を何度も出さない）。
  - **負例（埋め込みの学習）にも校正にも使わない**。
  あわせて「あなたの回答から見た基準」も、分離していないときは**境界を出さない**
  （以前は既定値がそのまま出て、「境界 0.350・取り違え 1,228」と計算結果のように見えていた）。
  分離度（AUC）を画面と要約ログに出す。
- 結果: 修正ジャーナルが「ユーザーが実際に答えたこと」だけになり、自動結合バー（残る C）を
  実測で決められる状態になる。⚠️ 一般則として、**機械が付けた印とユーザーの判断を
  同じ引き出しに入れない**——後から区別できなくなり、学習が静かに歪む。
- 関連: `FaceStore.markSamePhotoBlock` / `loadNegatives` / `FaceStore+Review` /
  `FaceStore+AnswerBasis` / ADR-45/146/148/149。

## ADR-151 アンカーは「種」には使うが「見本」には広げない
- 状態: 採用（`maxSeedPrototypes` 8→1・`anchorsByCluster` の既定 5→1）
- 文脈: ADR-130/132 で確認顔（アンカー）を再クラスタの種にした際、`prototypes`（似ている判定の
  見本）にも積んでいた。FG-NET で**順序（種にする）と見本（プロトタイプ）を分けて**測ったところ、
  見本にすると純度が k=1 で 0.877→0.812、k=5 で **0.509**（純度 0.8 未満の人物が 18→49 人）。
  順序だけなら 0.877→0.87 でほぼ不変。見本が増えるほど「別人への橋」が増える。
- 決定: 見本は **1 枚**（代表写真＝ユーザーが選んだ 1 枚）に絞る。同一性の固定は
  「種の重心をアンカーから作る」「アンカーをそのクラスタにピン留めする」で達成しており、
  見本を増やす必要はない。増分スキャン側（`anchorsByCluster`）も既定 1 枚。
- 結果: 乗っ取り防止（ADR-130）と学習の保持（ADR-132/134）は維持したまま、混入が減る。
  ⚠️ B3 マルチプロトタイプ（ADR-46）は **facenet 時代の測定で採用**したもので、
  arcface では逆に働いていた——**定数も設計もモデルに張り付く**（ADR-70 の教訓の再確認）。
- 関連: `FaceStore+Rebuild.maxSeedPrototypes` / `FaceStore.anchorsByCluster` /
  `face-accuracy.md` 2026-08-30 / ADR-46/70/130/132。

## ADR-150 「同じ人ですか？」と尋ねる下限は、しきい値を下回らせない
- 状態: 採用（arcface の下限 0.25→0.40・式を `max(floor, threshold)` へ）
- 文脈: 従来は「成長で離れた同一人物を拾う」ため、尋ねる下限をしきい値**より下**へ降ろしていた
  （ADR-68 追補2・arcface で 0.25）。FG-NET を本番設定で測ると、その帯の当たり率は **4.3%**
  ——3,200 対を尋ねて当たりは 137 対だけ。実機のユーザー回答でも「同じ人」と答えた対は
  **下位 5% が 0.669**で、下限 0.25〜0.40 の帯にはほぼ存在しなかった。
  レビューの質だけでなく、候補生成の重さ（1 枚 7 秒・ADR-142）にも効いていた。
- 決定: `mergeBandFloor = max(mergeCandidateFloor, threshold)`。arcface の下限は 0.40
  （尋ねる対 468・当たり率 14.3%＝3.3 倍）。校正でしきい値が上がったらそちらに合わせる。
- 結果: 尋ねる数が 1/7、当たり率が 3.3 倍。⚠️ 代償として FG-NET 基準では「拾える分裂」が
  72.9%→35.6% に下がる——**成長で離れた子供の分裂は別経路**（2 階層の束ね＝ADR-61、
  一覧からの明示操作＝ADR-133/147）で拾う設計であることを前提にした判断。
- 関連: `FaceTuning.mergeBandFloor` / `face-accuracy.md` 2026-08-30 / ADR-68 追補2 / ADR-142。

## ADR-149 校正は「同じ物差し」で、「分離するとき」だけ
- 状態: 採用
- 文脈: 実機 9,973 件の実測で 2 つの欠陥が確定した。
  1. **尺度の混在**: 校正しているのは「顔が人物に入る」しきい値（顔×重心）なのに、
     人物ペアの回答（重心×重心＝merge/notSame）まで正例・負例に混ぜていた。
  2. **分離しないデータでの最適化**: 顔レベルの正例（confirm・中央値 0.713）と
     負例（reassign・0.716）は **AUC 0.472＝コイン投げ以下**。そこから「最もよく分ける境界」を
     探すと、件数の多い負例（正例の 5.5 倍）を全部落とす端（0.974）が最適解になり、
     可動域の上限 0.40 に張り付く。**ユーザーが直すほど厳しくなる**の正体はこれ。
- 決定: (1) 校正の入力は**顔の尺度だけ**（confirm/sameGroup と reassign）。人物ペアの数字は
  「あなたの回答から見た基準」（ADR-148）で別に見る。(2) 分離度（AUC・重み付き）を測り、
  **0.60 未満なら校正しない**（プロファイル既定のまま）。
- 結果: 校正が上限に張り付く問題が根本から消える。ADR-141（確立した人物には引き上げ分を
  課さない）は保険として残す。⚠️ `reassign` の similarity は「**誤って入っていたクラスタとの**
  類似度」で、高くて当たり前——「この近さなら別人」の証拠ではない。
  **記録した数字が何の証拠になるかを、使う前に確かめる。**
- 関連: `FaceCalibration.separability` / `FaceStore.appendCalibrationSample` /
  `face-accuracy.md` 2026-08-30 / ADR-46/68 追補6/70/141/148。

## ADR-148 しきい値は「自分のデータ」で決める（Step 0/1）
- 状態: 採用（Step 0＝指標の整備／Step 1＝ユーザー自身の回答から基準を読む。Step 2/3 は未着手）
- 文脈: 「似ている度 80% 以上ならほぼ本人、70% でも本人のことが多い。基準は厳しすぎない？
  感覚で決めたくないので、データセットで適正値を決める方法論を」。
  ⚠️ 前提の整理: **人物どうしは自動では結合しない**（0.25 以上を「尋ねる候補」に出すだけ）。
  顔→人物は 0.35（校正 0.25〜0.40）＋サイズ上乗せ（最大 0.08）＋マージンゲート 0.04。
  そして画面の「似ている度」は**重心どうし**なので、顔どうしより高く出る——
  「80% なのに一緒にならない」は、しきい値が 0.80 だからではなく**結合を自動でやらない設計**だから。
- 決定:
  - **Step 0（指標）**: `ClusteringScore` に**人物ごとの分布**（純度・再現率の最悪値と下位 25%、
    純度 0.8 未満の人数）を足す。⚠️ 平均は小さいアルバムの被害を隠す——ADR-126 はそれで
    採用してしまい撤回した。以後、平均と一緒に最悪値を見る。
  - **Step 1（自分のデータ）**: 修正ジャーナルには「同じ人」（merge/confirm）と「別人」
    （notSame/reassign）の**類似度が既に記録されている**。これを種類ごと（人物どうし／顔と人物＝
    尺度が違うので混ぜない）に集計し、分布・中央値・ヒストグラム・**最もよく分ける境界**と、
    その境界での取りこぼし／取り違えを出す画面を作る（`AnswerBasisView`）。
    十分な回答数（8 件）が無ければ境界は出さない。
- 結果: 「厳しすぎる気がする」を、自分の写真での実測と突き合わせて判断できる。
  次（Step 2/3）は FG-NET/LFW での掃引（顔しきい値・サイズ上乗せ・**クラスタ結合バー**）と、
  「確認済み k 枚を模擬してアンカー化 → 確立した人物に緩める効果」の測定。
- 関連: `FaceEvalMetrics.ClusteringScore` / `FaceStore+AnswerBasis.swift` / `AnswerBasisView` /
  `face-accuracy.md` / ADR-46/126/141。

## ADR-147 「人物を調べる」はピープルの導線に置く（設定ではない）
- 状態: 採用（Developer Options の「クラスタリングの内訳」＝ADR-135 を格上げ）
- 文脈: 「内訳が便利なのでユーザー向けに格上げしたい。設定トップに『ピープルアルバム』を
  足すか、アルバムの自動処理か？」。**どちらでもない**——これは設定（好みの永続的な選択）ではなく、
  **特定の人物に対する調査と修正**。ユーザーが「この人、誰と混ざってる？」と思うのは
  その人物を見ているときで、動線はそこにある。
- 決定:
  1. 画面を `PeopleKit` へ移し（`PersonInspectorView`）、**人物メニュー**（ホームの人物カード長押し・
     人物アルバムの「…」）の「この人物を整理…」の隣に「この人物を調べる…」を置く。
     ピープル全体一覧のツールバーからも開ける（人物を選ぶところから始めたいとき）。
     Developer Options のリンクは残す（同じ画面・維持コストゼロ）。
  2. **数字は残し、言葉をユーザー語にする**。cos 0.412 →「似ている度 41%」、
     実効しきい値 →「必要 40%」、種 →「あなたの指定どおり保たれる」、
     負例 →「あなたが『別人』と直した」。判定の結論も
     「一緒にできる／似ている度が足りない／相手の写真が少なく判断できない／別の人と紛らわしい／
     同じ写真に一緒に写っている」と、読んで分かる形に。
  3. 国際化（`L(...)`・英語 base ＋日本語）。⚠️ 補間の中で関数を呼ばない
     （`\(percent(x))`）——カタログのキーは書式指定子に落ちるので、入れ子の括弧があると
     照合できない。カタログ検査のテスト側も括弧の深さを数えるよう直した。
- 結果: 開発者向けの内訳が、そのままユーザーの「なぜ？」に答える画面になった。
  トレードオフ: 用語を平易にしたぶん、開発時に見たい生の値（マージン幅・重心寄与数）は落とした
  ——必要になれば「詳細」トグルで戻せる。
- 関連: `PeopleKit/PersonInspectorView.swift` / `PeopleActions` / `AllPeopleView` /
  `DeveloperSettingsView` / ADR-135/137/145/146 / ADR-17（i18n）。

## ADR-146 「同じ写真に写っている」で統合できないとき、その写真を見せて直せるようにする
- 状態: 採用
- 文脈: 実フィードバック「近傍に同一人物を見つけて統合しようとしたら『同じ写真に一緒に写っている』
  と言われて統合できない。**どれが問題の写真か分からない**ので調整できない」。
  同一写真 cannot-link は物理的な制約（1 枚に同じ人は 1 回しか写れない）だが、
  **重複検出・写真の中の写真・鏡**では破れる。破れたとき、ユーザーには
  「できない」としか伝わらず、しかも拒否のたびに**「別人」を学習していた**
  ——直しても二度と統合できなくなる、逆向きの動きだった。
- 決定:
  1. **どの写真のどの顔がぶつかっているか**を返す（`samePhotoConflicts(between:and:)`）。
     診断画面の顔一覧の先頭に「重なっている写真」として、写真全体＋両方の顔を並べ、
     その場で片方を外せる（外した顔は新しい人物になる＝重なりが消えて統合できる）。
  2. **ユーザーが自分で選んだ統合では、拒否しても「別人」を学習しない**
     （`recordNotSameOnConflict: false`）。自動サジェストからの拒否は従来どおり記録する。
- 結果: 「できない」で終わらず、原因を見て直せる。⚠️ 制約が破れる例外があるなら、
  **例外を直す道**まで用意しないと、制約は user が越えられない壁になる。
  回帰テスト `SamePhotoConflictTests`（重なりの提示・外せば統合できる・拒否で学習しない）。
- 関連: `FaceStore.samePhotoConflicts` / `FaceStore.mergeClusters` / `FaceClusterInspectorView` /
  ADR-58（同一写真 cannot-link）/ ADR-135/137。
