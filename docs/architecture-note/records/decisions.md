# 設計判断の記録（ADR）— マスター

> このファイルが**設計判断の正本（マスター）**です。HTML（`docs/architecture-note/design-decisions/adr.html`）は、ここから必要なものを選んで記載した派生物であり、全件を転記するとは限りません。
>
> **運用ルール**
> - 設計上の判断をしたら、必ずこのファイルに 1 項追記する（網羅）。
> - HTML へ載せるかは別途指示で決める（取捨選択）。
> - フォーマット: `## ADR-N タイトル` ＋ **文脈 / 決定 / 結果（トレードオフ）**。番号は連番。
> - 撤回・変更時は項を消さず「状態: 置換（→ ADR-M）」のように追記して経緯を残す。

## テンプレート

```
## ADR-N タイトル
- 状態: 採用 / 置換(→ADR-M) / 廃止
- 文脈: なぜ判断が必要か。
- 決定: 何を選んだか。
- 結果: 得たもの／トレードオフ。
- 関連: コミット・ファイル・関連 ADR/事例。
```

---

## ADR-234 実機の台帳を控えて書き出し、Mac で遷移を回す
- 状態: 採用
- 文脈: ADR-233 で「壊れたら分かる」ようにしたが、**分かったあとに調べる手段**が無い。
  遷移を実機で 1 回試すには `reset()` が要り、10 万枚の再解析に数晩かかる。しかも本物の
  名前と家族グループを賭けることになるので、現実には押せない。
  ⚠️ 今日 `device-verification.md` に書いた F4 / F6 / F7 は、**この端末では実行できない**
  項目だった（数晩＋本物のデータを賭ける／モデル生成のやり直し）。
- 決定: 2 つ足す。**専用のテストアプリは作らない**——遷移の不具合は「10 万枚・1,300 人・
  8 年分の履歴・本物の名前とグループ」でしか出ないので、別アプリ＝別ライブラリにすると
  再現したい条件そのものを捨てることになる。加えて二重の実装は「テストアプリでは通るが
  本番は違う」を生む（ADR-196 の 11 述語・3 か所の前口上と同じ形）。
  - **手で控える／戻す**（`FaceLedgerBackup.takeManualSnapshot` / `restoreManualSnapshot`）。
    仕組みは ADR-186 の `StoreSnapshot` をそのまま使い、口だけを Developer Options に出す。
    ⚠️ 控えの名前は `<コンテナ名>-manual` にする——自動の控えと同じ名前だと
    「アプリの版が変わった最初の起動」で**手で取った控えが上書きされる**。
    ⚠️ 戻しても**開いているコンテナには効かない**（SwiftData がストアを掴んでいる）。
    画面に「要・再起動」を出す。
  - **書き出して Mac で回す**（`FaceLedgerBackup.exportForReplay` ＋ `FaceLedgerReplayTests`）。
    ⚠️ **写真本体は要らない**——顔の埋め込みは台帳の中にあるので、台帳ファイルだけで
    本物の規模・分布・名前・束ね・家族グループに対して `rebuildClusters` /
    `reapplyAssertions` / `pruneMissingPhotos` を何度でも回せる。**コピーに対して**やるので
    実機は壊れない。正解ラベルは無いが、見るのは精度ではなく**不変条件**（ADR-233）。
    - 再スキャンの再現は `facesAsScanInput()` で台帳の顔を**スキャンの入力の形に戻して**
      `recordScans` へ流す。顔検出をやり直さないので数秒で終わり、同じベクトルを入れるので
      精度は 1 ビットも変わらない——**見たい「持ち越しの論理」だけ**が動く。
    - 台帳が無ければ静かにスキップ（CI では常にスキップ）。置き場は `FACE_LEDGER_DIR`
      （顔の評価データセットの `FACE_EVAL_DIR` と同じ作法）。
  - スキーマは `FaceStore.ledgerSchema` に 1 つ置く（書き写すと、列を足したときに片方だけ
    古くなって開けなくなる）。
  - **書き出す時点で読める個人情報を外す**（`FaceLedgerRedaction`・既定 ON）。
    ⚠️ 相談で出た「テストの実行後に人物名を捨てればいい」は **2 つの点で成り立たない**:
    1. **後では遅い**。テストのあとで消しても、ファイルは既に本名を持って端末を出ている。
       外すのは書き出す時点でなければ意味がない。
    2. ⚠️⚠️ **名前は一番危ないものではない**。台帳の本体は**顔の 512 次元 identity 埋め込み**
       （生体テンプレート）で、これは**再生が使うものなので消せない**。ほかに
       `torsoEmbedding`・`FaceCorrection` の埋め込み・撮影日も入る。
       つまり書き出したファイルは**名前の有無に関わらず生体情報**——名前を外しても
       扱いの注意は変わらない。
    外せるのは「再生に要らないのに読める」ぶんだけ:
    - **人物名・グループ名** → 安定した仮名。再生が名前の**文字列**を要る場所は無い
      （`NameCarryoverMatching` は値を見ず、`AssertionCensus` は「名前が在るか」だけ見る）。
    - **写真のキー（refKey）** → 種別の接頭辞（"L-"/"C-"）を残したハッシュ。
      クラウドのキーは `C-/家族/2019/沖縄旅行/IMG_1234.jpg` のような**フォルダ名を含むパス**で、
      家族構成・行った場所・時期がそのまま読める。再生はキーを同一性の目印にしか使わないので
      置き換えても結果は 1 ビットも変わらない。
      ⚠️ `faceID` は `"<refKey>#<連番>"` なので**同じ置き換えで作り直す**（片方だけ変えると
      パスが faceID 側から漏れ、`coverFaceID` の参照も壊れる）。
      ⚠️ **接頭辞は残す**（ローカル/クラウドで分岐する処理が動かなくなる）。
      ⚠️ **塩を混ぜる**——`/家族/2019/…` のような当てやすいパスは、塩なしのハッシュなら
      総当たりで逆算できる。書き出しごとにランダムな塩にする。
    ⚠️ 検証は**バイト列**で行う（`FaceLedgerRedactionTests`）。SwiftData 越しに見るだけでは、
    別の列や WAL に残っていても気づけない。
  - **うっかりコミットを構造的に防ぐ**。リポジトリは公開なので「気をつける」に頼らず、
    `.gitignore` で `*-export/` と `*.store`（+`-wal`/`-shm`）を弾く。
    ⚠️ 実際に一番現実的な事故は「共有先を間違える」ではなく**公開リポジトリへのコミット**。
- 結果: 遷移の実験が「数晩・本物を賭ける」から「数秒・コピー」になる。F4/F6 は
  「Mac で確かめる」に置き換えられ、実機に残るのは**発火と規模**（BGTask が本当に起きるか、
  OS の顔品質、10 万枚のメモリと時間）だけになる。
  ⚠️ トレードオフ: 書き出したファイルは**外しても顔の特徴量が入ったまま**（生体情報）。
  `.gitignore` でコミットは防げるが、共有先は利用者が選ぶ。使い終わったら消す。
  アプリ内の控えはバックアップ対象外（`StoreSnapshot` が `isExcludedFromBackup` を立てる）。
  仮名にすると診断の出力が読みにくくなる（「太郎」ではなく「Person-a3f1」）ので、
  特定の人物を追うときだけ**本名のまま書き出す**口も残した（既定は外す側）。
  書き出しは開いている台帳をそのままコピーするので、**書き込みが静かなとき**に取るのが確実。
- 関連: `FaceLedgerBackup.swift`（新規）/ `FaceLedgerReplayTests.swift`（新規）/
  `DeveloperSettingsView.faceLedgerSection` / `FaceStore.ledgerSchema`。ADR-186/233。

## ADR-233 表明の国勢調査——遷移の前後で「利用者が作ったもの」を数える
- 状態: 採用（ADR-144 を名前の枚数から**表明の全部**に広げる）
- 文脈: 顔まわりの不具合は、ほぼ全部が同じ形で出る——**利用者が手で作ったもの（名前・束ね・
  家族グループ・確認顔・代表写真）が、機械の都合で黙って消える**。そして消えるのは
  **遷移のとき**だけ（夜の再クラスタ・版上げの再スキャン・世代の切り替え・写真の整理）。
  ⚠️ **定常状態では 1 つも出ない**。つまり「使っていれば気づく」が成り立たない。
  実フィードバック: ライブラリの解析が終わって新しい写真が流れなくなり、子供が大きくなって
  誤認識も出なくなると、**遷移を通る機会そのものが年に数回**になる——そのとき壊れていても
  誰も見ていない。能動的なデバッグが成立しない状況に入った。
- 決定: 「調べに行く」のをやめ、**遷移の前後を毎回突き合わせて、減ったら記録に残す**。
  - `AssertionCensus`（純ロジック）が名前・束ねの札・代表写真・確認顔・家族グループの
    メンバーを数え、`diff(before:after:)` が**減った／入れ替わった**ことだけを返す。
  - ⚠️ **突き合わせの鍵は写真（refKey）**。clusterID は遷移で振り直されるので、ID で
    突き合わせると正常な再クラスタでも「全員消えた」になり、警告が意味を失う。
  - ⚠️⚠️ **数だけでは足りない**。「別人が居座る」は数が減らない（メンバー数は 2 のまま、
    中身が別人）。実際にその不具合を踏んだので、**同じ ID が同じ人を指しているか**＝
    メンバーの写真が入れ替わっていないかも見る（`groupMemberSwapped`）。
  - ⚠️ **増えたことは報告しない**。再スキャンで人物が増えるのは正常で、報告すると本当の
    損失が埋もれる（ADR-231 で同じ行が毎分 30 回出て 256KB の記録を押し流した）。
  - ⚠️ **何も減っていなくても 1 行は出す**。「出ていない」と「そもそも走っていない」を
    区別できないログは後から役に立たない（ADR-207 で同じ失敗をしている）。
  - 仕掛ける場所は 4 つ: `rebuildClusters`（夜）・`pruneMissingPhotos`（写真の整理）・
    版上げ/手動の再スキャン（持ち越しが全部消化できたところで）・`promoteShadow`（世代）。
    再スキャンは**持ち越しが残っている間は報告しない**（「まだ戻っていない」を「失った」と
    言ってしまう）。
  - 数え上げは**射影クエリ 1 回**（1,300 人なら 1,300 往復になっていた形・ADR-119）。
    回数は `AssertionCensusTests` が数えている。
- 結果: 遷移で表明が失われたら、能動的にデバッグしなくても診断ログに残る。
  今まで踏んだ顔まわりの不具合のうち **7 件はこれで検出できたはず**（残る 1 件＝別人の
  居座りも `groupMemberSwapped` で拾える）。トレードオフ: 遷移のたびに全顔の射影 1 回
  （8 万顔で数百 ms）が増える。夜と手動操作の経路だけなので、前面の体感には出ない。
- 関連: `AssertionCensus.swift`（新規・純）/ `AssertionCensusTests.swift`（新規）/
  `FaceStore+Rebuild` / `FaceStore+Prune` / `PeopleEngine` / `PeopleEngine+Generation`。
  ADR-119/144/207/231/232/234。

## ADR-232 全消去して作り直すときは、名前だけでなく「表明」を持ち越す
- 状態: 採用（ADR-51/169 の持ち越しを、名前以外にも広げる）
- 文脈: 顔パイプラインの版を上げると全再スキャンになる（`migrateScanVersionIfNeeded`）。
  ADR-51 は「**写真の重なり**で旧人物と新人物を突き合わせて名前を戻す」仕組みを入れたが、
  持ち越していたのは **`name` と `memberRefKeys` だけ**だった。同じ重みの表明が 2 つ落ちていた:
  - **束ね**（ADR-61 の `personGroupID`）——成長で複数クラスタに分かれた子供を 1 人にまとめた指定。
    再スキャン後はほどけ、一覧に同じ子が何人も並ぶ。
  - **ピープルグループの所属**（家族・チーム）——グループは clusterID で人物を指すので、
    `store.reset()` が `PersonCluster` を消した時点で**指し先が二度と解決しない**
    （`PeopleGroupRecord` は残る＝記録はあるのに中身が空）。
  さらに 3 つの穴があった: (a) 控えの対象が「名前付き」に絞られていたので**無名のまま
  グループに入れた人物はそもそも控えられない**。(b) `reset()` が古い clusterID を
  グループに残すので、再スキャンで**同じ番号を割り当てられた別人が家族に居座る**
  （「消える」より悪い）。(c) 手動の再スキャン（`PeopleEngine.reset`）は控えを取っておらず、
  版上げの経路だけが守られていた。
- 決定: 持ち越しの荷物を `CarriedAssertion`（名前・`personGroupID`・所属グループの UUID・
  `memberRefKeys`）にし、**照合は名前と同じ「写真の重なり」1 本**にする。
  - 対象は「名前・束ね・グループ所属のどれかがある人物」＝**無名でも控える**
    （`assertedClusterEntries`）。
  - `reset()` は**グループのメンバーを空にする**（ID は意味を失うので残せない・行は消さない）。
  - グループのメンバーは**足すだけ**にする（`remapPeopleGroupMembers`）。再スキャンは数晩に
    分かれるので、毎回上書きすると前の晩に戻した人が消える。生きていない ID は落とすが、
    対応するエントリは「残り」として持ち越しに積まれたままなので後の晩に戻ってくる。
  - 束ねの札は**負の側**を持ち越し専用にし、**いま台帳に在る札を見て空きを取る**
    （`carriedBundleTags(for:usedTags:)`）。`linkClusters` は新しい札に「束ねたクラスタ ID の
    最小値」＝ 0 以上を使うので、旧札をそのまま持ち込むと**無関係な束ねと同じ値になり得る**。
    ⚠️ **`-(old + 1)` のような式では足りない**（レビュー 1 周目で見つけた）。式は「新しい札と
    ぶつからない」だけを保証し、**2 回持ち越すと壊れる**——世代 1 で札 3 を -4 にしたあと、
    利用者の新しい束ねの最小クラスタ ID がたまたま 3 なら札は 3 になり、世代 2 では
    -4（そのまま）と 3（→ -4）が同じ値になって**別人が 1 人に融合する**。
    値から値への写像では「旧世代の番号と新世代の番号が同じ空間に住んでいる」ことを直せない。
    割り当ては**復元時**に行い、決めた札は残りのエントリへ**書き戻して**返す
    ——書き戻さないと次の晩にもう一度空きを取りに行って別の札になり、同じ子が 2 人に割れる。
  - モデル世代の切り替え（`promoteShadow`）は、**グループの器（id・名前）だけ**を先に新コンテナへ
    写し（`importPeopleGroupShell`）、メンバーは同じ持ち越しに埋めさせる。
    以前は「名前で結び直す」実装で、無名のメンバーは落ち、同名の別人は混ざっていた。
    ⚠️ ここも**ディスクの戻り待ちを踏み潰さない**（レビュー 1 周目）。戻り待ちは「どちらの
    ストアにも居ない人」なので新しいスナップショットには入らない。上書きすると——特に
    空なら**ファイルごと消える**ので——版上げの途中で新しいモデルが来た瞬間に、戻り待ちの
    名前・束ね・グループ所属が全部消えて二度と戻らない。`merged` を通す。
  - 手動の再スキャン（`PeopleEngine.reset(includingCorrections:)`）も控えを取る。消すのは
    学習（負例・確認）の側で、ユーザーが付けた名前やグループはそれとは別のもの。
  - ディスクの控えは**旧形式（`name`/`memberRefKeys` だけ）も読める**（読めなくなると
    持ち越し待ちの名前が丸ごと消える）。
  - **控えを踏み潰さない**。控えに残っているのは「まだ戻せていない人」＝ストアに居ないので
    新しいスナップショットには入らない。上書きすると、再スキャンの途中でもう一度やり直した
    ときに戻り待ちが丸ごと消える。戻り待ちを先に置いて足す（重複は落とす・上限 1,000 件）。
    ⚠️ 重複判定は `memberRefKeys` を**並べてから**比べる——元が `Set` なので、
    アプリを開き直すと同じ写真の集合でも並びが変わる（Swift の Set の走査順はプロセスごと）。
  - **グループは代表クラスタだけで引かない**（`PersonInfo.clusterIDs`）。グループは
    代表 clusterID を持つが、代表は「名前つき → 写真の多い順 → ID 昇順」で**そのとき決まる**
    ので、束ねの中で入れ替わる。入れ替わった瞬間にその人がグループから消えていた
    （持ち越しが戻した ID が代表になるとは限らないので、この直しは持ち越しの前提でもある）。
    ⚠️ **編集画面も同じ見方をする**（`PeopleGroupSelection`・レビュー 1 周目）。表示だけ直すと、
    アルバムには出ているのに編集シートではチェックが付かず、「直そう」として押すと
    **同じ人物が 2 回記録に入る**（表示は重複排除で隠れるが、共有の写真キーは同じ束ねを
    2 度展開する）。「2 人以上」の判定も ID の数ではなく**人物の数**で見る。
    ⚠️ この判定は **UI ではなくロジック層（FaceCore）に置く**。PeopleKit は
    `#if canImport(UIKit)` の内側なので、UI 層に置くと macOS の `swift test` から見えず
    **テストが 1 度も走らない**（一度そう書いてコンパイルエラーで気づいた）。
    ⚠️ **既にメンバーの人は、表示フロアを下回っていても編集画面に出す**
    （`PeopleGroupSelection.selectable`・レビュー 3 周目）。基準は**シートを開いた時点の
    メンバー**で、いまのチェック状態ではない——チェック状態で絞ると、隠れていたメンバーを
    外した瞬間に行が消えて戻せなくなる（名前も打ち直していたら全部捨てるしかない）。
    ⚠️ 「2 人以上」は `memberCount`＝**解決できない記録上の ID も 1 人と数える**
    （レビュー 4 周目）。数えないと、ライブラリが変わった瞬間に**保存ボタンが永久に灰色**に
    なり、グループの名前も変えられず、居なくなったメンバーも外せなくなる。
    ⚠️ さらに **「2 人以上」はメンバーを変えるときの決まり**で、名前だけ直すのは人数を問わない
    （`allowsSave`・レビュー 5 周目）。`reset()` はメンバーを空にし、世代切り替えは空の器を
    作るので、問うと**再スキャンの数晩ずっと改名できない**＝同じ症状を別の入口から作る。
    名前だけのときは `updatePeopleGroup(memberClusterIDs: nil)` で渡さない
    （渡すとエンジン側の「2 人以上」ガードに当たって黙って捨てられる）。
- 結果: 版上げ・手動再スキャン・世代切り替えのどれを通っても、名前・束ね・家族グループが
  写真の重なりで戻る。トレードオフ: 数晩に分かれる間、家族グループは**一部のメンバーだけ**の
  状態で表示される（空より良いが、完全ではない）。「学習もリセット」でも名前とグループは残るので、
  真に空の状態から始めたい開発者はアプリを入れ直す必要がある。
- 関連: `CarriedAssertion.swift`（新規・純）/ `FaceStore+Rebuild.swift`
  （`assertedClusterEntries` / `reapplyAssertions` / `remapPeopleGroupMembers` / `reset`）/
  `PeopleGroups.swift`（`importPeopleGroupShell`）/ `PeopleEngine.swift`
  （`snapshotAssertionsForRescan`）/ `PeopleEngine+Generation.swift`（`promoteShadow`）/
  `NameCarryoverTests.swift`。ADR-51/61/130/132/134/169/186/231。

## ADR-231 ピープルグループの所属も「ユーザーの表明」として種にする
- 状態: 採用（ADR-132/134 の適用範囲を広げる）
- 文脈: 再クラスタで種になるのは「名前・確認顔・代表写真・束ね（`personGroupID`）」を持つ
  クラスタだけで、**ユーザーが作った家族グループのメンバーであること**は条件に入っていなかった
  （`PeopleGroupRecord` の `memberClusterIDs` と `personGroupID` は別概念）。種でないクラスタは
  行ごと消えて顔は新しい ID へ配り直されるので、**無名の人物を家族グループに入れると、
  次の再クラスタで黙って消える**（共有フォルダからもその人の写真が消える）。
  しかも `PeopleGroupInfo.resolve` は解決できないメンバーを `compactMap` で**黙って落として**
  いたので、直したあとも効いているか分からなかった。
- 決定: 「グループの所属も表明として扱う」を、**行が消え得る全ての経路**に通す。
  - `FaceSeedBuilder.ClusterRef` に `inPeopleGroup` を足し、**無名でもグループのメンバーなら種にする**。
    ⚠️ これは「無名のクラスタを機械の再編から守る」＝ADR-132 の範囲を広げる判断。
    広げる根拠は、**グループに入れる行為そのものがユーザーの表明**で、名前を付けるのと同じ重みだから。
    もう一方の道（再クラスタ後に ID を付け替える）は、無名クラスタが分裂・統合もするので
    1 対 1 に対応せず成立しない。
  - `resolve` に `unresolvedClusterIDs` を持たせ、`reloadPeopleGroups` が診断ログへ出す
    （**無音をやめる**）。⚠️ 人物一覧が空のうちは黙る——起動直後・再スキャン中は全メンバーが
    未解決に見え、本当の記録が偽の警告に埋もれる。
    ⚠️ **同じ内容は 1 回だけ書く**（レビュー 1 周目）。`reloadPeopleGroups` は `loadPeople`
    から呼ばれ実機では**毎分 30 回**走る。診断ログは末尾 256KB しか残らないので、二度と
    一致しないメンバー（写真を消した等）が 1 人居座ると**同じ行で記録を埋め尽くし、
    残したかった証拠を押し出す**。中身（どのグループが何人）が変わったときだけ書く。
  - メンバーの集合は**ストアで持つ**（`peopleGroupMembersCache`・ADR-119）。
    ⚠️ この集合は「行を消してよいか」と「フロアを免除するか」の両方が見るので、
    **1 顔ごと・1 クラスタごとに呼ばれる**——毎回引くと写真を 1 枚消すだけで
    クラスタ数ぶんの往復になる。グループは数個で変わるのは CRUD のときだけなので持っておき、
    書き換える全経路で捨てる（`invalidatePeopleGroupMembersCache`）。
    回数は `ScaleRegressionTests` が数えている（40 回聞いて fetch 0 回・書き換え後は見える）。
  - **枚数フロアも免除する**（`peopleClusters`・レビュー 4 周目）。⚠️ **行を残すだけでは
    足りなかった**——`peopleClusters` は枚数フロア（既定 3 枚）でクラスタを落とすので、
    0 枚になった行は `allPeople` に現れず、**家族グループからはやはり消えたまま**
    「解決できないメンバー」として永久に記録され続ける（編集画面からも外せない＝
    診断ログが同じ行で埋まる条件そのもの）。フロアは「たまたま写り込んだ人を人物として
    扱わない」ための機械の線なので、表明した人物には効かせない。
    ⚠️ ただし**免除は 2 段**（レビュー 5 周目）。無条件にすると、名前を付けた人の写真が
    全部消えたときに「Alice・0 枚」の行が一覧へ**永久に居座る**——人物を消す操作は無いので、
    名前を消すしか逃げ道が無い。**1 枚でも在る表明済みの人物は出す／0 枚は家族グループの
    メンバーだけ出す**（それはグループから外せば消える＝逃げ道がある）。
  - **メンバーの集合を読めなかったときは、それを覚えない**（レビュー 5 周目）。
    ⚠️ `countedFetchOptional` は throw で nil を返すので `?? []` で畳むと、**1 回の読み取り
    失敗が「メンバーは 1 人も居ない」という結論としてキャッシュに残る**——次にグループを
    書き換えるまで誰も捨てないので、その間の掃除・夜の再クラスタが「守るべき行は無い」と
    判断して消してしまう（この一連の修正が防ごうとしたものそのもの）。
    読めなかったら覚えずに空を返し、診断ログへ出す。
  - **行を消してよいかの判定にも入れる**（`FaceStore.isUserClaimed`・レビュー 1 周目）。
    種の条件だけ直しても、**写真を整理しただけで**無名のメンバーの行が消え
    （`pruneMissingPhotos`）、最後の 1 顔を外したときにも消えて（`FaceStore+Edit`）、
    同じ症状が別の 2 経路から出ていた。引数 `peopleGroupMembers` に**既定値を置かない**
    ——置くと呼び出し側が何も考えずに省略でき、「グループに入れた人だけが保護から漏れる」が
    また起きる（それがこの引数を足した理由そのもの）。集合は**ループの外で 1 回**作る。
- 結果: 家族グループのメンバーは再クラスタを跨いで残る。トレードオフ: グループに入れた無名の
  人物は「機械が配り直してよいクラスタ」から外れるため、その人物の純度は自動では改善されなくなる
  （ユーザーが外すまで固定）。これは名前付き人物と同じ扱いなので一貫している。
  **副産物**: 枚数フロアの免除で、**名前を付けた人の写真が 1〜2 枚になっても一覧から消えなく
  なった**。`people` の定義（`name != nil || count >= floor`）は元から「名前を付けた人は枚数に
  関係なく必ず出す」と書いてあったのに、その手前の `peopleClusters(minFaces: 3)` が黙って
  落としていた——名前も行も残っているのに一覧から見えず、利用者は消すことも直すこともできなかった。
  **宣言と実装が 2 層で食い違っていた**ので、片方（表示の線）だけ読んでも気づけない。
  ピープル一覧が断片で溢れないのは、表示の線そのものは変えていないから。
  **0 枚は出さない**（家族グループのメンバーを除く）——消す操作が無いので、居座ると直せない。
- **精度の計測について**: 「精度系はデータセットで測ってから採用」が原則だが、この仕組みは
  **利用者が作ったグループにしか反応しない**（`PeopleGroupRecord` が 1 件も無ければ
  `inPeopleGroup` は常に false）。FG-NET / LFW / PIPA にはグループが無いので、
  ハーネスの数値は**構造上まったく動かない**——測れないのではなく、測る対象が無い。
  実際、`FaceAccuracyEvalTests` / `FaceAgglomerationEvalTests` は差分前と同じ値で通る。
  代わりに固定したのは「グループのメンバーなら種になる／ならない」の対称なユニットテスト。
- 関連: `FaceSeedBuilder.swift` / `FaceStore+Rebuild.swift`（`buildSeeds`）/
  `PeopleGroups.swift`（`peopleGroupMemberClusterIDs` / `resolve`）/
  `FaceClusteringSetupTests.swift` / `PeopleGroupsTests.swift`。ADR-119/130/132/134/232。

## ADR-230 反映の合流は「終わったか」ではなく「どの版を読んだか」で決める
- 状態: 採用（ADR-95 で入れた合流に、版の条件を足す）
- 文脈: クラウド一覧の反映（`reflectCachedItems`）は、73k 行の実体化が同時に 2 回走るのを
  防ぐため「同じ口座の反映が走っていれば合流する」ようにしていた（ADR-95・diagnostics-38/65/66）。
  ⚠️ しかし合流先が持っているのは**始めた時点のスナップショット**でしかない。こちらが呼ばれたのは
  その後に版（`itemsRevision`）が進んだからなのに、無条件に合流すると**進んだぶんが一覧に
  出ないまま確定**する。しかも合流された側が `lastReflectedRevision` を**古い版で**更新するので、
  次の周期も「変わっていない」と判断されて**アプリを開き直すまで直らない**
  （初回同期の最後の数千枚・バックアップ直後の数枚がこれで消える）。
  `scheduleCacheRefresh` には後追いの再チェックがあったが、`loadItems()` の経路には無かった。
- 決定: 反映タスクの結果に「**反映できた版**」を添える（`(count, revision)`・中断は `nil`）。
  合流してよいのは `canJoinReflect(reflected:wanted:)`＝**こちらが見た版以上を反映した**とき
  だけ。古ければ上から見直して（別の呼び手が既に追いかけを登録していればそれに合流＝
  **2 本は走らせない**）、無ければ自分で 1 回反映する。版は増える一方なので輪は数回で抜けるが、
  アクターの割り込み順に頼った上限は事故になるので `maxReflectJoins` で明示的に切る。
  - あわせて、**同期ルートを変えた枝が世代（`loadGeneration`）を進めていなかった**のを直す。
    進めないと消去前のスナップショットが着地して**古いルートの一覧が戻り**、
    `lastReflectedRevision` まで新しい版で更新される＝以後ずっと直らない。
    `clearCache` とアカウント切替は既に「消す前に世代を進める」作法だったので、それに揃えた。
    ⚠️ ただしここは**消去が非同期**（`Task { await cache.clearAll(...) }`）なので、
    「消す前の 1 回」では**消している間に始まった読み込み**を捨てられない——その読み込みは
    新しい世代の札を持って素通りし、消した直後に古いルートの一覧を書き戻す。
    消したあとにもう一度進める（レビュー 3 周目）。
    （`clearAll` が `itemsRevision` を進める件は ADR-222〜225 で対処済み。）
  - ⚠️ 実際の競合はアクターの割り込み順で決まる＝壁時計では再現できないので、**規則を純関数に
    出して**固定する（`canJoinReflect`）。配管は既存の合流テストが見ている。
- 結果: 反映は最新の版に追いつく。実体化の回数はほぼ変わらない（古い合流を掴んだ呼び手だけが
  1 回追加で走る）。トレードオフ: 版が毎秒進む局面（バックアップ中の delta）では追いかけが
  1 本入り得るが、間引き（ADR-224）と無風の窓が上限を押さえている。
  ⚠️ **「2 本は絶対に走らない」ではない**（レビューで言い過ぎを直した）。上限
  （`maxReflectJoins`）に当たった稀な場合だけ、走っている反映と重なり得る——実体化が 1 回
  余分に走るだけで結果は正しく、`HeavyLoad` は同じ札の再入を数で持つので札も壊れない。
  完全に直列化するには鎖を持つ必要があるが、正しさは版の判定が担保しているので、
  そのための仕掛けは増やさない（ADR-197）。
- 関連: `DropboxPhotoStore.swift`（`reflectCachedItems` / `performReflectCachedItems` /
  `canJoinReflect` / `startSync` のルート変更枝）/
  `DropboxPhotoStoreReflectCoalesceTests.swift`。ADR-95/205/206/222/224。

## ADR-229 作り直しても同じになる表は、ディスクに置いて鍵で捨てる
- 状態: 採用
- 文脈: 実機ログ diagnostics-96 で、窓のピークが **617MB**（過去の 1,061MB からは大きく下がったが
  まだ高い）。内訳を追うと**候補の列挙ではなかった**——列挙中の footprint は 166MB で山になっていない。
  山は **CLIP テキスト塔（+260MB・178→438MB）と顔モデル（+85MB）が同時に載っていた**こと。
  そのテキスト塔は表示タグの概念埋め込み（314 語）を作るために載っており、
  ⚠️ **その表はメモリ上にしか無かった**ので**起動するたびに作り直して**いた（実機 13 秒）。
  314 語も同梱モデルも固定なので**答えは毎回まったく同じ**。
  diagnostics-65/66 で「569MB まで伸び、直後に `cancelPrewarm` で捨てられていた＝完全な無駄」と
  記録し、当時はゲートで**症状**を抑えたが、**毎回作り直す構造**は残っていた。
- 決定: 表をディスクに置く（`ConceptEmbeddingCache`・314×512×Float32 ＝ **628KB**）。
  - **いつ捨てるかは鍵が決める**（`MosaicSupport.ConceptTableFingerprint`・純ロジック）。
    ⚠️ **人の採番に頼らない**——採番は忘れられるが入力は忘れられない。混ぜるもの:
    モデルの素性（`mobileclip_config.json` の `model`/`pretrained`/`embedDim`/`contextLength`）・
    **モデルへ渡す文字列そのもの**（語の追加/削除/修正・並び・テンプレート変更を一度に拾う）・
    語彙ファイルの大きさ・ファイル形式の版。区切りを挟んで連結する（`["ab","c"]` と
    `["a","bc"]` を区別する）。**モデルを識別できないときは鍵を作らない＝使わない**。
  - **完全な表だけ保存する**。作る側は埋め込みが nil の語を黙って飛ばすので、欠けた表を
    保存すると以後ずっと欠けたままになる（作り直していた頃は次の起動で埋まり得た）。
    読む側は**大きさの検査が完全性の検査**にもなる（語数 × 次元 × 4）。
  - **Float16 にしない**。表示タグは「最上位との差が `margin`=0.04 以内」を採るので丸めが
    境界のタグを入れ替え得る。628KB と 314KB の差は取るに足らない（`PhotoEmbedding` が
    Float16 なのは 6.7 万行あるからで、314 行に同じ理屈は要らない）。
  - 置き場所は `.applicationSupportDirectory`。⚠️ Caches だと OS に消されて、気づかないまま
    また 13 秒と 260MB を払う（それを無くすのが目的）。
  - **キャッシュが在るときは `prewarm` のゲートを待たない**。628KB を読むだけならモデルは
    不要＝重い処理と食い合わない。ゲートが守りたいのは「中断できないモデルのロード」なので、
    それが起きない経路は通す。従来はここで降りてしまい、忙しい起動では表が作られなかった。
  - 並びは語リストの順そのまま（読む側は語を持たず順序で対応づける）。⚠️ 並びが変わった表を
    読むと**全タグがずれる**（cat のベクトルに dog の名前が付く）ので、並びも鍵に入れる。
- 結果: 起動ごとの 13 秒と +260MB が消える見込み。窓のピークは **617MB → 約 360MB** の見込み
  （要実機計測）。おまけに**起動直後からタグが出る**（従来は表ができるまで Vision タグのみ）。
  ⚠️ ADR-223 の「顔モデルと CLIP の塔を同時に抱えない」は、この順序（CLIP が先・顔が後乗り）に
  効いていなかった。塔がそもそも載らなくなるので**副作用としてそちらも解消する**
  ——ADR-223 を改訂する必要は無い（当初そう見立てたが誤りだった）。
- 関連: `MobileCLIPKit/ConceptEmbeddingCache.swift` / `MobileCLIPConfig.swift` /
  `CLIPDisplayLabeler.swift` / `MosaicSupport/ConceptTableFingerprint.swift`
  （`ConceptTableFingerprintTests` 13 本）。ADR-223・ADR-228・diagnostics-65/66/96。
  ⚠️ 実機で確かめること: `device-verification.md` の D 節「概念表がディスクから読まれること」。

## ADR-228 前面でも、一定時間使われていないモデルは手放す
- 状態: 採用（ADR-223 の「前面では手放さない」を限定的に改める）
- 文脈: 常駐メモリの棚卸しで、**解放点が 1 つも前面で発火しない**ことが分かった。
  ADR-223 の解放点は「窓の終わり」と「顔スキャン 1 巡」、ADR-226 追補で「背面化」が増えたが、
  どれも `BackgroundYield.scenePhase != .active` を条件に持つ。そのため
  **前面で検索を 1 回すれば CLIP テキスト塔が載り、critical 圧迫か背面化まで
  載りっぱなし**になる。アプリを開いたまま置いてある時間は珍しくない。
  ⚠️ **数字の出どころに注意**（レビュー 33 周目）: ADR-223 が引く「505MB」は
  **塔を読んだ後のプロセス全体の footprint**（diagnostics-88）であって、塔そのものの
  大きさではない。塔の取り分は 505MB からその時の下地を引いた差で、**未計測**。
  ⚠️ ADR-223 の「前面では手放さない」は誤りではない——あれは**窓の終わりに手放すか**という
  問いへの答えで、「前面で放置され続けた場合」は問いの外にあった。
- 決定: **最後の推論から一定時間（既定 5 分）まったく使われておらず、解析も走っていなければ、
  前面でも手放す**。線引きは純ロジック `MosaicSupport/ModelIdlePolicy`（他の方針型＝
  `BackgroundYield` / `HeavyWorkTiming` と同じ場所）。
  - 最終利用は **モデルごとに**記録する（`noteCLIPInference` / `noteFaceInference`）。
    呼ぶのは**同梱モデルを使う推論の入口 5 か所**（CLIP のテキスト/画像/画像バッチ、
    顔の 1 枚/バッチ）。Vision だけの経路（`VisionTagAdapter` /
    `FacePerceptionAdapter+Vision`）は自前モデルを読まないので数えない。
    ⚠️ **1 つの記録にまとめてはいけない**（レビュー指摘）。まとめると、写真を眺めている
    だけで走る CLIP の推論（表示タグ＝`CLIPDisplayLabeler` が数分おき）が、何時間も
    使っていない顔モデル（300〜650MB）を引き止める——ADR-228 がいちばん減らしたい
    「開いたまま眺めている」場面で減らなくなる。
    記録は推論の**開始時**（ゲートに入る前）。終了時にすると、ゲートで順番待ちしている
    長い推論が「使っていない」と見えてしまう。
  - ⚠️ **判定と「記録を消す」は不可分**（`ModelIdleTracker.consumeIfIdle`・レビュー 3 周目）。
    `note()` は推論のスレッドから、判定はメインから呼ばれるので、「判定 → 手放す → 消す」と
    3 段に分けると途中に `note()` が割り込み、**たった今使い始めた印を最後の消去が消す**
    ——そのうえでモデルを取り上げることになる。錠の中で 1 つにすれば、割り込みは
    消去の前か後のどちらかに定まる。
  - 「解析が走っているか」の式は**1 組だけ**（`HeavyWorkScheduler` の `isCLIPBusy` /
    `isFaceModelBusy`）。**前面も背面もモデルごとに**渡す——合成（`isAnalysisRunning`）は
    レビューで消した。「どちらかが走っていれば両方残す」だと、顔スキャン中に背面へ
    落ちただけで CLIP テキスト塔（505MB）が残る。⚠️ **背面のほうが効く**（jetsam に
    殺される場所）のに、前面だけ直して背面を見落としていた。
  - ⚠️ **`.background` ハンドラの順序**（レビュー指摘）: 前面のみモードのブーストは
    `appLeftForeground()` で終わらせて**から**解放を呼ぶ。逆順だと、解放の時点で
    セッションがまだ active＝「走っている」と見えるため**何ひとつ解放されず**、
    1 行あとにセッションが終わっても次の窓まで誰も解放しに来ない。
  - 解放のログは**モデルごと**（`CLIP released (…)` / `face model released (…)`）。
    まとめると、片方だけ手放した回を実機ログから区別できない＝上の退行に気づけない。
    ⚠️ **モデルごとに分けたのはレビュー 10 周目**。1 つにまとめていた間は、顔スキャンが
    走っているだけで CLIP テキスト塔（505MB）まで手放せなかった（顔スキャンは CLIP を
    使わないのに）。前面のブーストは 10 分以上続くので実際に効く。
    ⚠️ 記録（時刻）だけで足りない理由: トリクルは操作のたびに譲るので、**走っているのに
    5 分以上推論していない**ことがある。時刻は「使った」を、busy は「これから使う」を表す。
    ⚠️ **これで背面側の挙動も変わった**（レビュー 6 周目で明文化）。従来の背面の条件は
    「窓の仕事」と「ブースト」の 2 つだけで、前面のトリクルが埋め込み中に背面へ落ちると
    **走っているその処理からモデルを取り上げて**いた。埋め込み・顔スキャン・生成を
    足したのはその穴を塞ぐためで、意図した変更。代償は「フラグが立ちっぱなしだと
    解放されない」だが、影響は解放の見送りだけで**解析は止まらない**
    （`isGeneratingAlbums` に時間切れの安全弁が要ったのは、あちらが仕事を塞ぐ判定だったから）。
  - タイマーは**足さない**。`AnalysisDriver` のアイドル監視（5 秒刻み・前面にいる間だけ）に
    相乗りする。
- 結果: 前面で放置したときの常駐が減る。⚠️ **減る量は未計測**——
  「505MB」は塔を読んだ後のプロセス footprint なので、そのまま差として使えない
  （レビュー 33 周目で訂正。効果を過大に見積もると、35 秒の再ロードという代償との
  釣り合いを誤る）。実機で `PERF TICK footprint=` の解放前後を見るまでは数字を書かない。代償は
  「5 分以上空けた次の検索が再ロード待ちになる」こと。⚠️ **線を短くしてはいけない**——
  再ロードは実機 10〜35 秒で、アイドル解放が成り立つのは「誰も待っていない時間に払うから」。
  検索して結果を眺めている数分で手放すと、その前提が崩れる。
- 関連: `MosaicSupport/ModelIdlePolicy.swift`（線引き＋`ModelIdleTracker`・テスト 13 本）/
  `MobileCLIPKit/CoreMLModelSupport.swift`（`noteCLIPInference` / `noteFaceInference` /
  `releaseIfIdle(clipBusy:faceBusy:)`）/
  `MobileCLIPRuntime.swift`・`FaceModelRuntime.swift`（記録の呼び出し）/
  `HeavyWorkScheduler.releaseModelsIfIdleInForeground` / `AnalysisDriver.startIdleWatch`。
  ADR-223・ADR-226 追補・ADR-196。
  ⚠️ 実機で確かめること: `device-verification.md` の D 節
  「前面で放置したときに手放すこと（ADR-228）」。⚠️ **Z6 は別件で使用済み**
  （ADR-205・消えた同期ルート）なので番号を振らない。

## ADR-227 全件を読む処理は、使い捨ての `ModelContext` でページ分けして読む
- 状態: 採用（ADR-224 の実測から一般化）
- 文脈: 常駐メモリの棚卸し（クラウド 10.2 万枚・実機 `phys_footprint` 680〜840MB）で、
  **いちばん大きく、いちばん解放されない塊**が SwiftData だった。
  `propertiesToFetch` は列を絞らず（ADR-224 の実測）、**長生きの `ModelContext` は実体化した行を
  登録し続ける**ので、全件 fetch の 1 回がそのまま常駐になる。`ModelContext.reset()` は
  リポジトリ全体で 0 件、使い捨てコンテキストは 4 か所だけだった。
- 決定: **読み取り専用の全件走査は使い捨ての `ModelContext` で行う**。大きい台帳は
  さらに**カーソルでページ分け**して、ページごとに値へ写して捨てる
  （`AutoAlbumStore` の埋め込みページングと同じ手）。
  - ⚠️ **書き込みには使わない**。使い捨てで取った `@Model` を本体のコンテキストへ渡さない。
    書き戻しがある経路（再クラスタ等）は「読みは使い捨て・書きは本体」に分けてから移す。
  - 適用済み: `TagStore`（`allTags` / `allHumanCounts` / `allOcrTexts` / `topTags` / `tagVocabulary`）、
    `FaceStore.qualityReport`（夜の再クラスタ直後に必ず走る・顔 10 万件 × 埋め込み 1KB）、
    `BackupStore.recordedLocalIdentifiers` / `localToCloudPaths`。
  - **書き戻しがある経路は「読みはページ・書きは対象だけ」に分ける**。適用済み:
    `FaceStore+Edit.repairSamePhotoViolations`（**前面のタップごと**に走るのに顔を全件読んでいた。
    直す顔は普通 0〜数件なので、値だけページ読みして違反を見つけ、**外す顔だけ**を本体の
    コンテキストで引いて書き換える）。代表選びは値版 `bestCoverFaceID` に切り出して規則を共有。
  - 未適用（次）: `FaceStore+Rebuild` の全顔読み（clustering の中核で書き戻しも多い＝
    「読みは値・書きは faceID 指定」への分解が要る）、`BackupStore` の残り、`clearPerception`。
- 結果: 読むために積んだぶんが読み終わりで返る。代わりにページごとにコンテキストを作る手間が増える
  （実測では全件 1 回と同程度）。
- 関連: `Tags/TagStore.forEachRecordPage` / `Store/AutoAlbumStore`（先例）/ ADR-224・ADR-119・ADR-122。

## ADR-226 背面では、見ていない画像を抱えて眠らない
- 状態: 採用（ADR-223 の考え方を画像キャッシュへ広げる）
- 文脈: 実機（クラウド 10.2 万枚）で `phys_footprint` が常時 680〜840MB、窓の頭で **1061MB**。
  サムネのメモリ上限は端末の予算の約 5%（60〜192MB）で、**クラウドと端末の 2 系統**ある。
  背面では誰もサムネを見ていないのに抱えたままで、**背面のアプリは footprint の大きい順に
  落とされる**（ADR-223 と同じ理由）。見ていない画像のために jetsam の的を大きくしていた。
- 決定: `scenePhase == .background` で全キャッシュの上限を下限（既定 16MB）まで絞り、
  `.active` で元に戻す（`MemoryImageCache.setBackgroundMode(_:)`）。中身はディスクに残るので、
  戻ったら再デコードで埋まり直す。
  - ⚠️ 絞るのは **background だけ**。`.inactive`（Control Center・通知バナー・App スイッチャー）で
    捨てると、戻った瞬間に再デコードの山になる。
  - ⚠️ 背面のあいだに圧迫が来ると「一定時間後に元へ戻す」予約が入る。その復帰でも**戻さない**
    （戻すと絞った意味が消える）。
  - 背面で走る顔スキャンはキャッシュ済みサムネを使うが、メモリに無ければディスクから読む
    （`ThumbnailDecode.limiter` で同時数は有界）。
- 結果: 背面の常駐が下がる。代わりに前面へ戻った直後の数十枚はディスクから読み直す。
- **追補（同じ考えをモデルと大きな配列にも）**: 背面で手放すのは画像だけではない。
  - **Core ML の塔**: ADR-223 の解放点は「窓の終わり」「顔スキャン 1 巡」の 2 つだけで、
    **前面の検索で読んだテキスト塔（実測 505MB）は背面へ落としても次の窓まで残る**。
    背面へ入った時点で、**窓もブーストも走っていなければ**手放す
    （走っている解析から取り上げると、その場で 10〜35 秒の再ロードが始まる）。
  - **作り直せる大きな配列**: AI アルバムの下ごしらえ（台帳全行・約 30MB／`nil` を入れる経路が
    どこにも無かった）、解析候補（約 12MB／期限切れでも捨てていなかった）、顔の候補（約 10MB）。
- 関連: `ImageCacheKit/MemoryImageCache.swift` / `MosaicPhotosApp.swift` /
  `HeavyWorkScheduler.releaseModelsIfIdleInBackground` / `AutoAlbumEngine.releaseSuggestionSnapshot` /
  `MemoryImageCacheBackgroundTests`。ADR-223・ADR-20。

## ADR-225 作り直しの間引きは「実測した所要」から決める（人物一覧・クラウド一覧）
- 状態: 採用（ADR-95 / ADR-224 の一般化）
- 文脈: 「連続する変更を 1 回にまとめる」間引きを、固定の静止時間（人物一覧 700ms・
  クラウド一覧 0.4 秒）で入れていた。**1 回の作り直しがその静止時間より長いと、間引きは効かない**
  ——終わった直後にまた予約が入り、走り続ける。実機で 2 回踏んだ:
  - クラウド一覧（diagnostics-92）: 前面で 3 分に 40 回・1 回 1.2〜2.1 秒。
  - 人物一覧（diagnostics-93）: 18 分に **160 回・合計 414 秒**（1 回平均 2.6 秒・最大 8.7 秒）。
    顔の `@ModelActor` は 1 本なので、その間ずっと写真の人物名・レビュー候補・顔スキャンが待つ。
- 決定: 静止時間を **`max(下限, 直近の所要 × 4)`**（頭打ちつき）にする。
  小さなライブラリは従来どおり素早く、育つほど自分で空く。
  - クラウド一覧: 下限は前面 0.4 秒 / 初回同期 5 秒 / 背面 30 秒、頭打ち 10 秒。
  - 人物一覧: 下限 0.7 秒、頭打ち 15 秒。
- 結果: 重い端末ほど間引きが効く（回数が所要に反比例する）。代わりに一覧の反映は最大で
  頭打ちのぶん遅れる——どちらも「最終的に正しければよい」表示なので許容する。
- ⚠️ **固定値の間引きを見たら疑う**。「静止時間 < 1 回の所要」なら、その間引きは存在しないのと同じ。
- 関連: `PeopleEngine.reloadQuietMilliseconds` / `DropboxPhotoStore.refreshInterval` /
  `MosaicSupport.DebouncedTask.schedule(quietMilliseconds:)` / `PeopleReloadThrottleTests`。

## ADR-224 クラウド一覧の作り直しは「静かになってから」＋「背面では回数の上限」
- 状態: 採用（ADR-95 の間引きを置き換え）
- 文脈: バックアップ中はアップロードのたびに delta が届き、そのたびに 73,936 行を実体化して
  `DropboxFileItem` を作り直していた（実機 diagnostics-87: 2 分 15 秒で 21 回）。
  1 度目の対処で「静かになってから 1 回」（無風 1.5 秒）にしたが、**実機では効かなかった**
  ——delta は **3 秒おき**に届くので、無風 1.5 秒を毎回満たす（diagnostics-89: 5 分で 60 回・
  合計 102 秒）。「変化が止まったら」は、変化が一定間隔で続く相手には効かない。
- 決定: 2 つを重ねる。(1) 無風（既定 1.5 秒）を待つ、(2) **前の反映からの最短間隔**を守る
  ——前面 0.4 秒 / 初回同期 5 秒 / **背面 30 秒**。背面は誰も一覧を見ていないので回数を絞る。
  ⚠️ 背面を 0 回にはしない: 解析候補の列挙（`AnalysisCandidates`）が `items` を読むので、
  新しいクラウド写真がいつまでも解析対象にならなくなる。
  初回同期の完了など「もう変化が来ない」最終反映は `immediate` で待たない。
- 結果: 背面の作り直しが 1/10 以下になる。代わりに背面では一覧が最大 30 秒古い
  ——見ている人はいないので、前面へ戻る時点で最新になっていればよい。
- **追補（実機ログ diagnostics-90）**: 回数だけでなく**1 回の重さ**も問題だった。窓の開始で
  `cachedItems`（98,951 行 × 全列）が走り、フットプリントが **821MB** まで跳ねていた。
  さらに新しく足した `cachedContentHashes`（9.9 万行の射影）は、顔スキャン・バックアップと
  重なると **17.1 秒**かかり、その間キャッシュの actor が塞がっていた。
  - **表示用の配列を計算の入力に使わない**。解析候補の列挙は `CloudPhotoRef`
    （パスと撮影日だけ）の射影を使う（`cloudPhotoRefs()`）。ついでに「items が空なら読み込む」
    （ADR-85 の回避策）も不要になった——台帳は画面を開いていなくても埋まっている。
  - **作り直さず、増減で直す**。content_hash の表は `applyDelta` が増減のぶんだけ更新し、
    `itemsRevision` が一致する限り引き直さない（メモリ圧迫では捨てる＝作り直せる）。
  - **間隔は作り直しの実測時間から決める**（追補 2・実機ログ diagnostics-92）。前面 0.4 秒固定では、
    バックアップ中の delta（3 秒おき）に対して毎回作り直していた（3 分で 40 回）。
    `refreshInterval = max(下限, 直近の所要 × 4)`（頭打ち 10 秒）——小さなライブラリは素早いまま、
    9.9 万件なら自分で 6 秒空ける。下限は前面 0.4 秒 / 初回同期 5 秒 / 背面 30 秒。
- 関連: `Store/DropboxPhotoStore.swift`（`scheduleCacheRefresh` / `currentRefreshInterval`）/
  `Cache/DropboxCacheStore.swift`（`cachedContentHashes` / `cachedPhotoRefs`）/
  `Models/CloudPhotoRef.swift` / `PhotosFeatureKit/AnalysisCandidates.swift` /
  `DropboxPhotoStoreReflectCoalesceTests` / `CloudContentHashProjectionTests`。ADR-95・ADR-119。

## ADR-223 夜の処理枠が終わったら、同梱モデルを手放して眠る
- 状態: 採用（**前面の扱いは ADR-228 が限定的に改めた**——下記「前面では手放さない」は
  「窓の終わりに手放すか」への答えで、前面で放置され続けた場合は見ていなかった）
- 文脈: 実機ログ diagnostics-88 で、夜の窓のフットプリントが **650MB** に達していた
  （CLIP テキスト塔で 505MB → 顔モデルで 630〜650MB）。窓が終わってもモデルは常駐したままで、
  アプリはその後 30 分眠る。**背面のアプリは footprint の大きい順に落とされる**ので、
  誰も使っていない 300〜500MB を抱えたまま jetsam の候補になっていた。
  これまでの解放は**critical 圧迫のときだけ**で、圧迫が来なければ永久に持ち続けていた。
- 決定: 窓の終わり（`bgtask: end`）で `PerceptionModels.releaseForIdle` を呼び、CLIP 両塔と
  顔モデルを手放す。次の窓で読み直す（7〜17 秒）。窓は 30 分おきなので割に合う。
  ⚠️ **前面では手放さない**——検索・フル画像のタグ表示が次の操作で再ロード待ちになる。
  判定は `BackgroundYield.scenePhase`（状態の出典はここだけ・ADR-196）。
  **窓の中のピーク**も同じ考えで下げる: 顔スキャンが 1 巡したら（残作業 0 なら）**顔モデルだけ**
  先に手放す（`PeopleEngine.onScanFinished` → `PerceptionModels.releaseFaceModelIfDone`）。
  窓はこのあとタグ付け → CLIP 埋め込みへ進むので、持ち越すとモデルを 2 つ抱えることになる。
  ⚠️ 残作業があるなら手放さない（すぐ読み直すことになる）。
- 結果: 眠っている間の常駐が下がり、次に起こされるまで生き残りやすくなる。窓の中のピークも
  「顔モデル＋CLIP 塔」の二重持ちが消える。代わりに窓ごとにロードのやり直しが入る
  （窓の頭で 7〜17 秒）。**効果の確認は実機**（`device-verification.md`）。
- 関連: `MobileCLIPKit/CoreMLModelSupport.swift`（`PerceptionModels`）/ `MobileCLIPRuntime` /
  `FaceModelRuntime` / `HeavyWorkScheduler`。ADR-122（重い一括ロードは札を立てる）。

## ADR-222 同じ Dropbox に繋がっている人へ、写真をコピーせず「解析結果だけ」を公開する
- 状態: 採用
- 文脈: Dropbox に家族を接続すると、**写真そのものは接続した時点で相手からも見える**。ところが
  解析（タグ・CLIP 埋め込み・顔・人物名・撮影日）は各自の端末の中にしか無く、同じ写真を
  もう一度ダウンロードして解析し直していた（6.8 万枚で数時間 × 人数）。人物名も引き継がれない。
  既存の共有（ADR-112/183）は**写真のコピーと対**で解析を置く仕組みなので、
  「既にクラウドにある写真」には使えない——コピーを作る意味が無いからだ。
  送信側は共有セットに入れた写真の解析しか書き出さず、受信側も**家族フォルダ配下の写真**としか
  突合していなかった。
- 決定:
  - **公開**: クラウド写真ぜんぶの解析を `<root>/<端末>/Analysis/.mosaic-share/shard-xx.json` へ置く。
    形式は共有セットと同じ（鍵は Dropbox の `content_hash`）なので、受信側の取得・検証・取り込みは
    そのまま使える。端末ごとに別フォルダなので、同じ Dropbox の複数の端末が互いを上書きしない。
    写真はコピーしない。
  - **上げ方**: 1 枚 1〜2KB（CLIP 埋め込みが主）で 6.8 万枚なら 100〜200MB になるので、
    **変わったシャードだけ**を 1 回 8 個まで（`AnalysisPublishPlanning`・指紋は FNV-1a）。
    続きの位置はシャード名の昇順で持ち、一巡させる（先頭だけ上げ直し続けない）。
  - **受信**: 家族フォルダに加えて `<root>` 直下の端末フォルダを 1 回ずつ一覧し、
    `Analysis` を持つものを取得対象に足す（自分の端末は除く）。突合の対象は
    **手元のクラウド写真すべて**に広げた——鍵は content_hash（同じ中身＝同じ写真）なので、
    置き場所で絞る意味が無い。撮影日の表の掃除も同じ広さに揃えた（狭いままだと、
    共有フォルダの外の写真の撮影日を「もう無い写真のもの」と見て捨てる）。
  - **設定**: 「写真の解析結果を共有」（`ShareSettingsKeys.publishAnalysisEnabled`・**既定 ON**）を
    クラウド共有のハブに置く。OFF なら 1 バイトも上げない。
  - **いつ回すか**: 夜間の処理枠の 1 手（`NightlyPlan.Step.publishAnalysis`）。回線が要るので
    `.shareImport` / `.shareSync` と同じ回線の条件の中、**取り込みのあと**（受け取った解析が
    自分の台帳に入ってから公開されるので、同じ写真を 2 人が解析し直す空回りが早く止まる）。
- 結果: 家族が Dropbox に繋いだだけで、相手の端末の解析がそのまま効く（解析し直さない・人物名も届く）。
  代償は Dropbox の容量（解析ぶん・写真の 1% 未満）と夜間の上り通信。
  ⚠️ **公開する JSON は鍵を並べて書く**（`.sortedKeys`）。`Entry` の辞書は順序を持たないので、
  既定のエンコーダだと同じ中身でもバイト列が変わり、**何も変わっていなくても毎晩全部を上げ直す**。
  テストで実際に踏んだ。
- **追補（実機ログ diagnostics-83・同日）**: 最初は公開を**反映（`shareSync`）の後**＝手順の最後に
  置いたが、実機では**一度も順番が回ってこなかった**。反映は 1 回 500 件ずつコピーし、
  この端末には 9,265 件が残っていたため、窓（約 5 分）を毎回使い切っていた。
  公開は上限つき（8 シャード）で軽いので、**取り込みの後・反映の前**へ移した。
  あわせて、(1) 公開が**抜けるときも必ず診断ログを 1 行残す**ようにした——黙って return して
  いたため「順番が回らなかった」のか「回ったが抜けた」のかログから区別できず、原因に辿り着く
  のに実機ログ 1 本ぶん遠回りした。(2) 設定に「今すぐ公開」を足した（窓を待たずに実機で
  確かめられる・結果を画面にも出す）。
- **追補 2（実機ログ diagnostics-84・同日）**: 公開が毎回「クラウド写真が 0 件」で何もしなかった。
  写真の一覧を `DropboxPhotoStore.items`（表示用）から作っていたが、表示用アイテムは
  content_hash を**わざと持たない**うえ、`items` は画面を開いたときだけ作られる。
  台帳の射影 `cloudContentHashes()` へ向け直した。詳細と、同じ理由で空だった
  `cloudSourceHashProvider`（ADR-209）は `case-studies.md` を見ること。
- **追補 3（公開する端末は 1 台・既定 OFF）**: 実機で公開が動いたので、次は「2 台で ON にしたら
  どうなるか」を決めた。端末フォルダ（`<root>/<端末>/Analysis`）は分かれているので**ファイルは
  壊れない**（利用者の懸念だったが、上書きは起きない）。壊れない代わりに、**同じ写真の解析が
  台数ぶん積まれる**——1 台で 100〜200MB（6.8 万枚）なので 3 台なら 500MB 前後を同じ内容に使い、
  上りの通信も受け取り側の取り込みも台数ぶんになる。
  - **既定を OFF に変えた**（ADR-222 本体では ON だった）。「気づいたら容量を倍使っていた」を
    既定にしない。設定の説明にも「1 台だけで ON に」と明記する。
  - `<root>/.mosaic-analysis-owner.json` に**名乗り**（端末フォルダ・表示名・名乗った日・
    最後に公開できた日・枚数）を置く。公開の前に読み、別の端末が名乗っていたら
    **公開しない**で設定画面に注意を出す。
  - ⚠️ **止めきらない**。端末を失くす・機種変更はあるので、「この端末で公開する」で**いつでも
    引き継げる**（承諾は**その相手に対してだけ**効く＝3 台目が現れたらまた尋ねる）。
    相手の公開を止める手立ては無いので、相手側でも設定を切ってもらう必要がある——そう書いてある。
  - 名乗りは**変更が無かった回も更新**する（「この端末は生きている」＝引き継ぎの判断材料）。
  - ⚠️ **名乗りは公開より先に書く**（実機ログ diagnostics-91）。最後に書いていたら、窓が切れる
    間際の回でシャードと名乗りが揃って `upload failed (transport)` になった。名乗りが無いままだと
    次の端末は「誰も名乗っていない」と見て**二重に公開を始める**——この仕組みが防ぎたかったことそのもの。
    まだ自分のものでないときだけ先に書き、最後に「最後に公開できた日」を更新する。
    書けなかったら診断ログに残す（黙って落とさない）。
- **追補 4（1 回の大きさ・実機ログ diagnostics-85）**: 公開 1 回でフットプリントが 637MB・所要 67 秒。
  全シャード（256 個）を作ってから 8 個を選んでいたため。**先に見るシャードを決め、そのシャードの
  写真ぶんだけ**解析を取る形へ逆転した（ピーク 1/32・問い合わせ 49 回 → 8 回）。詳細は `case-studies.md`。
- 関連: `Share/AnalysisPublishPlanning.swift` / `Share/AnalysisOwnership.swift` / `AnalysisPublisher.swift` / `ShareAnalysisFetch.accountAnalysisRoots`
  / `MosaicPhotos/Share/ShareSupport.swift`（`CloudAnalysisPublisher`・`SharedAnalysisImporter`）
  / `NightlyWorkPolicy.swift` / `AnalysisPublishTests`。ADR-112・ADR-183・ADR-199。

## ADR-221 昼の線は 0.40 のまま、名前付き人物の重心を作り直す線を 0.40 → 0.20（実機相当の品質で計測）
- 状態: 採用
- 文脈: ADR-220 で、計測での品質が実機を代表していなかった（シミュレータでは OS の顔品質が取れない）
  と分かり、夜の平均連結へ入れる線を 0.10 にした。品質フロア 0.40 は、ほかに**昼の逐次割り当て**
  （`recordScan`＝これ未満は所属だけ）と**名前付き人物の重心の作り直し**（`FaceSeedBuilder`）で使っていた。
- 計測（`face-accuracy.md` 2026-09-22 昼の線・種の線）: 本物の `FaceStore`（メモリ上）で
  「最初のスキャン → 夜 → 最大 20 人に名前付け →（昼 → 夜）× 4 日」を回した（`QualityFloorEvalTests`・
  実機相当の品質）。
  - **夜の後の全体の F1 は、どの組み合わせでもほぼ同じ**（±0.001）。夜の平均連結が作り直すため。
  - 昼の線を下げると、その日のうちの精度は上がる（PIPA の昼の終わりの F1 平均 0.817 → 0.911）が、
    **名前付き人物の純度が下がったまま戻らない**（LFW 0.914 → 0.891・FG-NET 0.993 → 0.980）。
    名前付き人物のメンバーは夜の作り直しでも動かさない（ADR-132）ので、昼に誤って入った写りの悪い
    顔が夜になっても外れない。
  - 名前付き人物の重心を作り直す線を 0.20 にすると、名前付き人物の純度が上がる（LFW 0.914 → 0.952）。
    FG-NET・PIPA は不変（FG-NET の網羅は 0.694 → 0.685）。0.10 も同じ結果。
- 決定: 昼の線（`dayQualityFloor`）は 0.40 のまま。名前付き人物の重心の線（`seedQualityFloor`）を 0.20。
  2 つの線はストアごとの値にした（計測で差し替えるため）。`clusterRuleVersion` 10 → 11。
- 結果: 昼に下げて得られるのは一時的な精度で、失うのは利用者がいちばん大事にしている名前付き
  アルバムの純度（恒久的）——割に合わない。⚠️ 名前付き人物の「動かさない」決まりは、昼の誤りを
  固定する側にも働く。昼の割り当てに手を入れるときは、名前付き人物への入り方を必ず一緒に測る。
- 関連: `FaceStore.swift`（`dayQualityFloor` / `seedQualityFloor`）・`QualityFloorEvalTests`・
  `SeedQualityFloorTests`・ADR-132/210/220。

## ADR-220 平均連結へ入れる顔の品質の線を 0.40 → 0.10 に下げる（実機の品質で測り直した）
- 状態: 採用
- 文脈: 計測ハーネスはシミュレータで回すが、**シミュレータでは OS の顔品質
  （`VNDetectFaceCaptureQualityRequest`）が取れず、本番コードは nil を 1.0 として扱う**。
  そのため計測での品質は「1.0 × 減点（ぼけ・向き・目閉じ・明るさ）」だけで決まり、1.0 か 0.20 の
  二択に近かった。実機では OS の値が効き、同じ顔の多くが 0.1〜0.4 になる（Mac の Vision で実測:
  PIPA の中央値 0.20・LFW 0.29・FG-NET 0.44）。品質フロア 0.40 の位置は、**実機の品質では一度も
  測られていなかった**。
- 計測（`face-accuracy.md` 2026-09-22 実機相当の品質）: Mac で OS の品質を取り
  （`scripts/os_face_quality.swift`）、キャッシュの「1.0 × 減点」と合成して実機の品質を再現した。
  - 今の線（0.40）のままだと、実機では PIPA の顔の 538 / 802 がどの人物にも入らない（F1 0.545）。
    LFW も F1 0.801。
  - 平均連結へ入れる線を 0.10 にすると、PIPA F1 0.960・LFW 0.980・FG-NET 0.733。純度も上がる
    （PIPA 0.990 → 0.995・LFW 0.891 → 0.986・FG-NET 0.884 → 0.935）。旧モデル（facenet）でも同じ向き。
  - 線を 0（足切り無し）まで下げても結果はほぼ同じ。第2パスの線は結果をほとんど動かさない。
- 決定: 夜の再クラスタで**平均連結に入れる顔の線**（`FaceAgglomeration.Config.inclusionFloor`）を
  0.10 にする（両プロファイル）。これ未満だけを第2パス（所属だけ）へ回す。
  昼の逐次割り当て・種の重心・レビューで尋ねる顔の線（`FaceStore.qualityFloor` 0.40）は**変えない**
  （そこは実機相当の品質で測っていない）。`clusterRuleVersion` 9 → 10。
- 結果: 平均連結は「全員と全員の平均」で測るので、写りの悪い顔が混ざっても 1 枚で山が引きずられない。
  **捨てるより入れたほうが純度も再現率も上がる**（「低品質は捨てずに不確かさとして扱い、極端なものだけ
  外す」という考え方と一致）。代償は、写りの悪い顔だけの小さな人物（断片）が増えること（断片の自動吸収
  ADR-154 と、10 枚以上の表示フロア ADR-125 が受け止める）。FG-NET の最悪の 1 人の純度は 0.552 → 0.487。
  ⚠️ **品質が絡む計測は、以後 `os-quality.json` を使った実機相当の品質で行う**（事例に記録）。
- 関連: `FaceAgglomeration.swift`・`FaceTuning.swift`・`FaceStore+Rebuild.swift`・
  `scripts/os_face_quality.swift`・`FaceAgglomerationEvalTests.inclusionFloorOnDeviceQuality`・
  `InclusionFloorTests`・ADR-53/66/90/217。

## ADR-219 赤ちゃんの顔どうしで撮影日が 3 年を超えて離れていたら、似ていても同じ人にしない
- 状態: **撤回（2026-09-22・同日）**。FairFace のラベルは CC BY 4.0 だが、元写真（Flickr / YFCC-100M）には
  非商用の CC が含まれるため、ユーザーの判断で FairFace 由来のもの（判別器の数値・学習スクリプト・
  埋め込みの取り出しテスト・ライセンス表示）をすべて外した。決まりの仕組み（`FaceAgglomeration.BabyRule`・
  `BabyProbe` の型・単体テスト）は残し、`FaceTuning` では無効（判別器なし）。`clusterRuleVersion` は 9 のまま。
  権利のはっきりした学習材料か、同梱の CLIP による判定が用意できたら、同じ計測で再評価する。
  併せて、同梱している AuraFace-v1（Apache 2.0）をライセンス画面に追加した（表示漏れ）。
- 当初の状態: 採用
- 文脈: 兄弟の赤ちゃん時代の顔はよく似ていて、顔の埋め込みだけでは取り違える。一方、赤ちゃんの顔は
  2〜3 年しか続かず、兄弟は赤ちゃんの時期がずれる（ユーザーの提案）。ADR-60 の「年齢帯で全員を分ける」
  は網羅が半減して不採用だったが、今回は**まとめることを禁じるだけ**で、対象も赤ちゃんの顔だけ。
- 決定:
  - 夜の再クラスタ（平均連結・ADR-217）で、1 つの人物に入る赤ちゃんの顔の撮影日の幅が 3 年を超える
    組み合わせを作らない（小さな山の段階も、山どうしをまとめる段階も、種＝名前付きの人物にも）。
  - 赤ちゃんかどうかは、**既にある顔の埋め込み**に線形判別器（数値 513 個）を当てて決める＝追加の推論なし
    （`BabyProbe`）。判別器は FairFace（CC BY 4.0・年齢区分「0〜2 歳」）を本番と同じ処理で埋め込んで学習
    （`scripts/train_baby_probe.py`・`FairFaceEmbeddingTests`）。アプリには数値だけを入れ、画像は含まない。
    ライセンス画面に FairFace の出典を表示。
  - しきい値は FairFace の検証用分割で「赤ちゃんでない顔の取り違え 10%」の位置（検証用分割は取り違え
    やすい 3〜9 歳が大半なので、実際の取り違えはずっと少ない: FG-NET の 10 歳以上で 0%）。
  - 判別器は AuraFace の埋め込み空間に固有なので `FaceTuning.arcFace` にだけ持たせる（facenet は無し）。
  - `clusterRuleVersion` 9 → 10。
- 計測（`face-accuracy.md` 2026-09-22 赤ちゃん）: FG-NET（学習に不使用・架空の誕生年で家族を作る）で、
  別人の山に入った 0〜2 歳の顔 29 → 25 / 107、純度 0.922 → 0.931、最悪の人物の純度 0.539 → 0.563、
  純度 0.8 未満の人物 6 → 4、再現率は下がらない（0.603 → 0.608）。PIPA（大人中心・本物の撮影日）は全指標が不変。
- 結果（トレードオフ）: 効果は小さめ。HAC だけで赤ちゃんの取り違えが既に減っていることと、判別器が
  FG-NET の 0〜2 歳を 55% しか見つけないことが上限を決めている。本物の兄弟（赤の他人より似ている）では
  効果が大きいと予想するが、撮影日付きの兄弟データが無く測れない＝実機で確かめる。
  FairFace の元写真は Flickr の CC 写真（写真ごとに CC の種類が違い、非商用も含む）。アプリに入るのは
  学習した数値だけ。
- 関連: `BabyProbe.swift`・`BabyProbe+AuraFace.swift`（生成物）・`FaceAgglomeration.BabyRule`・
  `FaceAgglomerationTests`（決まりを外すと落ちることを確認）・`FaceAgglomerationEvalTests.babyRule`・
  `LicensesView`・ADR-60/61/217/218。

## ADR-218 クラウドの顔の撮影日は「EXIF の撮影日時」だけを使う（アップロード時刻を混ぜない）
- 状態: 採用
- 文脈: 顔の撮影日（時期グループ・ADR-61、赤ちゃんの時期の決まり）は、クラウド写真では
  空のままだった（`FacePerceptionAdapter` の「クラウドは当面 nil」）。クラウドの一覧には撮影日の列
  （`CachedDropboxItem.captureDate`）があるが、**EXIF が取れればその値、取れなければアップロード時刻
  （`client_modified`）のまま**で、値を見てもどちらか区別できない。アップロード時刻を撮影日に使うと、
  数年ずれた日付で「赤ちゃん同士が離れている」と判定してしまう。
  - 顔の解析でダウンロードしている 1024px 画像からは読めない: Dropbox の縮小画像
    （`get_thumbnail_batch`）は元写真のメタデータを写さない（公式の記述）。
- 決定:
  - キャッシュに **EXIF の撮影日時だけ**の列（`exifCaptureDate`）と、その問い合わせ済みの印
    （`exifProbedAt`）を足す。値は `get_metadata` の `media_info.time_taken`＝Dropbox が元写真の
    EXIF から読んだ値。アップロード時刻は決して入れない。
  - この列より前に問い合わせた写真は、EXIF だったか分からないので**一度だけ問い合わせ直す**
    （既存の少しずつ問い合わせる仕組み `fillMissingCaptureDates` の対象を `exifProbedAt == nil` に）。
  - 顔の解析は束の頭でクラウドの path をまとめて引き、撮影日を載せる（ネットには出ない）。
  - 記録済みの顔は、夜の再クラスタの判断の前（`rebuildClustersIfNeeded`）に、撮影日が空の
    クラウドの顔をまとめて埋める（読み出し 2 回・ADR-119）。
- 結果: クラウドの顔にも撮影日が入る（EXIF の無い写真は空のまま＝推し量らない）。
  代償として、既存ユーザーは約 6.8 万枚の `get_metadata` をもう一度だけ裏で問い合わせる
  （1 枚 1 往復・4 並行・背景の処理枠でのみ）。
- 関連: `CachedDropboxItem.swift`・`DropboxCacheStore.exifCaptureDates`・
  `FacePerceptionAdapter`（`cloudCaptureDates`）・`FaceStore+CaptureDates.swift`・
  `CaptureDateProbeTests`・`CloudCaptureDateFillTests`・ADR-61/201。

## ADR-217 夜間の再クラスタを「平均連結」に替える（1 枚ずつ最寄りへ入れる逐次方式をやめる）
- 状態: 採用
- 文脈: 夜間の再クラスタは、顔を品質順に 1 枚ずつ「いちばん似ている山」へ入れる逐次方式だった。
  山に顔が入るたびに重心が動くので、父親似の息子の顔が 1 枚入ると重心が息子寄りになり、次の息子の
  顔が入りやすくなる（混入が混入を呼ぶ・ADR-130「自分のアルバムが丸ごと娘になった」の形）。
  入れる順番でも結果が変わる。クラスタリング側の定数は ADR-56〜69 で絞り切っていた。
- 決定: 名前・確認・代表写真・束ねのある人物（種）は `FaceSeedBuilder` が固定したまま、**残りの顔を
  平均連結の階層的クラスタリング（HAC）でまとめる**（`FaceAgglomeration`）。山どうしの近さは
  「山 A の全員と山 B の全員の、すべての組の類似の平均」で、いちばん近い組から順にまとめる。
  - 数万枚への対応: (1) 先に「山の全員との平均類似 ≥ 0.55」の小さな山を作る、(2) 各山は近い相手
    30 個だけを覚える。平均連結の値は単位ベクトルの和の内積 ÷ 件数の積で 1 回で出る。
    LFW で「1 枚ずつから全部の組」と結果が完全一致・時間 1/200、近い相手だけでも 100% 一致。
  - 決まり: 種どうしはまとめない（ADR-153）・同じ写真の顔を含む山どうしはまとめない（ADR-54）・
    負例は `FaceClustering.place` と同じ式で拒否（ADR-140）。
  - 線はプロファイル（`FaceTuning.agglomeration`）: arcface は小さな山 0.55・まとめる線 0.40、
    facenet は 0.65・0.60。**校正後のしきい値は使わない**（昼の逐次割り当てにだけ効く）。
  - 第2パス（写りの悪い顔を所属だけ）は従来どおり、まとめ上がった人物に対して行う。
  - `clusterRuleVersion` 8 → 9（既存ユーザーも次の夜に作り直される）。再クラスタを
    `HeavyLoad.span("faces.rebuild")` で申告（ADR-122）。
- 計測（`face-accuracy.md` 2026-09-22）: FG-NET / LFW / PIPA のすべてで、純度・最悪の人物の純度・
  下位 25% の純度が上がった。LFW は F1 0.892 → 0.981、PIPA は 0.880 → 0.937。
- 結果（トレードオフ）: **成長写真の子供は分かれやすくなる**（FG-NET の再現率 0.710 → 0.603）。
  分かれた人物はレビュー（「同じ人ですか？」）と束ねで直せるが、混ざった人物は気づきにくく
  直しにくい——混ざる側を減らす方針（ADR-153）に沿う。
  メモリは小さな山の数 × 512 次元の和（2 万山で 40MB）＋近い相手探しの一時領域（同 40MB ×2）。
  実機の所要・ピークは `device-verification.md` の E で確かめる。
- 次: 赤ちゃんの顔どうしで撮影日が数年離れていたら別人とする決まり（兄弟の取り違え対策）。
  FG-NET の上限計測で取り違えが半分以下になったが、赤ちゃん判定の学習データ（FG-NET は学術用途）と
  クラウド写真の撮影日が課題（検討中）。
- 関連: `FaceAgglomeration.swift`・`FaceStore+Rebuild.swift`・`FaceTuning.swift`・
  `FaceAgglomerationTests` / `FaceAgglomerationEvalTests`・ADR-54/130/140/153。

## ADR-216 重心の凍結（ADR-210 の後半）を撤回する — FG-NET / LFW で一度も発動しなかった
- 状態: 採用（ADR-210 のうち「散らばりで重心の成長を止める」を廃止。記録の修正・整合検査・散らばりの記録は残す）
- 文脈: ADR-210 は「前夜に測った散らばり（重心からの距離の品質重み付き中央値）が `1 − しきい値`
  を超えた人物は、日中の追加スキャンで重心を動かさない」を入れた。効果は未計測だった。
  名前の無い人物は夜の再クラスタで一から作り直されるので、効くのは昼の追加スキャンのあいだだけ。
- 計測: 本番の流れ（夜に全顔から作り直して散らばりを測る → 昼に追加スキャン）を再現した
  （ハーネスはコミット 9f8a963 の `CentroidFreezeEvalTests`・`face-accuracy.md` の 2026-09-21 節）。
  - **本番のバー（0.65）では一度も凍結しない**。散らばりの最大は FG-NET（年齢順）0.383・
    LFW 0.471。顔は重心との類似がしきい値以上でしか入らないので、中央値がバーを超えることは
    構造的にほぼ起きない。凍結あり／なしの F1 は全条件で完全に同一。
  - バーを 0.15〜0.40 に下げても（作り直さない最悪の場面）: FG-NET 0.6832 → 0.6863（+0.003）、
    LFW 0.8373 → 0.8297〜0.8333（**悪化**）。
- 決定: 凍結（`FaceClustering.Cluster.centroidFrozen`・`FaceClusterHealth.shouldFreezeCentroid`・
  スキャン時の復元・診断ログ・判定の内訳の項目）を撤去する。散らばりの測定と記録
  （`PersonCluster.spread`）は、事後監査を尋ねる順番（ADR-69）と判定の内訳で使うので残す。
- 結果: 「足したか」（`Assignment.contributed`）は、凍結が無くなったので place では常に
  「割り当てた＝足した」になる。記録の事実性（ADR-210 前半）はそのまま成り立つ。
  **しきい値から導いたバーは、その値に分布が届くかを先に確かめる**（今回は届かないことが
  式から読めた）。
- 関連: ADR-210・`FaceClusterHealth.swift`・`face-accuracy.md`（2026-09-21 重心の凍結）。

## ADR-215 連写・服装による連結（ADR-211/212）を撤回する — PIPA の計測で効果が無かった
- 状態: 採用（ADR-211・ADR-212 を廃止）
- 文脈: ADR-211/212 は「顔のデータセット（FG-NET / LFW）には連写も服装も無いので測れない。
  実機の付け替え率で決める」として入れた。しかし**連写・服装・撮影時刻・人物 ID を全部持つ
  データセットが存在した**——PIPA（People In Photo Albums・Flickr の個人アルバム・
  頭の矩形＋人物 ID、撮影時刻は Oh et al. ICCV'15 の配布物）。本物の連写（同じアルバムで
  3 秒以内）が全体で 4,040 組ある。
- 計測: PIPA のうち連写の多いアルバムから 826 枚（顔 959・正解と対応 802）。本番の再クラスタと
  同じ順序（品質順の割り当て → 第2パス → 連写 → 服装）を再現して採点した
  （`face-accuracy.md` の 2026-09-21 PIPA 節）。
  - 連写（本番設定 3 秒・IoU 0.5）: 繋いだ顔 **1・正解 0**。掃引しても最良 28.6%。
  - 服装（本番設定 バー 0.90）: 繋いだ顔 **5・正解 0**。バーを 0.70〜0.95 で掃引しても最良 55.6%。
  - 比較の基準である第2パスは **92.5%**（300 件）。
  - 全体の B-Cubed F1: 連結なし **0.8796** → 両方 **0.8738**（下がる）。
  - 上限も小さい: 第2パスの後に人物へ入っていない正解付きの顔は 111/802 で、そのうち
    連結の対象（重心を作った顔と同じ連写・場面に居る）はごく一部。完璧に当てても 1% 未満。
  - 胴体の埋め込み自体は同じ場面の同じ人／別人を AUC 0.807 で分けるが、**第2パスが
    取りこぼした難しい顔**に限ると顔の類似が低すぎ、服の一致だけでは別人を弾けない。
- 決定: 連写（`TemporalLinking`）・服装（`TorsoLinking` / `TorsoRegion` / 胴体の CLIP 推論 /
  `FaceTuning.torsoFaceFloor` / Developer Options の 2 つのトグル）を撤去する。
  ADR-210（重心の記録）・ADR-213（統合の帯）・ADR-214（代表顔）は計測と無関係なので残す。
  - `DetectedFace.torsoEmbedding` は台帳（FacesV1）の列なので**列は残し**（ADR-186）、
    再クラスタが残った値を空にして容量を返す。`linkSource`（face / secondPass / absorb / user）は
    第2パスの付け替え率を見るのに使えるので残す（temporal / torso の値だけ消す）。
- 結果: 夜間の処理枠から「顔 1 つにつき CLIP 1 回」の上乗せ（約 3 割）が消える。
  「データセットで測れない」と書く前に、**連写・場面・服装を持つデータセットを探す**
  （今回は探せば見つかった）。ハーネスは再現用にコミット 8a7c2d3 に残した。
- 関連: `scripts/fetch_pipa_eval.py`・コミット 8a7c2d3（`PIPALinkingEvalTests`）・
  `face-accuracy.md`（2026-09-21 PIPA）・`case-studies.md`・ADR-211/212。

## ADR-214 代表顔は「見た目の良さ」だけで選ばない（重心から遠い顔を候補から外す）
- 状態: 採用
- 文脈: 代表顔は**アンカー**になる（ADR-130/132）。名前を付けた人物の代表写真は
  再クラスタで動かない錨として扱われ、`FaceSeedBuilder` が種の向きをそこから作る。
  ところが選び方は「品質 ＋ 笑顔 +0.3 ＋ 顔の大きさ +0.2」＝**見た目の良さだけ**だった。
  混ざり込んだ別人の顔が、たまたま大きく・笑顔で・綺麗に写っていれば代表になる。
  そうなると ADR-130 の実害（「私」のアルバムが丸ごと娘になった）が**自己増幅する**——
  誤った錨が毎晩その別人を引き寄せる。
- 決定: 候補を**重心との類似が校正後しきい値以上の顔**に絞ってから、従来のスコアで選ぶ。
  ⚠️ 絞って 0 人になったら**絞らない**（代表が消えると人物が一覧から落ちる）。
  同点のタイブレーク（faceID の小さい方・ADR-139）はそのまま。
- 結果: 「この人物らしくない顔」が錨になる経路が閉じた。見た目のスコア自体は変えていないので、
  健全な人物での選ばれ方は変わらない。
- 関連: `FaceStore.bestCoverFace` / `CoverFaceSelectionTests` / ADR-130/132/139。

## ADR-213 「どこまで機械がやってよいか」を 1 つの表にする
- 状態: 採用
- 文脈: 似ている度合いに応じた行動（自動で寄せる／チェック済みで見せる／尋ねる／無視）の
  目盛りが 5 か所に散っていた——`FaceTuning.autoAbsorbBar` / `autoSuggestBar` /
  `mergeCandidateFloor`、`FaceStore+Absorb` の形の定数（断片の上限 2 枚・吸収先 11 枚・
  1 位 2 位の差 0.05・1 回の上限 500）、`FaceStore.coOccurrenceNotSame`。
  どれも**同じ 1 本の軸の上の目盛り**なのに、帯として通して見られる場所が無かった。
- 決定: `MergePolicy`（純ロジック）に集める。`Bars`（absorb / preselect / askFloor）と
  `action(similarity:isFragmentToPerson:bars:)`、および形の定数。
  **値は 1 つも動かしていない**（`face-accuracy.md` の実測で決まっているもの）。
  呼び出し側の名前（`FaceStore.absorbMaxPhotos` 等）は別名として残す。
- 結果: 「人物どうしは、どれだけ似ていても自動で結合しない」（ADR-153）が
  **表の形で 1 か所に書いてある**ようになり、テストで直接固定できるようになった。
- 関連: `MergePolicy.swift` / `MergePolicyTests` / ADR-153/154/155/162/150。

## ADR-212 服装（胴体）を 2 つ目の手がかりにする（同じ場面の中だけ・所属だけ）
- 状態: 廃止（→ADR-215・PIPA で繋いだ顔の正解率 0%・B-Cubed F1 低下。2026-09-21 撤去）
- 文脈: 実ライブラリでは顔の**約 48% が品質フロア未満**で、埋め込みだけを頼りにしている。
  横を向いた・ぶれた・小さく写った顔は、どれだけ正しく埋め込んでも情報が足りない
  （第2パスの正解率は FG-NET 実測で 88%＝表示される顔の 12% は別人のアルバムに載る）。
  一方その写真には**同じ人を指すもう 1 つの証拠**がある——同じ場面で同じ服を着ている。
- 決定: 顔の下（幅 2 倍・高さ 2.5 倍）を切り出して**同梱の CLIP** で埋め込み
  （新しいモデルは足さない）、次の条件を**全部**満たすときだけ所属を足す:
  1. 同じ場面（既定 1 時間以内で連なる写真）の中だけ。日をまたげば服は別物。
  2. 両方が胴体を持つときだけ。片方でも欠ければ無音で顔だけの判断に戻る。
  3. **胴体だけで人物を作らない**。繋ぎ先はその場面で**顔が**確立している人物に限る
     （お揃いの服・制服で人物が丸ごと混ざるのを防ぐ）。
  4. **重心には決して足さない**（`sum`/`count` 不変）。間違いは 1 枚外せば直る。
  5. 1 位と 2 位が紛らわしければ繋がない（ADR-57 と同じ考え方）。
  - **尺度の混ぜ方**: 顔（ArcFace）と胴体（CLIP）はコサインの分布がまったく違う
    （CLIP は無関係な画像でも 0.5 を超える）。そこで**順位は重み付き和（顔 7 : 胴体 3）、
    可否は相対差**で決める。1 つの顔について複数の人物を比べる限り、どちらの尺度も
    その顔の中では一定なので重み付き和は正しい順位づけになり、採否は尺度に依らない。
    絶対値を使うのは「胴体が実質同じ写りか」を見る 1 か所だけ（暫定 0.90）。
- 結果: 顔が使えない写真を、人物の基準を汚さずに拾えるようになった。
  ⚠️ **計測できない部分がある**。FG-NET / LFW は顔のクロップだけで、場面も服装も持たない
  ——ここはデータセットでは決められない。そこで (1) バーを保守的に高く置き、
  (2) `linkSource`（face / secondPass / temporal / torso / absorb / user）を行と修正記録の
  両方に残し、(3) **根拠別の付け替え率**と (4) バー別の分布を診断ログに出す。
  断片吸収のバーを実機の分布で決めた ADR-162 と同じ手順を踏む。
  ⚠️ 代償は夜間の推論が顔 1 つにつき CLIP 1 回ぶん増えること（顔モデルは 1 顔 3 クロップ）。
  Developer Options で降ろせるようにした（設定 1 つの代償は表の行 1 つ・ADR-197）。
- 関連: `TorsoLinking.swift` / `TorsoRegion.swift` / `FaceStore+Linking.swift` /
  `FacePerceptionAdapter`（胴体クロップ）/ `DetectedFace.torsoEmbedding` / `linkSource` /
  ADR-66/70/140/162/197/211。

## ADR-211 連写は「同じ位置」で繋ぐ（同一写真 cannot-link を連写全体へ広げる）
- 状態: 廃止（→ADR-215・PIPA で繋いだ顔 1 件・正解 0。2026-09-21 撤去）
- 文脈: 連写は数秒のあいだ同じ人が同じ位置に写るが、**そのうち何枚かは必ずぶれる**。
  ぶれた顔は品質フロア未満になり、埋め込みだけを頼りに最寄り人物へ賭ける第2パス
  （ADR-66）に回される。しかし連写には埋め込みより強い証拠がある——
  **同じ写真群の同じ位置に写っている**。埋め込みが当てにならない顔ほど、この証拠は確実。
- 決定: 撮影日が 3 秒以内で連なる写真を 1 つの連写とみなし、矩形の IoU 0.5 以上で
  写真をまたいだ**トラック**を作る。トラックの中で**重心を作った顔**（membership だけの顔は
  証拠にしない＝推測が推測を裏づける循環を断つ）が 1 つの人物で一致していれば、
  同じトラックの未割当の顔にその人物の**所属だけ**を足す。
  - **cannot-link の連鎖**: 「1 枚の写真に同じ人は 1 回しか写らない」（ADR-54）は
    写真ごとの制約だったが、**1 枚でも居合わせた 2 本のトラックは連写のあいだずっと別人**。
    居合わせた 2 本が同じ人物を指したら、どちらかが必ず間違っているので**両方の証拠を捨てる**。
- 結果: モデルを 1 つも足さずに（位置と時刻だけで）、顔が使えない写真を拾えるようになった。
  足すだけ・既存の割り当ては動かさない・重心を汚さないので、入れても既存の判定は 1 つも変わらない。
  ⚠️ こちらもデータセットでは測れない（FG-NET / LFW に連写は無い）。`linkSource` で
  実機の付け替え率を追う（ADR-212 と同じ）。
- 関連: `TemporalLinking.swift` / `FaceStore+Linking.swift` / `TemporalLinkingTests` /
  ADR-54/66/212。

## ADR-210 「重心に寄与したか」は事実の記録にする／散らばりで重心の成長を止める
- 状態: 一部廃止（「散らばりで重心の成長を止める」は FG-NET / LFW で一度も発動せず撤回→ADR-216。記録の修正・整合検査・散らばりの記録は採用のまま）。当初: 採用（`unresolved-problems.md` の P1「再クラスタ後の寄与の記録が計算と食い違う」を解決）
- 文脈: 重心（`sum`/`count`）は**増分で更新される**——スキャンで足し、付け替えで引き、
  再クラスタで作り直す。増分更新は一度ずれると自力では戻らず、しかも**その場では何も起きない**。
  実際に次の食い違いが入っていた:
  - 種の計算（`FaceSeedBuilder`）は**品質フロア以上の顔だけ**を `sum` へ足すのに、
    書き戻し（`FaceStore+Rebuild`）は「**留めた顔はすべて寄与した**」と記録していた。
  - さらに `FaceRef.contributes` が**記録された `contributesToCentroid` を優先**して読んでおり、
    判断が自分の過去の記録を参照する**自己参照**になっていた。
  実ライブラリでは顔の約半数がフロア未満なので、30 枚の人物で `count == 4` なのに
  30 行が「寄与した」と言う状態になる。あとで数枚外すと `count` が尽き、**人物が丸ごと消える**。
  ⚠️ 回帰テストで確認: 低品質の顔を 8 枚外しただけで `count` が **6 → 0** になった。
- 決定:
  1. `contributesToCentroid` は**事実の記録**にする。書いてよいのは実際に `sum` へ足した
     当人だけで、後から品質で推し量らない。`FaceClustering.place` が
     `Assignment(clusterID:contributed:)` を返し、`FaceSeedBuilder` は実際に足した faceID を返す。
     再クラスタの材料の選び方は最初の割り当てと同じ「品質フロア以上か」だけで決める。
  2. **整合検査**（`FaceCentroidAudit`）を再クラスタの**前**に挟む。全顔はもう手元にあるので
     追加の読み出しは要らない。作り直したあとでは、ずれていたことに誰も気づけなくなる。
  3. **散らばり**（`FaceClusterHealth.spread`＝重心からの距離の品質重み付き中央値）を
     夜間に測って記録し、(a) 事後監査で尋ねる順番に使い、(b) 散らばりすぎた人物は
     **重心の成長を止める**（所属だけ受け取る＝`centroidFrozen`）、(c) 判定の内訳に出す。
     バーは **`1 − しきい値`**（＝過半のメンバーが今日のしきい値ではもう入らない状態）で、
     校正にもモデルのプロファイルにも自動で追随する——**新しい目盛りを 1 つも増やさない**。
  4. 種を作れなかったクラスタの `pinned` を取り消す（孤児を作ってから
     `repairOrphanFaces` で直す形をやめる・`unresolved-problems.md` の既知の粗さ）。
- 結果: 記録と実体が食い違ったまま毎晩受け継がれる状態が構造的に作れなくなった。
  ⚠️ 平均ではなく**中央値**なのが要点——平均だと「横顔が 1 枚入っている健全な人物」と
  「半分が別人のクラスタ」が同じ値になる。
  ⚠️ 凍結は顔を**弾かない**（所属は付ける）。ADR-59（外れ値を抜く）は成長データで正当な顔まで
  落として不採用になったが、ここは網羅を 1 枚も減らさない。
- 関連: `FaceCentroidAudit.swift` / `FaceClusterHealth.swift` / `FaceStore+Linking.swift` /
  `FaceSeedBuilder.swift` / `FaceStore+Rebuild.swift` / `CentroidRecordTests` /
  `FaceInvariantTests` / ADR-45/58/59/66/69/130/132/135/141/187/198。

## ADR-209 共有の反映は「記録と実在を合わせる」から「望ましい姿との差分」へ
- 状態: 採用（ADR-112 / ADR-172 / ADR-183 の実装を置換）
- 文脈: 家族共有は**サーバーの無い分散同期**——2 台の端末が Dropbox という
  ただのファイル置き場を介して同期する。解析データ（シャード）は ADR-183 で
  **状態を持たない**形（手元と遠隔の `content_hash` を比べるだけ）に整理済みだったが、
  **写真側だけは「記録が真実」のまま**だった:
  4 状態の状態機械（`pending`/`waitingBackup`/`copied`/`failed`）＋ `sharedPath` ＋
  `sharedContentHash` を持ち、記録と実在が食い違うたびに直し方を足してきた——
  採用（宛先が既に在れば記録だけ更新）・自己修復・ドリフト検知・`autorename` 残骸の掃除・
  **墓標 2 種**（消したフォルダ／消す予定だったファイルを覚えておく）・
  変更操作と反映の排他（反映をキャンセルして 8 秒待ち、止まらなければ操作を中止）。
  食い違いが生まれる根は 2 つ——Dropbox の `copy_batch` は**クライアントが諦めても
  サーバー側で完走する**（ジョブ取り消しの API が無い）ことと、
  **宛先名をアイテムごとに採番して覚えていた**こと。
  ⚠️ 不変条件のランダム化テストを張ったところ、**8 種の操作列のうち 3 種が壊れた**
  （孤児が残る／メンバーの写真が無い／落ち着いた反映が毎回書き込む）。
  どれも既存のテストでは出ていなかった。
- 決定: **宛先名を中身から決め**、反映を差分にする。
  ```
  宛先名 = "<元の名前の幹>--<SHA256(refKey|content_hash) の先頭 8 桁>.<拡張子>"
  望ましい = { 宛先名(メンバー) : 元が解決できるメンバー }
  コピー   = 望ましい − 実在        削除 = 実在 − 望ましい
  ```
  同じ写真は必ず同じ名前になるので、**どこへコピーしたかを覚える必要が無い**。
  フォルダも同じ規則で差分を取る（どのセットも持たないフォルダは消す）。
- 結果: 上の仕組みが**すべて消えた**。`ShareItem` は「このセットにこの写真が入っている」
  だけを持つ。変更操作（メンバー更新・削除・単枚除外）は**記録を書き換えるだけ**になり、
  Dropbox 側は次の反映が片付ける。3 件の欠陥は差分方式で解消し、20 種の操作列で緑。
  行数は 3,880 → 3,444、削除 1,365 行に対し追加 922 行。
- トレードオフ（どれも意図して受け入れた）:
  - **共有フォルダのファイル名が元と違う**（`IMG_1234--3f9a2c1d.jpg`）。受信側アプリの
    表示には出ないが、家族が Dropbox アプリで直接見ると印が見える。
  - **セットフォルダ直下は完全に管理される**——望ましい集合に無いファイルは名前の形に
    関係なく消す。最初は「このアプリが置いた名前だけ消す」安全弁にしたが、
    **旧方式のコピーが永久に残って家族に同じ写真が 2 枚見えた**。重複の害の方が大きい。
    守るのは**サブフォルダ**（`.mosaic-share/` と家族が作ったもの）。
  - **一時的な食い違いを許す**。反映中にメンバーが変わっても最悪「不要な 1 枚を作って
    次で消す」だけ。孤児が永久に残らないので、反映を止めて待つ必要が無い。
  - 移行の専用コードは**書いていない**。旧方式の名前は望ましくない側に回るので、
    次の反映が消して新しい名前でコピーし直す（サーバーサイドコピーなので転送は無い）。
- ⚠️ **印は digest を切り詰めない**（16 進 64 桁・全部）。最初は 8 桁（32 ビット）にしていたが、
  1 セット 12,941 枚の実績に対して誕生日問題で**約 1.9% が衝突**する——衝突すると
  辞書の代入で片方が黙って落ち、**家族に届かない写真が誰にも気づかれずに生まれる**。
  桁を伸ばして確率を下げる（＋衝突時に印を伸ばす）案も書いたが、
  **全部付けて「衝突」という概念ごと消す**方が単純で安全（利用者の判断）。
  衝突しないと決まれば、それを解く仕組みも、落ちた写真を記録する仕組みも要らない。
  代償はファイル名が長くなること——幹は 80 文字までに切り、Dropbox のパス長 260 に収める。
- ⚠️ **順序: コピー → ファイルの掃除 → フォルダの掃除**。フォルダの掃除を先にやると、
  **コピー元が掃除対象のフォルダに在る**場合に元を消してからコピーすることになり、
  コピーが永久に失敗する（クラウド写真は共有ルートの中にも在り得る）。
- ⚠️ 解析データは「コピーを**投げた**写真」ではなく「**成功した**写真」のぶんだけ作る
  （レビュー 1 周目）。失敗した宛先を混ぜると、受信側に突合できないエントリが届き、
  次の反映でシャードを上げ直すことにもなる。`copy_batch` の応答を見て数える。
- ⚠️ 「バックアップ待ち」は**共有メンバー − バックアップ済み**で、差し引きは
  **呼び出し側（`BackupRunner`）が行う**（ADR-119）。ストア側で求めると
  バックアップ記録の全件（数万行）が要るが、呼び出し側は同じ集合を既に持っている
  （`computePending` の `doneIDs`）——1 回のバックアップで同じ数万行を 2 度読むことになる。
- ⚠️ メンバー数は**1 回の反映で 1 回だけ数える**（ADR-119・レビュー 2 周目）。
  進捗を出すために `refresh()` をセットごとに呼んでおり、その中で毎回全メンバーを
  射影していた——1 回の反映で「メンバー総数 × セット数」行を触る。
  数えた結果を使い回す。回数は `share.itemTotals` で見られる（規模テスト付き）。
- ⚠️ **セットが 1 つも無い状態を「失敗」にしない**（レビュー 3 周目）。共有ルートは
  セットができて初めて作るので、提供を ON にしただけの利用者は一覧が `path/not_found` に
  なる——これをエラーにすると、まだ何も共有していない人の画面に赤字が出たまま消えない。
- ⚠️ 安全弁は 2 つだけ残す: **一覧が取れなかった回は何もしない**（実在が分からないまま
  動くのが最悪）。**望ましい集合が空なのにメンバーが居る回は消さない**（元の解決に
  失敗した回に共有フォルダを空にしない。メンバーも 0 なら空が正しいので消す）。
- ⚠️ 併せて直したもの: `syncNow()` が**走行中の呼び出しを待たずに返して**いた。
  「今すぐ反映」を押しても夜間の反映が走っていると何もせず戻る（呼び手は終わったと思う）。
  走行中の回を待つようにした——テストも同じ形で騙されていた。
- 関連: `SharePlanning.swift` / `ShareNaming.swift` / `ShareSyncEngine+Sync.swift` /
  `ShareModels.swift` / `ShareInvariantTests.swift` / ADR-112 / ADR-172 / ADR-183。

## ADR-208 リファクタの行き先は「測り直してから」決める——可変状態の数え方を直した
- 状態: 採用
- 文脈: 「どこを直すか」は複雑度スコアボード（`complexity-scoreboard.md`）を正本にしている。
  その最重要指標「可変状態の数」（構造系で重み 13＝最大）が、**行頭の `var` を無差別に
  数えていた**。つまり次の 3 つが同じ列に入っていた。
  - 型が持ち越す**格納プロパティ**（本来数えたいもの）
  - **関数内のローカル変数**（手続きの中で閉じている）
  - **計算プロパティ**（由来が 1 つに決まる派生値＝**状態ではない**）
  実測すると歪みは小さくなかった——顔クラスタ後処理の 166 は **57%（95 個）がローカル変数**、
  解析状況/ブーストの 43 は **47%（20 個）が計算プロパティ**。順位そのものが動く。
  しかも計算プロパティは**状態を減らす書き方**なので、加点すると**逆向きの誘導**になっていた。
- 決定: 「状態」は**型のスコープに置かれた格納プロパティ**だけを数える
  （`didSet`/`willSet` 付きも含む）。ローカル変数は数えない——手続きの長さの代理指標としては
  意味があるが、それは `maxBranches` が既に見ている。
  ⚠️ Codable の DTO フィールドは**数えたまま**にする（型が持ち越す値ではある）。
- 結果: 構造リスクの順位が変わり、実際に直す価値のある場所（分岐 39〜46 の巨大関数）へ
  向けられるようになった。トレードオフは第 3 回までの「状態」列と**直接比較できない**こと
  （台帳にその旨を明記した）。
- ⚠️ 追記（同日・レビュー 2 周目）: **この修正自体にもバグがあった**。折り返した
  関数シグネチャ（`func f(a: Int,` / `        b: Int) -> T {`）を跨げず、本体の
  ローカル変数を数えていた——顔クラスタ後処理を 63 と報告していた（正しくは 13）。
  **`--selftest` を足した**（`python3 scripts/complexity_scoreboard.py --selftest`）。
  指標を変えるときは、まず「壊れていたときの入力」を 1 件足すこと。
- 教訓: **地図を信じて歩く前に、地図が正しいか測る。** 重み 13 の指標が 3 つの別物を
  数えていたので、これまでの「次にどこを直すか」の判断材料が歪んでいた。
  計測の道具も、コードと同じく検証の対象にする。
- 関連: `scripts/complexity_scoreboard.py`（`stored_state`）/ `complexity-scoreboard.md` /
  ADR-198（同じ制約で構造だけを動かした先例）。

## ADR-207 顔の残作業は「スキャンしていなくても答えられる値」にする
- 状態: 採用
- 文脈: 「すべて解析済みです」が嘘になり得た。`PeopleEngine.remaining` は
  **スキャン中しか意味を持たない**（`FaceTagger` の `defer` が終わりに必ず `onProgress(0)`
  を返し、スキャンが止まると 0 に戻る）。つまり「全部終わった」と「譲って途中で畳んだ」と
  「そもそも始められなかった」が**すべて同じ 0**。読み手は 3 つとも「終わった」と解釈する。
  - ブースト（「今すぐ解析」）は顔の残作業を抱えたまま完了表示を出し、
    OS へ `setTaskCompleted(success: true)` を返していた。
  - 夜間の窓（`gatherInputs`）も同じ値を読む。`kick` は `.background` の Task を起こすだけで
    `remaining` が書かれるのはスキャンの `onProgress` なので、**新しい窓では常に 0**
    ——ADR-163 の「解析を起こしてから顔の残数を測る」修正が効いていなかった。
    埋め込みが 0 で顔だけ残っている窓にアルバム生成が入り、diagnostics-72 の共倒れが再発し得た。
  - さらに、回線が許可されないときクラウド分は `todo` から**外れる**ので、Wi-Fi の無い端末で
    端末内写真を配り終えると `todo` が空になる。クラウドの顔は残っているのに「もう無い」。
- 決定: 「この実行の残り」と「本当の残作業」を**別の事実として分ける**。
  - `onProgress` は従来どおり「この実行の残り」（進捗バー・`drainUntilIdle` が読む。
    ここに回線待ちのぶんを混ぜると、Wi-Fi の無い夜に窓が畳めなくなる）。
  - `onBacklog(todo:deferred:)` を足し、**畳んだ時点の本当の残り**＋**回線待ちで外した
    クラウド分**を返す。`defer` の 0 固定をやめる。
  - `PeopleEngine.faceBacklog: Int?` が最後に測った値を保つ（**止まっても 0 に戻さない**）。
    nil＝この起動でまだ測っていない。
  - `measureBacklogIfUnknown(candidateRefKeys:)` で**スキャンせずに測れる**ようにし、
    駆動役がスキャンを起こせなかった回に呼ぶ（走査済みの refKey を 1 回引くので
    **まだ測っていないときだけ**）。
  - 完了判定（`AnalysisSession`）と夜間の窓（`gatherInputs`）はこちらを読む。
- 結果: 残作業があるのに「すべて解析済み」と出なくなる。夜間の窓が顔の残作業を実際に測れる
  （ADR-163 の意図がようやく成立する）。トレードオフは、起動あたり 1 回、走査済み refKey の
  取得（75k 行）が余分に走り得ること——スキャンが始められなかった回だけで、
  スキャンが走ればその中で同じ取得をしている。
- ⚠️ 混ぜてはいけない: `onProgress` に回線待ちのぶんを足すと `drainUntilIdle` が 0 にならず、
  Wi-Fi の無い夜は窓を期限まで空転させる。**進捗と残作業は別の問いである**。
- ⚠️ 追記（レビュー 7 周目・同日）: **同じ読み違いが 3 か所にあった**——完了判定・
  夜間の枠配分・**停滞検出（ADR-87）**。3 つめは「沈黙を検出しにいく」仕組みなのに、
  入口が `pending > 0` なので顔の停滞を**一度も検出できなかった**（それ自体が沈黙していた）。
  名前が誘っていたので `remaining` → **`scanProgressRemaining`** に改名した
  （「この実行の進捗」であって残作業ではない、と名前で言う）。残作業は `faceBacklog`。
- ⚠️ 追記（レビュー 6 周目・同日）: **完了の「引き金」と「言い分」も別の問い**だった。
  引き金（`isFinished`）は「いま動かせるものが無いか」で、本当の残作業を混ぜると
  **誰も減らせない数が残り続けてブーストが永久に終わらない**（進捗も止まったまま）。
  言い分（`stopReason`）の側で本当の残作業を見る。
  併せて `.blocked([])` が画面で「すべて解析済みです」に落ちる穴を塞いだ
  ——理由の無い未完了を表す道が無かったので、嘘へ丸められていた（`.incomplete`）。
- 関連: `FaceTagger.swift` / `PeopleEngine.swift` / `FaceScanControl.swift` /
  `AnalysisDriver.swift` / `AnalysisSession.swift` / `HeavyWorkScheduler.swift` /
  `FaceBacklogTests.swift` / ADR-163 / ADR-196。

## ADR-206 照合は「いちばん要る端末でいちばん走らない」ようになっていた——順番と、クラウド側にも同じ仕組みを
- 状態: 採用
- 文脈: 週次の照合が 2 つとも成立していなかった。
  - **バックアップ台帳**: 夜間の手順は `backup → … → reconcile` の順で、`.startBackup` は
    投げっぱなしで `isRunning` を立てる。照合は `guard !isBusy`（走行中のアップロードと
    一覧が食い違うため）で門前払いされる。ADR-180 で 1 回あたりの上限を外してから、
    積み残しのある端末は窓いっぱい busy のままになった——つまり
    **照合がいちばん要る端末で、照合だけが永久に走らない**。しかも照合は
    オフロードの緊急停止（ADR-202）の**唯一の発火点**なので、
    「唯一のコピーが消えた」ことに誰も気づけない状態だった。
  - **クラウドのキャッシュ**: 一覧を取り直して掃除する処理は初回同期の中にしかなく、
    差分が「消えた」通知を一度でも取りこぼすとその行は永久に残る（ADR-205 の実機事例）。
    バックアップ台帳には週 1 回の照合があるのに、クラウド側には無かった。
- 決定:
  1. **照合はバックアップより前**。ただし期限の週だけ（`NightlyPlan.Inputs.backupReconcileDue`）。
     52 回に 1 回しか手順が変わらないので、ふだんの窓の性質は保つ。
  2. **失敗した回は間隔を縮める**（7 日 → 1 日）。刻印を始める前に置くのは正しい
     （通信断の端末が毎晩全件一覧を投げないため）が、そのままだと窓が切れた回のぶん
     緊急停止が丸 1 週間遅れる。
  3. **`missingOffloadedPaths` にも `listedAt` を渡す**。`reconcile(remote:listedAt:)` は
     `backedUpAt > listedAt` で走行中のアップロードを守っているのに、緊急停止の判定には
     その保護が無く、`guard !isBusy` に守られているだけだった。並走したら
     **在る写真を「消えた」と読んで誤発動する**。誤発動は自動オフロードを止めるので実害がある。
  4. **クラウドのキャッシュも週 1 回、全件を見直す**。新しい仕掛けは足さない——
     掃除の処理は初回同期の Step 4 が持っており、「最後に全件を見た時刻」は
     `DropboxSyncState.initialSyncCompletedAt` がそのまま持っている。
     期限が来たら初回同期をやり直すだけでよい（`CloudReconcilePolicy.isDue`）。
     判定は `syncOnce`（起動時）と `pollLoop`（起動しっぱなし）で**同じ式**を使う
     （`shouldReconcileNow`）。**前面では走らせない**（`BackgroundYield.allows(.cloudMonolith)`）。
     見直しの回は進捗を画面へ出さない——キャッシュは既に埋まっていて、
     利用者から見れば何も起きていないのが正しい。
- 結果: 照合が実際に走るようになり、緊急停止が機能する。クラウドの取りこぼしが
  最長 7 日で自力回復する。トレードオフは (a) 期限の週はバックアップの開始が
  照合のぶん遅れる、(b) 週 1 回 `list_folder` を全件ぶん引く（`applyDelta` は
  変わっていない行を書かないので、書き込みは増えない）。
- ⚠️ 追記（レビュー 1 周目・同日）: **見直しの失敗で、動いていたポーリングを止めない**。
  初回同期をやり直す形にしたので失敗の作法まで引き継いでいたが、初回同期の失敗
  （まだ何も無い＝止めてエラーを出す）と見直しの失敗（揃っているキャッシュの手入れに
  失敗しただけ）では正しい後始末が違う。既存のカーソルでポーリングへ戻り、
  そのルートはこの起動のあいだ再挑戦しない。**見直しは 1 本ずつ**（ルート 3 本は
  同じ日に期限を迎えるので、一斉に引くと窓を食い潰す）。確保は判定の直後に置く。
  ⚠️ **解放は「区間の終わり」に置く**（クラウドレビューの指摘）。`defer` に置いたら、
  `initialSync` が `return await pollLoop(...)` で末尾呼び出しするせいで
  **pollLoop が返るまで解放されず**、「1 本ずつ」が「最初の 1 本だけ」になっていた。
- ⚠️ 罠: `pollLoop` の期限判定は **longpoll の後**に置く。周の先頭に置くと、`syncOnce` が
  「まだ動かしてよい時間ではない」と判断して poll へ戻した瞬間にまた true を返し、
  通信を 1 度もせずに回り続ける（タイトループ）。
- 関連: `NightlyWorkPolicy.swift` / `HeavyWorkScheduler.swift` / `BackupEngine.swift`
  （`isReconcileDue`）/ `BackupReconcilePolicy.swift`（`stampAfterFailure`）/
  `BackupStore.swift`（`missingOffloadedPaths`）/ `CloudReconcilePolicy.swift` /
  `DropboxSyncEngine.swift` / ADR-166 / ADR-180 / ADR-202 / ADR-205。

## ADR-205 「そこに無い」は一時エラーではない——`path/not_found` を恒久の失敗として扱う
- 状態: 採用
- 文脈: 実機ログ（diagnostics-82・受信側シミュレータ）で、家族の共有フォルダ `/MosaicShare`
  が Dropbox 上から無くなったあと、`list_folder/continue` が `409 path/not_found` を
  返し続けていた。アプリはこれを**一時エラー**として 30 秒ごとに投げ直し、**18 日間・101 回**
  同じ 409 を受けていた。カーソルはキャッシュ DB で `AAawutvUVUlJ…` のまま 1 歩も進んでおらず
  （他の 2 ルートは毎起動更新されている）、そのルートのキャッシュ **8,513 行**が
  「一覧には出るが開けない写真」として残り続けた——利用者から見ると
  **タップすると「ファイルがありません」**。クラウドの一覧を取り直して掃除するのは
  **初回同期のときだけ**なので、一度この状態に落ちると**誰も直しに来ない**。
  同じ根が撮影日の穴埋めにも出ていた: `files/get_metadata` が `path/not_found` を返す
  死んだ行を「通信できなかった回」と同じに扱って記録しなかったため、候補（撮影日の降順＝
  アップロード時刻を持つ死んだ行が先頭）から外れず、**残り 80,172 件が 63 回連続で不変**・
  **734 回の 409** を費やして本来の穴埋めが 1 枚も進んでいなかった。
- 決定: 409 の本文に `not_found` があるものを **`SyncError.isPathNotFound`** として
  `isCursorReset` と同じ「やり直しても無駄」の側に分類する。
  - 差分ループ: そのルートのキャッシュ行を落とし、カーソルを捨て、そのルートの同期を**止める**。
    キャッシュは作り直せるので、フォルダが戻れば次の初回同期で復元される。
  - 撮影日の問い合わせ: 「訊いた」ことだけ記録して先へ進む（`captureDate` は触らない）。
  - 偽 Dropbox: 差分カーソルに**根のフォルダを持たせ**、その根が消えたら `path/not_found` を
    返す（本物と同じ）。番号だけのカーソルでは、この壊れ方をテストで作れなかった。
- 結果: 「開けない写真が一覧に残り続ける」が自力で直る。撮影日の穴埋めが死んだ行に
  飲まれなくなる。トレードオフは、**一時的に見えなくなっただけの共有フォルダでも
  キャッシュを捨てる**こと——戻ったときは初回同期をやり直すぶんの通信がかかる。
  `path/not_found` は Dropbox が「確かに無い」と言っている応答なので、この賭けを取る。
- 関連: `DropboxSyncEngine.swift`（`isPathNotFound`・差分ループ）/
  `DropboxPhotoStore+Location.swift`（`probeMediaInfo`）/ `FakeDropboxServer.swift`
  （根つきカーソル・`removeFolder`）/ `MissingRootRecoveryTests.swift` /
  `CaptureDateProbeNetworkTests.swift` / ADR-201 / ADR-203 / 事例「ファイルがありません」。

## ADR-204 テストの偽 Dropbox は「1 本のルール表」で壊す
- 状態: 採用
- 文脈: 偽サーバーへ失敗の種類を足すたびに「プロパティ＋セッター＋ハンドラ内の `if`」が
  1 セット増え、**注入口が 21 個**になっていた。ADR-196 でゲート判定を 11 の述語から
  1 つの表へ畳んだのと**同じ増え方**で、放っておくと「どの失敗がどの経路に効くのか」が
  読めなくなる（そして読めない偽物は、黙って嘘をつく）。
- 決定: 仕込みを **`inject(Fault)` 1 本**にまとめる。
  - `Fault` は「**どのエンドポイントの・どのパスに・何回**」＋ `Effect`。
  - `Effect` は 5 つ——`status(code, retryAfter:)` / `truncateBody`（200 だが本文が切れる）/
    `delay` / `networkError` / `drop`（応答しない）。**新しい壊れ方はここへ 1 行足す**。
  - 判定は**受け口 1 か所**で行い、各ハンドラには `if` を置かない。
  - `pendingFaults()` で**使われなかった仕込み**を検査できる。⚠️ これが要点——
    テストが緑でも、仕込んだ失敗が起きていなければ何も確かめていない。
  - 既存の 21 個は**薄い別名**として残す（`failUploads` は `inject` を呼ぶだけ）。
    移行のために 338 本を書き換えない。
- 結果:
  - 得たもの: 新しい異常を足すコストが「`Effect` に 1 行」まで下がった。
    仕込みが 1 行で読め、使われたかも検査できる。
  - トレードオフ: 別名が残るぶん API は二重に見える。新しいテストは `inject` を使う。
  - ⚠️ `drop` は**キャンセルされるまで戻らない**。キャンセルの効かない呼び出しに使うと
    テストが止まる（意図して使うこと）。
- 関連: `Packages/DropboxCore/Sources/DropboxTestSupport/FakeDropboxServer.swift` /
  `Tests/DropboxCoreTests/FakeDropboxServerTests.swift`（偽物そのもののテスト）。ADR-196。

## ADR-203 差分カーソルの失効は「やり直し」であって「一時エラー」ではない
- 状態: 採用（偽 Dropbox にカーソル失効を足して初めて再現できた）
- 文脈: Dropbox は `list_folder/continue` に対して稀に **`reset`**（HTTP 409）を返す。
  失効したカーソルは**二度と有効にならない**ので、投げ直しても永久に通らない。
  ところが同期の poll ループは、これを他の通信エラーと同じ一時エラーとして扱い、
  **30 秒ごとに同じ死んだカーソルを投げ続けていた**。利用者から見ると
  「新しい写真がいつまでも出てこない」（しかもアプリは動いているように見える）。
  この経路は台本どおりに答えるスタブでは作れず、**一度も試されていなかった**。
- 決定: `reset` だけを見分け、**カーソルを捨てて初回同期からやり直す**。
  - 見分け方は「409 ＋ 本文に `reset`」（`SyncError.isCursorReset`）。
  - `DropboxCacheStore.resetSyncCursor` がカーソルと「初回同期の完走」印を落とす。
    **キャッシュの写真の行は消さない**——初回同期は「取ってきた一覧に無いものを消す」形で
    収束するので、消すと逆に画面が空になる時間が生まれる。
  - `syncLoop` は「やり直しが要るか」を受け取って、要るなら最初から選び直す。
- 結果:
  - 得たもの: 失効から自動で立ち直る（テストでは 0.14 秒で 4 件目を取り込む。
    直す前は 6 秒待っても取り込めない）。
  - トレードオフ: やり直しは一覧の取り直しなので通信量が増える。ただし失効は稀で、
    代わりに得るのは「同期が永久に止まらない」こと。
  - ⚠️ **ループにする以上、無限に回らないことが要る**。`Task.isCancelled` を毎周見ており、
    停止・アカウント切り替えで抜ける。`reset` が延々と返り続ける状況では初回同期を
    繰り返すが、それは本物の Dropbox では起きない（失効は一度きり）。
- 関連: `Packages/DropboxCore/Sources/DropboxCore/Sync/DropboxSyncEngine.swift` /
  `Cache/DropboxCacheStore.swift`（`resetSyncCursor`）/
  `Tests/DropboxCoreTests/SyncRecoveryTests.swift`。ADR-44。

## ADR-202 オフロードは、唯一のコピーが失われたら**止めてから知らせる**
- 状態: 採用（利用者の指示）
- 文脈: オフロードは端末の原本を消すので、クラウドのコピーが**唯一のコピー**になる。
  それが Dropbox 側で消された／差し替えられた場合、その写真はもう戻らない。
  週次の照合（ADR-166）は実体の無いバックアップ記録を外すだけで、**オフロード台帳には
  触らず、利用者にも知らせなかった**——アルバムには出続け、開くと取得に失敗する。
  「なぜか表示できない写真」として現れ、原因も、**自分の操作が原因だったこと**も伝わらない。
  偽 Dropbox の通しシナリオを書いていて表に出た（テストで挙動を固定した）。
- 決定: **止めてから知らせる**。照合が「実体の無いオフロード済み写真」を見つけたら、
  1. **自動オフロードをその場で 0（オフロードしない）へ落とす**（設定の強制変更）
  2. 写真の一覧の**上に赤い帯**で知らせる（`sourceNotice`・押すとオフロード設定へ直行）
  3. 設定画面に「止めた事実・理由・該当ファイル」を出し、**「確認した」を押すまで
     自動オフロードを設定させない**（ピッカーを無効化）
  4. 確認しても**自動オフロードは自動では戻さない**（再開は利用者が選ぶ）
- なぜ設定を勝手に変えるのか: 本来は避けるべき（ADR-197 の「利用者の選択肢を勝手に消さない」）。
  ただしここは**写真が失われ続ける**経路で、次の夜に同じことが起きる。
  「止める」は可逆で、失われた写真は不可逆——**可逆な側に倒す**。
  代わりに、止めた事実と理由を**必ず画面に出す**（黙って変えない）ことを条件にする。
- 誤発動をさせない担保: 判定は**完全な一覧でだけ**行う。`listFolder` はページの途中で
  失敗したら部分結果でなく nil を返し、照合そのものが見送られる（`ReconcileScenarioTests`）。
  不完全な一覧で判定すると、実在する写真を「消えた」と読んで緊急停止が誤発動する。
- 結果:
  - 得たもの: 唯一のコピーを失ったことに**利用者が気づける**。同じ経路が自動で繰り返されない。
  - トレードオフ: 一覧の上に赤い帯が出る（確認するまで消えない）。伝えたい内容が
    取り返しのつかない事実なので、出しっぱなしでよいと判断した。
  - トレードオフ: 「知らせ」の枠（`SourceNotice`）を PhotoSourceKit に足した。
    ⚠️ **ここに何でも出し始めると、本当に伝えたいときに読まれなくなる**。
    現状の利用者はこの 1 件だけ——足すときは同じ重さのものに限る。
- 関連: `Packages/BackupKit/Sources/BackupKit/OffloadHalt.swift`（新規）/ `BackupStore.swift`
  （`missingOffloadedPaths`）/ `BackupEngine.swift`（照合での検知・`offloadHalt`）/
  `OffloadSettingsView.swift` / `Packages/PhotoSourceKit/.../Views/SourceNotice.swift`（新規）/
  `MosaicPhotos/Home/SourceHostView.swift`。ADR-39/40/166/197。

## ADR-201 クラウド写真の撮影日時は 1 枚ずつ取りに行き、「無かった」ことも憶える
- 状態: 採用（利用者の判断: 「撮影時間ですが、遅延取得したい」）
- 文脈: Dropbox は **`list_folder` / `list_folder/continue` / `get_thumbnail_batch` で
  `media_info` を返さない**（2019-12-02 以降・公式 SDK の記述で確認）。したがって一覧から
  得られる日付は `client_modified`＝**アップロード時刻**であり、撮影日時ではない。
  自分がバックアップした写真は台帳の `creationDate` で上書きでき（ADR-128 追補）、
  家族共有は解析データで渡せる（ADR-199）。残るのは**そのどちらにも載らない写真**
  ——他端末からのカメラアップロード、Dropbox 側で直接追加された写真。
  一覧では一生取れないので、選択肢は「1 枚ずつ `files/get_metadata` を叩く」
  「まとめて引く口を作る」「仕様と割り切る」の 3 つだった。
  ⚠️ 16 周目のレビューで「`include_media_info` の付け忘れが原因」と報告したのは**誤り**で、
  17・18 周目に撤回・訂正済み。API の制約であって、こちらの不具合ではない。
- 決定: **1 枚ずつ遅延取得する**。ただし次の 3 つを規則として型に埋める。
  1. **1 回の往復で撮影日時と撮影地の両方を取る**（同じ応答に入っている）。
     撮影地の解決（`location(for:)`）と同じ経路に統合した。
  2. **訊いた事実を記録する（無かったことも）**——`CachedDropboxItem.captureDateProbedAt`。
     EXIF の無い写真は何度訊いても無いので、記録しないと 6.8 万枚ぶんの往復を毎回繰り返す
     （CLAUDE.md 性能原則 3「無いものを繰り返し探さない」）。
     **無効化経路は contentHash の変化**＝中身が差し替わったら `applyDelta` が印を外して訊き直す。
  3. **後続の同期が、訊いて得た日付を上書きしない**。一覧の日付はアップロード時刻なので、
     素朴に代入すると直した並びが毎回元へ戻り、しかも全行がダーティになって
     ディスク書き込みの山になる（実機で 1.07GB の警告を出した形）。
- 進め方: 前面の操作を邪魔しないよう、**場所スキャンと同じ条件**（電源＋回線＋非操作＝
  `BackgroundYield.allows(.cloudTrickle)`）でだけ少しずつ進める。1 塊 12 枚・並行 4
  （独立した RPC なので待ちを重ねる＝性能原則 1。並行数は Dropbox のレート制限に配慮して固定）。
  一巡すれば「訊いた」印が全件に付いて止まる。
- 結果:
  - 得たもの: どこから来た写真でも、いずれ撮影日時で正しく並ぶ。撮影地も同じ往復で埋まる。
  - トレードオフ: **並びが後から動く**。穴埋めが進むと写真の位置が変わる（一覧は日付順）。
    一度きりの収束なので受け入れる。⚠️ 開いたまま見ている最中の並び替えについては
    `unresolved-problems.md` の既知の課題（全画面のラベルと中身の食い違い）と同じ性質。
  - トレードオフ: 6.8 万枚の一巡には時間がかかる（1 枚あたり数秒・条件を満たす間だけ進む）。
    新しい写真ほど先に直るよう**新しい順**に訊く。
- 関連: `Packages/DropboxCore/Sources/DropboxCore/Store/DropboxPhotoStore+Location.swift` /
  `Cache/DropboxCacheStore.swift` / `Cache/CachedDropboxItem.swift` /
  `MosaicPhotos/HomeView.swift` /
  `Packages/DropboxCore/Tests/DropboxCoreTests/CaptureDateProbeTests.swift`。
  ADR-128 追補 / ADR-199 / `unresolved-problems.md`（この項は解決済みとして除去）。

## ADR-200 バックアップのメタデータ台帳に「唯一の書き手」を置く（オフロードの印が届かなかった件）
- 状態: 採用（利用者の判断: 「オフロードはまだ使われていないので、後方互換性を破壊して
  あるべき設計にしてよい」）
- 文脈: v2 の台帳は「カタログ（`<root>/.mosaic/catalog.json`）＋撮影月シャード
  （`<root>/.mosaic/meta/<YYYY-MM>.json`）」の 2 層で、**読む側はカタログに載っている
  シャードしか開かない**（ADR-38）。つまり書き手には 2 つの義務がある——
  (1) シャードを `<root>` の下に置く、(2) 置いたシャードをカタログに登録する。
  ところがこの義務は**呼び出し側の責任**になっていた。バックアップ（`BackupRunner`）は
  両方守るが、オフロード（`OffloadService`）は**どちらも守っていなかった**:
  書き先を「写真のパスの親」から作っていたため、写真が年月フォルダへ移った（ADR-176）あとは
  `<root>/2025/2025-08/.mosaic/…` という誰も見ない場所へ書き、カタログにも登録しなかった。
  しかも書き込み自体は成功するので台帳は「送信済み」になり、**二度と書き直されない**。
  帰結は「オフロードした写真は再インストール後に復元できない」＝写真の喪失。
- 決定:
  1. **`BackupMetadataStore` を唯一の書き手にする**。`root` を持ち、**パスから推測しない**。
     シャードを書いたら**自分でカタログを更新する**（呼び出し側に選択肢を与えない）。
     `apply`（バックアップの追記）も `mark`（オフロードの印）も同じ型を通る。
  2. **オフロードはバックアップルートを受け取る**（`OffloadService.backupRoot`）。
     管轄外のパス（保存先を変える前・別端末から引き継いだ記録）には**書かない**——
     推測せず、未送信として残す。
  3. **印は「起きた事実」として、あとから来たエントリに消させない**。
     `DropboxBackupMetadata.merging` が `offloadedAt` / `verifiedAt` を引き継ぐ。
     バックアップ側のエントリは印の存在を知らない（常に nil）ので、素朴な上書きだと
     再送キューの古い 1 枚が流れただけで印が消え、ユーザー削除と区別できなくなる。
  4. **再送キューの消費口を 1 つにする**（`PendingMetadataStore.takeAll`）。
     本体とジャーナルを畳んでから空にするので、(a) 送信中に届いた背景アップロードの行が
     巻き添えで消えない、(b) 1 回の実行で同じ行を 2 度送らない、(c) 「送る写真が 0 枚」の回も
     ジャーナルが空になる。追記と取り出しは**同じ錠**の下に置く。
  5. 再送の経路も**カタログを更新する**（シャード一覧だけ・名前は触らない）。
     登録されないシャードは読まれないので、前回書けなかった月が取り残されていた。
- 結果:
  - 得たもの: 「書いたのに読まれない」が構造的に起きなくなった（カタログ更新を忘れる余地が無い）。
    メタデータの取りこぼし（人物名・アルバム・位置情報）が 3 つの経路で塞がった。
  - 後方互換性: **破棄してよいという利用者の判断**に従い、誤った場所（`<root>/<年>/<年-月>/.mosaic/`）
    へ書かれた印の救済は**作らない**。オフロードは未使用のため実害が無い。
    ⚠️ 過去の試用で作られた空の `.mosaic` フォルダが残っていることがある＝**手で消してよい**。
  - トレードオフ: 印 1 回につきカタログの往復が 1 つ増える（オフロードは 1 回あたり数枚〜
    数十枚なので無視できる）。
- 関連: `Packages/BackupKit/Sources/BackupKit/BackupMetadataStore.swift`（新規・
  `MetadataShardWriter` を置換）/ `OffloadService.swift` / `BackupEngine+Offload.swift` /
  `BackupRunner.swift` / `PendingMetadataStore.swift` /
  `Packages/DropboxCore/Sources/DropboxCore/Models/DropboxBackupMetadata.swift`。
  ADR-38 / ADR-40 / ADR-171 / ADR-176。

## ADR-199 共有写真の撮影日は解析データで渡す（Dropbox の日付は共有コピーでは当てにならない）
- 状態: 採用（実フィードバック「共有フォルダの表示が撮影時間順でないです。Dropbox へのアップロード順になっている？」）
- 文脈: 受け取った共有アルバム（`SharedAlbumPhotosView`）は `MergedPhotoStore` が撮影日昇順で並べる。クラウド写真の撮影日は Dropbox が返す `time_taken ?? client_modified`（`DeltaPageParser`）。⚠️ Dropbox は **2019-12-02 以降、`list_folder` / `list_folder/continue` / `get_thumbnail_batch` で `media_info` を返さない**（`include_media_info` も無視される）。したがって**すべてのクラウド写真**で `time_taken` は nil になり、日付は `client_modified`＝「提供者が反映した時刻」へ落ちる＝**反映順（実質アップロード順）に並ぶ**。※ **訂正（レビュー 18 周目）**: 当初ここには「`copy_batch_v2` のサーバーサイドコピーだから `time_taken` が付かない」と書いていたが誤り。コピーの作り方は無関係。**決定そのものは変わらない**——データを返さない API に対して解析データで撮影日を渡すのは正しい対処で、誤っていたのは理由の説明だけ。
  自分のバックアップ副本には同じ問題への対処が既にあった（ADR-128 追補: 台帳の撮影日で上書き）。ただしその上書きは**バックアップ台帳のパス**を引くので、家族フォルダ配下のパスは 1 件も当たらない。さらに受信側には台帳そのものが無く、`ShareAnalysisData.Entry` は撮影日を運んでいなかった（顔の `Face.d` にはあったが、顔が 1 つも無い写真には何も無い）。**受信側が撮影日を復元できる経路が 1 本も存在しなかった。**
- 決定: 撮影日を解析データに 1 枚あたり約 20 バイト（JSON の `"d":<epoch秒>`・メモリ上は 8 バイトの `Double`）載せる。
  1. `ShareAnalysisData.Entry.d`（epoch 秒）を追加。検証は `Face.d` と同じ範囲（NaN・非現実値は「日付なし」へ落とす）。
  2. 送信側は `PhotoEnrichment.captureDate`（`AutoAlbumStore.captureDates(forRefKeys:)` のまとめ引き・2 列射影）から載せる。**撮影日だけの写真も載せる**——解析が空でも並び順はそれで直る。
  3. 受信側は `ShareImportPlanning.Batch.captureDates` で受け、`SharedCaptureDateStore`（Application Support の小さな JSON・パス小文字 → 撮影日）へ貯める。
  4. 表示は**既存の撮影日上書きに合流**させる。Composition Root が `backupCopyIndexProvider` の中で台帳と共有の表を重ねるだけで、`MergedPhotoStore` は無改造。`localIdentifier` は nil なので副本隠しの対象にはならない。
- 結果:
  - 受け取った共有アルバムが撮影日順に並ぶ。**表示の仕組みは 1 つのまま**（バックアップ副本と同じ道を通る）。
  - 撮影日は**モデル版でゲートしない**。タグ・CLIP・顔は版が一致したときだけ取り込むが、撮影日はモデルに依存しない写真の事実なので、版が食い違う相手からでも並び順だけは直せる。
  - ロールアウトは自動。送信側の解析データは毎回組み直して content_hash で比較するので、`d` が増えた時点でシャードが上がり、受信側の rev が変わって取り込まれる。**逆に言えば、提供者のアプリが更新されるまで既存アルバムの並びは直らない。**
  - 置き場所は Caches ではなく Application Support。Caches だと OS に消されて並びが黙って壊れるのに、解析データの rev は「取り込み済み」のままで**再取得されない**。
  - 掃除は「いま同期済みの家族フォルダ配下」を基準にする回だけ（一覧が取れなかった回に全消しすると並びが壊れる）。上限 5 万件、溢れたら**古い**撮影日から残す（撮影日を落とした写真は Dropbox の日付＝反映時刻に落ちるので、古い写真ほど上書きの価値が高い）。
- **追補（レビューで直した点・2026-09-19）**:
  - 受信側の読み取り能力に版を持たせる（`ShareAnalysisFetch.receiverCapabilityVersion`）。
    rev だけを鍵にすると、旧ビルドで一度取り込んだ解析データは**受信側が更新しても再取得されず**、
    撮影日が永久に届かない。解析データから新しく読む項目を足したら必ず上げる。
  - 上限超過で残すのは**古い撮影日**。撮影日を落とした写真は Dropbox の日付（≒反映時刻）に
    落ちるので、古い写真ほど上書きの価値が高い。逆向きだと古い写真が列の最新側へ飛ぶ。
  - 取り込みの再入防止の旗は `await`（トークン更新）の**前**に立てる。2 本同時に走ると
    撮影日の表が後勝ちで片方失われ、rev は記録済みなので取り直せない。
- **追補2（レビュー 2 周目・2026-09-20）**: 「取り直せる」は取り直す量が有界でなければ成り立たない。
  - 1 回の実行で取る解析データに上限（`maxFilesPerRun = 48`）。記録を捨てた回に全部を
    まとめて取ると 1 セットで 100MB 規模を同時に抱え、jetsam されると版だけ上がって
    記録は空＝毎回同じ山を作り直す無限ループになる。**一覧は全部見る**ので掃除は狂わない。
  - `record` は**収まらなかったパスを返す**。1 件でも落ちた解析データは取り込み済みにしない
    （rev で弾かれて永久に失うため）。書き込みに失敗した回も同じ扱い。
  - 掃除は Dropbox の同期状態が落ち着いているときだけ。初回同期の途中だと
    「まだ手元に無いだけ」の写真の記録を捨てる。
- 関連: `Share/ShareAnalysisData.swift` / `ShareImportPlanning.swift` / `SharedCaptureDates.swift`（新）/ `AutoAlbumStore+ShareExport.swift` / `AutoAlbumEngine+Share.swift` / `MosaicPhotos/Share/ShareSupport.swift` / `RootView.swift`。ADR-128 追補（バックアップ副本の撮影日）・ADR-112（家族共有）・ADR-183（解析データのシャード）。

## ADR-169 名前の持ち越しは「一対一対応」として解く（同名の別人を捨てない）
- 状態: 採用（レビューループの指摘・P1 dataLoss）
- 文脈: 版上げ（顔モデル・パイプライン）後の再スキャンでは、旧人物の名前を
  「旧メンバー写真との重なりが最大の新クラスタ」へ戻していた。ここに 2 つの欠陥があった。
  (1) **同名クラスタが既にあるとエントリを `continue` で捨てていた**——「太郎」が 2 人いるのは
  普通で、捨てると 2 人目の名前と旧メンバーの対応が**永久に失われる**（残りにも積まないので
  再試行もされない）。(2) 旧人物ごとの貪欲な最大選択は、**局所的な最良ペアが別の旧人物にとって
  唯一の対応先を奪う**——存在する一対一対応を見逃す。
- 決定: `NameCarryoverMatching`（純ロジック）で**最大マッチング**（Kuhn の増加道法）として解く。
  候補は重なりの多い順に見るので、同数戻せる割り当てが複数あるときは重なりの大きい対を優先する。
  既に名前が付いているクラスタは**候補から外すだけ**にして、エントリ自体は必ず生かす。
  対応先が決まらなかったものは**必ず残りとして返す**（次のスキャンで再評価）。
- 結果: 同名の別人が両方とも命名され、唯一の対応先が奪われなくなる。旧人物は多くて数十なので
  この規模では計算量は問題にならない。⚠️ 最大**枚数**（総重なり）の厳密最大化まではしていない
  ——最大**件数**を優先し、その中で重なりの大きい対を選ぶ近似（実データでは同数の割り当てが
  複数生じること自体が稀）。
- 関連: `NameCarryoverMatching.swift` / `FaceStore+Rebuild.swift`（`reapplyNames`→ ADR-232 で `reapplyAssertions` に改名）/
  `NameCarryoverTests` / ADR-51（命名の持ち越し）。

## ADR-170 人物の統合はピープルグループの参照も付け替え、取り消しで戻す
- 状態: 採用（レビューループの指摘）
- 文脈: ピープルグループ（ADR-113）は **clusterID で人物を指す**。人物を統合すると src の行は
  消えるのに、グループの参照は付け替えていなかったため、`PeopleGroupInfo.resolve` が
  「現存しないメンバー」として黙って落とす＝**家族グループから 1 人消える**。
  さらに取り消し（ADR-136）の控えにグループが入っていないため、付け替えを足すと今度は
  「顔と人物は戻るのにグループは統合後のまま」になる（レビューループが実際にこの退行を作り、
  受け入れ条件の判定で巻き戻された）。
- 決定: 統合時に `remapPeopleGroups(from:to:)` で参照を付け替える（統合先が既にメンバーなら
  重複させず取り除くだけ）。取り消しの控えには**全グループの構成**を入れて書き戻す
  ——グループはユーザーが作る数個〜十数個なので、影響範囲を操作ごとに見極めるより漏れがない。
  控えより後に作られたグループは触らない。
- 結果: 統合しても家族グループから人が消えず、取り消せば構成も戻る。
- 関連: `PeopleGroups.swift` / `FaceStore+Edit.swift` / `FaceStore+Undo.swift` /
  `PeopleGroupMergeUndoTests` / ADR-113 / ADR-136。

## ADR-198 「1 本だけ走らせる」を部品にし、手続きを表に出す（複雑さ上位 3 領域の整理）
- 状態: 採用（ADR-196 と同じ型の適用・ADR-85/95/130/132/140/141/151 は数値・規則とも不変）
- 文脈: `complexity-scoreboard.md` の計測で上位に出た 3 領域（解析トリクル起動・PeopleEngine・
  顔クラスタ後処理）を読み直したところ、**3 つとも同じ 2 つのパターンを手書きで持っていた**。
  | パターン | 手書き箇所 | 過去のバグ |
  |---|---|---|
  | single-flight ＋世代ガード | `Task<Void, Never>?` のホルダ **24**、世代カウンタ **7** | 「A をキャンセル後に B が始まり、A の終了が **B のハンドルを消す**」→ `GenerationHandle` を新設したが**1 か所でしか使われなかった** |
  | coalescing（走行中の要求を 1 回だけ拾い直す） | 保留フラグ **3**（`editFollowUpPending` / `needsAnotherPass` / `refreshRequested`） | 「キャンセルで抜けると要求が消えて数字が凍る」 |
  さらに:
  - `scheduleBackgroundFill()` が **117 行の単一 Task クロージャ**（二重起動抑止・世代ガード・
    ゲート待ち・ラベラのウォーム・候補の準備・P1/P2 の順序と上限）で、**テストは 2 本**。
    「P1 に上限が無いと埋め込みが永久に飢餓する」（ADR-85）は**コメントにしか無かった**。
  - `PeopleEngine` は public 約 50・結合 44 で、上の 2 パターンを**3 つ**（再読込の間引き・
    編集の後追い・スキャン）抱えていた。
  - `rebuildClusters()` は 254 行・分岐 61。クラスタリングのノブ 10 個が**スキャン時と再クラスタの
    2 か所にコピー**され、ADR-130/132/140/141 の規則は `@Model` を直接いじる中に埋まっていて
    **単独で呼べなかった**。
- 決定:
  1. **共通部品**（`SingleFlightTask` / `DebouncedTask`・MosaicSupport）。`start` / `restart` /
     `coalesce` / `stop` / `waitUntilIdle` と、鏡写し用の `onStateChange`。過去に踏んだ 2 つの
     バグを回帰テストで固定した。⚠️ 鏡写しを**本体の `defer`** でやると、明け渡した旧タスクが
     遅れて終わったときに後続の表示を落とす——世代を知っている部品側から通知する
     （実装中にテストが捕まえた: `restart` で `false → true` とばたついていた）。
  2. **`TricklePlan.steps`**（純）。ウォーム／タグ（上限つき）／埋め込みの手順を表にし、
     `scheduleBackgroundFill` は 35 行の反映だけに。コメントにしか無かった 4 つの不変条件
     （上限が有限・回線 NG ならローカルのみ・ゲート閉ならウォームしない・必ず埋め込みで終わる）
     をテストで固定した。
  3. **`PeopleEngine` の 3 つの小状態機械を部品へ**。`isScanning` は `scan.isRunning` の委譲に
     （`SingleFlightTask` が `@Observable` なので SwiftUI の追従は保たれる）。
  4. **`FaceScanControl`**（断面プロトコル）。駆動役・ブースト・状況モデルは 8 メンバーだけを見る。
     ⚠️ **ビューは使わない**——`@Observable` の追従は具象型でしか効かず、プロトコル越しでは
     再描画されない。断面は「ポーリングで読む側」専用。
  5. **顔の純ロジック 3 つ**: `FaceClusteringSetup`（ノブの一元化・2 コピーを 1 つに）・
     `FaceNameFollowing`（名前の追随）・`FaceSeedBuilder`（種の構築＝ADR-130/132/140/141 の規則）。
- 結果: 手書きの世代・保留フラグが 6 か所消え、テストが**新規 40 本**増えた
  （MosaicSupport 129→141、FaceCore 247→268、AutoAlbumCore 231→238）。
  トリクルのテスト/kLOC は 7.1 → 13.7、顔クラスタ後処理は 66.2 → 74.5。
  ⚠️ **設計を 2 か所、実装中に変えた**（どちらも提案どおりでは目的を達しないと分かったため）:
  - (a) `FaceScanController` を**別クラスに切り出す案は取り下げ**た。スキャン本体が `store` /
    `tuning` / 版上げ移行 / 命名の持ち越し / 影の世代の昇格など **13 個の内部に依存**しており、
    切り出すと逆参照（`weak var engine`）を持つだけで結合は減らない。断面プロトコルに変更。
  - (b) 顔の再クラスタを**全顔の値型スナップショットにする案は取り下げ**た。86k 件 × 512 次元 ×
    4 バイト ≒ **176MB** を一度に確保することになり、ADR-6/119/122 が扱ってきた問題そのものを
    再現する（ハーネスは FG-NET/LFW の小規模なので**捕まえられない**）。メタデータだけを値にし、
    埋め込みは `embedding(faceID)` クロージャで 1 枚ずつ復号する形にした。
  ⚠️ 挙動変更は 1 つだけ: `FaceNameFollowing` で**同数のときの行き先を決定的にした**
  （ID の小さい方）。旧実装は `Dictionary.max(by:)` で、同じデータでも実行ごとに違う人物へ
  名前が移り得た（再現できない不具合）。
- **精度は動いていない**: `face-accuracy.md` の中核指標（B3 の P/R/F1・pair F1・クラスタ数・
  分裂）は変更前後で**完全一致**。⚠️ この確認の過程で、**ハーネスの 2 指標が元から非決定的**
  （純度<0.8 の人数・レビュー候補の当たり数が実行ごとに ±1）であることが判明した——
  変更前どうしを突き合わせて初めてノイズと分かった。`face-accuracy.md` に「どの指標が決定的か」
  の表を追記し、`unresolved-problems.md` に記録した。
- 教訓:
  - **同じ形の状態管理が 20 か所を超えたら、部品が無いのではなく「使われていない」。**
    `GenerationHandle` は既にあったのに 1 か所でしか使われず、他は手書きで、それぞれ別々に
    バグっていた。**部品を作るだけでは足りない——置き換えるところまでやる。**
  - **「数値が動いたら中止」のゲートは、指標の再現性を確かめてから使う。** 確かめずに使うと
    ノイズを退行と読み違える（実際に 1 度読み違えた）。ADR-119 の「フレーキーなテストは
    バグを隠す」は、ハーネスにも当てはまる。
  - **リファクタの設計は、実装して初めて分かることがある。** 上の (a)(b) はどちらも
    「提案時には見えず、コードを読んで初めて分かった」。**提案どおりに押し通さない。**
- **追補（同日・レビュー 13 件）**: ADR-196/197/198 をまとめてレビューしたところ 13 件出た
  （HIGH 3・MEDIUM 7・LOW 3）。全件修正し、回帰テストを 6 本足した。重いものだけ:
  - **止めた作業を待てない**（自分でも独立に発見）。`stop()` がハンドルを捨てるので
    `waitUntilIdle()` が即座に返り、`PeopleEngine.reset` が「止まってからストアを消す」を失った。
    → `retiring` にハンドルを控えて待つ。
  - **QoS の退行**。`launch` が素の `Task { }` で、置き換えた 3 か所の `Task(priority: .background)`
    が**静かに消えていた**（駆動役や SwiftUI の `.task` から起こされて昇格）。→ `priority:` を追加。
  - **「再解析」が通知バナー 1 つで止まる**。`fill` を共用したため停止経路も共有していた。
    → 専用ハンドル（`reanalyze`）へ分離。
  - **名前が消える**（ADR-130 の再発）。行き先の「無名」をスナップショットで固定したため、
    2 人が同じクラスタへ合流すると 1 人目の名前が上書きされる。→ 反映側で 1 つずつ落とす。
  - **ADR-163 のゲートが死んでいた**。顔の残作業を「解析を起こす前」に測っていたため常に 0。
    ⚠️ **この対処は効いていない**（11 周目に判明・`unresolved-problems.md` を参照）。
    測る位置は直ったが、顔スキャンは背景タスクを起こすだけなので、測る時点ではまだ 0 のまま。
    → 窓の手順を 2 段階（`analysisStep` → `remainingSteps`）にして順序を戻した。
  - **回線ポリシーの免除を撤回**（ADR-196 の挙動変更の 1 つ）。「全力で解析」の一部として
    免除したが、**利用者の実費**（68k 件のサムネをセルラーで取得し得る）。クラウド分を
    飛ばしたことは `.blocked([.networkBlocked])` で正直に報告する——嘘の完了は起きない。
  ⚠️ **テストを 2 回自分で壊した**: 新規テストが `withCheckedContinuation` の同期ミスで
  ハングし、優先度のテストは `await` による**優先度の昇格**を測っていて誤診しかけた。
  並行処理のテストは門を 1 つに寄せ、優先度の観測に `await` を使わない。
- 関連: `SingleFlightTask.swift` / `TricklePlan.swift` / `FaceScanControl.swift` /
  `FaceClusteringSetup.swift`（＋`FaceNameFollowing`）/ `FaceSeedBuilder.swift` /
  `complexity-scoreboard.md` / ADR-85 / ADR-95 / ADR-119 / ADR-130 / ADR-132 / ADR-140 /
  ADR-141 / ADR-151 / ADR-163 / ADR-186 / ADR-196 / case-studies「レビューが拾った 13 件」。

## ADR-197 単純化で消してよいのは「状態」であって「選択肢」ではない（ADR-195 の一部撤回）
- 状態: 採用（ADR-195 の一部を撤回・ADR-193 と ADR-80 の設定を復活）
- 文脈: 実フィードバック「AI 解析の状況に前は『アプリを離れた時の解析』という設定がありましたが、
  今はなくなっているようです。他にも『画像分析を控えめに動かす』の設定がなくなっています。
  **シンプルにする＝設定項目を消して良い、ということではありません。**なぜ消しました？」
  - 事実: 2 つとも ADR-195 の同じコミットで消していた。とくに「アプリを離れたときの解析」は
    **利用者自身が要望して**作った設定（ADR-193）で、その約 1 日後に自分で消している。
  - 当時の理屈と、その誤り:
    1. **控えめ**: 「トリクルは 1 単位ごとに譲るのだから前面で動いても代償は無い → 選ばせる
       意味が無い」と判断した。しかし「1 単位で譲る」が答えているのは**応答性**だけで、
       **電池と発熱**には答えていない。「手に持って使っている間は一切動かしてほしくない」は
       正当な要求で、それを一方的に取り上げていた。代償を 1 種類しか見ていなかった。
    2. **アプリを離れたときの解析**: この設定の目的を「自動再開の制御」と捉え、自動再開を
       廃止するなら設定ごと不要と判断した。しかし**インジケータがうるさいという動機は
       自動再開とは独立**で、手動の「今すぐ解析」でも継続タスクに載ればインジケータは出る。
       目的の一部が消えただけで設定を丸ごと消したのは行き過ぎだった。
  - 手続きの誤り: 設計案の箇条書きに「ADR-193 の 3 段設定を廃止」と混ぜ、**全体の承認をもって
    設定の削除まで承認されたものとして扱った**。利用者自身が要望した設定を消すなら、
    それだけを取り出して確認すべきだった。
- 決定: 2 つとも戻す。**単純化の対象は状態機械であって、選択肢ではない**。
  1. **「アプリを開いている間も解析する」（既定 ON）** — 旧「画像分析を控えめに動かす」の復活。
     名前は**肯定形**にした（旧設定は ON が「動かさない」で意味が反転していて読みにくかった）。
     実装はゲートの表（ADR-196）に行を 1 つ足すだけ＝`Blocker.foregroundAnalysisOff`
     （課す仕事: 前面でアクティブなとき・処理枠以外／免除: `.boost`）。
     **表にしておいたおかげで、設定を戻すコストが行 1 つ＋トグル 1 つで済んだ**——
     構造の単純化と選択肢の多寡は独立だという実例になった。
  2. **「アプリを離れても解析を続ける」（既定 ON）** — 旧 3 段の復活。ただし自動再開が
     無くなったので「自動再開」の列が消え、**実質 2 択**に畳んだ（旧 3 段目
     「この画面を開いている間だけ」は、自動再開が無い今 2 段目と同じ意味になる）。
     OFF は「解析を止める設定」ではなく**継続タスクを使わない**という意味で、結果として
     ロック画面と Dynamic Island のインジケータが出なくなる。夜間の自動解析はどちらでも動く。
     実装は `AnalysisSession.submitContinuedTask()` の頭で設定を見るだけ。
- 結果: 既定値は**どちらも現状の挙動のまま**なので、戻したこと自体では動作が変わらない。
  移行はしない（旧 `heavywork.conservative` の値は読まない）——旧設定を明示的に OFF にしていた
  利用者は新設定の既定 ON と同じ挙動になり、既定のままだった利用者は ADR-195 以降の挙動が続く。
  ⚠️ 代償: ゲートの条件が 1 行増え、設定が 2 つ増えた。ADR-195 で目指した「初めに 1 回だけ
  設定すれば、あとは自動」は既定値で達成されており、**増えたのは既定を変えたい人のための出口**。
- 教訓:
  - **単純化で消してよいのは「状態」であって「選択肢」ではない。** ADR-195 が正しく消したのは
    再開・中断の印・クールダウン・手動/自動の区別（＝状態機械）。設定はその巻き添えだった。
  - **「代償が無い」と判断する前に、代償の種類を数える。** 応答性・電池・発熱・通信・
    プライバシーは別々の軸で、1 つ潰しても他は残る。
  - **利用者が自分で要望した機能を消すなら、それだけを取り出して確認する。** 設計案の
    箇条書きに混ぜて全体の承認を得るのは、承認を得たことにならない。
- 関連: `HeavyWorkTiming.foregroundAnalysisKey` / `BackgroundYield.Blocker.foregroundAnalysisOff` /
  `ForegroundAnalysisSettingTests` / `AppSettingsKeys.analysisContinueAfterLeaving` /
  `AnalysisSession.continueAfterLeaving` / ADR-80 / ADR-193 / ADR-195 / ADR-196。

## ADR-196 「動かしてよいか」を 1 つの表にする（ゲート・窓の手順・画面の所有権）
- 状態: 採用（ADR-25/80/107/179/188/195 の述語を置換・ADR-194 の残りを撤去）
- 文脈: 保守負担を実測（fix コミットの密度・退行率・可変状態・結合・継ぎ足しコメント）して
  上位 3 領域を並べたところ、**3 つとも同じ 1 つの問題の現れ方**だった。証拠:
  1. **解析を起こす前口上が 3 コピー**。「候補の列挙（8.5 万件）→ 消えた写真の掃除 →
     顔スキャン → タグ/埋め込み」の 4 手が `AnalysisDriver.kick` / `AnalysisSession.runLoop` /
     `HeavyWorkScheduler.runHeavyWork` に別々に書かれ、3 つとも微妙に違った
     （候補キャッシュの有無・`scheduleBackgroundFill` と `restartBackgroundFill` の違い）。
  2. **「なぜ動かないか」の実装が 2 つ**。ゲート（`BackgroundYield.heavyWorkAllowed`）と
     画面（`AnalysisBlockerDiagnosis`・63 行）が同じ質問に別々のコードで答えており、
     ゲートが閉じているのに「すべての条件を満たしています」と言える状態だった。
  3. **入口と譲りが別の述語**。`PeopleEngine.startScan` の入口は `heavyWorkAllowedLocal`、
     1 単位ごとの譲りは `heavyShouldPause()`。前者は `HeavyLoad` と `isGeneratingAlbums` を
     見ないので、「入ってよい」と言われて `scannedRefKeys()`（75,000 行）を読んでから
     譲り待ちに入り、60 秒で 0 枚のまま畳む（diagnostics-62/63「入口代だけ払う」）。
  4. **「誰が聞いているか」がグローバル変数 4 つ**（`isAppActive` / `isAppInBackground` /
     `sessionActive` / `debugForceHeavyWork`）で、**処理枠はそのうち 2 つに嘘をついて**
     ゲートを開けていた。`restoreAppActive` という引数はその嘘の後始末専用で、
     戻し忘れが 1 回レビューで指摘されている。
  5. 述語は 11 個あり、`shouldPause(powerOK:)` は**完全な死にコード**、
     `debugForceHeavyWork` の doc（「生成との相互排他だけは維持する」）は**実装と矛盾**していた。
- 決定: 3 つを 1 つの設計として直す。
  1. **ゲートを 1 つの表にする**（設計 A）。軸は「どの仕事か」（`HeavyWork`＝通信が要るか ×
     始めたら譲れるか ＋ 処理枠）と「誰が聞いているか」（`Exemption`＝none/boost/debug）の 2 つだけ。
     条件は `Blocker` の表（`applies(to:)` と `skippableBy`）で、判定は純関数
     `blockers(_:for:exemption:)` 1 つ。呼び手は `verdict(for:)` を呼ぶだけで**部分集合を選べない**
     ——入口と譲りが同じ式になる。状態は `scenePhase` と `exemption` の 2 つに減らし、
     一時変更は `withScenePhase` / `withExemption` の**スコープ**で入る（手で戻さない）。
     画面は `verdict(for:).blockers` を描くだけになり、`AnalysisBlockerDiagnosis` は削除。
  2. **夜間窓を「手順の表」にする**（設計 B）。`NightlyPlan.steps(Inputs) -> [Step]` を純ロジックに
     出し、`runHeavyWork` は反映だけ（142 行 → 実行 40 行＋判断 60 行）。見送り・スキップも
     `Step` として残すので、窓が何を決めたかが `RunTimeline` に 1 行で出る。
     デバッグ実行は同じ `steps()` を `.debug` 免除と短い制限時間で回すだけになった。
  3. **画面とブーストの所有権を移す**（設計 C）。数え直しループの持ち主をビュー階層から
     `@State` のモデル（`AnalysisStatusModel`）へ移し、3 つの旗（`refreshInFlight` /
     `refreshRequested` / `refreshRequestedFresh`）を撤去。ブーストは `onStart` で駆動役へ委譲し、
     前口上のコピーを 3 → 1 にした。終わりの判定はゲートに理由を聞き、残作業があるのに
     畳んだ場合は `.finished` ではなく **`.blocked([Blocker])`** で止める。
- 結果:
  | | 前 | 後 |
  |---|---|---|
  | ゲート述語 | 11（うち死にコード 1） | 1（＋言い換え 2） |
  | 「誰が聞いているか」の状態 | 4（窓は 2 つに嘘をつく） | 2（スコープで出入り） |
  | 「なぜ動かないか」の実装 | 2 | 1 |
  | 解析の前口上 | 3 コピー | 1 |
  | テストできない判断 | `runHeavyWork` 142 行（0 本） | `steps()`・ゲート表（純・33 本） |
  昨夜記録した 10 件の指摘は全部これで解消した（前面の一枚岩・空振りの起こし・`.boostEnded` の
  no-op・電池の再開・入口の緩さ・嘘の完了・`RunTimeline` の氾濫・規模テストの欠如）。
  ⚠️ **意図的な挙動変更が 3 つ**:
  (a) **電池の下限（20%）を表の行にした**＝ブーストでも外れない。以前は電源ポリシー「常に」だと
      `backgroundAllowed()` が無条件に真で、下限はブーストのループの中にしか無く、
      「電池のため止めた」と言った直後に方針が同じ処理を再開していた。
  (b) **`startScan` の入口を譲りと同じ厳しさにした**。一括ロード中・生成中は入らない代わりに、
      起こし直しは駆動役が確実に行う（`alreadyRunning` で走行中は無視、空振りはバックオフ）。
  (c) **クラウド分の可否をゲートから引く**ようにした（`verdict.blocks(.networkBlocked)`）。
      以前は `FaceTagger` / `PhotoTagger` が `NetworkStateMonitor` を直読みしており、
      ブーストが回線ポリシーを免除しているのにクラウド写真を落とし、さらに「すべて解析済み」と
      嘘の完了を出していた。
      ⚠️ **追補（ADR-198 のレビュー）**: このうち「ブーストが回線ポリシーを免除する」部分は
      **撤回**した。免除すると 68k 件のサムネをセルラーで取得し得る＝**利用者の実費**で、
      「Wi-Fi のみ」は費用のために選ばれた設定だから。ゲートから引く形は維持し、
      クラウド分を飛ばしたことは `.blocked([.networkBlocked])` で正直に報告する
      （嘘の完了という本来の問題は解決したまま）。
  ⚠️ 代償: `HeavyWork` が 5 ケース（local/cloud × trickle/monolith ＋ window）になった。
  通信の要否と譲れるかを 1 つに畳むと、通信の要らない一枚岩（AI アルバムの本番化・ドリフト
  再評価＝台帳と埋め込みを読むだけ）まで Wi-Fi 待ちで止まる——実際に実装中 1 度そうした。
  ⚠️ テストは `environmentOverrideForTesting` で環境を組み立てる。条件を 1 つの表にした結果、
  判定が**実行マシンの状態**（低電力モード等）に左右されるようになった（実際に 1 本落ちた）。
- 関連: `BackgroundYield.swift`（表）/ `BackgroundGateTests`（表の検証・総当たりの不変条件）/
  `NightlyWorkPolicy.swift`（`NightlyPlan.steps`）/ `NightlyPlanTests` /
  `AnalysisDriver.swift`（唯一の前口上）/ `AnalysisDriverPolicyTests` / `AnalysisDriverScaleTests` /
  `AnalysisStatusModel.swift` / `AnalysisWindowHealth.swift`（旧 `AnalysisBlockerDiagnosis`）/
  ADR-25 / ADR-79 / ADR-80 / ADR-95 / ADR-107 / ADR-119 / ADR-179 / ADR-188 / ADR-195 /
  case-studies「上位 3 領域が同じ 1 つの問題だった」。

## ADR-195 解析は「常設の方針」で自動的に進め、「今すぐ解析」は無状態のブーストにする（セッション概念の置換）
- 状態: 採用（ADR-189/193/194 を置換・ADR-191 の一部を置換）→ **一部撤回（→ ADR-197）**: 「控えめ」と「アプリを離れたときの解析」の**設定を消したのは誤り**だった（消すべきだったのは状態機械だけ）。状態機械の廃止と無状態ブーストは維持
- 文脈: 「夜間解析が進まない」に対して ADR-189（押した事実の永続化＋自動再開）→ ADR-191
  （前面継続への降格）→ ADR-193（3 段の設定）→ ADR-194（台帳で終わりを確定）と積み上げた結果、
  「今すぐ解析」の周りに **5 つの軸**（手動/自動・前面/背面・画面を開いているか・電源・やり残しの印）
  が絡み、レビューループ 9 周で 45 件の指摘、うち多数が同じ場所の退行だった（case-studies）。
  利用者の要望は「機能が複雑。いちいち確認せず、初めに 1 回設定すればあとは自動で進んでほしい」。
  根本原因は、**前面で解析を起こす駆動役が存在しなかった**こと——起動時の 1 回と電源/回線の
  復帰時だけで、`PeopleEngine.startScan` は前面では拒否していた。だから「控えめ OFF（前面アイドルで
  動く）」は実質機能せず、その穴を「セッションの自動再開」で埋めようとして、再開・中断・印・
  クールダウン・画面の可視という**状態**が生まれ、停止経路と再開経路の組み合わせが増えるたびに穴が開いた。
- 決定:
  1. **駆動役を 1 つ置く**（`AnalysisDriver`）。条件が変わった瞬間（起動・前面復帰・20 秒アイドル・
     電源・回線・ブースト終了（熱の回復や写真の追加はアイドル中 120 秒ごとの再評価で拾う））に方針（`BackgroundYield.heavyWorkAllowedLocal`）を見て、
     許されていれば既存のトリクル（`scheduleBackgroundFill` / `startScan`）を起こす。トリクルは
     差分処理で、操作されれば 1 単位ごとに譲り、60 秒開かなければ畳む（ADR-95）。畳んだら次の
     条件変化で駆動役がまた起こす。**「再開」という概念は無い**——残作業そのものが状態。
     判断は純ロジック `AnalysisDriverPolicy`（背面は処理枠の管轄／ブースト中は重ねない／
     アイドルの契機は 120 秒に間引く）。顔の候補列挙（8.5 万件・数秒）は 10 分だけ使い回す。
  2. **「控えめ」の軸を廃止**（4 軸 → 3 軸: 自動処理・電源・回線。発熱は安全弁）。前面は
     「20 秒触っていない」だけを要求し、触れば即譲るのは常時の挙動（ADR-79 の 20 秒は不変）。
     一枚岩（生成・本番化）は非アクティブ限定のまま（ADR-107）。移行: 旧 5 段の保存値は
     `migrationPlan` で 0→paused・3→always・4→always＋unrestricted・それ以外→enabled。
  3. **「今すぐ解析」は無状態のブースト**。押した間だけ `sessionActive` でゲートを免除し、
     継続タスクに載る。終わり方は `finished / user / expired / lowBattery / leftApp` の 5 つで、
     **どれも印を残さない**。前面を離れたら前面のみモードは止める（`appLeftForeground`＝ADR-191 の
     降格は「前面にいる間」だけに縮小）。OS に止められたら方針に戻るだけ（充電＋アイドルなら
     駆動役が続きを進める）。「画面を消さない」はブースト中だけ効く（従来どおり）。
  4. **廃止**: `AnalysisContinuation`（3 段の設定＝ADR-193）・`analysis.sessionPending` ほか永続キー 5 つ・
     `resumeIfPending`・クールダウン・画面の可視状態の報告（`analysisScreenLifecycle`）・
     `pendingScanCount` による終わりの確定（ADR-194）・「前回の解析は中断されました」バナー。
- 結果: 利用者が触る設定は「自動で解析する」＋電源＋回線の 3 つで、**1 回決めれば以後は自動**。
  充電しながらアプリを開いたままでも 20 秒放置で進み、閉じても処理枠で進み、「今すぐ解析」は
  「今すぐ全力で」以上の意味を持たない。ロック画面のインジケータが出るのは押したときだけ
  （Apple の「継続タスクは利用者が始めた作業に」とも整合＝ADR-193 の審査上の留保が消える）。
  コード: `AnalysisContinuation.swift` 削除・`AnalysisSession` 約半分・旧テスト 2 スイート削除・
  `AnalysisDriverPolicyTests` 追加。⚠️ トレードオフ: (a) 前面アイドルで解析が走るので、放置から
  戻った直後に短い引っかかりを感じ得る（1 単位で譲る・速度段階で調整可）。(b) OS に止められた
  あと電源も回線も無ければ方針どおり進まない——画面にそう出す（`.expired` の文言）。
  (c) 背面は依然として OS の処理枠次第（ADR-191 の留保は不変）。
- 関連: `AnalysisDriver.swift`（駆動役＋`AnalysisDriverPolicy`）/ `AnalysisSession.swift`（ブースト）/
  `HeavyWorkTiming.allows`（3 軸）/ `BackgroundYield.heavyShouldPause`（ブースト中は生成と UI 譲りだけ）/
  `PeopleEngine.startScan`（前面でも方針が許せば動く）/ `AnalysisDriverPolicyTests` /
  `HeavyWorkTimingTests` / ADR-25 / ADR-79 / ADR-80 / ADR-95 / ADR-107 / ADR-182 / ADR-188 /
  ADR-189 / ADR-191 / ADR-193 / ADR-194 / case-studies「『今すぐ解析』の周りが 9 周壊れ続けた結論」。

## ADR-194 「終わったか」はライブのカウンタで推測せず、台帳に聞いて確定する
- 状態: **置換（→ ADR-195/196）**——再開そのものを廃止したため「終わりの確定」も不要になった。旧: 採用（ADR-189 の追補・レビューループ 5 周の結論）
- 文脈: 中断された解析セッションの自動再開（ADR-189）で、「もう残作業は無いのに毎回
  セッションを起こす」を避けようとして、**顔スキャンの残り枚数をライブのカウンタから
  推測**する仕組みを入れた。ここから 5 周連続で欠陥が出た——毎回直したつもりが、
  次の周で別の穴として返ってきた:
  1. `people.remaining` は**スキャン中しか更新されない**（新しいプロセスでは常に 0）。
  2. `UserDefaults.integer` は**欠損も 0** にする（「未観測」と「残り 0」が潰れる）。
  3. 顔スキャンは**残作業を抱えたまま畳む**（クラウドのサムネ未取得・譲り待ち・回線 NG）ので、
     「スキャンが終わった＝残り 0」ではない。
  4. その対策に入れた「スキャン中に残り 0 を観測したら本物」という札も、`isScanning` が
     **スキャン開始直後（todo 列挙前）と後処理中**に true のままなので誤って立つ。
  つまり、どの観測点を選んでも「まだ始まっていない / 途中で畳んだ / 本当に終わった」を
  区別できない。**推測そのものが間違った道具**だった。
- 決定: 推測をやめる。
  - **再開の判定から残り枚数を外す**。印（`analysis.sessionPending`）が立っていれば再開する。
  - **終わりの確定は台帳に聞く**。ライブの値で「終わったように見えた」ときに 1 回だけ
    `PeopleEngine.pendingScanCount(candidateRefKeys:)`（`FaceStore` への実クエリ）を呼び、
    0 なら `.finished`（印を下ろす）、残っていれば `.expired`（印を残して次の機会へ）。
  - 持ち越し用の `UserDefaults` キーと観測フラグは**削除**する。
- 結果: 空振りのセッションは「起きて数秒で終わり、印が下りる」1 回で自然に収束する
  （毎回起きることはない）。判定に使う値が**権威ある 1 つ**になり、観測窓の罠が消えた。
  ⚠️ 代償: 終わりの確定で 1 回だけ DB クエリが増える（候補集合に対する count・セッション終了時のみ）。
- 追記（同日・レビュー 7 周目の指摘）: 台帳に聞くときは **`FaceTagger` が実際に対象にする
  集合と揃える**。タガーは回線が許可されないときクラウド（"C-"）を今回の対象から外すので、
  全候補で数えると「いつまでも残っている」ことになり、セッションが終われず
  **前面に戻るたび再開し続ける**（8.5 万件の列挙つき）。候補は走っているスキャン自身が
  持っているもの（`PeopleEngine.scanCandidates`）を使う——数え直さない。
  あわせて、やり残しを次回に回した終わり方には専用の理由 `.deferred` を設けた
  （`.expired` を流用すると「iOS に止められた」と嘘の説明を画面に出し、
  `setTaskCompleted(success: false)` を繰り返して OS の受け入れを悪くする）。
- 関連: `AnalysisSession.runLoop`（`remainingFaceWork` による確定）/ `resumeIfPending` /
  `PeopleEngine.pendingScanCount` / `PeopleEngine.scanCandidates` / `FaceTagger`（回線での絞り込み）/
  ADR-189 / ADR-119（規模に比例する呼び出し）/ case-studies「レビューループが拾った…」。

## ADR-193 「アプリを離れたときの解析」を 1 本の軸で選べるようにする（継続タスクを使う場面）
- 状態: 置換（→ ADR-195）→ **一部復活（→ ADR-197）**: 自動再開の列は消えたが、「継続タスクを使うか（＝インジケータを出すか）」の選択は 2 択として戻した。旧: 3 段の設定を廃止。旧: 採用（ADR-182/189/191 の追補）
- 文脈: iOS 26 の継続タスク（`BGContinuedProcessingTask`）が働き始めると、ロック画面と
  Dynamic Island に進捗インジケータが出る。実フィードバック「ちょっとうるさいかも」。
  ⚠️ **このインジケータはアプリから消せない**——OS が付ける進捗 UI で、そもそも進捗を報告しない
  タスクは OS が殺す。つまり選べるのは「表示」ではなく**継続タスクを使う場面**だけ。
  とくにうるさかったのは、ADR-189 の自動再開が**アプリを開くたびに**継続タスクを取り得たこと。
- 決定: **軸は 1 本**（ADR-80 の教訓＝電源・回線・自動処理を混ぜない）。
  「アプリを離れたときの解析」を 3 段で選ぶ（`AnalysisContinuation`）。
  | 段 | 手動の「今すぐ解析」 | 自動再開 | インジケータ |
  |---|---|---|---|
  | **続ける**（既定） | 継続タスク | **充電中に限り**継続タスク | 解析中に出る |
  | 押したときだけ続ける | 継続タスク | 充電中＋この画面を開いている間だけ | 押したときだけ |
  | この画面を開いている間だけ | 前面のみ | 同上 | 出ない |
  - **夜間の処理枠（BGProcessingTask）はどの段でも動く**。設定画面にもそう書く
    （「解析が止まる設定」と読まれるのが一番まずい）。
  - **自動再開は電源接続が必須**（全段共通・新規制約）。外出先でアプリを開いただけで走り出し、
    電池を食う／インジケータが出るのを防ぐ。手動タップは従来どおり免除（電池 20% の安全弁は別途）。
  - 継続タスクを使わない段では、自動再開は **AI 解析の状況を開いているときだけ**
    （見ていない前面で重い処理を走らせない＝ADR-25）。判定は純ロジック
    `AnalysisContinuationPolicy`（3 段 × 手動/自動 × 電源 × 画面）。
- 結果: 既定は現行動作のままで移行不要。しかも既定 ＋ 電源条件の組み合わせで、
  「電池で持ち歩いている最中にアプリを開いたらインジケータが出る」という一番うるさいケースは
  既定のまま消える。⚠️ **審査上の留保**: Apple は継続タスクを「ユーザーが始めた作業に使え／
  自動のメンテナンスに使うな」としており、既定（自動再開でも継続タスクを取る）は
  「利用者が押した作業の再開である」という説明で立てている（押した事実は
  `analysis.sessionPending` に永続化してある）。もし指摘を受けたら**既定値を「押したときだけ」に
  変えるだけで対応できる**——設定の構造そのものが退避路になっている。
- 関連: `AnalysisContinuation.swift`（設定＋純ロジック）/ `AnalysisSession.start(autoResume:)` /
  `resumeIfPending(statusScreenOpen:)` / `AIAnalysisStatusView`（Picker・`.task` での再開）/
  `AnalysisContinuationPolicyTests` / ADR-25 / ADR-80 / ADR-182 / ADR-189 / ADR-191。

## ADR-192 「動いていない時間」を診断できるようにする（実行タイムライン＋OS の終了理由）
- 状態: 採用
- 文脈: diagnostics-81 の実機ログを受け取っても、**「12 時間 処理枠が来なかった理由」に
  答えられなかった**。理由は 3 つで、どれも設計側の落ち度だった。
  1. **アプリが動いていない時間のことは、その時間には書けない**。にもかかわらず、次に動いた
     瞬間に「空白について分かること」（前回どう終わったか・前回の窓からの経過・端末条件）を
     何も書いていなかった。
  2. **OS は答えを持っていたのに、それを捨てていた**。`HangDiagnostics` は MetricKit を
     購読していたが `didReceive(_ payloads: [MXMetricPayload])` が**空実装**で、
     `MXAppExitMetric`（メモリ上限＝jetsam・ウォッチドッグ・BGTask 期限超過などの終了理由）も
     `MXCrashDiagnostic` も読んでいなかった。
  3. **流量の桁が違うものを 1 つのログに混ぜていた**。末尾 256KB の診断ログは Dropbox の同期と
     顔検出の行で埋まり（実測 47%）、1 日数十行しかない起動・処理枠の記録が押し出されて消えた。
- 決定:
  1. **実行タイムライン**（`RunTimeline`・`Caches/run-timeline.log`）を分ける。書くのは起動・
     予約・処理枠の開始/終了・解析セッションの開始/停止・終了理由だけ。同じ 256KB でも数か月残る。
     Developer Options から閲覧・共有できる（診断ログと同じ画面を使い回す）。
  2. **次に動いた瞬間に空白を説明する**: `RunTimeline.noteState()` のパンくず（idle / window /
     session）を `UserDefaults` に残し、起動時に前回の終わり方を要約する——`window` のまま
     終わっていれば「処理枠の途中で iOS に終了させられた疑い」。あわせて端末条件
     （バックグラウンド更新・充電・電池・低電力・熱・footprint・空きメモリ）と、
     **OS に積まれている予約**（`getPendingTaskRequests`）を記録する。
  3. **MetricKit の終了理由を読む**: `MXAppExitMetric` の背面/前面カウンタを日本語ラベルで
     台帳へ（0 件は書かない）。`MXCrashDiagnostic` も記録する。1 日 1 回・直前 24 時間ぶんが届く。
- 結果: 「窓が来ない」が沈黙ではなく**記録**になる。次のログでは「予約は積まれていたか」
  「前回は窓の途中で殺されたか」「iOS は何を理由に終了させたか」に、ログだけで答えられる。
  ⚠️ MetricKit は即時ではない（1 日 1 回）ので、当日中に答えが要る場面には間に合わない——
  そこはパンくずと環境スナップショットで埋める。
- 関連: `RunTimeline.swift` / `HangDiagnostics`（`MXMetricPayload` / `MXCrashDiagnostic`）/
  `HeavyWorkScheduler.environmentLine()` / `logPendingRequests(context:)` /
  `DiagnosticsLogView`（ログを差し替え可能に）/ `RunTimelineTests` / ADR-106 / ADR-117 /
  diagnostics-81。

## ADR-191 OS の都合で解析を終わらせない（前面継続への降格）＋「なぜ進まないか」をアプリが言う
- 状態: 採用（ADR-182/189 の追補）→ **一部置換（→ ADR-195）**: 降格は前面にいる間だけ・やり残しの印と自動再開は廃止。「なぜ進まないか」の表示と窓の間隔のログは継続
- 文脈: 実フィードバック「電源ケーブルを繋いでアプリも動かしっぱなしなのに、やりたい事が
  できないなんて事はないはず」。まったくそのとおりで、**電源＋前面という理想的な条件では
  OS の処理枠を待つ必要はない**。にもかかわらず、(1) 継続タスク（`BGContinuedProcessingTask`）が
  OS に止められると、アプリが前面にあっても `stop(.expired)` で解析ごと終わっていた。
  (2) 処理枠が来ない理由（バックグラウンド更新 OFF・低電力・未充電・発熱）は端末の設定で
  決まるのに、利用者に見える場所が無く、Mac に繋いで Console で `dasd` を読むしかなかった。
  実機 diagnostics-81 では、充電中にもかかわらず 22:36 以降 **12 時間 窓が 1 度も来ていない**。
- 決定:
  1. **期限切れ＝終了にしない**。継続タスクが止められたとき、アプリが前面なら
     `.foregroundOnly` へ**降格して走り続ける**（`setTaskCompleted` は必ず 1 回呼ぶ）。
     背面なら従来どおり止め、やり残しの印を残す（ADR-189 が次の前面復帰で再開する）。
  2. **前面のみモードでは UI へ譲る**（`BackgroundYield.sessionYieldsToUI`）。画面が生きている
     前提なので、写真を見ている最中に ANE と CPU を奪わない（継続モード＝画面が無い前提は
     従来どおり全力）。
  3. **「自動で進まない理由」を画面に出す**（`AnalysisBlockerDiagnosis`＝純ロジック）。
     自動処理 OFF / バックグラウンド更新 OFF / 低電力 / 未充電 / 発熱 / 回線を、直しやすい順に
     並べる。理由がどれも無いのに**半日以上 枠が来ていない**ときは、その旨と
     「App スイッチャーで終了したか、iOS がメモリ確保のため終了した可能性」＋
     「今すぐ解析を押せば続きから進む」を案内する。
  4. **窓の間隔を診断ログに残す**（`bgtask: begin (前回の窓から N 分)`）。「窓が来ない」は
     沈黙として現れるので、来たときに間隔を書いておかないと後から数えられない。
- 結果: 電源＋前面なら OS の裁量に関係なく進む。枠が来ないときも、利用者は理由を画面で見て
  自分で直せる（あるいは「今すぐ解析」で回避できる）。⚠️ 残る制約: **画面を消してロックした
  状態**では、依然として OS の処理枠と継続タスク（iOS のバグ FB19916760 の対象）に依存する。
  そこは Apple の仕様どおりで、アプリ側からは動かせない——だから「画面を消さない」既定 ON と
  自動再開で埋める。
- 関連: `AnalysisSession.continueInForegroundOrStop()` / `applyModeGates()` /
  `BackgroundYield.sessionYieldsToUI` / `AnalysisBlockerDiagnosis` /
  `AIAnalysisStatusView.blockersSection` / `HeavyWorkScheduler.minutesSinceLastWindow()` /
  `AnalysisBlockerDiagnosisTests` / ADR-182 / ADR-189 / diagnostics-81。

## ADR-190 自分の通信で自分を詰まらせない（429 は Retry-After で待つ・投入は小分け・空振りのポーリングは間合いを空ける）
- 状態: 採用
- 文脈: diagnostics-81 の処理枠では、バックアップが 1 枠で **813 件**を背景 URLSession へ渡し、
  Dropbox から **429 を 713 回**受けて **132 枚を「3 回失敗」で捨てて**いた。429 は
  `classify` の既定分岐（`.retry`）に落ちるため**試行回数を消費**しており、混雑した枠では
  3 回があっという間に尽きる。さらに、上げたファイルは**自分が監視しているルート**へ落ちるので
  `longpoll` が `changes=true` を返し続け、「longpoll → delta（表示対象の増減 0）→ すぐ longpoll」を
  **10 分で 202 周**（1 周 ≈ 0.7 秒）回していた。`changes=false` 側には最小待ちがあるのに、
  `changes=true` 側だけ素通りだったのが原因。実害は三重で、(1) 往復と SwiftData 書き込みが
  処理枠（解析と同じ資源）を食う、(2) API レートを消費して**自分のアップロードが 429 を受ける**、
  (3) 末尾 256KB の診断ログを埋め尽くし（実測 47%）、**夜間の証拠が押し出されて消える**。
- 決定:
  1. **429 は失敗ではない**。`ResponseOutcome.rateLimited(retryAfter:)` を新設し、試行回数を戻して
     `Retry-After`（ヘッダ／本文の `retry_after`・1〜900 秒にクランプ）だけ投入を止める。
  2. **1 回に渡す数に上限**（`maxEnqueuePerFlush = 40`）。転送は枠の外で OS が続けるので、
     一度に渡す利点は無く、渡した数だけレート制限を誘発する。
  3. **ポーリングは空振りで間合いを空ける**（`SyncPollPacing`）。1 周あたり最低 1 秒、
     「変化ありと言われたのに表示対象の増減が 0」の周が続いたら 1→2→4→8→16→30 秒と延ばす。
     **実変化が 1 件でもあれば即リセット**するので、本物の更新への追従の速さは変えない。
- 結果: 空振りのポーリングが 85 周/分 → 8 周/分以下になり、処理枠と API レートが解析へ回る。
  診断ログも夜間ぶんが残る。⚠️ トレードオフ: 自分以外が上げた変化も、空振りが続いた直後は
  最大 30 秒遅れて反映される（longpoll の性質上もともと即時ではない）。
- 関連: `SyncPollPacing.swift` / `DropboxSyncEngine.pollLoop` / `BackgroundUploadSession`
  （`classify` / `split(limit:)` / `retryAfterSeconds`）/ `SyncPollPacingTests` /
  `BackgroundUploadTests` / ADR-181 / diagnostics-81。

## ADR-189 利用者が始めた解析は「押した事実」を永続化し、中断されたら自動で再開する
- 状態: **置換（→ ADR-195）**——押した事実の永続化と自動再開を廃止。残作業は常設の方針で駆動役が進める。旧: 採用（ADR-182 の追補）
- 文脈: 「今すぐ解析」（ADR-182）のセッションは**メモリ上の存在**で、永続化も自動再開も無かった。
  端末をロックすると継続タスクが止まる iOS のバグ（FB19916760）、OS の期限切れ、プロセス終了の
  どれでも消え、**消えたことが誰にも分からない**（プロセスごと死んだ場合は停止マークすら残らない）。
  実フィードバック「夜に解析して寝たのに、朝になっても進んでいない」の実体はこれで、
  diagnostics-81 のログにもセッションのマークは 1 つも無く、25 分ずつプロセスが吊るされていた。
- 決定: 押した時点で `analysis.sessionPending` を立て、**「全部終わった」「利用者が止めた」
  だけが印を下ろす**（`AnalysisSessionPolicy.keepsPendingFlag`＝純ロジック）。前面復帰と起動時に
  `resumeIfPending()` が残作業を確認して**自動再開**し、中断の事実と理由を診断ログと
  AI 解析画面に出す。⚠️ ただし OS が継続タスクを受けなかったとき（前面のみモード）は
  **自動再開しない**——見ていない前面で重い処理を走らせるのは ADR-25 の約束を破るため、
  印は残したまま次の機会に譲る。電池が下限を割っている（電源なし）ときも再開しない。
- 結果: 中断は「気づかれずに永久に止まる」から「次にアプリを開いたら続きから」に変わった。
  ⚠️ トレードオフ: 自動再開は画面を点けたまま（既定 ON）走るので、アプリを開いている間は
  画面が消えない。止めたい利用者は「停止」を押す（押せば印は下りる）。
- 関連: `AnalysisSession.start(autoResume:)` / `resumeIfPending()` / `AnalysisSessionPolicy` /
  `AppSettingsKeys.analysisSessionPending` / `MosaicPhotosApp`（scenePhase）/ `RootView` /
  ADR-182 / ADR-25 / diagnostics-81。

## ADR-188 ゲートは「やめる合図」ではなく「待つ合図」。そして解析は自分の取得で自分を止めない
- 状態: 採用
- 文脈: diagnostics-81 の夜、OS がくれた処理枠 2 回（4分19秒・4分56秒）で CLIP 埋め込みは
  **合計 24 枚**しか進まなかった（残り 35,583 枚・推論時間の合計は 0.37 秒＝ANE は遊んでいた）。
  2 つの独立した原因があった。
  1. **ゲートを「やめる合図」に使っていた**: 埋め込みは `while !heavyShouldPause()` で回しており、
     入口で閉じていると**ループに一度も入らず実行ごと終了**する。実機ログは
     `embed loop entry (pause=true)` のあと期限切れまで無音——**枠の残り 3 分が丸ごと捨てられた**。
     閉じていた理由はバックアップの一括ロード（`HeavyLoad`）とサムネのドレインで、
     どちらも数秒〜数十秒で開くものだった。タグ付けの段には `waitWhilePaused` があったのに、
     埋め込みの段にだけ無かった（非対称が原因）。
  2. **解析が自分の取得で自分を止めていた**: クラウド写真の埋め込みは表示用と同じ
     `thumbnail(for:)` を通り、バッチャはドレイン中に `cloudThumbnailBusy`（＝「UI が忙しい」の印）を
     立てる。その印は `uiBusy` → `heavyWorkAllowedLocal` → `heavyShouldPause()` に直結するので、
     **サムネを取りに行くという行為そのものが停止条件**になっていた（`clip.pauseWait` が
     2.5 秒あたり 4〜7 回）。顔スキャンだけは ADR-179 で取得経路を分けて回避済みだったが、
     CLIP とタグには同じ手当てが入っていなかった。
- 決定:
  1. `BackgroundTrickle.runPhasesWaitingForGate` を新設し、埋め込みループを**待つ**側へ変える。
     待ちの上限（60 秒）を超えたときだけ畳む＝実行中フラグは握り続けない（ADR-95 は不変）。
  2. サムネ要求に**目的**を持たせる（`ThumbnailPurpose.display / .analysis`）。解析は
     `analysisThumbnail(for:)` で中優先レーン（表示の後ろ・先読みの前）に並び、
     `cloudThumbnailBusy` を**名乗らない**。UI ビジーは「表示のための取得が残っているか」だけで決める。
  3. ゲート側も ADR-179 の規則へ揃える: `BackgroundYield.analysisShouldYieldToUI` ＝
     **前面で誰かが見ている間だけ譲り、背面ではメモリ圧迫だけ見る**。顔アダプタのインライン判定も
     これに一本化した。
- 結果: 処理枠の残り時間が埋め込みに使えるようになり、解析の自己停止が消えた。
  表示は従来どおり最優先（人が見ているサムネが解析の行列に埋もれない）。
  ⚠️ トレードオフ: 背面ではサムネのドレイン中でも解析が回るので、バックアップ・共有と
  帯域を share する（資源が別なので ADR-180 の並行と同じ判断）。
- 関連: `BackgroundTrickle.runPhasesWaitingForGate` / `AutoAlbumEngine+Recognition`（埋め込みループ）/
  `DropboxThumbnailBatcher`（`ThumbnailPurpose`・`displayInFlight`）/ `DropboxPhotoStore.analysisThumbnail` /
  `BackgroundYield.analysisShouldYieldToUI` / `AnalysisYieldGateTests` / `ThumbnailPurposeTests` /
  `GateWaitingPhaseLoopTests` / ADR-179 / ADR-95 / ADR-131 / diagnostics-81。

## ADR-187 クラスタ ID は再利用しない・ユーザーが表明した人物の行は消さない
- 状態: 採用
- 文脈: 実フィードバック「ピープルアルバムの中身が、気がついたらごっそり別人になっていた」
  「AI アルバムで『XX ではない』を選んでも効かない」。コードを追うと ID の再利用があった:
  新しいクラスタの ID は**既存クラスタの最大 ID + 1**で採番していた（`loadClustering` の `nextID`・
  再クラスタの `minimumNextID` も同じ）。最大 ID の人物が消える——写真の削除・付け替えで最後の顔が
  抜ける（`removeFromCluster` は最後の 1 顔でクラスタ行ごと削除）・掃除（ADR の幽霊掃除で 14 クラスタ）
  ——と、**次に生まれた別人が同じ ID を受け取る**。ID で参照している側（ピープルグループのメンバー・
  共有セットの作成元 `person(N)`・開いたままの人物アルバム・消えたクラスタを指したままの顔）は
  そのまま別人を指す。同じ理由で、`solePerson` が返す人物と `removePhoto` の対象が食い違い、
  「XX ではない」が 0 件（対象なし）で空振りし得る。
- 決定: (1) **クラスタ ID の高水位**（消えた ID も含む最大値）を UserDefaults に持ち、新規 ID は必ず
  その先から（`FaceStore.clusterIDHighWater` / `noteClusterIDs`・インメモリの店はテストどうしで
  繋がらないよう店内に持つ）。(2) **ユーザーが表明した人物（名前・束ね・代表写真）の行は消さない**
  （`isUserClaimed`）——最後の顔が抜けても空の人物として残す（次に本人の顔が来れば同じ名前で復活）。
  掃除（`pruneMissingPhotos`）も同じ規則。(3) 消えたクラスタを指したままの**孤児の顔**は起動時と掃除の
  前に未割当へ戻す（`repairOrphanFaces`）。
- 結果: ID は単調に増え、参照側が別人を指すことは起きない。既に起きてしまった入れ替わりは
  自動では戻せない（どの ID が誰だったかの記録が無い）——グループ・共有セットは作り直し。
  テスト: 消えた最大 ID を新人物が受け取らない（高水位を外す変異で落ちる）・名前付きの行は最後の顔が
  抜けても残る・孤児の修復（`ClusterIdentityTests`）。
- 関連: `FaceStore.swift`（高水位・`isUserClaimed`）/ `FaceStore+Edit.removeFromCluster` /
  `FaceStore+Prune`（掃除の規則・`repairOrphanFaces`）/ `PeopleEngine.makeWithOffMainStore` / ADR-130 / ADR-132。

## ADR-186 モデル更新は索引を「少しずつ」新モデルへ移す（行ごとの版・顔は影の世代・台帳は消さない）
- 状態: 採用
- 文脈: 学習済みモデル（CLIP・顔）は今後も更新される。従来はモデル更新＝版番号を上げて**索引を一斉に
  作り直す**（CLIP は全写真を未埋め込みに戻す＝検索が空になる、顔は全消去→再スキャン＝ピープルが痩せる）。
  8 万枚の再解析には数週間かかり、その間ユーザーは劣化を我慢する。実フィードバック:
  「写真を少しずつ順番に新しいモデルで解析していけるように」「DB 構造が変わって解析データが
  無くなったら嫌」。
- 決定: 索引の性質で 3 通りに分ける（`ModelGeneration` の doc に一覧）。
  1. **シーンタグ**: 行ごとの版（既存・`PhotoTagRecord.version`）。変更なし。
  2. **CLIP 埋め込み**: 行ごとに `PhotoEmbedding.modelID`（optional・nil＝列導入前＝現行）。
     `ModelGeneration.clip` を変えると、未埋め込みが尽きた後に**旧モデルの行を新しい写真から上書き**
     （`staleEmbeddingRefKeys`・`PhotoTagger`）。検索は現行モデルの行だけを使う（空間を混ぜない）。
     旧テキストタワーを 1 リリース同梱して未移行の行も当てる二重空間検索は、検索側の対応
     （空間ごとのクエリ埋め込み）とセットで最初のモデル更新時に実装する（`retainedClipTextTowers` の注記）。
     `perceptionVersion` は知覚ロジックの版として残すが、モデル更新では上げない。
  3. **顔**: クラスタという全体構造を持つので行の版では混ぜられない。同梱モデルの ID
     （face_config.json の `model`＝`FacePerceptionProvider.modelID`）が現行世代（UserDefaults
     `faces.activeModel`・既定 "auraface-v1-r100"＝コンテナ "FacesV1"）と違えば、**影の世代**
     `Faces-<id>` を別コンテナで育てる（`PeopleEngine.shadowStore`・スキャンは影へ・表示は旧のまま）。
     網羅が候補の 90% に達したら `promoteShadow`: 名前は写真の重なりで移し（既存の持ち越し・ADR-51/169）、
     グループは名前で結び直し、現行世代を差し替える。**旧コンテナは消さない**（1 リリース残す）。
     影を育てている間は版上げの全再スキャン（`migrateScanVersionIfNeeded`）を止める。
- 台帳を消さないための約束（ユーザーの学習結果＝人物名・束ね・修正）:
  - スキーマ変更は **optional 列の追加だけ**（軽量マイグレーション）。**コンテナ名の採番はしない**
    （採番＝旧ストアの破棄。`AutoAlbumV10` の慣習はキャッシュ向けの逃げ道で、台帳には使わない）。
  - FacesV1 は `.ledger` 方針に変更（壊れても削除せず退避）。
  - `StoreSnapshot`: 台帳はアプリの版が変わった最初の起動で**開く前に控え**を取り、開けなければ控えから
    戻して 1 回だけ再試行（`makeResilientModelContainer`）。
- 持ち越せないもの: 埋め込みで記録した修正（`FaceCorrection` の負例・確認）。空間が違うので意味を持たず、
  新世代には効かない（同じ誤りが再び出ることがある）。今後の修正記録は写真の組を併記する方向。
- 結果: モデル更新の日に検索が空になったり、ピープルが痩せたりしない。AI 解析画面に「モデルの更新」節
  （索引の残り・顔の影の世代の進み具合）。テスト: 埋め込みの移行（新しい順・legacy 扱い・上書き・
  検索除外）・影の世代（旧に書かない・網羅不足なら切り替えない・切り替えで名前とグループが移り旧は残る）・
  控えの取得と復元。
- 関連: `PerceptionCore/ModelGeneration.swift`（方針の一次記述）/ `PhotoEmbedding.modelID` /
  `AutoAlbumStore.staleEmbeddingRefKeys` / `PhotoTagger` / `FaceStore.containerName(for:)` /
  `PeopleEngine+Generation.swift` / `MosaicSupport/StoreSnapshot.swift` / `ResilientModelContainer` /
  `EmbeddingModelMigrationTests` / `FaceModelGenerationTests` / `StoreSnapshotTests` / ADR-31 / ADR-51 / ADR-70 / ADR-169。

## ADR-185 ディスクキャッシュは「1 つの予算を全キャッシュで共有し、価値の低い順に捨てる」（総容量の 10%・変更可）
- 状態: 採用（ADR-184 を置換）
- 文脈: キャッシュが 5 つ（端末サムネ・Dropbox サムネ・Dropbox 本体画像・顔アバター・spool）あり、
  上限がそれぞれ別の場所に別の既定で置かれていた（端末サムネ 500MB→ADR-184 で 10%・Dropbox サムネ
  **50MB**・本体 200MB・アバター無制限）。Dropbox サムネ 50MB は 2,000 枚ぶんで、クラウドの顔スキャンが
  「キャッシュ済みサムネだけ」を使う設計（ADR-88）と噛み合わず、スキャンできる写真もそこに縛られていた。
  「枚数から割り当てを計算する」案は今の枚数のスナップショットで線を引くことになり、写真が増えれば
  引き直し、ユーザーが違えば別の線＝筋が悪い（実フィードバック）。
- 決定: キャッシュごとの上限を**持たない**。予算 B＝端末の総容量 × 10%（設定で 5% / 20% / 固定 GB）を
  全キャッシュが共有し、合計が B を超えたら**価値の低い層から**捨てる:
  第 3 層 本体画像（先読みしただけ → 開いたもの）→ 第 2 層 顔アバター → 第 1 層 サムネ（端末・Dropbox を
  区別せず、最も古く触った方から）。各層に床（本体 10%・派生 2%・サムネ 50%）。安全弁として
  空きが 2GB を切る手前で予算を縮める。spool は一時（転送が終われば消える）なので予算の外。
  実装は `ImageCacheKit` の `CacheBudget`（予算）・`CacheBudgetPlanner`（1 手の決定＝純ロジック）・
  `CacheBudgetCoordinator`（actor・参加者に「いくら捨てるか」だけ配る）。各キャッシュは
  `BudgetedCache`（usage / oldestAccess / evict(bytes:)）で参加し、書き込み後に `noteGrowth()` を
  呼ぶだけ。判定はまとめて（3 秒デバウンス）行う——旧実装は Dropbox が書き込みごとに 6.8 万行を
  fetch して容量判定していた（ADR-119 型）。使用量は起動時に 1 回集計し以後は増減で追う。
  設定は Storage 画面の 1 つに一本化し、各ソースのディスク上限の Picker は撤去
  （旧キーは残置・未使用）。
- 結果: 枚数もユーザーも見ない規則になった。サムネはライブラリの成長に合わせて勝手に増え、
  捨てられるのは総容量の 10% を使い切ったとき（128GB 端末で 30 万枚超）。本体画像は「サムネが
  使っていない残り」を自動的に使う。テスト: 予算・安全弁・層の順序・同層の古い順・床・協調役の収束
  （`CacheBudgetTests`）。
- 追記（2026-09-06・同日の後続コミット）: 割合の選択肢に **30% / 40%** を足した（現行は
  5 / 10 / 20 / 30 / 40% ＋固定 GB 2〜100。`CacheBudget.percentChoices`）。クラウド中心の使い方では
  10% で足りないという実フィードバックによる。あわせて Picker 表示の "%%" エスケープ崩れと未翻訳を修正。
- 関連: `ImageCacheKit/CacheBudget.swift` / `ThumbnailCache`（`BudgetedCache`）/
  `DropboxCacheBudgetParticipant` / `DropboxCacheStore+Eviction`（`evict(kind:bytes:)`・先読み優先）/
  `FaceAvatarBudgetParticipant` / `StorageSettingsView` / ADR-88 / ADR-184。

## ADR-184 端末写真サムネのディスク上限は「総容量の 10%」を既定にし、設定で変えられるようにする
- 状態: **ADR-185 で置換**（同日。キャッシュごとの上限そのものを廃止）
- 文脈: 実フィードバック「ローカルサムネのディスク上限が少ない。ディスクの 10% はあってよい。変更もできるように。
  メモリの節にあるがメモリではない」。既定は 500MB 固定・選択肢は 2GB まで。1 枚 20〜40KB のサムネで
  500MB は 1.5 万枚ぶんにしかならず、8 万枚のライブラリではグリッドを往復するたびに捨てて作り直していた。
  ディスクは RAM と違って余裕がある。
- 決定: `diskLimitMB` の 0 を Auto＝**端末の総容量の 10%**（500MB〜50GB にクランプ・`ThumbnailDiskBudget`）
  とし既定にする。選択肢は Auto / 1 / 2 / 5 / 10 / 20 GB。旧既定（200/500MB）で保存されていた値は
  起動時に Auto へ引き上げる。設定画面は「サムネイル（メモリ）」「サムネイル（ディスク）」の 2 節に分け、
  ディスクの説明に「使うのは実際に閲覧した分だけ・上限で古いものから削除」を明記。
- 結果: 128GB 端末で約 12.8GB＝30〜60 万枚ぶん＝実質「一度作ったサムネは捨てない」。
  Dropbox 側のサムネ上限（既定 50MB）は据え置き（別途判断）。
- 関連: `ThumbnailDiskBudget.swift` / `ThumbnailCache.swift` / `LocalPhotoSettingsView.swift` /
  `ThumbnailDiskBudgetTests`。

## ADR-183 共有の解析データ（旧称サイドカー）をシャード化し、反映は共有ルートの再帰一覧 1 回で・作成元へ自動追従
- 状態: 採用（ADR-112 / ADR-166 の実装を置換）
- 文脈: 共有アルバム（人物・AI アルバム）は写真が増減する。反映は `copy_batch`（サーバー側コピー・
  転送なし）と `delete_batch` で軽いが、**解析データが 1 セット 1 ファイル**（CLIP 1.4KB＋顔 1.4KB/顔）
  で、12,941 枚のセットは 20〜30MB。写真が 1 枚増減するたびに送信側は丸ごと作り直して上げ、受信側は
  丸ごと落として全部デコードしていた（既取り込み分はデコード後に捨てる）。さらに反映のたびにセットごとに
  `create_folder`＋`list_folder`＋解析データ実在確認の `list_folder`（セット数 × 3 往復）、受信側も
  セットごとの `list_folder`（N+1）。作成元（人物・AI アルバム）が育っても共有セットは手動更新だった。
- 決定:
  (A) **シャード化**: `<セット>/.mosaic-share/shard-<xx>.json`（content_hash 先頭 2 桁・空は置かない）。
  写真ごとの解析結果は不変なので、触れたシャードだけが動く。送信側は**状態を持たない**——共有ルートの
  一覧にある各シャードの `content_hash` と、手元で組んだシャードの `content_hash`（同じ計算＝
  `DropboxContentHash`・決定的エンコード）を比べて「無い／違う → 上げる、余る → 消す」
  （`ShareAnalysisPlanning.plan`）。消された解析データの復元（ADR-166）は自然に含まれる。
  旧 `analysis-v1.json` は受信側が読み続け、送信側はシャードを置いたら消す。
  (B) **一覧は 1 回**: 送信側は共有ルートの `list_folder(recursive)` 1 回で全セットの写真・解析データの
  実在を得る（`RemoteShareIndex`）。セットの `create_folder` は一覧に無いときだけ。受信側も家族フォルダごとに
  再帰 1 回で全シャードの rev を得て、変わったものだけ落とす。rev の記録は一覧に無いパスを捨てる
  （旧「500 件超で末尾 300 件」はシャード化で取り込み済みの記録まで捨てて再取得を誘発する）。
  (C) **作成元への自動追従**: 処理枠の共有反映の直前に `refreshAllFromSource()`（作成元が現存する
  セットだけ・孤児は触らない）。反映の差分はシャード単位なので夜間に回しても軽い。
- 結果: 増減 1 枚あたりの転送は 25MB → 1 シャード（数十〜数百 KB）。往復はセット数 × 3 → 2〜3。
  受信側の再デコードは変わったシャードだけ。送信側の解析データ状態（`analysis dataChecksum`）は不要になった
  （モデルの列は残置）。テスト: シャード分割・差分計画・決定的エンコード・受信側の 1 回一覧と差分取得・
  旧形式の読み取り・作成元追従（`ShareAnalysisShardTests`）。既存の復元テスト（ADR-166）はそのまま通る。
  本物の Dropbox には接続していない。代わりに `FakeDropboxServer` を**本物に近づけた**——
  `list_folder` のページング（`has_more` → `list_folder/continue`・ページ幅を設定可）、再帰一覧に
  中間フォルダのエントリを含める（本物は必ず含む）、アップロードの `content_hash` は本物と同じ計算
  （独立に計算した既知ベクトルで検証済み）。その上で 3 セット × 60 枚・ページ幅 50 で「2 回目の反映は
  コピーもアップロードも無く一覧は 1 回」、1 枚外して「空になったシャードだけ消え他は触らない」、
  受信側がページを跨いで全シャードを拾う、を固定（`ShareLargeScaleTests`）。ページング打ち切りと
  「常に上げる」の変異で落ちることを確認。
- 関連: `ShareAnalysisData.swift`（`shards` / `shardPath`）/ `ShareSyncEngine+Sync.swift`
  （`RemoteShareIndex` / `ShareAnalysisPlanning` / `updateAnalysisData`）/ `ShareAnalysisFetch.swift` /
  `ShareSyncEngine.refreshAllFromSource` / `DropboxShareCopier.listFolder(recursive:)` /
  `HeavyWorkScheduler` / ADR-112 / ADR-166。

## ADR-182 「今すぐ解析」を解析セッション 1 本に統合し、iOS 26 の継続タスクに載せる
- 状態: 採用（ADR-165 の「開いている間」と旧「今すぐ処理（30 分）」を置換）
- 文脈: 処理枠（BGProcessingTask）は OS 裁量で、数分のロックでは開かず、開いても 5 分前後
  （diagnostics-73/74）。利用者が「充電台に置いて放置している間に進めたい」のに、手動の入口が
  「30 分ブースト」「開いている間」の 2 つに割れ、どちらもロックすると止まった（背面＝吊るされる）。
  調査の結果（処理枠の章 1.1）、iOS には CPU 時間を増やす権限は無く、使えるのは
  (a) 前面（無制限）、(b) 背景 URLSession（転送のみ）、(c) iOS 26 `BGContinuedProcessingTask`
  ——利用者が始めた作業をアプリを閉じても続け、進捗を Live Activity に出す新 API。
  Apple は自動のメンテナンス・バックアップ用途を禁じているが、ボタンで始める解析は適合する。
- 決定: 入口を **「今すぐ解析」1 ボタン・1 進捗・1 停止**にする（`AnalysisSession`・アプリ層）。
  タップで `BGContinuedProcessingTaskRequest`（`strategy = .fail`）を submit し、受理されれば
  **継続モード**（アプリを閉じても続く・Live Activity に進捗・そこから停止可）、拒否／シミュレータなら
  **前面のみモード**（画面を離れると止まる）に自動で落ちる。走らせる中身は処理枠と同じトリクル
  （顔 → タグ → 埋め込み）で、ゲートは `BackgroundYield.sessionActive` で開ける——熱・一括ロード保護・
  生成との相互排他（メモリ）は免除しない。**一枚岩（生成・AI アルバムの本番化）は起こさない**。
  進捗は `AnalysisSessionPolicy.progressUnits`（残作業の最大値を分母・モデルロード中も「準備中」の
  目盛りを進める＝報告しないタスクは OS に殺される）。停止条件は残作業ゼロ／停止操作／
  OS の expiration／電源なしで電池 20% 未満（`shouldStopForBattery`）。
  処理枠との衝突は `isActive` で避ける（処理枠は解析の起動を飛ばし、前面復帰の `stopForForeground`
  もセッションの作業を止めない）。旧「30 分ブースト」（`manualBoostUntil`）は撤去。
- 結果: 充電台＋「今すぐ解析」で処理枠を待たずに進む。**既知の制約**: 端末を本当にロックすると
  継続タスクが止まる iOS のバグ（FB19916760・DTS が「端末を起こす assertion の付け忘れ」と認めた・
  26.2 beta 3 でも未修正）。直るまでは「解析中は画面を消さない」（既定 ON）が命綱。ロックで止まっても
  差分は残り、次の処理枠か次のタップで続きから進む。バックアップは別の関心事として今回は含めない
  （同じ仕組みで「今すぐバックアップ」を後から足せる）。
- 関連: `MosaicPhotos/AnalysisSession.swift` / `AnalysisSessionPolicy.swift` /
  `AnalysisSessionPolicyTests` / `BackgroundYield.sessionActive` / `AIAnalysisStatusView` /
  `Config/Info.plist`（`com.kanai.MosaicPhotos.analyze`）/ ADR-165 / ADR-179 / 処理枠の章 1.1。

## ADR-181 夜間バックアップは背景 URLSession に持ち出す（fire-and-forget・OS が窓の外でも上げ続ける）
- 状態: 採用（ADR-180 の「残る限界」への回答）
- 文脈: ADR-180 で夜間バックアップは毎窓・上限なしになったが、アップロードは前面の `URLSession`
  なので **BGTask の窓（約 5 分）を出られない**。1 枚 1〜2 秒で 150〜300 枚／窓が天井で、
  6 万枚の上げ直し（ADR-175）は何百晩もかかる。窓の中でしか動けないのは「応答を待って台帳に
  書く」構造のせいで、転送そのものは OS（`nsurlsessiond`）に任せられる。
- 決定: 夜間実行だけ、写真を **spool（`Caches/BackupSpool/<uuid>.bin` ＋ 意図 `<uuid>.json`）**
  に置いて背景 `URLSession`（identifier 固定・`sessionSendsLaunchEvents`）の `uploadTask(fromFile:)`
  へ渡し、応答は**窓の外・別プロセス起動でも**受け取る。
  ```
  窓の中（数十 ms／枚・ディスク速度）                      窓の外（OS 任せ・何時間でも）
   読む → hash → spool に書く → uploadTask ── 転送 ──▶ 応答 → hash 照合 → ジャーナル → 台帳「済み」→ spool 削除
  ```
  **不変条件（ADR-40 / ADR-171 を崩さない）**: (1) 「済み」は応答の `content_hash` が投入時の
  hash と一致したときだけ（`BackgroundUploadSession.classify`）。(2) メタデータのジャーナル →
  記録の順（`BackgroundSettlement.perform`）。(3) 応答が来ない／失敗した spool は次の窓で
  再投入（3 回で諦める＝次回の通常対象に戻る）。(4) **409 は前面経路へ**——hash 照合と autorename
  は `uploadOne` の仕事なので、印（`conflict`）を付けて次の窓で前面経路に回す。
  (5) 転送中の写真は次の窓の対象から外す（spool が「転送中」の台帳を兼ねる）。
  上限は `BackgroundUploadPolicy`（400 枚／1 GB／空き 3 GB 未満は積まない）——積んでも速くならず
  ディスクを食うだけ。上限で窓を閉じ、残りは次の窓（捌けた分だけ空く）。
  手動実行は前面経路のまま（進捗を目で追える・上限で待ち時間が有界）。
- 結果: 窓の中の仕事が「読んで置く」だけになり、1 窓で数百枚を OS に渡せる。転送は夜通し続き、
  完了は `AppDelegate.handleEventsForBackgroundURLSession` で起こされて台帳へ落ちる
  （台帳更新が**全部終わってから**完了ハンドラを呼ぶ＝早く呼ぶと更新の途中で吊るされる）。
  制約: 背景で投入した転送は OS が裁量扱い（電源＋Wi-Fi 時に流す＝夜間条件と同じ）。トークンは
  投入時のものなので、4 時間を超えて遅れた転送は 401 → 次の窓で再投入。spool は Caches
  なので OS に消されうるが、消えても写真が次回の対象に戻るだけで失うものは無い。
  テストは純ロジック（意図・spool・上限・振り分け・応答分類・落とし方の順序）を macOS で回し、
  hash 照合とジャーナル順序は**変異で落ちること**を確認済み。
  追記（同日レビュー）: 初版は「この実行で積んだ分があるときだけ OS へ渡す」だったが、期限切れで
  積んだまま渡せなかったジョブは次の窓で「転送中」扱い＝対象外になり、新しく積むものが無い窓では
  **誰も拾わず永久に残る**。渡す処理を無条件にした（`flushIsUnconditional`）。
  バックグラウンド処理枠全体の時系列・資源・見積りは `deep-dive/background-window.html` に専用章を設けた。
- 関連: `BackupKit/BackgroundUpload/`（`UploadSpool` / `BackgroundUploadSession` /
  `BackupRunner+BackgroundUpload` / `BackupEngine+Settle`）/ `MosaicPhotos/AppDelegate.swift` /
  `BackgroundUploadTests` / ADR-40 / ADR-171 / ADR-180。

## ADR-180 夜間のバックアップは解析と並行して毎窓走らせ、1 回あたりの上限を外す
- 状態: 採用（ADR-72 の順番回しを置換）
- 文脈: 実フィードバック「バックアップは SSD とネットワークしか使わず CPU は限定的。画像解析と
  並行して動かしてよい。現状あまり動いていない」。コードを追うと、動かない理由は 3 つあった。
  (1) 夜間の起動条件が「埋め込みの残作業が無い窓」→「4 回に 1 回」と譲っていた（ADR-72）。
  (2) 「1 回あたり 10 枚」の既定上限が夜間にも当たり、5 分の窓に 10 枚しか上がらない
  ——ADR-175 で 6 万枚を上げ直す状況では何年も終わらない。(3) 窓の期限（約 5 分）で止まる。
  一方、資源の観点では並行して構わない: バックアップは 1 枚読んで上げる trickle で、
  CPU/ANE を使う解析とは重ならない。ADR-72 が譲らせた根拠（メモリのピーク）は、
  当時の一括読み込みの話で、いまの実装には当たらない。
- 決定: (1) 毎窓、解析と**並行して**起こす（見送りを廃止）。(2) 夜間（自動）は**枚数の上限を外す**
  ——夜間には窓の期限という別の上限がある。手動実行の上限は従来どおり（待ち時間を有界にする）。
  (3) 窓の期限での停止は維持（「済み」は hash 検証後にしか付かないので中断は安全＝ADR-40）。
- 結果: 窓のあいだ、解析とバックアップが同時に進む。1 窓で上がる枚数は「窓の長さ ÷ 1 枚の所要」
  （実測 1 枚 1〜2 秒なら 150〜300 枚／窓）。
  ⚠️ **残る限界**: アップロードは前面の `URLSession` なので窓の外には出られない。
  `URLSession` の背景転送に載せれば OS が窓の外でも上げ続けるが、hash 検証つきの台帳更新
  （ADR-40）を fire-and-forget に作り替える必要があり、別途の設計になる。
  並列アップロード（2〜3 本）も、台帳の順序とレート制限の計測をしてから。
- 関連: `HeavyWorkScheduler.swift` / `BackupEngine.swift`（`isNightlyRun`）/
  `NightlyUploadLimitTests` / ADR-40 / ADR-72 / ADR-163 / ADR-175。

## ADR-179 夜間の顔スキャンは「画面が無い」ので UI への譲りをしない／空振りが続いたら畳む
- 状態: 採用
- 文脈: 実フィードバック「ピープルの夜間解析が進まなくなった」。コードを追うと 2 つの欠陥が
  重なっていた。(1) クラウド写真の解析画像取得（`cloudAnalysisImages`）は `BackgroundYield.uiBusy`
  なら**空を返す**。`uiBusy` には `cloudThumbnailBusy`（Dropbox サムネ取得のドレイン中）が含まれ、
  夜間ウィンドウでは画面が無いのに、バックアップ・共有・生成の準備がサムネ取得を走らせて
  これが立ち続ける。残作業はすべてクラウド写真（20,078 枚）なので、**全バッチが空**になる。
  (2) `FaceTagger` は「全件未解析のバッチ」を**進めて次へ**としていたため、画像が一切取れない
  状態が続くと todo 全体を空で歩き切って `finished — scanned=0` で終わり、次の窓でまた同じ。
  ログ上は「動いた」ように見えるのに 1 枚も進まない。
- 決定: (1) UI への譲りは**アプリがアクティブなときだけ**。非アクティブ（夜間）ではメモリ圧迫
  （jetsam の保護）だけを見る。(2) 連続 3 バッチ（48 枚）で 1 枚も解析できなければ**今回は畳む**
  （原因は続いているので窓を無駄にしない）。畳んだことと進捗を診断ログに残す。
- 結果: 夜間の顔スキャンがサムネ取得の有無に左右されなくなる。空振りは最大 48 枚で止まり、
  次の窓に回る。
- 関連: `AutoAlbumAdapters.swift`（`cloudAnalysisImages`）/ `FaceTagger.swift` /
  `FaceTaggerRecordingTests` / ADR-92 / ADR-163。

## ADR-178 キーセット・ページングは並びと `>` の順序を揃える（`.lexical`）
- 状態: 採用（ADR-143 の補足・**実害の疑いあり**）
- 文脈: 「別の人かもしれない写真」の上限撤廃（ADR-177）でページ送りを書き、規模テストの
  **変異検証**で「160 顔中 100 顔しか読めず、5 顔が二重」が発覚した。原因は
  `SortDescriptor(\.refKey)` の既定が数字を意識した順（"dsc5" < "dsc49"）で並べる一方、
  `#Predicate` の `>` は単純な文字列比較で、**並びと比較の順序が食い違う**こと。
  カーソル `"…-49"` の次のページに `"…-5"`（辞書順で大きい）が再び現れ、`"…-100"` 以降
  （辞書順で小さい）が二度と来ない。
  同型のコードが **`AutoAlbumStore` に 2 箇所**あった——`allEnrichedPhotosLite`（AI アルバム・
  旅行・検索の母集団）と `enrichmentVectorPage`（意味検索のベクトル）。テストで再現すると
  埋め込みは 160 件中 100 件、メタ一覧は 5,200 件が 5,755 件（重複）だった。
  実データの refKey（`L-<UUID>`・`C-/写真/2024-05-03/dsc03019.jpg`）は数字を含むので、
  **実機でも意味検索の母集団から写真が黙って抜けていた可能性が高い**。ADR-143 で潰した
  `Duplicate values for key` の別経路でもある。
- 決定: キーセット・ページングの `SortDescriptor` は必ず `comparator: .lexical` を指定する
  （3 箇所すべて）。テストは**件数と一意性**を見る（回数だけだと「少ないほど良い」に見えて
  取りこぼしを見逃す）。
- 結果: 3 箇所とも全件をちょうど 1 回ずつ返す。実機では次の再評価で母集団が増える可能性がある
  （欠けていた写真が戻る）。
- 教訓: **変異検証は書いた本人の思い込みを壊す。** 「fetch 回数が 4 倍以下」という緩い assert は
  1 顔 1 fetch の変異にも通ってしまい、そこから本物の欠陥まで辿り着いた。
- 関連: `AutoAlbumStore.swift` / `FaceStore+Explain.swift` / `EmbeddingPagingOrderTests` /
  `ScaleRegressionTests.outlierScanIsPaged` / ADR-143 / ADR-119。

## ADR-177 「別の人かもしれない写真」は人物の大きさで諦めない
- 状態: 採用
- 文脈: 実フィードバック「（未確認）顔が N 枚あり…と出る。候補を上げられないか」。
  `outlierFaces` は 8,000 顔で打ち切っていた——`faces(inCluster:)` が埋め込み（1KB/顔）ごと
  全件を materialize するため。しかし混入は**いちばん大きな人物でこそ起きる**ので、
  大きい人物ほど調べられないのは本末転倒。
- 決定: 埋め込みは採点に要るので射影では省けない。代わりに**ページで読み**（500 件）、
  手元には「似ている度の低い上位 `limit` 件」だけを残す（有界）。`tooManyMembers` は撤去。
- 結果: 5,000 顔の人物でも候補が出る。読み出しはページ数に比例（規模テストで固定）。
- 関連: `FaceStore+Explain.swift` / `FaceDecisionExplainTests` / ADR-96 / ADR-164 / ADR-178。

## ADR-176 バックアップの写真は撮影年月のフォルダに分ける
- 状態: 採用（ADR-175 の補足）
- 文脈: 写真本体を `Backup/` 直下に平置きしていた。8 万枚規模だと Dropbox の Web/アプリで
  開くだけで重く、`list_folder` の一覧取得も長い（実フィードバック「フォルダで整理した方が
  いいのでは」）。ADR-175 で台帳をリセットして全部上げ直すタイミングなので、構成を変えるなら
  いまが最も安い。
- 決定: `Backup/<YYYY>/<YYYY-MM>/<ファイル名>`（撮影日不明は `Backup/undated/`）。
  月の決め方はメタデータのシャード（`.mosaic/meta/<YYYY-MM>.json`）と**同じ関数**
  （`BackupMetadataV2.shardName`＝UTC 固定）を使い、「この月の写真とそのメタデータ」が
  対応するようにする。年で 1 段挟むのは `Backup/` 直下に月フォルダが百数十個並ぶのを
  避けるため。`.mosaic/` は直下のまま（数十ファイル・カタログの入口は固定パスの方が復元に有利）。
  共有セットの中は**平置きのまま**（人物・アルバム単位で数百件が上限。家族が開いたとき
  月フォルダをまたぐ方が不便）。
- 結果: 月あたり数百枚に収まる。台帳・オフロード・共有はいずれもフルパスを記録して参照し、
  照合の一覧は再帰なので、下流に「直下にある」前提は無い（確認済み）。
  同名衝突は月が分かれることでむしろ減る（同月なら既存の autorename 経路）。
  ⚠️ 実際のアップロード経路は `PHAsset` 依存で単体テストできないため、フォルダの導出
  （純関数）だけをテストで固定した。実機では最初のバックアップ後に Dropbox 上で確認する。
- 関連: `BackupLayout.photoFolder` / `BackupRunner.swift` / `BackupPhotoFolderTests` / ADR-38 / ADR-175。

## ADR-175 Dropbox の配置を 1 ルートにまとめる（`<root>/<端末>/Backup` と `Share`）
- 状態: 採用
- 文脈: バックアップ（`/MosaicPhotos/<端末>/`）と共有（`/MosaicShare/<端末>/`）が**別のルート**で、
  端末フォルダが 2 箇所に散り、Dropbox 上で「どこに何があるか」が分かりにくかった
  （実フィードバック「わかりやすく整理したい」）。
- 決定:
  ```
  /MosaicPhotos/<端末>/Backup/   ← 写真本体と .mosaic メタデータ
  /MosaicPhotos/<端末>/Share/    ← 共有セット（People-◯◯ / Album-◯◯ …）
  ```
  (1) パスの組み立てを `BackupLayout`（純ロジック）に集約する。**冪等**——配下のパスを渡されても
  `…/Backup/<端末>/Backup` と二重にならない（実装直後に二重化し、テストが捕まえた）。
  (2) 共有ルートの設定（`/MosaicShare`）は**廃止**し、バックアップルートから導出する。
  設定画面の入力欄は表示だけにする。
  (3) **既存データは移行しない**（ユーザー判断）。バックアップは配置の版（`layoutVersion`）で
  台帳をリセットして新配置へ**上げ直す**。共有セットは記録を `.pending` へ戻して新配置へ
  **コピーし直す**。旧フォルダは Dropbox に残り、片付けは人が行う（設定画面に案内を出す）。
  (4) **オフロード台帳は消さない**——オフロード済みの写真は端末に無く、旧パスのクラウド代替が
  唯一の実体。参照を失うと辿れなくなる。
  (5) 受信側: 家族が `/MosaicPhotos` ごと共有してきても `Backup/` の写真を共有アルバムに
  しない（`Share/` 配下だけを見る）。旧配置（`/MosaicShare`）の家族フォルダは従来どおり。
  (6) 表示: 自分の共有ルート（新）と旧ルート（設定に残っていれば）を Cloud/All から隠す。
- 結果: 端末ごとに 1 箇所を見れば済む。トレードオフ: 初回は**写真の再転送**が起きる
  （新パスには 409/hash 照合が効かない）。家族は共有フォルダの**招待し直し**が要る。
- 関連: `BackupLayout.swift` / `BackupEngine.swift`（`resetForLayoutChangeIfNeeded`）/
  `ShareSettingsKeys.swift` / `ShareSyncEngine+Sync.swift`（`migrateFolderIfNeeded`）/
  `SharedAlbumDiscovery.swift` / `ShareSupport.swift`（`ShareVisibility`）/
  `DeviceBackupRootTests` / `BackupLayoutChangeTests` / `ShareScenarioTests` / ADR-41 / ADR-112。

## ADR-171 メタデータは 1 枚ごとに追記し、完了記録より先に永続化する
- 状態: 採用（残課題 P1・dataLoss）
- 文脈: 人物名・アルバム・位置は**実行の最後にまとめて**送っていた。夜間ウィンドウは毎回
  `expired` で終わるので、途中終了すると**それまでの全件**が失われる。しかも写真本体は
  進捗台帳に載って次回の対象から外れるため、**二度と作られない**（端末を消すと復元不能）。
- 決定: 追記ジャーナル（1 行 1 エントリの `.jsonl`）を作り、**アップロード成功のたびに 1 行**
  足す。本体（JSON 全体）に書かないのは、毎回まるごと書き直すため枚数の 2 乗に比例するから。
  順序は「**メタデータを永続化 → 完了記録を保存**」。永続化できなければ完了記録も作らない
  （次回この写真をもう一度通す。実体は Dropbox にあるので再アップロードは起きない）。
  送信が確認できた後にだけジャーナルを消す（本体へ書いてから消す＝順序も逆にしない）。
- 結果: 途中終了しても、それまでに上げた全写真のメタデータが残る。壊れた行は読み飛ばす
  （途中終了はファイル末尾を壊し得るので、1 行の破損で残り全部を失わない）。
- 関連: `PendingMetadataStore.swift` / `BackupRunner.swift` / `PendingMetadataJournalTests`。

## ADR-172 共有の墓標は「不在を確認できるまで」消さない
- 状態: **撤回（→ ADR-209）**。反映が差分になり、墓標という仕組みごと不要になった——
  遅れて完走したコピーが何を作っても「望ましくない」側に入るので、次の反映が消す。
  以下は当時の記述。（残課題 P1・ADR-112 の補足）
- 文脈: 発行済みの `copy_batch` は、クライアントが諦めてもサーバー側で走り続ける。単枚の墓標を
  猶予（15 分）で捨てていたため、**その後にジョブが完走すると外したはずの写真が残り続ける**
  ——記録には無いので以後どの反映でも掃除されず、家族には見えたまま。
- 決定: 単枚の墓標は**時間で消さない**。削除を投げたうえで親フォルダを 1 回ずつ見て、
  **実在しないことを確かめられたものだけ**消す。一覧が取れない（通信断・フォルダ消失）ときは
  残して次の反映で再確認する。無限に増えないよう件数の上限（5,000・古い順に破棄）だけ置き、
  そこに達したら記録に残す（確認が延々できていない＝異常事態）。
  フォルダ墓標は従来どおり猶予で消す（フォルダは作り直せば済み、残す害の方が大きい）。
- 結果: 遅れて完走したコピーも次の反映で掃除される。
- 関連: `ShareSyncEngine+Sync.swift` / `ShareSettingsKeys.swift` / `ShareTombstoneRetentionTests`。

## ADR-173 端末写真の削除は、実行の直前にもう一度バイト列で照合する
- 状態: 採用（残課題 P1・wrongDeletion）
- 文脈: オフロード（実削除）は実行前に照合しているが、そこから実際の削除までに
  **台帳の書き込みと OS の確認ダイアログ**が挟まる。ダイアログはユーザーが放置すれば
  何分でも開いたままで、その間に写真が編集・置換されても、クラウド側の同一コピーが消えても、
  最初の照合結果は古いまま。`modificationDate` が変わらない置換もあるので日付では足りない。
- 決定: `deleter.delete` の**直前に**、対象（≤ limit・往復は有界）を**バイト列から**取り直して
  照合し、hash が変わったもの・クラウド側が消えたものを外す。外した分は台帳から戻す
  （消さないものを「オフロード済み」にしない）。
- 結果: 検証後の変更・置換・クラウド側の消失を検出して削除を見送る。
  ⚠️ **残る窓**: 最終照合から `delete` 呼び出しまでの間は原理的に閉じられない
  （PhotoKit に原子的な compare-and-delete が無い）。ただし削除は「最近削除した項目」へ入るので、
  30 日間は復元できる。この 2 点をもって受け入れ可能と判断する。
- 関連: `OffloadService.swift` / `OffloadFinalRecheckTests` / ADR-168。

## ADR-174 画像は「開始時のアカウント」を照合してから保存・配達する
- 状態: 採用（残課題 P2）
- 文脈: 一覧の読み込みには世代＋アカウントの札（`shouldApplyLoad`）があるが、**画像経路には
  無かった**。フル画像・原本の取得は「ダウンロード → デコード」と待ち合わせが複数あり、
  その間にアカウントが切り替わると、旧アカウントの画像が**新しいキャッシュへ書かれる**
  ——同じパスの写真として別人の画像が出て、キャッシュを消すまで直らない。
- 決定: 取得の開始時に札を控え、(1) **キャッシュへ保存する直前**と (2) **返す直前**の両方で
  照合する。合わなければ保存も配達もしない（`shouldDeliverImage`）。
- 結果: 切替を跨いだ画像が混ざらない。⚠️ 同一パスに対する新旧 2 本のダウンロードが並走した場合の
  書き込み順序までは直列化していない（切替時にキャッシュを消しているので窓は狭い）。
  完全な直列化・原子的公開は、実害が観測されたときに検討する。
- 関連: `DropboxPhotoStore.swift` / `DropboxPhotoStore+Images.swift` /
  `DropboxImageDeliveryGuardTests`。

## ADR-168 バックアップは「今見えているもの」を上げる——編集済みは fullSizePhoto ＋ 実形式に沿った名前
- 状態: 採用
- 文脈: `BackupAssetReader` は `resources.first(where: { $0.type == .photo })` を最優先していた。
  `.photo` は**原画**で、写真アプリで編集した写真では画面に見えているものではない。
  一方オフロード（実削除）の適格判定は「バックアップ後に編集された」（`modificationDate >
  backedUpAt`）しか見ないので、**初回バックアップより前に編集した写真は素通り**する。
  削除直前の hash 再検証も同じ読み取りを通るため原画同士で一致し、`.eligible` → 実削除。
  編集結果はクラウドにもアプリにも残らず、「最近削除した項目」の保存期間を過ぎたら復元できない。
- 決定: (1) `.fullSizePhoto`（編集結果）があればそれを上げる。無ければ従来どおり `.photo`
  （未編集の挙動・名前とも不変＝既存バックアップを上げ直さない）。
  (2) 編集結果の名前は「**原画の stem ＋ `-edited` ＋ 実データの形式に対応する拡張子**」。
  `.fullSizePhoto` の `originalFilename` は "FullSizeRender.jpg" のように写真ごとに一意でなく、
  原画の拡張子を流用すると **`.HEIC` という名前の JPEG** ができる（共有＝`ShareTempFile` は
  Dropbox 上の名前をそのまま一時ファイル名にするので、他アプリへ誤った型で渡る）。
  拡張子は UTI → 先頭バイトの形式判定の順で決め、**`DeltaPageParser.imageExtensions`
  （Cloud 一覧の唯一の出典）に無い／決められないときは名前を作らずスキップ**する。
  (3) 編集レンディションが端末に無い（iCloud 最適化）ときは**原画へフォールバックしない**
  ——フォールバックすると「上げたものと見た目が違う」状態が復活する。
  (4) 読み取り結果に `isEditedRendition` を持たせ、オフロードの skip 理由を
  「編集結果が未バックアップ（クラウドは原画）」と言い分ける。
- 結果: 編集済み写真は現在の見た目が保全され、旧版で上げた編集写真は hash 不一致で
  **削除されない**（ドライランに理由が出る）。トレードオフとして、編集レンディションが端末に
  無い写真はバックアップ対象から外れる（喪失はしないが被覆率がその分下がる）。
  遡及はしない——旧版で上げた編集写真の Dropbox 側は原画のままで、差分判定を
  localIdentifier ベースから hash ベースへ変えるまで再アップロードされない（残課題）。
- 関連: `BackupRenditionNaming.swift`（新規・純ロジック）/ `BackupAssetReader.swift` /
  `OffloadService.swift` / `DeltaPageParser.swift`（`imageExtensions` を public 化）/
  `BackupRenditionNamingTests` / `OffloadRestoreFidelityTests` / `OffloadSafetyTests` /
  `MosaicPhotosTests/BackupEditedRenditionFixtureTests`（opt-in・実ライブラリ）/ ADR-40。

## ADR-163 夜間ウィンドウでは「1 つの仕事を終わらせる」——生成と解析を同じ窓で走らせない
- 状態: 採用（ADR-72 のバックアップ公平性ルールを生成にも広げる）
- 文脈: 実フィードバック「顔の認識が動いていない気がする」。実機 diagnostics-72 を見ると
  **動いてはいる**が、5 分の BGTask 窓で 280 枚しか進んでいない。内訳は
  「実作業 22 秒（1 枚 80ms）／2.5 秒休止 × 35 バッチ＝87 秒／残りは譲り待ち」。
  譲りの相手は**アルバム生成**で、生成は `isGeneratingAlbums` を立てるため
  `heavyShouldPause()` が真になり解析が止まる。しかも生成は窓内に終わらず毎回
  `generate: aborted`（このログでは中断 3 回・完了 0 回）。窓は「止まった解析＋終わらない生成」で
  空転していた。残りは 20,078 枚（すべてクラウド）で、この速度では数十日かかる。
- 決定: (1) 残作業（顔・埋め込み）があるうちは**アルバム生成を見送る**。ただしバックアップと
  同じく連続見送りの上限（4 回）を置いて順番を回す（生成も飢えさせない）。
  (2) 顔スキャンの既定を「8 枚 / 2.5 秒休止」→「**16 枚 / 0.5 秒休止**」にする。前面では
  スキャンを始めない（`isAppActive` で弾く）ので長い休止の相手は居らず、譲りは
  `shouldPause` が 1 枚ごとに見る。バッチ倍増はクラウドのサムネ往復（1 回 約 870ms）を半減させる。
- 結果: 1 つの窓が「解析を進める窓」か「生成を終わらせる窓」のどちらかになる。
  効果は次の実機ログの `faces: finished — scanned=N` と `bgtask: defer generate n/4` で確認する。
- 関連: `HeavyWorkScheduler.swift` / `FaceTagger.swift` / ADR-72（バックアップの公平性）/
  ADR-107（一枚岩は非アクティブ限定）/ 事例「顔の認識が動いていないように見える」。

## ADR-167 人物名も共有する（既定 ON・設定で切れる／受信側は「提案」として扱う）
- 状態: 採用
- 文脈: 共有解析データ（ADR-112）は顔（bbox・identity 埋め込み・品質・笑顔・撮影日）を運ぶが、
  **人物名は運んでいなかった**。そのため受信側では「同じ人だ」というまとまりはできるのに、
  名前は「Person 42」のままで、家族全員がそれぞれ名前を付け直す必要があった。
- 決定: 解析データの `Face` に `n`（名前）を足し、**既定で共有する**。
  設定「人物名も共有する」（`shareNamesEnabled`・共有を提供する画面）で送らない選択もできる
  ——名前は個人情報で、送らない選択ができること自体に意味がある。
  受信側は名前を**提案**として扱う: (1) **既に名前がある人物は上書きしない**
  （受信のたびに相手の呼び方へ書き換わるのは事故に近い）、(2) 同じ人物に複数の候補が来たら
  **多数決**（1 枚の外れ値で人物名が決まらないように）、(3) 名前は外部入力なので
  空白を落とし 64 文字で切り詰める。
- 結果: 家族の端末でも同じ人に名前が付いた状態になる。名前を送らない設定でも顔は共有され、
  グルーピングは従来どおり成立する（切り分けが独立している）。
- 関連: `ShareAnalysisData.swift` / `ShareSettingsKeys.swift` / `FaceStore+ShareExport.swift`
  （`applySharedNames`）/ `ShareNameTests` / `SharedNameImportTests` / ADR-112。

## ADR-166 共有と実体の整合を、開かなくても保つ（夜間反映・解析データ復元・週 1 照合）
- 状態: 採用
- 文脈: 実装状況を点検したところ、共有まわりに 3 つの穴があった。
  (1) **解析データが消えても復元されない**——アップロードは「内容が変わったか」だけで判定して
  おり、実在を確認していなかった。写真は自己修復されるのに解析結果だけ取り残される。
  (2) **バックアップの実体が消えても気づけない**——`reconcileWithDropbox()` は
  Developer Options からの手動のみ。共有は「バックアップ済みのコピーを複製」する仕組みなので、
  実体が消えると共有の自己修復もコピー元が無くて失敗し続ける。
  (3) **反映のきっかけが弱い**——「起動 25 秒後」「バックアップ完走後」「手動」だけで、
  アプリを開かない日が続くと反映も自己修復も走らない。
- 決定: (1) 解析データは**内容が同じときだけ実在確認**して、無ければ復元する
  （list_folder 1 回・変化があればどのみち上げるので追加コストは最小）。
  (2) `reconcileIfDueWeekly()` を足し、夜間ウィンドウから**週 1 回**照合する。
  時刻は**始める前**に記録して、失敗続きでも毎晩の全件一覧を避ける。
  (3) 共有の反映と受信を**夜間ウィンドウに入れる**（回線ポリシーは各処理の内側が見る）。
- 結果: アプリを開かなくても、共有の内容と Dropbox の実体が一致へ向かう。
- 関連: `ShareSyncEngine+Sync.swift` / `BackupEngine.swift` / `HeavyWorkScheduler.swift` /
  ADR-112 / ADR-163（窓の使い方）。

## ADR-165 「この画面を開いている間、解析する」を正面から用意する
- 状態: 採用
- 文脈: 背景ウィンドウ（`BGProcessingTask`）は **1 日に数回・1 回数分**しか来ない
  （実機 diagnostics-72: 5 分の窓のあと 8 時間半ゼロ）。2 万枚規模の積み残しは、
  自動処理だけでは何日経っても終わらない。従来の逃げ道は Developer Options の
  「重い処理のゲートを常に開く」だったが、**デバッグ用トグルを本来の用途外で使う**形で、
  戻し忘れると通常利用が重くなる。
- 決定: 設定 →「AI Analysis」に **「この画面を開いている間、解析する」** を置く。
  押している間だけ (1) 待機・電源のゲートを開き、(2) 画面を消灯させない
  （`isIdleTimerDisabled`＝消灯すると背景扱いで中断するため）、(3) 顔スキャンと
  背景処理を起こす。**停止・画面を離れる（`onDisappear`）で必ず元に戻す**。
- 結果: 「今すぐ終わらせたい」に正面から応える経路ができ、デバッグトグルの流用が要らなくなる。
  ⚠️ 外すのは**待機・電源のゲートだけ**——発熱（`ThermalGate`）と一括ロード保護（`HeavyLoad`）は
  `heavyShouldPause()` の手前にあるので外れない。「見ているから全力」と「端末を痛めない」を両立させる。
- 関連: `Settings/AIAnalysisStatusView.swift` / ADR-80（4 軸のゲート）/ ADR-163（窓の使い方）。

## ADR-164 取り消しの控えは射影で読む（大きな人物こそ戻したい）
- 状態: 採用（ADR-136 の補足）
- 文脈: 「1 対 1 の確認で間違えることがある。戻す機能がほしい」。機能は既にあるが
  （`FaceReviewView` のツールバー）、実機ログには `undo skipped — too many faces (5017)` が並び、
  **大きな人物の統合だけ控えが取れない＝戻すボタンが出ない**状態だった。
- 決定: 上限の理由は行数ではなく `embedding`（1 枚 約 1KB）の materialize だったので、
  控えの fetch を**射影**（faceID / clusterID / confirmedAt / contributesToCentroid）にし、
  1 手あたりの上限を 5,000 → **50,000** に上げる。無制限にはせず、控え全体の総量にも
  上限（60,000 顔）を置いて古い手から捨てる。
- 結果: 実ライブラリの統合（最大 5 千枚規模）が戻せるようになる。常駐は総量上限で有界。
- 関連: `FaceStore+Undo.swift` / `FaceUndoTests` / ADR-136 / ADR-96（射影で読む）。

## ADR-162 断片吸収のバーは 0.75 のまま。判定不能帯は実機の分布で決める
- 状態: 採用（ADR-154/155 の補足。K=2 の据え置きを再確認）
- 文脈: 実機で自動吸収が 1 件も発火しない（433 断片中 333 がバー 0.75 未満）。
  「バーが厳しすぎるのでは」を確かめるため、FG-NET / LFW でバー×K を掃引した。
- 決定: **バー 0.75・K=2 を据え置く**。根拠は (1) 0.60 未満は FG-NET でコイン投げ（50%）・
  LFW では K>2 の吸収が全滅、(2) 0.65〜0.80 は**両データセットとも発火せず判定不能**。
  判定不能帯を動かすかは、実機の分布（`faces: absorb` の「バー別に寄る数」）で決める。
- 結果: 「機械を大胆にする」方向は、いま手元にあるデータでは支持できない。ご要望
  （1〜2 枚の断片をいちいち確認したくない）に応える筋は**尋ね方を安くする**方——
  まとめて確認の候補下限を下げ、高い帯を既定でチェック済みにする——が次の候補として残る。
- 関連: `face-accuracy.md` 2026-08-31 の項 / `FaceStore+Absorb.swift`（バー別分布のログ）/
  ADR-154 / ADR-155 / ADR-148（ユーザー回答の分布）。

## ADR-161 `List` の行の中に遅延グリッド（LazyVGrid）を置かない
- 状態: 採用
- 文脈: 「調べる」画面で **間違い候補を『さらに表示』（24→48 枚）した直後**にアプリが落ちた。
  端末のクラッシュレポート（`bug_type 309`・`EXC_BREAKPOINT / SIGTRAP`）の落ちたスレッドは
  `UpdateCoalescingCollectionView.layoutSubviews` → `-[UICollectionView _updateVisibleCellsNow:]`
  が**7 段の再入**→ `-[UICollectionViewCompositionalLayout invalidateLayoutWithContext:]` →
  `libswiftCore._assertionFailure` で、アプリのコードは 1 フレームも無い。
  診断ログの足あと（ADR-159）は
  `inspector.report: rendered neighbors=15 outliers=48 ← inspector.report … ← people.reassignFace: done …`
  ——**付け替え後の読み直しで 48 枚を描いたところ**だと分かった。
  SwiftUI の `List` は UICollectionView 実装で、行の中の遅延グリッドは自分の高さを決めるたびに
  レイアウトを無効化する。件数が増えると再計算が収束せず、UIKit 側の assertion に当たる。
- 決定: `List` の行に `LazyVGrid` を置かず、**固定列数で自分で行に刻む**
  （`FaceGridRows.chunked` ＋ 1 行＝`HStack`）。高さが確定するので何件並べても再計算が回らない。
  `ScrollView` の中の `LazyVGrid` は従来どおり（そちらは問題にならない）。
- 結果: 「さらに表示」で件数が増えても落ちない。刻み方（順序・端数・件数）は純ロジックの
  テストで固定した。列数は固定 4（適応列と見た目はほぼ同じ）。
- 関連: `PeopleKit/FaceGridRows.swift` / `PersonInspectorView.swift` / `FaceGridRowsTests` /
  事例「『さらに表示』した直後に落ちる」・ADR-159（足あとが位置を特定した）。

## ADR-160 AI アルバムの再評価は「遅れている本だけ」流す
- 状態: 採用
- 文脈: 実機からディスク書き込みの資源警告（`diskwrites_resource`）が 2 通届いた——
  **33 分で 1,073.75MB**（限度は 24 時間で 1,073.74MB）。もっとも重いスタックは
  `AIAlbumService.rankedSearch` → `AIAlbumSearcher.searchWithPool` →
  `AutoAlbumStore.enrichmentVectorPage`＝**台帳の埋め込みページ送り**だった。
  ドリフト検知は「保存済みの**最小** `evaluatedEmbedCount`」というアルバム横断の 1 判定で、
  1 本でも遅れていれば `refresh` が全 5 本を作り直す。1 本の作り直しは埋め込み 85,090 件
  （約 1KB/件）を頭から流すので、**追いついている本のぶんだけ丸ごう無駄**になっていた。
- 決定: 起動条件（1 本でも遅れていれば動く）はそのままに、**作り直す対象をアルバム単位**で
  選別する（`AIAlbumDrift.needsFullEvaluation`＝純ロジック）。判定は従来と同じ 3 条件
  ——解釈器の版・相対日付の日跨ぎ・埋め込みの進み差が閾値超——をアルバムごとに見る。
  `refresh(onlyDrifted:)` で skip した本数を診断ログに出す。
- 結果: 追いついている本は 1 ページも流さない。実機の構成（5 本中 1 本だけ遅れ）なら
  I/O は 1/5 になる。手動の再評価（`refresh(current)`）は従来どおり全本を作り直す。
- 関連: `AIAlbum/AIAlbumDrift.swift` / `AIAlbumService+Refresh.swift` / `AIAlbumDriftTests` /
  事例「33 分で 1GB のディスク書き込み警告」。

## ADR-159 落ちた痕跡を必ず端末ログに残す（Swift trap ＋ 同期書き込み ＋ 直前の操作）
- 状態: 採用
- 文脈: 「今朝、使ってみたら落ちまくり」の診断ログ（8/31）に、クラッシュの行が **1 本も無かった**。
  35 分で 4 回の再起動があるのに、何が起きたか一切分からない。理由は二重だった:
  (1) `NSSetUncaughtExceptionHandler` は **ObjC 例外しか通らない**（Swift の `fatalError` /
  `precondition` / 配列外 / SwiftData trap は素通り）。(2) その例外ハンドラすら
  `DiagnosticsLog.append`＝`queue.async` で書いていたため、**書く前にプロセスが終わっていた**
  （前日 diagnostics-69 で例外行が残ったのは、たまたまキューが流れていただけ）。
- 決定: 三点をセットで入れる。
  (a) `DiagnosticsLog.appendNow`（同期書き込み）を作り、クラッシュ経路はこちらを使う。
  (b) `CrashSignals`＝致命シグナル（SIGABRT/SIGILL/SIGTRAP/SIGSEGV/SIGBUS/SIGFPE）に
  ハンドラを掛け、**async-signal-safe な `write(2)` だけ**で 1 行残してから既定ハンドラへ戻す
  （標準クラッシュレポートは潰さない）。ハンドラ内でアロケートしないよう、fd と文言は
  install 時に用意し、時刻は付けない。
  (c) `Diagnostics.breadcrumb(_:)`＝**直前の操作**を固定長バッファに 1 つだけ覚え、
  クラッシュ行と一緒に書き出す。ログ行は増やさないので高頻度の操作にも置ける。
- 結果: 次に落ちたとき `CRASH SIGTRAP — …（直前の操作: people.mergePerson 123→456）` が残る。
  **jetsam（SIGKILL）だけは捕まえられない**が、行が無いこと自体が「OOM か watchdog」を示す
  情報になる（消去法が成立する）。
- 関連: `MosaicSupport/CrashSignals.swift` / `Diagnostics.swift` / `CrashSignalsTests` /
  事例「落ちまくったのに、ログにクラッシュの行が 1 本も無い」。

## ADR-158 一覧の ID は表示側でも一意化する（データの異常でアプリを落とさない）
- 状態: 採用
- 文脈: 人物・場所などの**メンバー限定アルバム**を開いた瞬間に `NSInternalInconsistencyException — supplied item identifiers are not unique` で落ちた（実機 diagnostics-69・547 枚のローカル写真のうち 65 枚が重複）。グリッドの土台 `UICollectionViewDiffableDataSource` は、スナップショットに同じ識別子が 2 つ入ると**例外を投げる**。メンバー一覧の出どころは 6 画面（人物 / 人物グループ / 場所 / 端末アルバム / 自動アルバム / 共有）あり、どこか 1 つが重複を作れば同じ結末になる。
- 決定: 二段で塞ぐ。(1) **入口**＝`MergedPhotoStore.forMembers` で localID を先勝ちで一意化する（6 画面が必ず通る 1 箇所）。落とした件数は診断ログに出し、どの画面が重複を作っているかを次のログで特定できるようにする。(2) **出口**＝`PhotoCollectionView` のスナップショット構築で `uniquedByID` を通す。表示は読み取りにすぎないので、**データが壊れていても落とさず出せるところまで出す**（ADR-143 と同じ原則）。
- 結果: 重複が残っていてもクラッシュしない。副作用として「同じ写真が 2 枚並ぶ」も消える。純関数（`uniquedByID` / `uniquedIdentifiers`）は macOS のテストで固定した（先勝ち・順序保存・セクション割り当て後も一意）。根本（誰が重複を作ったか）は診断ログで次に特定する。
- 関連: `Packages/PhotoSourceKit/Sources/PhotoSourceKit/Support/UniqueByID.swift` / `Views/PhotoCollectionView.swift` / `Packages/PhotosFeatureKit/Sources/PhotosFeatureKit/MergedPhotoStore+Members.swift` / `UniqueByIDTests` / 事例「メンバーアルバムを開くと落ちる」・ADR-143。

## ADR-157 「走って 0 件」と「走っていない」を必ず区別できるようにする（断片吸収の記録）
- 状態: 採用
- 文脈: 断片の自動吸収（ADR-154）は `absorbed > 0` のときだけ記録していた。実機ログ（diagnostics-69）には再クラスタが 2 回あるのに吸収の行が 1 行も無く、**「0 件だった」のか「早期 return で走らなかった」のかログから判定できなかった**。同じ形の困りごとは UI 側でも指摘されている（「候補が無いのかバグか分からない」）。
- 決定: `FaceStore.absorbFragments` は**必ず** 1 行記録する。早期 return（クラスタ 2 未満・吸収先ゼロ）にもそれぞれ理由付きの記録を置き、通常終了では `absorbed / into / fragments / targets / skipped（bar・margin・blocked の内訳）/ tooBig / bar` を出す。呼び出し側（`PeopleEngine`）の重複記録は削除する。
- 結果: 次の実機ログで「吸収が動いたか」「どの条件で見送ったか」「上限を上げれば何件が対象になるか（tooBig）」が 1 行で分かる。ADR-155 で保留した「断片の上限を 2 枚から上げるか」の判断材料が実測で得られる。
- 関連: `Packages/FaceCore/Sources/FaceCore/Faces/FaceStore+Absorb.swift` / `PeopleEngine+Review.swift` / ADR-154 / ADR-155。

## ADR-156 一覧は「さらに表示」で伸ばす（ページ送りにしない）
- 状態: 採用
- 文脈: 「人物を調べるの似ている人・別の人かもしれない写真で、次の候補も見たい。
  表示数を増やす機能でもページ送りでもよい」。あわせて「まとめて確認でも全体画像を見たい」。
- 決定:
  1. **「さらに表示」で件数を伸ばす**（近傍 +15・間違い候補 +24）。ページ送りにしなかったのは、
     ここが**比べながら見る画面**だから——前の並びが消えず、戻る操作も要らない。
     実装も足し算だけで、次に増やす軸（例: 束ねた人物の候補）にも同じ形で付けられる。
     人物を切り替えたら件数は最初に戻す（前の人の「もっと見る」を持ち越さない）。
  2. **まとめて確認にも「写真全体」トグル**（1 対 1 の確認は ADR-91 で先に入っていた）。
     あわせて全体表示は `.fit` で収める——正方形の枠に `.fill` で入れると左右が切れて、
     「全体を見たい」という目的を果たさない。
- 結果: 候補を掘り下げられる／顔だけでは判断が付かない場面で状況を見て決められる。
  ⚠️ 最初から全件を出さないのは、写真の多い人物で読み込みが重くなるため（`outlierFaces` は
  メンバー全員の埋め込みを触る）。
- 関連: `PersonInspectorView` / `FaceBatchReviewView` / `FaceStore.decisionReport(outlierLimit:)` /
  ADR-91/135/137。

## ADR-155 断片の上限（2 枚）は据え置く——データセットが「上げると誤る」としか言わないため
- 状態: 採用（ADR-154 の上限の根拠づけ）
- 文脈: 「2 枚以下という数字に意味はあるのか。3 枚や 5 枚も自動でよいのでは」。
  当て推量だったので、**バー × 断片の大きさ K** をデータセットで掃引した
  （face-accuracy.md 2026-08-30 3 回目）。
- 測定結果:
  - FG-NET（真の断片が多い）: **どの K でも吸収 0 件**。年齢差が極端でクラスタ対の類似度が
    0.6 を超えず、バー 0.75 では発火しない。
  - LFW（真の断片がほぼ無い）: 発火する帯（0.50〜0.55）で吸収したものは **全部間違い**
    （K≤5 で 5 件・K≤10 で 12 件、正答率 0%）。**K≤2 はどのバーでも一度も誤らなかった**。
- 決定: **上限は 2 枚のまま**。「大きい断片ほど発火し、しかも間違える」という傾向しか
  出ていない以上、推測で上げない。代わりに実機で判断できるよう、
  **「上限だけが理由で見送った数」（tooBig）** を結果と診断ログに出す。
- 結果: 次は実機の数字（tooBig の量と、2 枚以下の吸収が正しかったか）で上限を動かせる。
  ⚠️ **どちらのデータセットも実機を代表していない**——FG-NET は真の断片があるが類似度が低く、
  LFW は類似度は届くが真の断片が無い。実機は「真の断片が高い類似度で存在する」第 3 の状況で、
  だからこそ**手元のデータで測る**（ADR-148）の出番になる。
- 関連: `FaceStore.absorbMaxPhotos` / `FragmentAbsorbResult.skippedTooBig` /
  face-accuracy.md 2026-08-30（3 回目）/ ADR-148/154。

## ADR-154 断片（1〜2 枚）は自動で寄せる。人物どうしは寄せない
- 状態: 採用（`clusterRuleVersion` は据え置き＝夜間の再クラスタ後に走る）
- 文脈: 実フィードバック「1〜2 枚のグループがたくさんあって、4,000 枚の人物と同一人物。
  でも確認画面にはあまり出てこない。1 枚ずつ確認なんてしたくない」。調べると 2 つあった。
  1. **レビューの母数が `minFaces`（3 枚）で切られていた**——断片は候補にすらなっていなかった。
  2. ADR-153 で「自動結合はしない」と決めたが、その判断材料は**人物どうし**の対だった。
- 決定:
  1. **候補側の下限を 1 枚に**する（基準＝アンカー側は従来どおり確立した人物・ADR-123）。
  2. **断片だけは自動で寄せる**（`absorbFragments`）。ADR-153 と矛盾しないのは、
     **失敗の代償が非対称**だから——断片 1〜2 枚が入っても 4,000 枚の重心は動かず、
     間違いは 1 枚外せば直る（ADR-133/137）。確立した人物どうしを機械が混ぜると
     取り返しが付かない（ADR-130 で経験した）。
     条件: 断片は 2 枚以下・無名・アンカーなし・束ねなし／吸収先は確立した人物
     （名前かアンカーがあり 11 枚以上）／近さ `autoAbsorbBar`（arcface 0.75）以上かつ
     **2 位の人物と 0.05 以上の差**（紛らわしければ人に尋ねる）／同一写真・負例・
     「別人」記録があるものは触らない。
  3. 機械の判断は**修正ジャーナルにもアンカーにも残さない**（ADR-152）。
     夜間の再クラスタ後に自動実行し、件数を診断ログへ。手動実行の導線も置く
     （「人物を調べる」→「小さなグループをまとめる」）。
- 結果: 断片が確認画面に出るようになり、明らかな分は機械が寄せる。
  ⚠️ 「自動化するか」は精度ではなく**失敗の代償**で決める（ADR-153 と同じ物差しで、
  結論が逆になった例）。回帰テスト `FragmentAbsorbTests`（寄せる／紛らわしいと寄せない／
  ユーザーが名前を付けた人物は触らない／同一写真は触らない）。
- 関連: `FaceStore+Absorb.swift` / `FaceStore+Review.batchReviewItem` / `FaceTuning.autoAbsorbBar` /
  ADR-123/130/133/152/153。

## ADR-153 人物どうしは自動結合しない。「あらかじめ選んで見せる」までにする
- 状態: 採用（残していた「C: 自動結合バー」の結論）
- 文脈: 「似ている度 80% 以上ならほぼ本人。自動で繋げられないか」を実測で判断した。
  実機 7,710 件の「同じ人」回答と、汚れを分離した「別人」回答（統合拒否の自動記録を
  時間的近接で除いた 10 件）で、バーごとの効果を出した（face-accuracy.md 2026-08-30 2 回目）。
- 決定: **自動結合はしない。** 理由:
  1. ユーザー自身が **0.885 / 0.920 の対を「別人」と答えている**（家族＝似た顔が多いライブラリ
     では当然）。安全なバーが存在しない。
  2. 0.90 まで上げても自動化できるのは yes 対の **2.5%**——得るものが小さい。
  3. 失敗の代償が非対称。人物が丸ごと混ざる事故は、ADR-130 で実際に起きて何日も尾を引いた。
  代わりに **0.85 以上を「あらかじめ選んだ状態」で見せる**（まとめて確認の候補を選択済みにする）。
  この帯の正しさは 98.2%（判断らしい負例だけなら 99.9%）で、ユーザーは**外すだけ**で済む。
- 結果: 一括の作業量は下がり、間違いは**起きる前に見える**。⚠️ 一般則:
  **自動化の可否は「精度」ではなく「失敗の代償」で決める。** 98% は表示の判断には十分でも、
  取り返しの付きにくい変更には足りない。
- 関連: `FaceTuning.autoSuggestBar` / `FaceBatchReviewItem.Candidate.preselected` /
  `FaceBatchReviewView` / face-accuracy.md 2026-08-30（2 回目）/ ADR-130/148/150。

## ADR-152 「写真の事実」と「ユーザーの判断」を同じ引き出しに入れない
- 状態: 採用
- 文脈: ADR-146 で「ユーザーが自分で選んだ統合の拒否では『別人』を学習しない」ようにしたが、
  **まとめて確認からの統合**は対象外だった。実機ログ（2026-08-30）でも、
  1 分間に 10 件の `notSame` が類似度 0.64〜0.79 で入っており（＝一括統合で同一写真の拒否が
  連続した痕跡）、負例と校正の材料が汚れ続けていた。
  同一写真の重なりは**写真の事実**であって、ユーザーが「別人だ」と答えたわけではない。
- 決定: 同一写真で統合できなかった対は `samePhotoBlock` という**別の種類**で記録する。
  - レビューの「もう尋ねない」判定には使う（同じ対を何度も出さない）。
  - **負例（埋め込みの学習）にも校正にも使わない**。
  あわせて「あなたの回答から見た基準」も、分離していないときは**境界を出さない**
  （以前は既定値がそのまま出て、「境界 0.350・取り違え 1,228」と計算結果のように見えていた）。
  分離度（AUC）を画面と要約ログに出す。
- 結果: 修正ジャーナルが「ユーザーが実際に答えたこと」だけになり、自動結合バー（残る C）を
  実測で決められる状態になる。⚠️ 一般則として、**機械が付けた印とユーザーの判断を
  同じ引き出しに入れない**——後から区別できなくなり、学習が静かに歪む。
- 関連: `FaceStore.markSamePhotoBlock` / `loadNegatives` / `FaceStore+Review` /
  `FaceStore+AnswerBasis` / ADR-45/146/148/149。

## ADR-151 アンカーは「種」には使うが「見本」には広げない
- 状態: 採用（`maxSeedPrototypes` 8→1・`anchorsByCluster` の既定 5→1）
- 文脈: ADR-130/132 で確認顔（アンカー）を再クラスタの種にした際、`prototypes`（似ている判定の
  見本）にも積んでいた。FG-NET で**順序（種にする）と見本（プロトタイプ）を分けて**測ったところ、
  見本にすると純度が k=1 で 0.877→0.812、k=5 で **0.509**（純度 0.8 未満の人物が 18→49 人）。
  順序だけなら 0.877→0.87 でほぼ不変。見本が増えるほど「別人への橋」が増える。
- 決定: 見本は **1 枚**（代表写真＝ユーザーが選んだ 1 枚）に絞る。同一性の固定は
  「種の重心をアンカーから作る」「アンカーをそのクラスタにピン留めする」で達成しており、
  見本を増やす必要はない。増分スキャン側（`anchorsByCluster`）も既定 1 枚。
- 結果: 乗っ取り防止（ADR-130）と学習の保持（ADR-132/134）は維持したまま、混入が減る。
  ⚠️ B3 マルチプロトタイプ（ADR-46）は **facenet 時代の測定で採用**したもので、
  arcface では逆に働いていた——**定数も設計もモデルに張り付く**（ADR-70 の教訓の再確認）。
- 関連: `FaceStore+Rebuild.maxSeedPrototypes` / `FaceStore.anchorsByCluster` /
  `face-accuracy.md` 2026-08-30 / ADR-46/70/130/132。

## ADR-150 「同じ人ですか？」と尋ねる下限は、しきい値を下回らせない
- 状態: 採用（arcface の下限 0.25→0.40・式を `max(floor, threshold)` へ）
- 文脈: 従来は「成長で離れた同一人物を拾う」ため、尋ねる下限をしきい値**より下**へ降ろしていた
  （ADR-68 追補2・arcface で 0.25）。FG-NET を本番設定で測ると、その帯の当たり率は **4.3%**
  ——3,200 対を尋ねて当たりは 137 対だけ。実機のユーザー回答でも「同じ人」と答えた対は
  **下位 5% が 0.669**で、下限 0.25〜0.40 の帯にはほぼ存在しなかった。
  レビューの質だけでなく、候補生成の重さ（1 枚 7 秒・ADR-142）にも効いていた。
- 決定: `mergeBandFloor = max(mergeCandidateFloor, threshold)`。arcface の下限は 0.40
  （尋ねる対 468・当たり率 14.3%＝3.3 倍）。校正でしきい値が上がったらそちらに合わせる。
- 結果: 尋ねる数が 1/7、当たり率が 3.3 倍。⚠️ 代償として FG-NET 基準では「拾える分裂」が
  72.9%→35.6% に下がる——**成長で離れた子供の分裂は別経路**（2 階層の束ね＝ADR-61、
  一覧からの明示操作＝ADR-133/147）で拾う設計であることを前提にした判断。
- 関連: `FaceTuning.mergeBandFloor` / `face-accuracy.md` 2026-08-30 / ADR-68 追補2 / ADR-142。

## ADR-149 校正は「同じ物差し」で、「分離するとき」だけ
- 状態: 採用
- 文脈: 実機 9,973 件の実測で 2 つの欠陥が確定した。
  1. **尺度の混在**: 校正しているのは「顔が人物に入る」しきい値（顔×重心）なのに、
     人物ペアの回答（重心×重心＝merge/notSame）まで正例・負例に混ぜていた。
  2. **分離しないデータでの最適化**: 顔レベルの正例（confirm・中央値 0.713）と
     負例（reassign・0.716）は **AUC 0.472＝コイン投げ以下**。そこから「最もよく分ける境界」を
     探すと、件数の多い負例（正例の 5.5 倍）を全部落とす端（0.974）が最適解になり、
     可動域の上限 0.40 に張り付く。**ユーザーが直すほど厳しくなる**の正体はこれ。
- 決定: (1) 校正の入力は**顔の尺度だけ**（confirm/sameGroup と reassign）。人物ペアの数字は
  「あなたの回答から見た基準」（ADR-148）で別に見る。(2) 分離度（AUC・重み付き）を測り、
  **0.60 未満なら校正しない**（プロファイル既定のまま）。
- 結果: 校正が上限に張り付く問題が根本から消える。ADR-141（確立した人物には引き上げ分を
  課さない）は保険として残す。⚠️ `reassign` の similarity は「**誤って入っていたクラスタとの**
  類似度」で、高くて当たり前——「この近さなら別人」の証拠ではない。
  **記録した数字が何の証拠になるかを、使う前に確かめる。**
- 関連: `FaceCalibration.separability` / `FaceStore.appendCalibrationSample` /
  `face-accuracy.md` 2026-08-30 / ADR-46/68 追補6/70/141/148。

## ADR-148 しきい値は「自分のデータ」で決める（Step 0/1）
- 状態: 採用（Step 0＝指標の整備／Step 1＝ユーザー自身の回答から基準を読む。Step 2/3 は未着手）
- 文脈: 「似ている度 80% 以上ならほぼ本人、70% でも本人のことが多い。基準は厳しすぎない？
  感覚で決めたくないので、データセットで適正値を決める方法論を」。
  ⚠️ 前提の整理: **人物どうしは自動では結合しない**（0.25 以上を「尋ねる候補」に出すだけ）。
  顔→人物は 0.35（校正 0.25〜0.40）＋サイズ上乗せ（最大 0.08）＋マージンゲート 0.04。
  そして画面の「似ている度」は**重心どうし**なので、顔どうしより高く出る——
  「80% なのに一緒にならない」は、しきい値が 0.80 だからではなく**結合を自動でやらない設計**だから。
- 決定:
  - **Step 0（指標）**: `ClusteringScore` に**人物ごとの分布**（純度・再現率の最悪値と下位 25%、
    純度 0.8 未満の人数）を足す。⚠️ 平均は小さいアルバムの被害を隠す——ADR-126 はそれで
    採用してしまい撤回した。以後、平均と一緒に最悪値を見る。
  - **Step 1（自分のデータ）**: 修正ジャーナルには「同じ人」（merge/confirm）と「別人」
    （notSame/reassign）の**類似度が既に記録されている**。これを種類ごと（人物どうし／顔と人物＝
    尺度が違うので混ぜない）に集計し、分布・中央値・ヒストグラム・**最もよく分ける境界**と、
    その境界での取りこぼし／取り違えを出す画面を作る（`AnswerBasisView`）。
    十分な回答数（8 件）が無ければ境界は出さない。
- 結果: 「厳しすぎる気がする」を、自分の写真での実測と突き合わせて判断できる。
  次（Step 2/3）は FG-NET/LFW での掃引（顔しきい値・サイズ上乗せ・**クラスタ結合バー**）と、
  「確認済み k 枚を模擬してアンカー化 → 確立した人物に緩める効果」の測定。
- 関連: `FaceEvalMetrics.ClusteringScore` / `FaceStore+AnswerBasis.swift` / `AnswerBasisView` /
  `face-accuracy.md` / ADR-46/126/141。

## ADR-147 「人物を調べる」はピープルの導線に置く（設定ではない）
- 状態: 採用（Developer Options の「クラスタリングの内訳」＝ADR-135 を格上げ）
- 文脈: 「内訳が便利なのでユーザー向けに格上げしたい。設定トップに『ピープルアルバム』を
  足すか、アルバムの自動処理か？」。**どちらでもない**——これは設定（好みの永続的な選択）ではなく、
  **特定の人物に対する調査と修正**。ユーザーが「この人、誰と混ざってる？」と思うのは
  その人物を見ているときで、動線はそこにある。
- 決定:
  1. 画面を `PeopleKit` へ移し（`PersonInspectorView`）、**人物メニュー**（ホームの人物カード長押し・
     人物アルバムの「…」）の「この人物を整理…」の隣に「この人物を調べる…」を置く。
     ピープル全体一覧のツールバーからも開ける（人物を選ぶところから始めたいとき）。
     Developer Options のリンクは残す（同じ画面・維持コストゼロ）。
  2. **数字は残し、言葉をユーザー語にする**。cos 0.412 →「似ている度 41%」、
     実効しきい値 →「必要 40%」、種 →「あなたの指定どおり保たれる」、
     負例 →「あなたが『別人』と直した」。判定の結論も
     「一緒にできる／似ている度が足りない／相手の写真が少なく判断できない／別の人と紛らわしい／
     同じ写真に一緒に写っている」と、読んで分かる形に。
  3. 国際化（`L(...)`・英語 base ＋日本語）。⚠️ 補間の中で関数を呼ばない
     （`\(percent(x))`）——カタログのキーは書式指定子に落ちるので、入れ子の括弧があると
     照合できない。カタログ検査のテスト側も括弧の深さを数えるよう直した。
- 結果: 開発者向けの内訳が、そのままユーザーの「なぜ？」に答える画面になった。
  トレードオフ: 用語を平易にしたぶん、開発時に見たい生の値（マージン幅・重心寄与数）は落とした
  ——必要になれば「詳細」トグルで戻せる。
- 関連: `PeopleKit/PersonInspectorView.swift` / `PeopleActions` / `AllPeopleView` /
  `DeveloperSettingsView` / ADR-135/137/145/146 / ADR-17（i18n）。

## ADR-146 「同じ写真に写っている」で統合できないとき、その写真を見せて直せるようにする
- 状態: 採用
- 文脈: 実フィードバック「近傍に同一人物を見つけて統合しようとしたら『同じ写真に一緒に写っている』
  と言われて統合できない。**どれが問題の写真か分からない**ので調整できない」。
  同一写真 cannot-link は物理的な制約（1 枚に同じ人は 1 回しか写れない）だが、
  **重複検出・写真の中の写真・鏡**では破れる。破れたとき、ユーザーには
  「できない」としか伝わらず、しかも拒否のたびに**「別人」を学習していた**
  ——直しても二度と統合できなくなる、逆向きの動きだった。
- 決定:
  1. **どの写真のどの顔がぶつかっているか**を返す（`samePhotoConflicts(between:and:)`）。
     診断画面の顔一覧の先頭に「重なっている写真」として、写真全体＋両方の顔を並べ、
     その場で片方を外せる（外した顔は新しい人物になる＝重なりが消えて統合できる）。
  2. **ユーザーが自分で選んだ統合では、拒否しても「別人」を学習しない**
     （`recordNotSameOnConflict: false`）。自動サジェストからの拒否は従来どおり記録する。
- 結果: 「できない」で終わらず、原因を見て直せる。⚠️ 制約が破れる例外があるなら、
  **例外を直す道**まで用意しないと、制約は user が越えられない壁になる。
  回帰テスト `SamePhotoConflictTests`（重なりの提示・外せば統合できる・拒否で学習しない）。
- 関連: `FaceStore.samePhotoConflicts` / `FaceStore.mergeClusters` / `FaceClusterInspectorView` /
  ADR-58（同一写真 cannot-link）/ ADR-135/137。

## ADR-145 「出ない」は理由まで出す（空の表示を消さない）
- 状態: 採用
- 文脈: 「クラスタリングの内訳で間違い候補が出ないことがある。候補が無いのか**バグか分からない**ので
  （なし）などと出してほしい」。実際、空リストには **(1) 本当に対象が無い (2) メンバーが多すぎて
  計算を省いた (3) 顔が 1 枚も無い** の 3 通りがあり、セクションごと消していたため区別できなかった。
- 決定: 状態を値で返す（`PersonOutlierStatus`: computed / noMembers / tooManyMembers(limit,members)）。
  UI は**空でもセクションを出し**、理由を書く（「（なし）全部調べましたが外れはありません」
  「（未計算）顔が 4,800 枚あり上限 8,000 枚を…」）。あわせて上限を 3,000→8,000 に上げた
  ——枚数の多い人物こそ混入を見たい。
- 結果: 画面から消えない＝壊れているのか正常なのかが分かる。⚠️ 一般則として、
  **空の状態を黙って消さない**。「出ない」はユーザーから見れば故障と同じで、
  理由が書いてあって初めて情報になる。回帰テスト `outlierStatusExplainsEmptyList`。
- 関連: `FaceStore.outlierFaces` / `FaceClusterInspectorView` / ADR-137。

## ADR-144 育てたアルバムが痩せたら、記録に残す
- 状態: 採用（記録のみ・挙動は変えない）
- 文脈: 「ピープルアルバムの写真の全数が減っている気がする。ストレージの制約で何かが
  削除されたせいだったら嫌だ」。**確認した結果、容量による削除は無い**——Dropbox キャッシュの
  LRU（`enforceCapacity`）が消すのは**画像の実体と使用記録だけ**で、写真のメタデータ
  （`CachedDropboxItem`）も顔（`DetectedFace`）も消さない。顔の全消去は
  (1) 再スキャン版の migration (2) クラウド解析版の migration (3) Developer Options の
  リセット の 3 つだけで、いずれも診断ログに痕跡が残る（当該ログには無かった）。
  一方で、実際に痩せる原因は別にあった（ADR-140 の負例の一斉適用・ADR-141 の校正の張り付き）。
- 決定: 再クラスタの前後で**名前付き人物の枚数を突き合わせ**、減っていれば診断ログへ出す
  （合計と、5 枚以上かつ 2 割以上減った人物の上位）。台帳は変更しない。
- 結果: 「気のせいかもしれない」を次から数字で確かめられる。⚠️ ユーザーが育てたものが縮むのは、
  原因が何であれ**先に知らせるべき事象**——見つけたときには何度も直したあと、になりやすい。
- 関連: `FaceStore.namedShrinkReport` / ADR-140/141 / 事例「診断で数枚直したらアルバムが激減した」。

## ADR-143 台帳のページ送りはオフセットではなくカーソルで
- 状態: 採用
- 文脈: 実機で `Swift/NativeDictionary.swift:792: Fatal error: Duplicate values for key:
  'C-/写真/…/dsc03019.jpg'` でアプリが停止（直前のログは `[Tagger] embed: batch 2 done`）。
  AI アルバムの意味検索は埋め込みを**ページ送り**で読むが、`fetchOffset` を使っていた。
  夜間の索引付け（`PhotoTagger`）は**走査中に行を挿入する**ので、カーソルより前に 1 行入ると
  以降の全ページが 1 つずれ、**同じ写真が 2 回**返る。それを
  `Dictionary(uniqueKeysWithValues:)` に流し込んで trap していた。
- 決定:
  1. ページ送りは**キーセット**にする（`refKey > 前ページの最後` を昇順で `limit` 件）。
     挿入があってもずれない。`enrichmentVectorPage(after:limit:)` と
     `allEnrichedPhotosLite()` の両方を直す。
  2. 走査側にも重複ガード（`seen`）を置き、**同じ写真を二重に採点しない**。
  3. 写真キーで作る辞書は `uniquingKeysWith:` にする——**ここで trap すると
     アプリが落ちる**。表示のための集計が、データの一時的な重複でクラッシュしてはいけない。
- 結果: 索引付けと検索が同時に走っても落ちない。⚠️ 一般則として、
  **書き込みが続く表をオフセットで送らない**（SwiftData/SQL に共通）。
  回帰テスト `EmbeddingPagingTests`＝1 ページ目のあとにカーソルより前へ挿入し、
  2 ページ目に重複も取りこぼしも無いこと。
- 関連: `AutoAlbumStore.enrichmentVectorPage` / `allEnrichedPhotosLite` / `AIAlbumSearcher` /
  `TagCentroids` / ADR-119。

## ADR-142 ピープルの重い処理は「増えるほど遅くなる形」をやめる
- 状態: 採用
- 文脈: 実機ログ（diagnostics-68）で、**まとめて確認のカード生成が毎回 7 秒**
  （最悪 74.6 秒）、**人物一覧の読み込みが通常 0.44 秒のところ最大 13.3 秒**だった。
  どちらも「修正が増えるほど」「同時に走るほど」遅くなる形をしていた。
- 決定: 3 つ直す。
  1. **しきい値校正を O(n²) → O(n log n)**。候補しきい値ごとに `positive.filter { … }` を
     作っていたため、修正 8,868 件では 1 回の校正で数千万回の走査＋配列確保が走っていた。
     並べ替えて一度だけ舐める形に置き換える（結果は総当たりと同一・乱数で突合するテスト付き）。
  2. **校正の材料を作り直さない**。修正のたびに全件（8,868 行）を読み直していたのを、
     キャッシュへ**足すだけ**にする（記録・取り消し・リセット・プロファイル変更で破棄）。
  3. **ユーザーがピープルを触っている間は顔スキャンに譲らせる**。顔スキャンと人物一覧・
     レビュー候補・診断は**同じ `@ModelActor` を奪い合う**。レビュー表示中の保留（ADR-95）と
     同じカウンタを `shouldPause` にも効かせ、ピープル関連の画面（一覧・顔の管理・代表選び・
     整理・診断）に `.pausesFaceScan` を付ける。スキャンは差分ベースなので閉じれば続きから再開する。
  あわせて、次のログで内訳が分かるよう `people.batchReview.threshold/pool/faces` と
  `faces.calibrate` の計測点を足した。
- 結果: 「修正するほど遅くなる」を断つ。⚠️ 教訓として、**ユーザーの操作で増える集合
  （修正ジャーナル）は、規模テストの対象**——`ScaleRegressionTests` は人物数・写真数を増やすが、
  修正数は増やしていなかった。
- 関連: `FaceCalibration.calibratedThreshold` / `FaceStore.calibrationSamples` /
  `PeopleEngine`（`isBrowsingPeople`）/ `PeopleScreenHold.swift` / ADR-95/119。

## ADR-141 確立した人物には、校正の「引き上げ分」を課さない
- 状態: 採用（`clusterRuleVersion` は据え置き＝次の再クラスタから自然に効く）
- 文脈: 実機ログ（diagnostics-68）で、**校正しきい値が可動域の上限に張り付いていた**
  （arcface: 0.40 / 既定 0.35・positives 7,381・negatives 1,368）。arcface は**同一人物の平均
  類似度が 0.434** なので、0.40 の bar は同一人物の分布のほぼ真ん中を切る。結果、
  ユーザーが修正するほど校正が厳しくなり、**育てたアルバムが育たなくなる**——修正が自分の
  首を絞める向きに働いていた（同ログで人物 2,864 クラスタ・再クラスタで 11,630 顔が移動）。
  ユーザーの言葉: 「人間が判断してグループがある程度大きくなったら、もうその
  ピープルアルバムは安定している」。
- 決定: **アンカー（確認顔・代表写真）を持つ成熟クラスタ**（メンバー ≥ 11）には、
  校正で上がったぶんを課さない（実効しきい値の上限を既定値に戻す）。候補の打ち切り下限も
  緩い方に合わせる。新しい人物の分離・無名クラスタには校正値がそのまま効く。
- 結果: 修正を重ねても、確立した人物は本人の顔を取り込み続ける。トレードオフは、
  確立した人物ほど混入を受けやすくなること——ただし混入は 1 枚ずつ外せる（ADR-133/137）し、
  負例は相対判定で効く（ADR-140）。
  ⚠️ **データセット計測では検証できない**（FG-NET/LFW にはユーザー修正が無く、校正が動かない）。
  実機の体感で判断する類の変更なので、次のログで人物数・アルバム枚数の推移を見る。
- 関連: `FaceClustering.assign`（`baseThreshold` / `anchoredClusterIDs`）/ `FaceStore.loadClustering` /
  `FaceStore+Rebuild` / ADR-46/70/140。

## ADR-140 負例で「既にいるメンバー」を一斉に外さない
- 状態: 採用（ADR-45/132 の補正）
- 文脈: 実フィードバック「診断画面で数枚を『この人ではない』『もしかしたら別の人』にしたら、
  その調査対象のアルバムが**激減**した」。実機ログ（diagnostics-68）でも、負例 1,368 件を抱えた
  状態の再クラスタで **11,630 顔が移動**していた。
  原因は ADR-132 で入れた「負例に一致するメンバーは留めない」規則が**絶対のしきい値**
  （`negativeSameThreshold`＝arcface 0.45）だけで判定していたこと。この線は
  **本人の顔どうしの類似度（同一人物平均 0.434）より低い**ので、外した 1 枚が本人に似ていると
  **アルバムのほぼ全員がその負例に一致**して一斉に外れる（合成データで 12 枚→3 枚を再現）。
- 決定: 場面で分ける。
  - **既にその人物に入っているメンバー**（再クラスタ）: 外すのは「外したその顔と実質同じ顔」
    （`negativeDuplicateThreshold` 0.97＝連写・重複検出）だけ。アンカー（確認顔・代表写真）は
    そもそも外さない。混入はユーザーが 1 枚ずつ外す——そのための入口は増やした（ADR-133/137）。
  - **これから入ろうとする顔**（`assign`）: 従来どおり拒否するが、**相対で**判定する
    ——「外した顔の方が、候補クラスタ自身より `negativeMargin`(0.10) 以上近い」ときだけ。
    本人の顔を 1 枚外しただけで、その人物の顔が以後どれも入らなくなるのを防ぐ。
- 結果: ユーザーが 1 枚外しても、育ったアルバムが崩れない。トレードオフは「1 枚の指摘が
  同じ別人の**別の写真**にも自動で効く」範囲が狭まること（実質同じ顔のみ）。
  ユーザーの明示（「この人ではない」「別の人へ移す」）は当然そのまま効く。
  回帰テスト: `removingOwnFaceDoesNotEmptyTheAlbum`（1 枚外して 6→5 枚）＋既存の
  `userCorrectionEjectsMatchingFaces`（重複は外れる）。
- 関連: `FaceClustering.assign` / `FaceStore+Rebuild` / ADR-45/46/132 / 事例「診断で数枚直したら
  アルバムが激減した」。

## ADR-139 代表顔の選択は同点でも決定的にする
- 状態: 採用
- 文脈: CI（macOS fast）だけが落ち、手元では通る——を追ったところ、`PersonIdentityAnchorTests` の
  1 本が**8 回に 1 回**落ちていた（手元でも再現した）。原因は `bestCoverFace` が
  `max(by: coverScore)` で、**同点のときに fetch 順で決まっていた**こと。
  代表顔は見た目だけの話ではない: 命名・代表選択で**アンカー（確認顔）**になり、再クラスタで
  動かない錨になる（ADR-130/132）。混入した顔と本人の顔が同点だと、
  **混入がアンカーになる回**が生まれ、そのときだけ「指摘した顔と同一人物を留めない」が崩れる。
- 決定: 同点は `faceID` で決める（小さい方）。これで同じ台帳からは常に同じ代表顔が選ばれる。
- 結果: 実行ごとに結果が揺れない。⚠️ 品質・笑顔・顔の大きさが**そっくり同じ**顔は実際に起こる
  （連写・同じ場面）ので、これは合成データだけの話ではない。
  ADR-94（名前の主クラスタ選択）と同じ考え方で、**「最良の 1 つ」を選ぶ場所には必ず決定的な
  タイブレークを置く**。
- 関連: `FaceStore.bestCoverFace` / ADR-94/130/132 / 事例「テストが一括実行のときだけ落ちる」。

## ADR-138 表示中に写真ストアが差し替わったら、読み込みをやり直す
- 状態: 採用
- 文脈: 実フィードバック「人物アルバムで『他の人に束ねる』をすると、画面は束ねた先に変わるのに
  **写真が表示されない**。似た状況で他の画面でも起きた」。原因は 2 つとも「**古いものを掴んだまま**」:
  1. `PhotoSourceContentView` の `.task { await store.start() }` は**ビューの同一性が変わらない限り
     1 回しか走らない**。ストアのインスタンスを差し替えても新しいストアは start されず、
     `.idle` のまま＝空の画面になる。
  2. `PhotoCollectionView`（UIViewRepresentable）の `Coordinator` は `makeCoordinator()` で
     作られた **1 回きり**の存在で、`store` を `let` で抱えていた。差し替え後もサムネイル取得・
     先読みは**古いストア**に行くので、一覧の件数は新しいのにセルが埋まらない。
- 決定: (1) `.task(id: ObjectIdentifier(store))` にして、ストアが差し替わったら start をやり直す
  （`PhotoStore` を `AnyObject` 制約にして同一性で見分ける）。(2) `Coordinator.store` を `var` にし、
  `updateUIView` で `adopt(store:)` する。
- 結果: 人物アルバムの束ね・メンバー変更のように「同じ画面のまま中身が入れ替わる」操作が
  正しく描き直される。⚠️ この不具合は**ビルドもテストも通る**（SwiftUI のライフサイクルの話で、
  ロジックのテストには現れない）。同型の seam を足すときは「差し替わったらどうなるか」を必ず見る。
- 関連: `PhotoSourceContentView` / `PhotoCollectionView` / `PhotoStore` / `PersonAlbumView` / ADR-133。

## ADR-137 混入は「内側」からも探す ＋ 気づいた場所から直せる
- 状態: 採用（ADR-135 の内訳画面に追加）
- 文脈: 実フィードバック「近傍は良いのですが、**グループ内で重心から外れている、間違い候補の
  リスト**も出してほしい。近傍や間違い候補の画像一覧から**『別の人』を呼び出せる**ようにして、
  正しいピープルアルバムへ入れられるようにしてほしい」。近傍（他人との距離）は
  「分裂しているか」は見せるが、**内側に紛れ込んだ顔**は見せない——混入は内側で起きる。
- 決定:
  1. 内訳レポートに**重心から遠い順のメンバー**（`outlierFaces`・最大 24 件）を追加する。
     各顔に重心との類似度・確認済みか・**いまのしきい値なら合流しないか**（オレンジ表示）を付ける。
     メンバーが 3,000 を超える人物は計算しない（デバッグ画面のために全埋め込みを触らない）。
  2. 付け替え先ピッカー（`FaceReassignPickerView`）を「顔の管理」から**共有部品として切り出し**、
     間違い候補・近傍の顔一覧からも呼べるようにする。**誤りに気づく場所は 1 つではない**ので、
     直す入口も 1 つにしない（ADR-129/133 と同じ考え方）。付け替えは取り消しの対象（ADR-136）。
- 結果: 「この人物は誰で出来ているか」を内側から確認し、その場で正しい人物へ移せる。
  トレードオフ: 外れている＝誤りとは限らない（横顔・成長・照明）。確認済み（アンカー）の顔には
  印を付けて区別する。回帰テスト `outliersSurfaceContamination`（混入が先頭に出る）。
- 関連: `FaceStore+Explain.outlierFaces` / `FaceClusterInspectorView` /
  `FaceReassignPickerView`（PeopleKit・共有部品）/ ADR-135/136。

## ADR-136 ピープルの判定は「直前の 1 手」を取り消せる
- 状態: 採用（実行中のみ保持・永続化しない）
- 文脈: 実フィードバック「ピープルの確認をしていると、たまに、間違った！と思うことがある。
  戻るボタンは付けられる？」。確認は連続で答える画面なので、**間違いに気づくのは次のカードが
  出た直後**。そこで戻せないと、あとから顔の管理を開いて手で直すことになる——直し方を知って
  いないと戻せない、という一番きつい形になる。
- 決定: 操作の**直前に「触る前の行」を控え**（顔の所属・確認印・重心寄与／人物行の重心・件数・
  名前・代表・束ね）、取り消しはそれを書き戻す。この操作で記録した学習（`FaceCorrection`）は
  控えた時刻より後のものを消し、この操作で新しく作られた人物のうち顔が残っていない行を消す。
  対象は 1 対 1 の確認・まとめて確認・グループの分離・「この人ではない」「別の人へ移す」。
  控えは**実行中のメモリだけ**（10 手・1 手あたり 5,000 顔まで）で、再クラスタ・再スキャンの
  後は捨てる（戻す先が変わっているため）。
- 結果: レビュー画面のツールバーに「戻す」。1 対 1 の確認では**そのカードへ戻って答え直せる**。
  トレードオフ: アプリを終了すると控えは消える（起動を跨いだ取り消しは、間の再クラスタで
  戻す先が変わっていて危険なため、あえて持たない）。
  ⚠️ 逆操作を操作ごとに書かない——統合の逆は「顔を戻す＋消えた行を作り直す＋重心と名前を戻す＋
  学習を消す」で、**逆操作を 6 種類書けば 6 種類ぶんバグる**。状態の復元なら 1 つで済む。
  回帰テスト `FaceUndoTests`（統合・分離・控えの上限/破棄）。
- 関連: `FaceStore+Undo.swift` / `PeopleEngine.undoLastAnswer` / `FaceReviewView` /
  `FaceBatchReviewView` / ADR-45/46/134。

## ADR-135 顔クラスタリングのチューニングは「近さの図」ではなく「判定の内訳」で見る
- 状態: 採用（Developer Options・読み取り専用）
- 文脈: 「Maltego のようなグラフ表示で顔・人物の近似度を出すデバッグ機能があれば、チューニングの
  感覚や設定状況の把握ができそう」。実際に必要なのはそこの手前だった——直近の不具合
  （乗っ取り＝ADR-130、学習消失＝ADR-134）で効いていたのは類似度そのものではなく、
  **そこに何が上乗せされたか**（サイズ適応マージン）、**種になれるか**（名前/アンカー/束ね）、
  **負例が効いているか**だった。近さの図はそれを表示しない。
- 決定: 焦点にした人物と近傍を、**割り当て規則そのものの数字**で並べる画面を Developer Options に
  置く（`FaceClusterInspectorView`）。行ごとに cos / 実効しきい値（しきい値＋サイズ上乗せ）/
  結論（合流する・しきい値未満・サイズ上乗せで届かない・マージンゲート・負例が拒否・同一写真・
  別々の名前）と、相手の枚数・重心寄与数・アンカー数を出す。焦点人物側は「再クラスタの種になるか」を
  明示する。判定は純関数 `FaceDecisionExplain`（SwiftData 非依存）に置き、
  **`FaceClustering.assign` と同じ順序・同じ式**で評価する。
- 結果: 「なぜ合流しないか」が 1 画面で分かり、しきい値を動かす前に**どの制約が効いているか**を
  確かめられる。トレードオフは規則を 2 箇所に書くこと——`sizeMargin` は実装側と同じ式を共用し、
  ずれたら落ちるテスト（説明と `assign` の結論一致）を置いて担保する。
  グラフ描画（力学配置）は見送り: 近さは分かっても判定理由が出ないため。必要になれば
  この内訳の上に足せる（人物間類似度は同じ計算で出ている）。
- 関連: `FaceDecisionExplain.swift` / `FaceStore+Explain.swift` / `FaceClusterInspectorView.swift` /
  `PeopleEngine.decisionReport` / `FaceDecisionExplainTests` / ADR-130/132/134。

## ADR-134 ユーザーの表明（統合・束ね）を、再クラスタが忘れない
- 状態: 採用（`clusterRuleVersion` 8）
- 文脈: 実フィードバック「まとめて確認や『別の人と束ねる』で何度も調整しているのに、**再クラスタが
  終わると学習した内容を忘れる**気がする」。調べると、2 つとも**記録の残り方が弱かった**:
  1. **統合（merge）は顔に何の印も付けていなかった**。`FaceCorrection` に正例を 1 行残すだけで、
     これはしきい値校正にしか使われない。印が無い人物は再クラスタの種にならない（名前もアンカーも
     無い＝機械が作った断片と同じ扱い）ので、メンバーは毎回ばらされ、**しきい値未満の 2 群は
     また 2 つに割れる**。統合の逆操作（split）は負例を残すので生き残るのに、統合だけが消えていた。
  2. **束ね（`personGroupID`）はクラスタ行に載っている**のに、再クラスタは種以外の行を削除する。
     束ねた相手が無名クラスタなら、行ごと消えて**束ねが黙って無かったことになる**。
- 決定: (1) 統合時に**両側の代表顔 1 枚ずつをアンカー（`confirmedAt`）にする**——「同じ人」という
  表明を顔に刻む。以後その人物は種になり、ADR-132 のとおり構成が保たれる。
  (2) 再クラスタの種の条件に `personGroupID != nil` を加える（束ねた人物は行ごと残す）。
- 結果: まとめて確認・1 対 1 の確認・束ね・代表写真・命名——**ユーザーが触った人物はすべて種**に
  なり、夜の再クラスタで作業が失われない。トレードオフは、統合を重ねるほど種（＝動かない人物）が
  増えること。誤って統合したときは「顔の管理」や「この人ではない」で外せば負例として学習される。
  ⚠️ 既に消えてしまった過去の統合・束ねは**復元できない**（記録そのものが残っていない）。
  回帰テスト: `mergeSurvivesRebuild` / `groupingSurvivesRebuild`（どちらも修正を外すと落ちる）。
- 関連: `FaceStore+Edit.mergeClustersUnchecked` / `FaceStore+Rebuild.rebuildClusters` /
  ADR-132 / ADR-45/46/61 / 事例「学習したはずの統合・束ねが再クラスタで消える」。

## ADR-133 写真を見ている場所で、人物の誤りを直せるようにする
- 状態: 採用（ADR-129 の拡張）
- 文脈: 実フィードバック 3 点。(a)「人物アルバムで束ねたり『この人ではない』を選んでも、
  **ビューが更新されない**」。(b)「『この人ではない』だけでなく、**この人は別の人**という選択も
  付けて、相手を選べるようにして」。(c)「**人物アルバム以外でも**、1 人しか写っていない写真なら
  同じ操作を出して。**AI アルバムで家族の束を見ていて、家族じゃない写真があると気づいた**ときに
  直したい」。誤りは人物アルバムだけで見つかるわけではない。
- 決定:
  1. 人物アルバムは操作のたびに**人物とメンバーを引き直して描き直す**（`PersonAlbumView.reload`）。
     束ねると主クラスタが相手側へ移り得るので、**束ね先の人物として**開き直す（タイトルも追従）。
     メンバーが変わらないときは組み直さない（スクロール位置を無駄に失わない）。
  2. 「この人は別の人…」＝付け替え先を選ぶピッカー（`PhotoPersonPickerView` / 新しい人物も選べる）。
     外すだけの操作と違い、**付け替え先の確認顔（アンカー）として学習**される（ADR-46・ADR-132）。
  3. `photoContextActionProvider`（写真 ID → その写真ぶんの操作）を `PhotoSourceKit` に足し、
     アプリの写真画面すべて（`SourceHostView` 配下）に `.photoPersonActions` を付ける。
     出すのは**1 人しか写っていない写真**だけ——複数人ではメニューの文言でどの人か決まらないため。
     グリッドは長押しの瞬間に `UIDeferredMenuElement` で解決し、一覧の全セルぶんを先に引かない
     （規模比例の問い合わせを作らない・ADR-119）。
- 結果: 気づいた場所で直せる。トレードオフは、写真ビューを開くたびに「誰が写っているか」を
  1 枚ぶん問い合わせること（長押し／ページ送りの都度・1 件だけ）。
- 関連: `PersonAlbumView` / `PhotoPersonActions.swift` / `PhotoPersonPickerView.swift` /
  `PhotoSourceEnvironment`（`photoContextActionProvider`）/ `PhotoCollectionView` / `PhotoPageView` /
  `PeopleEngine.movePhoto` / `PeopleEngine.solePerson` / ADR-129。

## ADR-132 名前を付けた人物のアルバムは、機械の都合で組み替えない
- 状態: 採用（`clusterRuleVersion` 7・ADR-130 の続き）
- 文脈: 実フィードバック「すでに名前の付いているアルバムは、**よほどのことが無い限り 2 つに分けたり、
  構成するグループを切り分けたりは不要**。そういうケースは一人ずつ顔写真を確認する画面に出して、
  ユーザーが『この人ではない』と指摘して初めて分割を検討すればよい」。ADR-130 で種（重心・アンカー）
  は守られるようになったが、**メンバーは毎晩プールへ戻して割り当て直していた**ので、
  しきい値・マージン・「別のクラスタが後から育った」といった機械の都合だけで、
  名前付きアルバムの構成が毎晩変わり得た（1 枚だけ他人のアルバムへ移る、等）。
- 決定: ユーザーが表明した人物（名前・代表写真・確認顔のいずれかがある人物）の**既存メンバーは
  再クラスタで動かさない**（`pinnedCluster`）。外れるのは**ユーザーの指摘に一致した顔だけ**
  （`negativeRejects`＝「この人ではない」で外した顔と同一人物の顔・ADR-45）。重心は留めたメンバーの
  加重平均（＝再クラスタ前と同じ向き）、アンカーは `prototypes` として別に効く（上限 8＝
  確認を重ねた人物ほど再クラスタが重くなるのを防ぐ・ADR-119）。無名のクラスタ（機械が作った断片）は
  従来どおり自由に組み替える。疑わしい顔は**レビュー画面で尋ねる**（既存の A2 経路）。
- 結果: 名前付きアルバムは、ユーザーが触らない限り毎晩同じ。純度の自動改善は諦める代わりに、
  「気づいたら別人になっていた」が構造的に起きなくなる。誤りは (1) レビューで尋ねる
  (2)「この人ではない」で外す、の 2 経路でユーザーが直し、その指摘は同一人物の他の顔にも波及する。
  回帰テスト: `namedPersonKeepsMembersAcrossRebuild`（後から育った別人に 1 枚も取られない）/
  `userCorrectionEjectsMatchingFaces`（指摘した顔と同一人物は留めない）——どちらも該当箇所を
  外すと落ちることを確認済み。
- 関連: `FaceStore+Rebuild.swift` / ADR-130 / ADR-45/46 / ADR-123（レビューの基準は命名済み人物）。

## ADR-131 低優先レーンの「UI へ譲る」に上限を設ける（無制限に譲らない）
- 状態: 採用
- 文脈: 実フィードバック「顔の管理（この人ではない顔をタップして下さい）の画面と、代表写真を選ぶ
  画面で、サムネイルが**まったくロードされない**」。顔アバターは `HeavyImageLane`（低優先レーン）
  を通しており、`waitWhileUIBusy` は **UI がビジーな間ずっと**待つ実装だった。この「ビジー」には
  `cloudThumbnailBusy`（Dropbox サムネのドレイン中）が含まれ、数万枚のライブラリでは**分単位で
  立ちっぱなし**になる。結果、その間に開いた画面の画像は 1 枚も出ない。譲りは「操作の瞬間を
  邪魔しない」ためのものなのに、**表示を諦める**動作になっていた。
- 決定: 譲りに上限（既定 2 秒）を入れる（`waitWhileUIBusy(maxSeconds:)` /
  `HeavyImageLane.run(maxYieldSeconds:)`）。ユーザーが今見ている画面の画像（顔アバター）は
  0.5 秒、ディスクの小さな JPEG を読むだけの経路は**譲らない**。同時実行数のスロット（Gate）は
  常に効いているので、上限を切っても burst にはならない。
- 結果: 背景がどれだけ忙しくても画像は必ず出る。トレードオフはスクロール中に低優先の処理が
  最大 2 秒後に走り始めること（同時数は絞られたまま）。回帰テスト
  `HeavyImageLaneTests`＝「UI ビジーのままでも body は走る」（上限を外すと落ちることを確認済み）。
- 関連: `MosaicSupport/HeavyImageLane.swift` / `PeopleKit/FaceAvatarImage.swift` / 事例「顔の
  サムネイルが 1 枚も出ない」/ ADR-88。

## ADR-130 人物の同一性は「ユーザーの表明」で固定する（再クラスタで乗っ取らせない）
- 状態: 採用（`clusterRuleVersion` 6 ＝既存ユーザーは次の夜間処理で置き直す）
- 文脈: 実フィードバック「**まさに私の顔を認識しているアルバムが、いつの間にか丸ごと娘の顔写真に
  なっていた**。私の写真は People 9 とか別の人物として追い出されている」。再クラスタ
  （`rebuildClusters`）の**種の作り方**が弱かった: (a) **代表写真（cover）がアンカーではなかった**
  ——1 対 1 の確認をしていない人物（まとめて確認だけで育てた人物）はアンカーが 0 個。
  (b) その場合の種は「現重心を 1 票」だけで、**count=1** はサイズ適応マージン（ADR-58）が最も
  厳しく効く状態＝**1,000 枚の確立した人物が、作り直しの瞬間だけ「生まれたての 1 顔クラスタ」**に
  なり、本人の顔すら入れない。(c) 重心が別人へ寄っていれば、そのまま別人のアルバムになる。
  名前はクラスタ ID に付いているので、**中身だけが入れ替わっても名前は残る**。
- 決定: ユーザーの表明を同一性の土台にする。
  1. **代表写真を選ぶ＝人物確定**（`setCover` が `confirmedAt` を立てる）。**名前を付ける＝人物確定**
     （アンカーが 0 の人物は、その時点の代表顔をアンカーにする）。ユーザー提案どおり。
  2. 再クラスタの種は**アンカー（確認顔＋代表顔）から重心を作り**、アンカーは**そのクラスタへ固定**
     （既に別クラスタへ流れていても引き戻す）。
  3. 種の `count` は**以前の規模を引き継ぐ**（向きはアンカー、大きさ＝動きにくさは実績）。
  4. **名前は人に付いていく**: アンカーの無い命名済み人物（本 ADR 以前のデータ）の顔が過半まとめて
     別クラスタへ移ったら、名前もそちらへ移す。名前だけ元の ID に残ると、そこへ流れ込んだ別人が
     その名前のアルバムになる。
- 結果: 代表写真・確認・命名のいずれかをした人物は、重心が別人へ寄っていても再クラスタで
  入れ替わらない。既に壊れたアルバムは版上げの再クラスタで置き直される（アンカーが無いまま
  壊れた人物は 4 で名前が本人側へ戻る／それでも合わなければ、代表写真を選び直せば以後は固定）。
  トレードオフ: アンカーは強い制約なので、**誤った顔を代表写真にすると誤りも固定される**
  （「この人ではない」で外せば負例として学習される・ADR-45）。
  回帰テスト `PersonIdentityAnchorTests`（3 本とも、修正を外すと落ちることを確認済み）。
- 関連: `FaceStore+Rebuild.swift` / `FaceStore+Edit.swift`（rename）/ `FaceStore+People.swift`
  （setCover）/ `PeopleEngine+Review.swift`（`clusterRuleVersion` 6）/ ADR-45/46/58/126 / 事例
  「命名済みアルバムの中身が別人と入れ替わる」。

## ADR-129 「この写真はこの人ではない」を、写真を見ている場所から直せるようにする
- 状態: 採用
- 文脈: 実フィードバック「ピープルアルバムのサムネイル／全画面から『この人ではない』を呼べるように
  してほしい。**全体像や前後関係で違うと気づく**ことがある」。従来は「顔の管理」＝顔の切り抜きを
  並べた画面からしか外せなかったが、**顔だけを見ても気づけない誤り**がある（後ろ姿・似た兄弟・
  同じ日の別人）。気づいた場所で直せないと、覚えて画面を移動するという負担が要る。
- 決定: `PhotoSourceKit` に**写真ごとの追加操作**の seam（`PhotoContextAction` ＋
  `photoContextActions` 環境値）を置き、グリッドは**長押しメニュー**、全画面は**「…」メニュー**に出す。
  ⚠️ `PhotoSourceKit` は顔の仕組みを知らない（`FaceCore` に依存しない）ので、
  **表示名と実行**を呼び出し側（人物アルバム）が渡す——`faceHighlightProvider` と同じ形。
  実行は `PeopleEngine.removePhoto(itemID:from:)`：写真 ID の解決は `faceHighlights` と
  **同じ規則**（`refKeyCandidates`）に揃える（別々に解くと「黄枠は出るのに外せない」が起きる）。
- 結果: 気づいた場所で 1 手で直せる。外した顔は ADR-45 の**負例として学習**されるので、
  再スキャン・再クラスタでも同じ誤りは再発しない。取り消しは従来どおり「顔の管理」から
  別の人物へ付け替える。⚠️ 同じ人物の顔は 1 枚の写真に 1 つしか入らない（同一写真 cannot-link）が、
  実装は**その写真の顔をまとめて**外す——旧データや第2パスで不変条件が破れている行を残さないため。
- 関連: `PhotoContextAction` / `PhotoGridView` / `PhotoPageView` / `PersonAlbumView` /
  `FaceStore+Edit.removePhoto` / テスト `RemovePhotoFromPersonTests`。ADR-45（負例の学習）。

## ADR-128 バックアップ副本は「アルバムの中でも」隠し、撮影日を持たせる
- 状態: 採用
- 文脈: 実フィードバック「ピープルのグループアルバムが時系列で並んでいない。バックアップを
  新しい写真と認識しているのか」。調べると**別々の 2 つの欠陥が重なっていた**:
  (1) 副本を隠す索引（`backupCopyIndexProvider`）を、ホームの一覧には渡していたのに
      **メンバー限定ストア（人物・グループ・場所・アルバム）には渡していなかった**
      （diagnostics-57/58 の対処が一覧だけで止まっていた）。同じ写真が原本と副本の 2 枚で並ぶ。
  (2) アップロード時に `client_modified` を送っておらず、Dropbox は**アップロード時刻**を記録する。
      同期側の撮影日は `time_taken ?? client_modified` なので、EXIF から `media_info` が
      付かない写真では副本が「今日撮った写真」になる＝古い写真が列の先頭に出る。
- 決定: (1) `MergedPhotoStore.defaultBackupCopyIndexProvider`（Composition Root が設定）を
  `forMembers` が自動で渡す。⚠️ 隠すのは**原本が同じアルバムに居るときだけ**——オフロード済み
  （端末に原本が無い）写真まで隠すと、アルバムから写真が消える。
  (2) アップロードの `Dropbox-API-Arg` に `client_modified`（撮影日・秒精度 UTC）を載せる。
- 追補（同日・**まだ直っていなかった**）: (1)(2) だけでは足りなかった。⚠️ **隠すだけでは直らない**——
  顔がクラウド副本側だけで検出された写真は、原本がそのアルバムに居ないので隠れず、日付だけが
  アップロード時刻のまま残る。(2) はこれから上げる写真にしか効かない。
  → (3) **表示時に台帳の撮影日で上書きする**。アプリの台帳（`BackupAssetRecord.creationDate`）は
  元の `PHAsset.creationDate` を持っているので、Dropbox 側の日付（`time_taken ?? client_modified`）
  より信用できる。`MergedPhotoStore` が再構築時に差し替える＝**既にアップロード済みの写真も直る**。
- 結果: アルバムの重複が消え、並びが撮影日どおりになる（既存分も含めて）。
  ⚠️ 台帳に無い写真（アプリ以外で置いた Dropbox 写真）は従来どおり Dropbox の日付を使う。
- 関連: `MergedPhotoStore+Members` / `DropboxBackupUploader.upload(clientModified:)` /
  `BackupRunner.uploadOne` / テスト `MemberStoreBackupHidingTests`・`UploadClientModifiedTests`。

## ADR-127 写真の地図は「ズームで決まるグリッド」で畳む
- 状態: 採用（Step 1 実装済み・サムネ表示は Step 2）
- 文脈: 写真アプリのような地図ビューが欲しい（撮影地にピン・拡大縮小）。ライブラリは 86,000 枚規模で、
  **1 枚 1 ピンは成立しない**（描画も操作も破綻する）。一方で「場所」の索引はすでにあり
  （`PlaceScanner` が端末の EXIF/GPS と Dropbox メタから `PlaceCandidate` を作る）、作り直す必要はない。
- 決定:
  (1) **データ源は既存の `PlaceCandidate`**。地図を開いたときに索引を作り（`locatedCandidates`）、
      閉じたら解放する（86k 件を常駐させない）。構築は `HeavyLoad` の札つき（ADR-122）。
  (2) **集約はズームで決まるグリッド**（`PhotoMapClustering`・純ロジック）。step は画面の経度幅から
      決め（既定 12 列）、可視範囲（+30% マージン）だけを 1 パスで畳む。ピン数の上限は 120。
      ⚠️ 上限は安全弁で、**本体は粒度**——step が画面幅に追従するので、写真が何枚あってもセル数は
      `列数²` 程度で頭打ちになる（テストで固定）。
  (3) **SwiftUI の `Map` ＋ 自前アノテーション**。`MKMapView` の `MKClusterAnnotation` は使わない
      （件数バッジ・代表写真を出しにくく、集約規則をテストできない）。
  (4) 領域変更は `.onMapCameraChange(frequency: .onEnd)` → **オフメインで集約** → 世代照合で
      最新だけ反映（`MergedPhotoStore.rebuildItems` と同型）。メインへ渡すのは数百ピンだけ。
  (5) ピンのタップは `MergedPhotoStore.forMembers` で既存のグリッド/フル画面へ（`PlacePhotosView` と同型）。
- 結果: 写真枚数に依らず描画対象が有界。純ロジックが `swift test`（macOS）で回るので、
  「ズームで割れる」「上限を超えない」「範囲外を拾わない」を回帰として固定できる。
  ⚠️ `PhotoMapPin` の `==`/`hash` は**セルのキーと件数だけ**で判定する（members 数千件を
  SwiftUI の差分判定のたびに比較しない）。
- 追補（実フィードバック 2 件・Step 2）:
  - **「ピンが密集しすぎ」**: グリッドだけでは足りなかった。隣り合うセルの写真がそれぞれ境界側に
    寄っていると、重心どうしは 1 セルぶんも離れない。**画面に正規化した距離**（画面幅=1）で
    近すぎるピンを大きいほうへ**吸収**する（間引きではない＝写真は 1 枚も落ちない）。
    既定は画面幅の 14%（約 55pt）で、1 画面のピンは最大約 50 に収まる。列数も 12 → 8 へ。
  - **代表写真のサムネを表示**: ピンは 52pt のサムネ＋件数バッジ。クラウド写真のサムネは
    表示用サムネと同じ回線を通るので、(1) 可視ピンぶんだけ（数十に有界）、(2) セッション内
    キャッシュ（上限 200・地図を動かすたびに取り直さない）、(3) ピンが消えたら `task` の
    キャンセルで取得も止まる、で抑える。さらに**代表は端末内の写真を優先**する
    （同じセルに端末写真があれば通信ゼロで出せる。体感差は「新しさ」より「出るかどうか」）。
  - **戻ったら元の場所へ**: 写真を開いて戻ると初期位置（＝日本全体）に飛んでいた。原因は
    (1) push で `onDisappear` が走って索引を捨てていたこと、(2) 再表示で `.task` が索引を
    作り直しカメラを初期化していたこと。索引の生成を 1 回だけにし、破棄は画面（`@State`）の
    寿命に任せる。⚠️ **押して戻るだけで捨てない**——「閉じたら解放」と「押したら解放」は違う。
  - **初期位置は「写真がいちばん多い場所」**（`initialRegion`）。全体が収まる範囲だと開いた瞬間に
    見たいものが無い。⚠️ 国や都市は**コードに書かない**——既定粒度（約 2km）で最多のセルを選び、
    その近傍の写真の広がりを実測して範囲にする。どこに住んでいる人でも同じ規則で動く。
- 残課題: (a) 日付での絞り込み。(b) 経度 180 度線をまたぐ範囲は未対応。
  (c) 現在地表示は入れない（位置情報の許可を新たに求めないため）。
- 関連: `PhotoSourceKit/Places/PhotoMapClustering.swift` / `PlaceScanner.locatedCandidates` /
  `MosaicPhotos/Home/PhotoMapView.swift` / テスト `PhotoMapClusteringTests`。

## ADR-125 ピープルは「よく写っている人」だけ載せる（表示フロア）
- 状態: 採用
- 文脈: 実機でピープルが 1,000 人超。中身は「たまたま写り込んだ 5 枚の人」が大量で、
  本当に見たい数名が埋もれる（実フィードバック: 「2 枚とか 5 枚しか顔写真がない人は載せなくて良い」）。
- 決定: **表示フロア 10 枚**（`PeopleEngine.minPhotosForList`・「すべて表示」のフィルタで 3/5/10/20）。
  **名前を付けた人は枚数に関係なく必ず表示**する——名前はユーザーが関心を表明した唯一の印で、
  3 枚しかない親戚に名前を付けるのは普通に起きる。
  ⚠️ **表示だけの線**にする。学習（レビュー候補・名前解決・検索の接地・グループの解決）の母数は
  従来どおり 3 枚以上（`allPeople`）。表示から消すために学習材料まで捨てない。
- 結果: 一覧が実用的な長さになる。隠した人は**フィルタを下げれば戻る**（消していない）。
  トレードオフ: 5 枚の人を人物アルバムから開くにはフィルタを下げる必要がある。
- 関連: `PeopleEngine.people` / `allPeople` / `AllPeopleView`（フィルタ）/ テスト
  `PeopleDisplayFloorTests`。face-accuracy.md 2026-08-29。

## ADR-126 マージンゲートの免除は「校正で bar が上がったときだけ」効かせる
- 状態: **撤回**（実機で人物アルバムが崩れた。`clusterRuleVersion` 5 で元の規則へ戻す）
- ⚠️ **撤回の理由（2026-08-29・実フィードバック）**: 採用後の再クラスタで
  「枚数の少ない人物のアルバムが決定的におかしい。枚数の多い人物でも数枚おかしい」。
  データセット計測では LFW 混在で純度 −0.001・F1 同値＝「ほぼ無料」に見えていた。
  食い違いの理由は**分布**と**指標**の 2 つ:
  (1) 手持ちのデータセットに「主役 1 人 1,000 枚 ＋ 数枚ずつの他人が数百人」が無い。
  (2) **平均の純度は、小さいアルバムの体験を代表しない**。3 枚のアルバムに 1 枚混ざれば
      体感は「壊れた」だが、全体純度はほとんど動かない。ゲート免除は紛らわしい顔を
      「1 位のクラスタ」へ入れる規則で、**重心が不安定な小クラスタほど 1 位になりやすい**。
  再挑戦するなら「**小さいクラスタには免除しない**（成熟クラスタ限定）」条件とセットで、
  かつ**小さいアルバムだけを見る指標**（例: 3〜10 枚クラスタの混入率）を先に用意すること。
- 文脈: 名前を付けた数名（数百〜1,000 枚）が何十個にも割れる、という実フィードバック。
  調べると実機の校正値はプロファイル可動域の上限 0.40 に張り付いていた。
  ⚠️ しきい値を下げれば直る、ではない: 0.35 は FG-NET（加齢）に有利だが LFW（901人）では
  純度 0.852 vs 0.927・F1 0.892 vs 0.932 と明確に不利で、**人数の多いライブラリでは 0.40 が正しい**。
  そこで新しい計測軸（**混在シナリオ**＝重い数人＋長い尾・ADR-125 の計測）で全バリアントを測り直した。
- 決定: **マージンゲートの免除（rivalAwareMarginGate）を、校正値が既定より高いときだけ有効**にする。
  計測（thr=0.40）: LFW 混在の上位5人の分裂 **2.4 → 1.6（最悪 7 → 3）**・純度 −0.001・F1 同値、
  FG-NET 混在も F1 +0.001。一方 thr=0.35 では FG-NET が F1 0.790 → 0.759 と悪化するため、
  **既定値のままの端末では従来どおり**にする。`clusterRuleVersion` を 4 へ（夜間に全体再クラスタ）。
- 結果: 「校正が bar を上げた結果、同じ人の別クラスタどうしが恒常的に紛らわしくなった」状態だけを
  ほどく。ADR-68 で (a) を不採用としたのは**既定値・全体集合での計測**だったからで、
  条件を付ければ両立する——⚠️ 過去の不採用は「その条件での不採用」であって永久ではない。
- 残課題: 用意できるデータセットに「1 人 1,000 枚」は無い（LFW 上位5人は各 10 枚）。
  実ライブラリの頭の重さは未計測で、効果は実機ログ（`faces: rebuild done — clusters=…`）で追う。
- 関連: `FaceStore.rivalAwareMarginGateWhenCalibratedUp` / `FaceStore+Rebuild` /
  テスト `RivalAwareGateConditionTests` / face-accuracy.md 2026-08-29。ADR-57/58/68。

## ADR-123 人物レビューの基準は「名前を付けた人」に絞る
- 状態: 採用
- 文脈: 「人物を確認」「まとめて確認」で候補探しに数秒かかる（実機 `people.reviewItems 5.9 秒` /
  `people.batchReview.load 2.1 秒`）。原因は**基準を全人物に取っていた**こと——
  A1 の統合サジェストは人物数²（552 人で 15 万対）、境界の顔（A2）は「命名優先で並べた先頭 60 人」を
  **実体（埋め込み込み）で**引き、共起判定のために**全顔**を射影で読んでいた。
  ⚠️ 実フィードバックが方向を決めた: 「100 枚単位で名前を付けている人物は数名。その数名に近い
  写真を探せばよい。数枚しかない名前も付けていない人物をキーに検討する必要はない」。
- 決定: **注目人物（focus）＝命名済みクラスタ**（1 つも無ければ大きい順に 8 人）を基準にする。
  (1) A1 の候補対は**片側が必ず基準**（人物数² → 基準数 × 人物数）。
  (2) A2 の境界走査と A3 の事後監査は基準のみ（上限 `boundaryScanLimit`=60 を維持）。
  (3) 顔の読み出しは「先に重心だけで絞り、**基準＋候補に挙がったクラスタの顔だけ**を引く」
  （`faceDigests(inClusters:)`）。重心は `PersonCluster.sum` にあるので、絞り込みに顔は要らない。
  (4) 一括レビューのアンカーも focus から選ぶ（大きい無名より、名前を付けた人を優先）。
- 結果: 無名の断片が何人増えても候補探索は重くならない（規模テストで固定）。
  トレードオフ: **無名どうしの統合は提案しなくなる**。名前を付けた人へ畳む道（候補側には出る）は
  残るので、ユーザーの関心に沿って収束する。名前が 1 つも無いライブラリは従来どおり大きい順。
  ⚠️ これは**出題の選ばれ方**の変更で、クラスタリング精度（`face-accuracy.md`）は不変。
- 関連: `FaceStore.focusClusters` / `FaceStore.faceDigests(inClusters:)` / `FaceStore+Review` /
  テスト `FaceReviewFocusTests`・`ScaleRegressionTests`（無名 4 倍で fetch が増えないこと）。ADR-46/68/119。

## ADR-124 数秒かかる待ちには、必ず「出口」を出す
- 状態: 採用
- 文脈: 候補探索中は画面にスピナーだけが出て、**中断する手段が無かった**
  （実フィードバック: 「次の人物を探しているとき、UI が何も書かれていないのでキャンセルできない」）。
  待ち時間そのものより、待たされている側に出口が無いことが応える。
- 決定: `busyOverlay` に任意の**キャンセル**を足し、人物レビューの探索に付ける。
  中断は形だけにしない——`FaceStore` の候補生成が要所（クラスタのループ・重い読み出しの前）で
  `Task.isCancelled` を見て降りるので、押した瞬間に ModelActor が解放される。
- 結果: 探索が長いときでも閉じられる。実装コストは低く、同じ形（重い非同期＋オーバーレイ）へ
  横展開できる。⚠️ 中断可能にするだけでは足りず、**始める前の判定**（ADR-122）と対で考えること。
- 関連: `PhotoSourceKit/BusySpinner.busyOverlay(_:text:cancel:)` / `FaceReviewView` /
  `FaceBatchReviewView` / `FaceStore+Review`（`Task.isCancelled`）。

## ADR-122 起動の一括ロードは「札」を立て、中断できない背景ロードを重ねない
- 状態: 採用
- 文脈: 起動・復帰の数秒に、73k 件のキャッシュ実体化・68k 件のパスアルバム計算・86k 件のマージ・
  18k 件のアセット索引が**同時に**走り、そこへ背景の CLIP テキストタワーのロード
  （実測 12.9 秒・約 120MB）が重なって footprint が **569MB** に達した（実機 diagnostics-66）。
  背面 568MB では実際に jetsam で落ち、復帰＝コールドリランチになっている（diagnostics-65）。
  ⚠️ どれも単体では健全（0.3〜1.5 秒）で、**重なることだけが問題**。しかも重なった側の
  モデルロードは直後に `cancelPrewarm` で捨てられており、ピークを作っただけで成果はゼロだった。
- 決定: (1) `MosaicSupport.HeavyLoad` に「一括ロードが走っている」札を置く（ラベル別・多重可・
  30 秒で自動失効＝背景処理を永久に止めない安全弁）。(2) 起動の 4 経路
  （`cache.items` / `pathAlbum.fast` / `merged.rebuild` / `assetIndex`）が札を立てる。
  (3) `BackgroundYield.heavyShouldPause()` が札を見る＝**背景のトリクルとモデルロードが順番を待つ**。
  (4) `CLIPDisplayLabeler.prewarm()` は**ロードを始める直前**にもう一度ゲートを見て、閉じていれば降りる
  （Task の生成時点と実行時点は、プロセス中断を挟んで大きくずれる）。以後は 8 語ごとにも見る。
  ⚠️ **向きは一方通行**。背景ロードは起動処理を待つが、**起動処理が背景ロードを待つことはない**
  （ユーザーが待っている処理を、誰も待っていない処理のために遅らせない）。
- 結果: 起動時のピークが下がる。代償は「背景の準備が少し後ろへずれる」だけ（prewarm は
  未完了でも実害なし＝insight は Vision タグで即返る）。札は失効するので、事故で立ちっぱなしに
  なっても最大 30 秒で解ける。
- 関連: `MosaicSupport/HeavyLoad.swift`・`BackgroundYield.heavyShouldPause`・
  `CLIPDisplayLabeler.prewarm`・`DropboxPhotoStore.reflectCachedItems`・`PathAlbumGenerator.generateFast`・
  `MergedPhotoStore.rebuildItems`・`LocalAssetIndex`。テスト `HeavyLoadTests`。ADR-121（同じログの前半）。

## ADR-121 SwiftData ストア（@ModelActor）は専用のシリアルキューで走らせる
- 状態: 採用
- 文脈: 実機で「重い SwiftData の読み出しがメインスレッドを塞ぐ」ハングを何度も踏んだ
  （14.5s / 13.9s / 10.5s / 10.9s / 5.9s…）。原因は `@ModelActor` の既定 executor
  （`DefaultSerialModelExecutor`）が **ジョブを「呼び出し元のスレッド」でそのまま実行する**こと。
  MainActor から `await store.fetchAll()` と書くと、その fetch は**メインで走る**——`await` が
  あるのでコード上はオフメインに見え、レビューでも気づけない。ハング中のメインスレッドの
  スタック採取（ADR-106 の自前採取）で `AutoAlbumStore.allEnrichedPhotosLite` /
  `FaceStore.faceDigestsByCluster` がメインに載っているのを直接観測して確定した。
  **「init したスレッドに束縛される」という従来の理解は誤り**だった（生成スレッドは無関係）。
  そのため `Task.detached { Store() }`（オフメイン生成）では一切防げていなかった。
- 決定: 各ストアの `unownedExecutor` を**専用のシリアルキュー**（`ModelStoreExecutor.serialQueue`）に
  差し替える。対象は `AutoAlbumStore` / `TagStore` / `UsageStore` / `FaceStore` / `BackupStore` の 5 つ。
  キューに **QoS は付けない**（`.unspecified`）——固定すると UI 起点の読みまでその優先度に落ちる。
  未指定なら enqueue 側の優先度を引き継ぎ、前面の読みは高く・夜間バッチは低く走る。
  オフメイン生成は残す（コンテナを開く＝ストアファイルを触るディスク I/O をメインから外すため）。
- 結果: ストアの処理がメインに載らなくなる（＝規模に比例する fetch がそのまま前面ハングになる形が
  構造的に消える）。代償は毎呼び出しのスレッドホップ 1 回。actor の直列性はキューが保証するので
  SwiftData の要件（ModelContext を同時に触らない）は満たしたまま。
  回帰は**テストで固定**した（`ModelActorExecutorTests`・3 パッケージ）: MainActor から呼んで
  `Thread.isMainThread == false` を検証する。`unownedExecutor` を外すと落ちることを確認済み。
- 関連: `MosaicSupport/ModelStoreExecutor.swift`・各ストア・事例「SwiftData ストアがメインスレッドで
  走っていた（ロック解除後の 10.9 秒フリーズ・diagnostics-64/65）」。ADR-106（ハング中のスタック採取）。

## ADR-120 永続化は「変化検出してから書く」を既定にする
- 状態: 採用
- 文脈: OS が `diskwrites_resource` の報告を出した（12 分で 1.07GB・制限の約 117 倍）。
  原因は Dropbox 差分適用が既存行を無条件に代入し、`cachedAt = Date()` を毎回進めていたこと。
  実測は `inserted=0 / updated=109,679`——**1 件も新規が無いのに 11 万行を書き直していた**。
  「同じ値の代入なら安いはず」は誤りで、SwiftData/Core Data は代入そのものでダーティ化し、
  save のたびに SQLite へ書き戻る。書き込み量は発熱・電池・フラッシュ寿命に直結する（ADR-118 の発熱とも重なる）。
- 決定: `@Model` への更新経路は**先に変化を検出し、変化が無ければ 1 フィールドも触らない**。
  併せて (a) 「最終更新時刻」の類は**中身が変わったときだけ**進める（無条件に進めると必ずダーティになる）、
  (b) 変化ゼロなら版番号（`itemsRevision` 等）も据え置く（進めると読み手が全件を引き直す・ADR-95）、
  (c) 回帰テストは**書き込み件数**を数える（「同じ入力を 2 回適用して `updated == 0`」）。
  結果の件数や所要時間ではなく回数で見るのは ADR-119 と同じ理由。
- 結果: 変化検出の比較コスト（数フィールド）を毎行払う代わりに、無変化の同期がディスクに一切触らなくなる。
  比較漏れがあると更新を取りこぼす（表示が古いまま残る）ので、**変化する可能性のあるフィールドを
  網羅**すること、そして「変わったら更新される」側のテストも同時に置くことを条件とする。
- 関連: 事例「変わっていない 11 万行を毎回書き直していた」、
  `DropboxCacheStore.applyDelta`、`DropboxCoreTests/DeltaWriteVolumeTests`、ADR-95 / ADR-118 / ADR-119。

## ADR-119 規模で壊れるコードは「回数」の規模テストで止める
- 状態: 採用
- 文脈: 1 日で 8 件の性能バグを踏んだ（フル画面 18 秒・顔レビュー 27.8 秒のハング、
  密表示でメモリ 1GB 超のクラッシュ）。**すべて同じ形**だった——
  「1 回ぶんに見える呼び出し」が、実はライブラリ規模に比例していた。

  | 呼び出し | 見た目 | 実際 | 効いた規模 |
  |---|---|---|---|
  | `items.first { $0.id == x }` | 検索 | 全走査 × 毎回の String 生成 | 86,262 件 |
  | `gridIdentitySignature(...)` | 指紋 | 全件ハッシュ（ズームのたび） | 86,262 件 |
  | `requestImage(targetSize:)` | 1 枚取得 | 640×640＝1.6MB × 無制限並列 | 1,072 件/10 秒 |
  | クラスタごとの `fetch` | 取得 | クラスタ数ぶんの往復 | 1,316〜2,632 回 |
  | `isMarkedNotSame(a, b)` | 述語 | 対 × 記録数の 512 次元内積 | 52 万対 × R |

  共通するのは 3 点。(1) 書いた時点では**正しく速い**。(2) ライブラリが育って初めて出る。
  (3) 「動くこと」のテストでは永久に捕まらない。つまり**機能テストを増やしても防げない**。
- 決定: **規模退行テスト**を導入する。全件を触る処理には、規模を変えて
  **発行回数が比例して増えないこと**を assert するテストを 1 本置く。
  - **時間ではなく回数**を見る（CI で揺れない・原因が一意）。`PerfTrace.takeCounts()` で読む。
  - 数える場所は共通の入口に置く（`FaceStore.countedFetchOptional`）。個別に仕込むと漏れる。
  - 形は「規模 4 倍 → 回数は 2 倍以内」。厳密な O 記法ではなく、**比例していないこと**を見る。
- 結果: 導入した初日に、**自分が直したつもりで取り切れていなかった経路を 2 つ検出した**
  （`auditSplitItems` の足切り前 fetch、A2 境界走査の無制限クラスタ走査）。
  代わりに Suite の実行が 36 秒かかる（規模を作るコスト）。小さくすると意味が消えるので許容する。
- 落とし穴（実際に踏んだ）:
  - **fixture が意図した規模になっていない**。最初の版は埋め込みが似すぎて全員が 1 クラスタに
    合流し、N を増やしてもクラスタが増えず素通ししていた。`clusterCount` を assert する。
  - **打ち切り上限を跨がない規模で測ると意味がない**。上限 60 に対し 12/48 人で測っており、
    上限が効いているのか元から少ないのか区別できなかった。
- 関連: `ScaleRegressionTests` / `PerfTrace.takeCounts` / `FaceStore.countedFetchOptional` /
  事例「フル画面が 18 秒固まる」「密表示でメモリが 1GB を超えて落ちる」「別人記録の照合が対ごとに全走査」。

## ADR-118 発熱したら夜間処理を止める（遅くするだけでは充電が止まる）
- 状態: 採用
- 文脈: 「朝、起きるとスマホが熱くて、iOS が『冷めてから充電します』になっていて充電が
  終わっていない」という実フィードバック。**充電されないと翌晩も処理が進まない**ので、
  発熱だけして前に進まない悪循環になる。
  既存の対応は `BackgroundTrickle.thermalPauseMultiplier()`（`fair` で 1.5 倍、`serious` で 3 倍、
  `critical` で 8 倍に休止を延ばす）だけだった。これは**遅くしながら発熱し続ける**形で、
  熱が上がり続ける状況では充電の阻害を解消できない。**止める**判断が別に要る。
- 決定: 設定「熱くなったら処理を止める」を追加し、**既定 ON**にする。
  - 止める境界は `serious`（OS が「性能に影響が出ている」と言う段階。充電の抑制もこの辺から
    始まる）。`critical` まで待つと手遅れ。
  - 再開は `nominal` まで冷えてから＝**ヒステリシス**。同じ境界で止めて再開すると、境界付近で
    「止まる→少し冷える→再開→また上がる」を繰り返し、細かく動き続けて冷えない。
  - **設定より優先する**。デバッグ全開（`debugForceHeavyWork`）・手動ブーストでも止める。
    明示操作でも免除しないのは、目的が「速さ」ではなく**充電が終わること**だから。
  - 判定は純ロジック（`ThermalPolicy`）＋状態保持（`ThermalGate`）に分け、
    共通ゲート（`BackgroundYield.heavyWorkAllowed` / `heavyShouldPause`）の先頭に置く。
    ゲートを 1 箇所に集約してあるので、埋め込み・顔・タグ・生成すべてに一度に効く。
- 結果: 夜間に温まっても自動で降りるので、朝までに冷えて充電が完了する。代わりに
  **暑い環境では夜間処理が進まない**（設定で切れるが、既定では切らない——切ると「速いが
  充電されない」になり、トータルでは進まないため）。既存の休止延長（`fair` で 1.5 倍）は
  そのまま残す（止めるほどではない段階での予防）。
- 関連: `MosaicSupport/ThermalPolicy.swift`（新規）/ `BackgroundYield` /
  `BackgroundSettingsView` / `records/background-behavior.md`（5 軸目として追記）。

### 追補（2026-08-29・実フィードバック）: 満充電なら止めない／名前を実態に合わせる
- 文脈: 「バッテリー 100% なら止める必要がない」という指摘。そのとおりで、この停止が守っているのは
  **充電**であって温度ではない（上の文脈のとおり）。満充電なら守る対象が無い。
- 決定: 充電が **98% 以上**なら熱でも止めない（`ThermalPolicy.chargedEnoughLevel`）。
  100% ちょうどにしないのは、満充電付近では表示が 99〜100% を行き来し、iOS 自身も充電を絞るため
  ——「100%」条件ではほとんどの晩で成立しない。⚠️ 残量**不明**は満充電と読まない（従来どおり止める）。
- 決定: 設定の表示を「熱くなったら停止」→「**充電をバックグラウンド処理より優先**」へ。
  名前が実態と食い違うと「満充電でも止まるのはなぜ？」という当然の疑問に答えられない。
- 結果: 充電が終わった後の夜間はそのまま処理が進む（朝までの時間を使い切れる）。
  診断ログの遷移行に残量を併記（`thermal: pausing … battery=87%`）＝実機で条件を確かめられる。
- 関連: `ThermalPolicy` / `PowerStateMonitor.batteryLevelIfKnown` / `BackgroundSettingsView` /
  テスト `ThermalPolicyChargedTests`。


## ADR-117 ハングの犯人は「止まっている最中」に自分で採る（MetricKit だけに頼らない）
- 状態: 採用（[[ADR-106]] を補完。置き換えではない）
- 文脈: 実機 diagnostics-56 で、前面のメインが **78 秒間ほぼ完全に停止**した
  （ウォッチドッグの ping は 200ms 間隔＝正常時 10 秒あたり 46〜49 回。停止中は **2〜4 回**）。
  ところが**その 78 秒間、hang 行以外のログが 1 行も無い**——ログを書くのもメインの仕事だからで、
  「直前のログ行を疑え」方式は、背景処理が何本も並走している状況では因果を切り分けられない。
  ADR-106 で入れた MetricKit（`MXHangDiagnostic`・OS 採取のスタック）は正しい手段だが、
  **配信が 1 日 1 回まで**で、前面ハング 9 件を含む 5.5 時間のログに `HANG-DIAG:` は 1 行も無かった。
  原因究明が OS の配信タイミング待ちになり、その間の再発ログもまた読めない。
- 決定: ウォッチドッグが**ハングの継続中に**メインスレッドのスタックを自分で採る
  （`MainThreadStack`・`PERF hang.stack` 行）。`thread_suspend` → `thread_get_state`（arm64）→
  フレームポインタ鎖を辿る → `thread_resume` → `dladdr` ＋ Swift デマングル。
  - **suspend 中は一切アロケートしない**。メインが malloc のロックを持ったまま止まっている
    可能性があり、その間に確保するとデッドロックする。読み書きは起動時に確保した固定バッファのみ。
  - メモリ読み出しは `vm_read_overwrite`（失敗を返す）で行う。壊れた fp を直接 deref しない。
  - 実機 arm64 のみ（シミュレータ・macOS は空を返す）。`PerfTrace` 連動＝Developer Options の
    トグルが OFF ならウォッチドッグ自体が止まっており、採取も起きない。
  - しきい値 2 秒・**15 秒ごとに採り直す**。長い停止で 1 枚しか採らないと「同じ場所で止まり
    続けている」のか「別の重い処理が数珠つなぎ」なのかが区別できない（diagnostics-56 の 78 秒は
    約 6 秒のハングが 5 回続いた形だった）。判定は純関数 `shouldCaptureStack` に出してテストする。
- 結果: 次の実機ログで犯人が名指しできる。代わりに、ハング中にメインを数十マイクロ秒 suspend する
  リスクを負う（上記の無アロケーション設計と PerfTrace ゲートで抑える）。MetricKit は
  **翌日届く裏取り**として併用を続ける（採取タイミングが違うので互いの穴を埋める）。
- 関連: `MosaicSupport/MainThreadStack.swift`（新規）/ `MainThreadWatchdog.swift` /
  `Diagnostics.install()` / 事例「前面 78 秒の停止（diagnostics-56）」。[[ADR-97]]（偽ハングの除去）。

## ADR-116 時間差で効く記録には「持ち主」を持たせる（アカウント／保存先／世代）
- 状態: 採用
- 文脈: 再レビューで、性質の違う 4 か所が**同じ形**の欠陥を持っていた。
  共有の削除墓標（パスだけを鍵にしていた＝アカウント切替後に別アカウントの同名フォルダを消す）、
  保留メタデータの再送キュー（ファイル 1 本＝保存先を切り替えると前の保存先向けの人物名を書く）、
  夜間処理の実行ハンドル（世代なし＝終わった旧実行が次の実行のハンドルを消す）、
  BGTask の完了ラッチ（既に世代化済み・ADR 済み）。いずれも「記録を残しておいて後で効かせる」
  仕組みで、**効くころには文脈（アカウント・保存先・実行）が変わっている**。
- 決定: 時間差で効く永続記録・共有スロットには、必ず**持ち主の識別子**を含める。
  - アカウントは**指紋**（`BackupDeviceIdentity.fingerprint` の FNV-1a hex）で持つ。
    生の accountId は等値比較にしか使わないので保存・記録しない（資格情報を持ち回らない）。
  - 保存先（バックアップルート）を含めて名前空間を作る（`PendingMetadataStore(account:folder:)`）。
  - 実行の使い回しスロットは世代トークンで守る（`CompletionLatch` / `GenerationHandle`）。
  - アプリ側は Composition Root（`BackupEngine` / `AutoAlbumAdapters`）で seam に注入する。
    ロジック層は指紋の出どころを知らない（`accountFingerprint: @MainActor () -> String?`）。
- 結果: 切り替え直後の取り違え（他人のフォルダを消す・他の保存先へ書く・次の実行を止められない）が
  構造的に消える。代わりに、切り替えをまたぐと**前の持ち主の記録は掃除されずに残る**
  （墓標は猶予時間で失効するので実害は無いが、保留キューはファイルが残り得る）。
- 関連: `ShareSettingsKeys.tombstoneKey` / `PendingMetadataStore` / `MosaicSupport.GenerationHandle` /
  事例「再レビューで出た 11 件」。

## ADR-115 クラウド共有は「状態を持つ偽 Dropbox」でシナリオ検証する
- 状態: 採用
- 文脈: クラウド共有で実機障害が 3 度続いた（重複約 1,300 件の生成・掃除の空回りループ・
  再共有でのフォルダ二重化）。いずれも**単発の応答は正しく、2 回目以降の反映で悪化する**型で、
  当時のテスト（応答を順に返すだけのステートレスなスタブ）では**構造的に検出できなかった**。
  純ロジック（`SharePlanning`）のテストは充実していたが、それは「1 回の計画が正しいか」しか
  見ておらず、「計画→実行→再計画」の輪が閉じるかは誰も検証していなかった。
- 決定: **状態を持つ偽 Dropbox サーバー**（`FakeDropboxServer`・テストターゲット内の actor）を
  用意し、`ShareSyncEngine` をエンドツーエンドで回すシナリオテスト層を新設する。
  - サーバーはファイル表を保持し、`copy_batch` / `delete_batch` / `list_folder` /
    `create_folder` / `upload` / `download` / `get_metadata` を**実際に反映**する。
    衝突時の挙動（autorename の有無）・フォルダ削除の再帰・非同期ジョブも本番に合わせる。
  - **障害注入**を第一級の機能にする: `jobsTimeOutButComplete`（**サーバーでは完了するが
    クライアントにはタイムアウトを返す**＝実障害の引き金そのもの）、`rateLimitEveryNthRequest`
    （429）、`failCopyPaths`（部分失敗）。
  - 検証の主眼は**収束**（反映を 2 回・3 回走らせて増減しないか）と**データ安全性**
    （消してはいけないものが残るか）。
  - ポーリング設定をテストから注入可能にする（本番 0.5s×480＝4 分のままでは 1 テストに
    数十秒かかる。注入で 244 秒 → 2 秒）。
- 結果: 過去 3 回の実障害がすべて**テストで再現・固定**された。今後の変更で同じ穴が開けば
  CI で落ちる。テストは macOS の `swift test` で完結し、ネットワークも Dropbox アカウントも
  不要（12 シナリオ・2 秒）。
- トレードオフ: 偽サーバーは本番 API の**近似**であり、Dropbox 側の仕様変更や未実装の
  エラーコードは捕まえられない。実機ログ（diagnostics）による検証は引き続き必要で、
  両者は代替ではなく補完である。
- 関連: `Packages/BackupKit/Tests/BackupKitTests/FakeDropboxServer.swift` /
  `ShareScenarioTests.swift`。ADR-112（クラウド共有）。

## ADR-114 ドキュメント用スクリーンショットは「専用シミュレータ＋フリー素材」で自動生成する
- 状態: 採用
- 文脈: ヘルプ・設計資料・ストア用のスクリーンショットが必要。しかし運用ルールで
  **シミュレータのアプリ画面スクショは禁止**（個人写真が写り得るため）。はめ込み合成は
  実物と乖離して陳腐化するし、UI 変更のたびに作り直しになる。
- 決定: **使い捨ての専用シミュレータ「MosaicShots」を作り、フリーライセンス素材だけを
  入れて本物の画面を撮る**。個人写真が 1 枚も入らないので禁止事項に抵触しない。
  1. 素材: Lorem Picsum（Unsplash License・商用可・出典表記不要）をシード指定で決定的に取得
     （`scripts/fetch_screenshot_photos.sh`）。
  2. **EXIF 注入**（`scripts/inject_screenshot_exif.swift`・ImageIO のみ・外部依存なし）:
     ファイル名の都市キーから GPS と撮影日を決定的に割り当てる。これで**旅行アルバム・
     場所アルバム・日付グリッドが実データで埋まる**（実際に「那覇市 & 豊見城」「京都市 12 枚」
     等が生成されることを確認）。
  3. 環境構築（`scripts/make_screenshot_sim.sh`）: `simctl create`（既存機に触れない）→
     `addmedia` → install → launch。
  4. 撮影（`MosaicPhotosUITests/ScreenshotCaptureTests`）＋取り出し
     （`scripts/export_screenshots.py`・xcresult の添付を名前つき PNG へ）。
- 結果: UI 変更のたびに 1 コマンドで撮り直せる。日英・端末サイズの横展開も同じ仕組みで可能。
  制約: **ピープル（顔）だけは撮れない**（素材に顔が無い・実在人物の顔を公開資料に載せる
  判断は別途必要）。AI アルバム・端末アルバム・Dropbox 連携は操作/接続が要るため未取得。
- 落とし穴（実際に踏んだもの・再発防止）:
  - `simctl privacy grant photos` は iOS 26 ランタイムで**効かないどころか「決定済み(拒否)」
    を作りダイアログすら出なくなる**。権限は UITest でダイアログをタップして許可する。
  - `xcodebuild test` は既定で**デバイスをクローンして並列実行**する。クローンには
    `addmedia` した写真も権限も無く「Photo library access denied」になる。
    `-parallel-testing-enabled NO` が必須。
  - `PhotoCollectionView` のセルは accessibility 要素として `cells` に現れず、クエリは
    ホームのリスト行を拾う。撮影用途では**相対座標タップ**の方が確実。
- 関連: `scripts/{fetch_screenshot_photos.sh,inject_screenshot_exif.swift,make_screenshot_sim.sh,export_screenshots.py}`・
  `MosaicPhotosUITests/ScreenshotCaptureTests.swift`。素材と出力は `.screenshot_assets/`（.gitignore 対象）。

- 追記（相対日付アルバムの経時更新・コードレビュー指摘）: 「直近 30 日」のような条件は
  **写真が増えなくても範囲が動く**。ドリフト検知は埋め込み枚数の差しか見ておらず、増分評価は
  既存メンバーを維持するため、期間外になった写真が残り続けていた。→ `SavedInterpretation` に
  **フル評価の時刻**（`lastEvaluatedAt`・増分では更新しない）を持ち、`RelativeDateStaleness`
  （純ロジック）で「時間依存の日付条件があり、かつ日付境界を越えた」ならフル再評価する。
  `year` も年をまたぐと意味が変わるので時間依存に含める。
- 追記（削除・編集との競合・コードレビュー指摘）: 本番化とフル再評価は、開始時にアルバムを
  取得した後、解釈・検索・LLM 審査で**長く await する**。その間に削除されると最後の upsert が
  アルバムを**復活**させ、編集されると古い条件の結果が新しい内容を**上書き**していた。
  → アルバムごとの**世代**を持ち（作成/再設定/削除で進む）、保存の直前に
  「まだ存在し、条件と世代が開始時のまま」を確認する（`canCommit`）。
- 追記（増分評価の延期分・コードレビュー指摘）: クエリ埋め込みが取れず採点を見送った写真は、
  評価済みにもならず待機列にも残らないため、追加枚数がドリフト閾値（500）を超えるまで
  アルバムへ入らないままだった。→ `refreshIncremental` が**延期した refKey を返し**、
  エンジンが待機列へ戻す。再処理での二重加算を避けるため、評価済み件数は現実の
  埋め込み総数を上限に頭打ちする（既存値は下回らせない）。

## ADR-113 ピープルグループ（複数人物の名前付き束）
- 状態: 採用
- 文脈: クラウド共有（ADR-112）の共有単位を毎回選ぶのは面倒。家族・チーム・組織のような
  「人のまとまり」を一度作れば、閲覧（合成アルバム）にも共有（共有セット作成）にも使い回せる。
  ADR-61 の「人物束ね」（同一人物の複数クラスタを 1 人に束ねる）とは別概念であることに注意。
- 決定:
  1. `PeopleGroupRecord`（FacesV1 コンテナに additive 追加）: 名前＋メンバー clusterID のみ。
     **クラスタリングには一切影響しない**参照レイヤー（表示・共有のためのビュー）。
  2. メンバーは clusterID で参照し、表示時に現在の人物一覧へ**解決**する。再クラスタ等で
     一覧から消えた ID は表示から外すが記録には残す（復活したら再び現れる）。
  3. 表示はピープルカルーセルの**先頭**に、単人カードと見分けが付く見た目
     （メンバー顔 2×2 コラージュ＋person.2 バッジ＋アクセント枠）で並べる。
     タップで全メンバーの合成アルバム、長押しで編集/クラウド共有/削除。
  4. 写真キーの解決は必ず `memberRefKeys(forGroup:)` を使う。ホーム一覧の
     `PersonInfo.memberRefKeys` は遅延取得で**空**（ADR-95）のため、これに依存すると
     空アルバム・空共有になる（実装時に人物のクラウド共有で実際に踏みかけた罠。
     人物・グループとも「開く/共有する直前にストアから解決」に統一した）。
- 結果: グループ→「クラウド共有…」で家族全員分の共有セットが 1 操作で作れる。
  グループは静的スナップショットではなく**人物への参照**なので、メンバーの写真が増えれば
  アルバムも追従する（共有セットは作成時点のスナップショットのまま＝ADR-112 の決定を維持）。
- 関連: `FaceCore/Faces/PeopleGroups.swift`・`MosaicPhotos/Home/PeopleGroupViews.swift`・
  `PeopleGroupsTests`。ADR-61（人物束ね）/ ADR-95（遅延取得）/ ADR-112（クラウド共有）。

## ADR-112 家族共有＝「バックアップの射影」＋解析データ同梱
- 状態: 採用
- 文脈: 全量バックアップは維持したまま、選んだ写真（子供・旅行）だけを家族に共有したい。
  家族も本アプリを使う前提で、送信側で解析済みのベクトル・タグ・顔も引き継ぎ、受信側の
  再解析（サムネ DL＋推論）を省きたい。検討した代替: (a) 配置分割（共有分を共有フォルダへ
  移動して 1 実体）＝容量二重なしだがバックアップ完全性が家族の操作に依存する、
  (b) 共有リンク＝閲覧体験が悪い、(c) 外部無制限サービス（Amazon Photos 等）＝公開 API が
  無く自動連携不可。ユーザーは容量の二重計上を許容し、コピー方式を選択した。
- 決定:
  1. **共有＝サーバーサイドコピーによる射影**。共有セット（=`/MosaicShare/<セット名>/`）へ
     `files/copy_batch_v2` でコピーし、正本（バックアップ・原本）には一切触れない。
     セット削除・単枚解除はユーザー操作起点のみ（自動削除しない）。共有フォルダの家族への
     共有操作自体は Dropbox 側でユーザーが行う（sharing API 権限を持たない）。
  2. **解析は解析データで同梱**。`<セット>/.mosaic-share/analysis-v1.json` に、共有した写真
     **だけ**の解析（Vision タグ・OCR・人数・美的・CLIP 埋め込み・顔シグナル）を載せる。
     キーは **content_hash**（送信者と受信者で refKey が異なるため、パスにも refKey にも
     依存しない結合キー）。人物名・地名・バックアップ台帳は載せない。
  3. **受信側は版一致セクションのみ取り込み**（tag/perception/face の各版をヘッダに記載）。
     取り込みは既存レコードをスキップ（自前解析が常に優先）し、夜間タガーは取り込み済みを
     「処理済み」に採用してスキップ（`adoptImportedEmbeddings`）。顔は `recordScans` の
     逐次クラスタリングで受信側の人物へ自然に合流する（共有された子供の写真が受信側の
     命名済み人物アルバムに入る）。解析データは外部入力として防御的検証
     （サイズ/件数/次元/有限性の上限・不正エントリは黙って破棄）を通す。
  4. **ローカル写真はバックアップ経由・クラウド写真は原本から直接コピー**。未バックアップは
     waitingBackup で保留し、バックアップ完走後に自動反映（バックアップ隊列に割り込まない）。
  5. **自己修復**: 反映のたびにセットフォルダを実在照合し、外部削除は再コピー、元の
     content_hash 変化（編集）は再コピー。一覧が取れないときはセットをスキップ
     （実在不明のまま再コピーすると autorename が重複を作るため）。
  6. **送信側の二重表示対策**: 自分の共有ルートを items 反映から除外
     （`DropboxPhotoStore.setExcludedPathPrefixes`・同期/キャッシュは対象のまま）。
     受信側は家族フォルダを同期ルートへ追加して閲覧する（既存の複数ルート機構）。
- 結果: 追加通信は RPC のみ（コピーは Dropbox サーバー内・写真の再アップロードなし）。
  共有分の容量は二重計上（許容済み）。1 枚が複数セットに入るとコピーも複数（許容済み）。
  セット名変更は非対応（作り直しで代替・フェーズ 2 で move 対応）。アルバム共有は
  スナップショット方式（自動追従なし・フェーズ 2 でレビュー承認式を検討）。
- 関連: `BackupKit/Share/*`（ShareSyncEngine / SharePlanning / ShareAnalysisData /
  DropboxShareCopier / ShareAnalysisFetch / ShareHubView）、`AutoAlbumEngine+Share` /
  `PeopleEngine+Share` / `PhotoTagger`（採用スキップ）、アプリ `ShareSupport.swift`。
  テスト: `SharePureLogicTests` / `ShareCopierTests`（macOS）。
- 追記（複数ユーザー共有・実フィードバック）: 共有レイアウトに**端末フォルダを挟む**
  （`<root>/<端末名-短ID>/<セット名>/`）。家族が同じ共有フォルダを使い、たまたま同じ
  セット名（例「◯◯家」）を付けると**互いのファイルを上書き**してしまうため。
  短 ID はバックアップと同じ `BackupDeviceIdentity`（Keychain 永続）を再利用する（ADR-41）。
  受信側の発見（`SharedAlbumDiscovery`）は**階層数で決め打ちせず「写真が直接入っている
  フォルダ＝セット」**とし、ルートとの間に階層があればその要素を**提供者名**として表示する
  （同名セットが並んでも誰の共有か分かる）。旧来の `<root>/<セット名>/` 構成も引き続き動く。
- 追記（同名・異種の共有セット・実フィードバック）: AI アルバムとピープルグループに
  **同じ名前**を付けることは普通にある。「作成元キーが無ければ名前一致で既存セットを再利用」
  という規則をそのまま適用すると、片方を共有したつもりで**もう片方の共有を書き換える**。
  → 再利用は「作成元キーの完全一致」または「**種類（album/person/group）が同じ**かつ同名」に
  限定する。バッジ判定の名前フォールバックも同様に種類で絞る。
- 追記（フォルダ名の種類接頭辞＋同名禁止・実フィードバック「2 とか 3 が付くのはいけてない」）:
  上の「種類が同じかつ同名」規則だけだと、AI アルバムとピープルグループに同名を付けたとき
  Dropbox 上のフォルダが衝突し、サニタイズの連番（`沖縄 2`）で逃げることになる。連番は
  クラウドを見た人に意味が伝わらず、どちらの共有かも分からない。→ 二段で解く。
  (1) **種類の中では同名を禁止**する（`PeopleEngine.peopleGroupNameExists` /
  `AutoAlbumEngine.aiAlbumTitleExists`。作成・改名の両方で拒否し、UI は理由を出して保存を無効化）。
  (2) **フォルダ名に種類の接頭辞**を付ける（`ShareNaming.folderName(_:kind:)` →
  `Album-` / `Person-` / `People-`）。結果、最終パスは
  `<共有ルート>/<端末名-短ID>/People-<グループ名>/…` になり、種類違いの同名でも衝突せず
  連番は発生しない。連番ロジック自体は保険として残す（手動作成・旧セットは接頭辞なし）。
- 追記2（既存セットの移行・実フィードバック「接頭辞が付かない」）: 接頭辞は**作成時にしか
  付かない**ので、この変更より前に作った共有セットは素の名前のまま残る（＝同じ対象を
  共有し直しても再利用経路に入り、フォルダ名は変わらない）。作り直しを促すと写真を
  コピーし直すことになるため、**反映のたびにフォルダ名を検査し、必要なら
  `files/move_v2` で改名する**（サーバーサイド move ＝実体の転送なし・配下ごと移動）。
  記録側は `folderName` と各アイテムの `sharedPath` 接頭辞を張り替え、解析データの
  チェックサムは捨てて作り直す。**改名に失敗した回は記録を進めない**——記録だけ先に
  進めるとクラウド上の実体を見失い、全部コピーし直す事故になる（次回の反映で再試行）。
  移動元が無い（まだ 1 度も反映していない）場合は記録だけ直す。
- 追記3（旧レイアウトが直らない・実フィードバック「Dropbox 上のパスが直っていない」）:
  上の移行は接頭辞しか見ていなかったため、**端末フォルダが入る前のセット（共有ルート直下）**
  が残り続けた。しかも計画は `item.sharedPath`（記録済みのコピー先）をそのまま再利用するので、
  **フォルダを作る先だけ新レイアウトになり、写真は旧パスに書かれ続ける**（＝いつまでも直らない）。
  → 移行は「接頭辞」と「端末フォルダ」の両方を見て、旧レイアウトの候補パス
  （端末フォルダ有り/無し × 旧名/新名）を順に move する。1 つでも動かせたら記録の
  `folderName` と `sharedPath` を張り替える。作成元不明（`sourceKey` なし）のセットは
  種類が決められないので**名前は据え置き、置き場所だけ**直す。
- 追記4（共有の停止・実フィードバック「停止する機能がない」）: 共有を止める操作は
  設定 → クラウド共有 → セット詳細の「セットを削除」だけで、**共有した場所からは戻せなかった**。
  → 共有元（人物・ピープルグループ・アルバム）のメニューを共有状態で出し分け、
  共有中は「クラウド共有を停止…」を出す（`ShareSyncEngine.sharedSetID(sourceKey:name:)` で判定。
  照合規則は `createSet` の再利用と同じ＝作成元キー一致 → 同じ種類かつ同名）。停止＝共有フォルダ
  ごと削除で、**アルバム/人物・端末写真・バックアップには触れない**ことを確認文で明示する
  （相手が既に保存したコピーは取り消せない点も併記）。
- 追記5（移行の探索は一度きり）: 旧配置の候補パスを毎回の反映で探すと、**存在しないパスへの
  move が 1 セットにつき毎回 1 往復**発生する（規約: 無いものを繰り返し探さない）。
  `ShareSet.layoutVersion`（現在は 1）を印として持ち、検査が済んだセットは以後探索しない。
  移行が**失敗した回は印を付けない**ので、次回に再試行される。
- 追記6（削除の確実性・コードレビュー指摘 3 件）: 共有側の削除まわりに、いずれも
  「**記録は消えたのにクラウドには残る**」へ収束する穴があった。
  1. `delete_batch` の成否を**バッチ全体でしか見ていなかった**。ジョブが完了しても
     エントリ単位で失敗し得る（権限等）ため、消せていないのに成功を返していた。
     → エントリを 1 件ずつ検査し、`not_found`（＝元から無い＝目的達成）だけを成功に数える。
  2. 変更操作は反映を 3 秒待って**無条件に先へ進んでいた**。コピーのポーリングは最長 4 分あり、
     `sync(set:)` は途中で `isMutating` を見直さないため、削除後にコピーが完走し得た。
     → 反映を**キャンセルできる Task** で回し、止まってから進む。止まらなければ変更操作を
     **中止**する（食い違いを作るくらいならやらない・`SyncError.syncBusy`）。
     ⚠️ ガードで畳まれた `syncNow` では `syncTask` を張り替えない（張り替えると
     キャンセルが「既に終わった Task」に飛んで止まらない・自テストで検出）。
  3. リモート削除の失敗を無視して**記録だけ削除**していた（`updateSetMembers` / `removeItems`）。
     → 消せたときだけ記録を消す。未コピー分（`sharedPath` なし）は失敗時も外してよい。
  さらに、(2) はクライアント側のキャンセルだけでは塞ぎきれない——**Dropbox にジョブ取り消しの
  API は無く、サーバー側のコピーは走り続ける**。削除直後に完走するとフォルダが復活し、記録は
  既に無いので誰も掃除できない孤児になる。→ 削除したフォルダの**墓標**（パスと削除時刻）を
  15 分だけ持ち、以後の反映で「在るなら消し直す」。期限切れは捨てる（無いものを探し続けない）。
- 追記（機能の独立性・実フィードバック「バックアップしなくても受信は動くべき」）:
  クラウド共有は **「受ける」「提供する」「バックアップ」の 3 機能が独立**で、それぞれ
  別トグルで設定できる。依存は一方向のみ:「提供する」のうち**端末写真**の共有だけが
  バックアップ実体を前提とする（クラウド写真の提供・受信はバックアップ不要）。
  受信トグル OFF で家族フォルダの同期・取り込み・ホームの受信セクションが止まり、
  提供トグル OFF で共有メニュー・セット反映が止まる（互いに影響しない）。
- 追記（UI 導線・実フィードバック「どこに何があるか分かりにくい」）: (1) 名称は「共有」でなく
  **「クラウド共有」**（OS の共有シートと紛れるため）。(2) **受け取った共有アルバムはホームの
  独立セクション**（受信がある場合のみ表示・他のアルバムと同じカルーセル）に出す。発見は
  `SharedAlbumDiscovery`（純・家族フォルダ配下を第一階層フォルダ単位にグルーピング・
  ルート直下写真はフォルダ自身をアルバム扱い）。(3) 入り口は **Dropbox ハブと
  設定「Albums & Search」の両方**に置く（アルバム機能の一つとして辿れるように）。

## ADR-110 レキシコン外の語を「作成直後」に解決する（即時本番化＋活動語彙の一括拡充）
- 状態: 採用
- 文脈: 実フィードバック「バレエの<人名>がバレエと関係ない写真のまま直らない」。
  調査（diagnostics-49）で、認識能力の問題ではないと確認した——Vision 1,303 クラスには
  `ballet` / `ballet_dancer` が実在し、CLIP もバレエ認識は得意。真因は**タイミング**:
  「バレエ」が決定的レキシコンに無いため、作成/編集直後のプレビュー解釈では内容条件ゼロ
  （`no effective content → base`）＝人物の全写真になり、FM 解釈（バレエ→ballet）は
  夜間の本番化まで走らない。ユーザーには「ずっと直らない」ように見える。
- 決定（2 本立て）:
  1. **即時本番化（finalizeNow）**: 作成/更新の第 2 段として、決定的プレビュー表示の直後に
     FM 解釈＋フル評価＋証拠ゲート＋LLM 審査をその場で実行する（utility 優先度・スピナー継続・
     ユーザーの明示操作なので夜間ゲート＝ADR-107 の対象外）。どんな言葉でも作成から十数秒で
     正しい条件になる。**語彙接地だけは重心キャッシュ済みのときのみ**
     （`groundingCachedOnly`・コールド構築は数十分＝夜間の仕事）。全語が語彙に完全一致する
     場合は重心不要で即接地（exact ショートサーキット）。接地を見送ったら
     `pendingFinalization` を維持し、夜間の finalize が接地込みで仕上げる。
  2. **活動・イベント語彙の一括拡充**: レキシコンに約 70 語（バレエ/ダンス/発表会/運動会/
     各種スポーツ/楽器/行事/食べ物/自然）を追加。英語側は **Vision 実在識別子**
     （taxonomy ダンプで実在確認・アンダースコアは空白形＝tagHits の多語一致で当たる）を優先。
     作成した**瞬間**からタグ一致が効く。単漢字（庭/森/城 等）は漢字隣接ガードが守る。
- 結果: レキシコン外の語も「作成→プレビュー（1〜2 秒）→本番化（十数秒）」で解決。
  COCO 116 本＋回帰フロア全通過（変更の副作用なし）。新テスト: 活動語 4 本・
  アンダースコアタグ一致・最長一致占有（パンダ≠パン）・単漢字ガード。
  トレードオフ: 作成直後に FM 推論（数秒〜十数秒・utility）が毎回走る（作成は低頻度の
  明示操作なので許容）。重心コールド時の広い語（「風景」等）は夜間まで接地されない（従来どおり）。
- 関連: `AIAlbumService.finalizeNow` / `AIAlbumInterpreter.groundContentTerms(cachedOnly:)` /
  `ConceptExpander.hasCachedCentroids` / `JapaneseVisualLexicon`。[[ADR-23]]（解釈は作成時 1 回＝
  本来の姿へ回帰）[[ADR-107]]（夜間ゲートとの関係）[[ADR-109]]（実効内容語）。

## ADR-111 「この人物を整理」＝混入グループのサブグループ化と一括分離
- 状態: 採用
- 文脈: 実フィードバック「ピープルアルバムの中に複数の別人が多数含まれるグループがあり、
  1 つ 1 つバラすのは大変。全体の枚数も膨らんで見える」。既存の分割手段は 1 対 1 レビュー
  （事後監査の 2 分割カード・最大 5 件/回）と束ねの**全**解除のみで、多人数混入に追いつかない。
- 決定: 人物の長押しメニューに「この人物を整理」を新設。中身を自動でサブグループ化して
  代表顔＋枚数で一覧し、「別人」のグループへチェック → **1 タップで一括分離**する。
  - サブグループ化: 束ねた人物は構成クラスタ単位（分離＝束ねから外す＝部分解除）。
    各クラスタ内部は事後監査（`FaceClusterAudit`）の 2 分割を**再帰適用**（`recursiveSplit`・
    最大 6 群）して「互いに離れた塊」を切り出す。
  - 分離の学習: クラスタ内の一部の分離は `splitCluster`（負例記録つき＝再クラスタで再発しない）。
    最大グループは「残す側」として選択不可（全分離で空人物を作らない）。
- 結果: 多人数混入を 1 画面・数タップで解消できる。純ロジック＋ストア連携のテスト 4 本。
  トレードオフ: 束ねから外す分離は負例を記録しない（束ねは手動操作の取り消しであり、
  自動再統合はされないため）。監査が「連続した帯」と見る同一人物の成長分布は分割候補に
  出ない（意図どおり・ADR-69）。
- 関連: `FaceCore/PersonCleanup.swift` / `PersonCleanupView.swift` / [[ADR-69]]（事後監査）
  [[ADR-61]]（束ね）[[ADR-46]]（修正からの学習）。

## ADR-109 ハード接地語を内容語から除く（実効内容語＝二重計上の禁止）
- 状態: 採用（解釈器 v7）
- 文脈: ユーザー報告「バレエの<娘の名前>で、バレエ以外の写真も出てくる（OR になっていないか）」。
  調査の結果、spec 自体は単一節（人物 AND 内容）で正しいが、**評価側で AND が OR 化**していた:
  FM は「バレエの太郎」で content にも "太郎" を入れがちで、そのまま字句検索（LexicalSearch は
  人物名フィールドにも当たる）へ流すと、人物名チャネルが RRF **和集合**経由でバレエ証拠ゼロの
  本人写真を全部通してしまう。ハードで絞り済みの条件を内容語として**二重計上**する構造誤り。
  補助的に、英訳文（"Ballet of Taro"）の固有名詞が CLIP 採点を薄める問題も同根。
- 決定: 評価時に**実効内容語**（`QuerySpec.effectiveContentTerms`）を使う＝ハード条件
  （人物・場所）に接地済みの語を include から除く（大小無視・部分名は両方向包含で照合）。
  さらに (a) 内容語が全部ハード接地語だった（除外なし）クエリは**ハード絞り込み結果をそのまま
  返す**（「太郎」だけのアルバムを英訳文の CLIP band で欠けさせない）、(b) 人物/場所接地がある
  クエリの CLIP フレーズは英訳文でなく**内容語**を使う（固有名詞の混入排除）。フル評価
  （searchWithPool）と増分評価（queryVectors / refreshIncremental）に同一規則で適用。
  解釈は永続化済みのため解釈器を v7 に採番し、**版が古いだけでもドリフト再評価が発火**する
  よう `refreshIfDrifted` に stale 判定を追加（既存アルバムのメンバーを夜間に正す）。
- 結果: 「バレエの太郎」＝太郎∩バレエになる（テスト `HardGroundedContentTests` で固定）。
  COCO 116 本・回帰フロアは全通過（F1 0.896 不変）。既存アルバムは次の夜間窓で自動修正。
- 関連: `QuerySpec.effectiveContentTerms` / `AIAlbumSearch.swift` / `QueryEmbedder.phrase` /
  `AIAlbumService`。[[ADR-100]]（証拠主義）[[ADR-102]]（接地）[[ADR-107]]（夜間窓での再評価）。

## ADR-108 VLM キャプション（SmolVLM）を廃止する
- 状態: 採用（ADR-34/35 を置換＝キャプション機能ごと撤去）
- 文脈: 「AI 系機能の重さと検出への貢献」をユーザーと棚卸しした。キャプション生成は最も重い
  AI 処理（SmolVLM-500M・1 枚数十秒・モデル 877MB・fp16 デコーダ）なのに、検索パイプラインでは
  ランキングに一切参加せず、役割は「証拠ゲートの存在 1 票」「FM 審査の文脈」「情報パネル表示」
  のみ。網羅率アブレーション（台帳 S13・`SearchEvalCaptionAblationTests`＝廃止と同時に撤去、
  結果は台帳に永続）で **Δ(実機 1% − 0%) F1 = 0.0001＝検索寄与ゼロ**を計測。100% 網羅でも
  F1 +0.008 で、そこへ至るには夜間窓 30 日分以上が必要＝到達不能。しかも実機では顔スキャンに
  窓を奪われ、diagnostics-38〜46 の全セッションで生成 0 枚（動いてすらいなかった）。
  証拠役は humanCount（86%・タグ付けパスで無償）が既に代替している。
- 決定: 機能ごと廃止。夜間一括生成・審査時オンデマンド・「AI description」表示・キャプション窓
  （TurnTaking の順番回し＝ADR-86/93）・S7 の証拠不足優先（ADR-102 の一部）を撤去。
  `VLMRuntime`/`GPT2Tokenizer`/`CaptionedPhotosView`/`scripts/build_smolvlm.sh` 等を削除し、
  モデル実体（`MosaicPhotos/VLM/` 877MB）も削除＝**アプリが 877MB 軽くなる**。
  互換のため残すもの: `PhotoTagRecord.caption`（消すと TagsV1 スキーマ不整合で全シーンタグを失う）
  / `DropboxBackupMetadata.caption`（アップロード済み metadata JSON のデコード互換・新規は nil）。
- 結果: 夜間窓とメモリ（877MB 常駐候補）が全量、タグ・埋め込み・顔スキャンへ回る。
  失うもの: 情報パネルの説明文表示（生成済み約 1% 分も非表示）。将来もし写真の文章説明が
  必要になれば、その時点の軽量 VLM で「表示専用・オンデマンド」として設計し直す（検索の
  証拠としては使わない＝本 ADR の計測が根拠）。
- 関連: 台帳 `search-quality.md` S13 / [[ADR-34]][[ADR-35]]（旧設計・置換）/ [[ADR-86]][[ADR-93]]
  （キャプション窓・撤去）/ [[ADR-100]]（証拠主義）。コミットは本項と同時。

## ADR-107 一枚岩の重い処理は「非アクティブ限定」の厳格ゲートに分離する
- 状態: 採用
- 文脈: ユーザー報告「AI アルバムの機能は、いちいち画面が固まる。処理がバックエンドで動いていない」。
  diagnostics-46 で機序を特定: `heavyWorkAllowed` は控えめ OFF＋前面 20 秒アイドルで通るため、
  **AI アルバムの本番化（finalize）が 06:10:57 にフォアグラウンドで起動**していた。
  トリクル系（埋め込み・顔・タグ）は 1 単位ごとに `heavyShouldPause()` で譲れるので前面実行でも
  安全だが、finalize/ドリフト再評価/自動生成は**一枚岩**（FM 解釈＋フル検索 28s＋重心構築
  数分〜数十分＋85k 件 SwiftData）で、**始まるとユーザーが戻ってきても途中で譲れない**。
  以後の全操作が ANE ゲート待ち（実測 7.9s）・ModelActor 競合・CPU 競合で引っかかる＝
  「いちいち固まる」の正体。
- 決定: `BackgroundYield.monolithicHeavyWorkAllowed` を新設＝`heavyWorkAllowed` に加えて
  **非アクティブ（画面ロック・アプリ切替）限定**。手動ブースト（「今すぐ処理」）と
  デバッグ全開は明示操作なので従来どおり前面でも動く。`AutoAlbumEngine.refreshIfNeeded` の
  finalize / ドリフト再評価 / 自動生成をこのゲートへ移す。トリクル系と地名補正
  （セル単位で譲れる）は従来の `heavyWorkAllowed` のまま。
- 結果: 前面アイドルで一枚岩が始まらなくなり、「使用中に固まる」の主経路を断つ。
  トレードオフ: 控えめ OFF でも finalize は夜間 BGTask（またはロック中）まで遅れる＝
  AI アルバムの本番化が半日遅れることがある（プレビューは即時のまま）。急ぐ場合は
  「今すぐ処理」で前面実行できる。
- 関連: `MosaicSupport/BackgroundYield.swift` / `AutoAlbumCore/AutoAlbumEngine.swift`
  （refreshIfNeeded）。[[ADR-80]]（4 軸ゲート）[[ADR-106]]（diagnostics-46 の観測）。
  ※ 復帰直後の 9〜12s ハング自体は別件として MetricKit（ADR-106）の採取待ち。

## ADR-106 ハングの真因は OS 採取のスタック（MetricKit）で特定する
- 状態: 採用
- 文脈: 実機で「復帰直後・AI アルバム作成時にメインが 9〜12 秒止まる」が 3 セッション連続で
  再現（diagnostics-40/41/46）。自前ウォッチドッグ＋直前ログ行の突き合わせでは、同時進行の
  イベント（CLIP テキスト塔ロード 5.3s・顔モデルロード 10.9s）と**因果の区別がつかない**。
  ANE ゲート計測（ADR-101）は動いたが該当時刻に待ちは無く、ゲート起因は否定された。
  推測での修正を 2 回行い（prewarm 中断・モデルロードのキャンセルガード＝それ自体は正しい修正）、
  それでも再発している＝観測手段が足りない。
- 決定: MetricKit の `MXHangDiagnostic` を購読し、**OS がハング時に採取したメインスレッドの
  呼び出し木**を診断ログへ落とす（`HangDiagnostics`・`HANG-DIAG:` 行・上位 14 フレーム）。
  診断ペイロードは次回起動時などに届くため、次々回のログで真因が読める。
- 関連: `MosaicSupport/HangDiagnostics.swift` / `Diagnostics.install()`。[[ADR-97]]（偽ハングの除去）。
- 同時修正（diagnostics-46 の実測から）:
  - **重心構築の二重実行**: 夜間の finalize 2 本が同時に重心構築へ入り、51k 件の埋め込み走査が
    並走していた（スパン 2 本・壁時計 約30 分）。in-flight 合流を追加（`CLIPConceptExpander`）。
  - **スパンの suspend 汚染**: 「29 分」の大半は中断跨ぎの壁時計。中断を跨いだスパンは記録しない。
  - この二重構築が夜間窓を食い潰し、**キャプション窓が 4.5 分で expire**（0 枚）していた。
    合流＋キャッシュで次セッションから解消される見込み（次ログで確認）。

## ADR-105 凝集精錬は「視覚的に一貫したカテゴリ」専用（gate つき）＝ S12
- 状態: 採用
- 文脈: S9 の残弱点（food R=0.44・instrument R=0.62＝band の裾切り、bird への minaret 混入）。
  対策候補だった「凝集成長」（一貫している限り足す）を実験したところ、**band 置き換えでは
  現行より悪化**（マクロ 0.79 vs 0.862）——高原の弱いリンクで早停止する。
- 決定: band 選択を土台に**条件付き精錬**を重ねる。グループ自体の凝集 z ≥ 1.0 のときだけ、
  (a) 刈り込み（他メンバーとの限界凝集 z < 0.75 の混入を除く・最類似 1 件は常に残す）、
  (b) 成長（類似度順の続きを限界凝集 z ≥ 1.25 の間だけ足す）。seam は `CoherenceContext`
  （集合 z＋限界 z）へ拡張。
- 結果: Caltech マクロ 0.840→**0.860**（楽器 +0.156・食 +0.148・鳥 +0.066 / animal −0.026）。
  雑音語の門は閉じたまま。
- 教訓: **「意味カテゴリ＝視覚クラスタ」は常には成り立たない**。乗り物（車・飛行機・船）は
  意味的に 1 カテゴリでも視覚的には不均質（coh_z 0.15）で、凝集ベースの操作を掛けると壊れる
  （F1 0.4→0.25）。gate はこの反例から生まれた——凝集規則は適用可否そのものを凝集で判定する。
- 関連: `VocabularyGrounding`（refineGate/pruneBar/growBar） / `CoherenceContext` /
  `CLIPConceptExpander.coherenceContext`。数表: `records/search-quality.md` S12。
  同時実施: 「だけ」対応（人物除外として解釈）・人数条件（`.peopleExactly`＋humanCount 評価）・
  評価の質的改善（robust/specificity カテゴリ・回帰フロア `SearchEvalRegressionTests`）。

## ADR-104 検索品質はクエリ集ハーネスで計測してから判断する（継続運用）
- 状態: 採用（実フィードバック「この計測方法は今後も発展させながら活用していく」）
- 文脈: S0〜S11 の一連の改善で、クエリ集ハーネス（COCO 103 本＋Caltech 10 本・正解付き・
  決定的・macOS 数秒）が**10 件の実欠陥を実装前後に発見**した:
  否定の人物専用化 / 「無い＝いない」誤読 / humanCount 未使用 / 電車→車 / hot dog→dog /
  cougar face→人物語 / レキシコン入口不足 / 印象→象 / 連言先頭の人物否定漏れ /
  英語否定の肯定化。いずれも**体感レビューでは気づけない**か、気づいても原因を特定できない
  類の欠陥で、正解付き計測だけが検出・定量化・修正確認を一度にこなした。
  顔認識には既に同じ仕組みがあり（ADR-55・`face-accuracy.md`）、検索側にも恒久化する。
- 決定: `records/search-quality.md` を検索品質の**判断基盤**とし、台帳冒頭の運用ルールに従う。
  要点: (1) 検索まわりの変更は両ハーネスを回して差分を台帳へ、(2) クエリは正解が機械的に
  決まるものだけカテゴリ付きで追加、未対応の言い回しは knownLimitation 枠（数字は出す＝
  次の実装優先度になる）、(3) P を最優先・R の低下は天井計測で網羅率由来か規則由来かを
  切り分け、(4) 新規則・しきい値は負例つき掃引で採用判断。
- 結果: 現状値 COCO 96 本マクロ P=0.993 / F1=0.886、Caltech（接地込み）F1=0.840。
  発展の余地は台帳の limitation 枠と「残っている課題」節が常に示す。
- 関連: `SearchEvalTests` / `SearchEvalCaltechTests` / `SearchEvalCorpus` / `SearchEvalQueries` /
  `scripts/fetch_search_eval_datasets.sh` / `scripts/gen_centroid_fixture.py`。
  [[ADR-55]]（顔の同型）・[[ADR-100]]〜[[ADR-103]]（本手法が生んだ改善）。

## ADR-103 索引済み属性をクエリ言語へ昇格（笑顔・美的スコア）＝ S10
- 状態: 採用
- 文脈: 実フィードバック「子供・笑ってる写真・綺麗な写真のクエリも測って」。調べると
  笑顔（顔スキャンの `hasSmile`・代表顔選定にのみ使用）と美的スコア（ADR-78・ベストショット
  フィルタにのみ使用）は**索引済みなのに AI アルバムのクエリ言語から届かない**信号だった
  （ADR-100 の教訓「索引しているのに使っていないデータ」の残り 2 件）。
- 決定:
  - `QuerySpec.Condition` に `.smiling` / `.beautiful` を追加。評価は `QuerySignals`
    （笑顔実測・美的台帳＋分布適応しきい値）で行い、**「データが無い」は「満たさない」**
    （証拠主義・ADR-100 と同じ原則。網羅が進むほどアルバムが自然に埋まる）。
  - 「綺麗」のしきい値は ADR-78 のベストショットと**同一定義**（`PhotoQuality.adaptiveThreshold`）
    ＝「綺麗」の意味がフィルタとアルバムで食い違わない。
  - レキシコンに属性語（笑顔/笑って/綺麗/美しい…）と動物具体語（馬/象/キリン…）を追補。
    解釈は決定的（プレビューで即立つ・FM 不要）。
  - 本番結線: `FaceStore.smilingFaceCounts()` → `PeopleEngine` → Engine seam →
    `AIAlbumService.querySignalsIfNeeded`（条件があるアルバムだけ台帳を引く）。増分評価も同一規則。
- 結果（COCO・38 クエリ）: 子供 F1 0.927 / 綺麗 0.921 / 複合（綺麗×猫・綺麗×2024・笑顔×子供）
  すべて P=1.000。**笑顔は P=1.000 / R=0.103**——天井計測（索引 100% で P=R=1.000）により
  「配線は正しく、R は顔スキャン網羅率 11% の構造的限界」と切り分けた。
  数表は `records/search-quality.md`「属性クエリの追加と計測」。
- 未解決: 子供の**除外**は年齢証拠が無く人物除外へ丸められる（R=0.473・保守側）。
  対策候補: 顔パイプラインの時期分裂知識（`personGroupID`）を年齢層シグナルに転用。中期。
- 関連: `QuerySpec` / `QueryEvaluator`（`QuerySignals`） / `JapaneseVisualLexicon` /
  `AIAlbumInterpreter` / `AIAlbumService` / `FaceStore+People.smilingFaceCounts`。
  テスト: `AttributeConditionTests`。[[ADR-78]] [[ADR-100]] [[ADR-102]]。
- 追記（S11・103 本クエリのカテゴリ別評価）: クエリを 113 本へ拡張し、レキシコンを COCO 名詞へ
  一括拡充（約 50 エントリ）。評価がさらに 3 件の欠陥を発見して修正:
  (1) 「印**象**的な写真」の 象→elephant（単漢字は漢字隣接で不一致の一般規則へ。熊本・空港も同時解決）、
  (2) 「**人**と車と自転車が写っていない」の人物否定が連言を跨げず 1,989 枚混入
  （否定文脈判定へ一般化・P 0.477→0.997）、
  (3) "photos without dogs" が犬の肯定に化ける（英語否定の分解を実装・en F1 0.309→0.805）。
  「良い写真」「印象的な写真」等の言い回しは美的スコア条件へ寄せる設計
  （応えられる索引は 1 本＝答えたふりをしない）。最終: COCO 96 本マクロ P=0.993 / F1=0.886。
  カテゴリ別数表と limitation 枠は `records/search-quality.md`。

## ADR-102 検出性能の汎用底上げ（S5〜S8）— 全施策を計測とセットで実施
- 状態: 採用（S5/S6/S7）・不採用（S8=多言語テキスト塔・計測により棄却）
- 文脈: 実フィードバック「汎用的な検出性能向上を。ミニ AI の実装も含めて検討を」。
  弱点を層別に列挙し（`records/search-quality.md` の改善計画）、各施策を計測とセットで実施した。
- 決定と結果:
  - **S5a レキシコンの最長一致**: 「**電車**の写真」が 車→car にも当たり include=["car","train"]
    になっていた（COCO: train P=0.214 の正体。偽陽性の内訳を測ってタグ照合の無実が判明）。
    長い語から一致・範囲占有で **P 1.000 / F1 0.932**。肯定側マクロ F1 0.385→**0.581**。
  - **S5b タグ照合の単語境界化**: 双方向 `contains` を廃止（train⊂training 級の過剰一致）。
    トークン一致＋単純複数形のみ。同義・上位語は接地（重心）が受け持つ＝責務分離。
  - **S5c 内容語の語彙カタログ**: FM プロンプトへ実在タグ上位 60 を渡し「この中から選べ」
    （場所・人物の規律の横展開）。検証は実機の `aialbum.ground:`（FM はテストで動かない）。
  - **S5d 連言否定**: 「犬**と猫**が写っていない」の 犬 に否定が届かず肯定に立っていた。
    連言＋名詞を読み飛ばして否定表現を探す（ADR-100 追記と同時実施）。
  - **S6 高原の凝集度規則**: animal（上位全部正しいのに z=1.8 で接地不能）を解いた。
    当初仮説（ギャップ検定）は**実測で棄却**——高原に崖は無い（0.257, 0.254, 0.254…）。
    MAD-z も 1.51 で逆効果。**分布の形では「一貫した高原」と「雑音の高原」を区別できない**。
    意味情報＝上位候補どうしの重心相互類似（凝集度 z）が完全分離する
    （animal 2.24 / insect 2.44 vs 雑音語 0.17〜1.11・しきい値 2.0）。
    規則: 突出（z≥3・上限6）or 凝集した高原（z≥1.5 かつ凝集度z≥2・上限24・幅 top−1.0sd）。
    → animal F1 0→**0.77**・Caltech マクロ F1 **0.762**・接地 11/11・雑音語は全拒否。
  - **S7 証拠不足キャプションの優先配分**: 証拠ゲートで保留になった写真
    （`evidenceStarvedRefKeys`・上限 400・プロセス内）を夜間キャプションの**先頭**に回す。
    網羅 1% のキャプションを「いま証拠を必要としている写真」へ狙って配る。
    進捗判定（before/after）も優先分を含めて数える（含めないと「進捗なし」と誤読して窓を畳む）。
    検証は実機ログ（`aialbum.evidenceGate:` の deferred 件数の推移）。
  - **S8 多言語テキスト塔（ミニ AI 案）＝不採用**: Imagenette ベンチで
    (A) 現行＋英語 0.99 / (B) 多言語＋出荷画像塔 **0.14**（蒸留先が OpenAI 空間で datacomp と
    別物＝そのまま足すのは不可能） / (C) 両塔載せ替え 0.69 / (D) 載せ替え英語上限 0.96。
    載せ替えは全 86k 枚の再埋め込み＋約 300MB 増を要して 0.69 止まり＝翻訳（FM）＋接地の
    現行構成が優位。`scripts/bench_multilingual_query.py` に恒久化（再判定はいつでも可能）。
- 関連: `JapaneseVisualLexicon` / `AIAlbumSearch.tagHits` / `FoundationModelsQueryUnderstanding` /
  `AIAlbumCatalog.contentTags` / `VocabularyGrounding`（高原規則） / `CLIPConceptExpander.coherenceContext` /
  `AIAlbumVerificationCoordinator.evidenceStarvedRefKeys` / `AutoAlbumEngine+Recognition.runCaptionPhase`。
  計測: `records/search-quality.md`（S5/S6/S8 の数表）。[[ADR-100]] [[ADR-101]]。
- 追記（S9・クエリ集による総合評価）: クエリを 25 本へ拡充して測定→修正→再測定を回した。
  評価が 4 件の欠陥を発見（hot dog⊃dog の複合タグ誤一致・cougar face の人物語誤判定・
  レキシコンの入口不足・展開幅の保守性）。修正後: COCO マクロ F1 **0.927**、
  Caltech（接地込み広い語）F1 **0.840**。数表と残課題は `records/search-quality.md` の
  「クエリ集による総合評価」。展開幅は掃引で peak 2.0σ / plateau 1.5σ を採用。
- 教訓: (1) **犯人を測ってから直す**——train の過剰ヒットはタグ照合を疑ったが、偽陽性の内訳を
  測ったらレキシコンの部分一致だった。(2) **仮説も計測で棄却する**——ギャップ検定は実装前に
  データで死んだ。(3) ミニ AI の採否は**ベンチを書いてから**——0.14 という数字が
  「もっともらしい提案」を 1 時間で棄却した（ADR-32 FastVLM/Florence と同じ型）。

## ADR-101 クエリ語は「索引に実在する語彙」へ接地する（個別の対応表を書かない）
- 状態: 採用（機構・重心版の近さ計算とも。当初の text↔text 実装は計測により差し替え）
- 文脈: [[ADR-100]] で否定は汎用化したが、肯定側の語は接地されていなかった。
  ユーザーの問い「風景の検索条件はどうなっている？ 街中の写真は入らないはずだが」を
  実測して確かめたところ、`風景` は `landscape / scenery / outdoor` に展開され、タグ照合は
  **`outdoor` にだけ当たり、`mountain` にも `beach` にも当たらない**ことが分かった。
  つまり「広すぎるカテゴリ語だけ拾い、狭くて確実な語は取りこぼす」最悪の当たり方。
  街中写真は `outdoor` 経由で入る。「街の風景」と「風景」が同一クエリになる穴も見つかった。
  個別に「風景→山,海,…」と書けば直るが、語彙の数だけ破綻する（ユーザー指摘）。
- 決定（機構・採用）: **語彙の側から決める**。索引の台帳に実在するタグ語彙
  （`TagStore.tagVocabulary`・出現頻度上位・このライブラリに実在する語のみ）に対し、
  クエリ語との意味的な近さを測って上位を採る。規則は純ロジック `VocabularyGrounding` に置き、
  近さの計算は seam（`ConceptExpander`）にする。要点は 3 つ:
  - **完全一致を最優先**（語彙に在る語は展開しない）。
  - **突出していなければ接地しない**。絶対しきい値ではなく z スコアで見る（尺度非依存）。
  - **肯定と否定で畳み方を変える**。接地できなかった語は、肯定なら残す（CLIP のソフト採点が
    受け持つ）が、**否定なら捨てる**——索引に無い概念の不在は検証できず、除外に使うと
    [[ADR-100]] で直した「証拠が無いのに除外した気になる」誤りの再来になる。
  - 否定も展開する（「犬が写っていない」なら子犬・犬種も除外する）。
- 実測による否定的結果（重要）: 近さを **CLIP のテキスト同士**の類似度で計算する実装を書いて
  COCO 80 クラスで測ったところ、**使い物にならなかった**。CLIP テキスト塔は異方性が強く、
  どの語も語彙全体と 0.67〜0.83 の高い値で並ぶ。z スコアで見ると:
  | 語 | z(top) | 上位 | 判定 |
  |---|---|---|---|
  | pizza / train / cat / car / dog | 4.3〜6.1 | 正しい | ✅ |
  | people | 3.8 | person | ✅ |
  | outdoor | 2.6 | cup, bench | ❌ 誤接地 |
  | **food** | 1.6 | pizza が埋もれる | ❌ 取りこぼし |
  | landscape / scenery | 1.7 | car, bird | ❌ |
  **まさに必要だった `food → pizza` が判別できない**ため、この実装は production に結線していない
  （`CLIPConceptExpander` はコードとしては残すが Composition Root でコメントアウト）。
- 差し替え（採用）: 近さは **text↔image** で測る。各タグを「そのタグが付いた写真の CLIP **画像**
  埋め込みの重心」で表し、クエリ文と比べる（`TagCentroids` / `CLIPConceptExpander`）。
  画像埋め込みは全写真ぶん既にある（`PhotoEmbedding`）ので**新規の推論はゼロ**。
  **Caltech-101（101 クラス）で計測**（COCO は物体中心で上位語の階層が作れないため別途取得）:
  接地できた語 3/10 → **10/10**、正解集合に対する F1 **0.300 → 0.761**。
  副産物として、タグの意味が**このライブラリの写真**で定義される（雪山ばかりのライブラリなら
  `mountain` はそういう意味になる）。詳細と語ごとの結果は `records/search-quality.md`。
- 関連: `AIAlbum/VocabularyGrounding.swift`（機構・純）/ `QuerySpecSanitizer.replacingExclusions`
  / `AIAlbumInterpreter.groundContentTerms` / `Tags/TagStore.tagVocabulary`
  / `MobileCLIPKit/CLIPConceptExpander.swift`（保留）/ `scripts/gen_grounding_fixture.py`（計測）。
  テスト: `VocabularyGroundingTests`。
- 教訓: **一般的な仕組みを作っても、中身の関数が悪ければ意味がない。**
  「CLIP の埋め込み空間で近い語を採る」は一見もっともらしいが、テキスト同士の比較は
  CLIP の設計外の使い方だった。作ったら測る——測っていなければ、正しそうな設計のまま
  壊れたものを出荷するところだった。
  z スコア判定は**標本数に依存する**（n が小さいと理論上の最大 z が (n-1)/√n までしか出ない）
  ことにも注意（語彙 7 語では 3.0 に到達し得ない）。

## ADR-100 「証拠が無い」を「写っていない」と読まない／否定は語ごとに汎用で扱う
- 状態: 採用（[[ADR-24]] の証拠主義を、除外条件について徹底する）
- 文脈: 実フィードバック「『人の写っていない風景』に人やものが写っている写真が多い」。
  当初は人物除外への点対応を提案したが、「検索される用語は多岐にわたる。もっと汎用的な対応を」と
  指摘され再設計した。調べると**汎用の仕組みは既に半分あって途中で止まっていた**:
  場所・人物名は `AIAlbumCatalog` に接地して「カタログから選べ」と制約しているのに、
  内容語だけは LLM が英単語を創作し、索引側に実在するか誰も確かめていなかった。
  そのうえで 3 つの具体的欠陥があった。
  1. **否定が人物専用**。`hasPeopleNegation` という固定文字列判定しか無く、
     「犬が写っていない写真」は否定が無視されて `dog` が**肯定**に立ち、犬の写真を返していた。
  2. **「無い」を「いない」と読む**。`(faceCounts[id] ?? 0) == 0` で、顔スキャン未実施の写真を
     「人なし」と判定していた。顔スキャンの網羅率は実機で 11%＝89% が無証拠で通過。
  3. **索引済みなのに未使用**。`humanCount`（Vision 上半身検出）は夜間タグ付けで既に計算・
     保存されているのに検索から一度も読まれていなかった。網羅率は約 86%（顔の 7 倍以上）で、
     上半身検出なので後ろ姿や小さい顔も拾える＝「人がいない」の担保に適する。
- 決定:
  - **否定は語ごとに文脈で判定する**（`JapaneseVisualLexicon.excludeTerms`）。語の直後の
    「が写っていない/がない/なし/以外…」、英語は直前の "without/no…" を見る。
    否定された語は**肯定に立てない**。人物否定もこの中に集約し、呼び手は人物を特別扱いしない。
  - **除外だけのクエリを有効な選択として扱う**。肯定語が無くても「犬が写っていない写真」は
    成立する。旧実装は空を返していた（＝否定を汎用化しても 1 枚も出ない）。
    除外語は決定的レキシコンで接地済みなので、旧障害（解釈失敗で全件アルバム）は再発しない。
  - **人物証拠は `humanCount` を主軸に、顔スキャンを補助にする**。どちらにも記録が無い写真は
    **通さない**（証拠が無いことを不在の証拠にしない）。索引が進めば自然に入る。
  - **証拠ゲートをチャネル対応にする**。人物除外には**人物について語れる証拠**を要求する。
    風景タグ（beach, sky…）は人の有無について何も語らないのに、旧実装はそれで通していた。
- 結果（COCO val2017・4,952 枚での実測。台帳は `records/search-quality.md`）:
  除外つきクエリのマクロ F1 **0.125 → 0.619**。実障害そのものの「人が写っていない写真」は
  precision **0.490 → 1.000**（誤混入 2062 枚 → 0 枚）で、recall は 0.878 → 0.876 とほぼ不変。
  **人物以外（犬 F1 0.016→0.930、車 0.028→0.926）でも同等に改善**＝点対応ではなく汎用化。
  肯定のみのクエリの F1 は全段階で不変＝肯定側を壊していない。
  トレードオフ: 証拠の無い写真を除外するぶんアルバムは小さくなる（索引の進行で埋まる）。
- 関連: `AIAlbum/JapaneseVisualLexicon.swift` / `AIAlbumInterpreter.swift` / `AIAlbumSearch.swift`
  / `AIAlbumVerificationCoordinator.swift` / `Tags/TagStore.swift`。
  計測: `scripts/fetch_search_eval_datasets.sh` / `SearchEvalTests` / `records/search-quality.md`。
- 未解決（台帳に記載）: **内容語のカタログ接地**が未着手。「食べ物」→ `food` と解釈されるが
  索引側は `pizza`/`cake`/`sandwich` で持っており当たらない。場所・人物でやっている
  「カタログから選ばせる」規律を内容語へ広げるのが次の段階。
- 追記（連言否定・2026-08-11）: 「犬**と猫**が写っていない写真」で否定が 猫 にしか届かず、
  **dog が肯定に立って犬の写真を返す**ことをプローブで確認した。否定判定は語の直後だけを
  見ていたため。連言（と・や・、）＋名詞の並びを読み飛ばしてから否定表現を探すよう修正
  （`skippingConjoinedNouns`）。「山と人が写っていない風景」も mountain+people の除外になる。
  英語の連言（"without dogs and cats"）は未対応（FM 解釈が受け持つ・レキシコンは補助線）。
- 教訓: **索引しているのに使っていないデータ**があった（`humanCount`）。索引の追加時に
  検索側への結線を仕様として持たないと、計算コストだけ払って効果ゼロになる。
  クエリ言語（`QuerySpec.Condition`）が固定 enum である限り同じことが起きるので、
  「索引できるもの」と「クエリで表現できるもの」を対応付けて管理する必要がある。

## ADR-99 診断ログの閲覧は「末尾を・非同期で・行単位に」出す
- 状態: 採用
- 文脈: 実フィードバック「デバッグログを見るところで固まる」。`DiagnosticsLogView` は
  `.task { text = DiagnosticsLog.shared.recentText() }` としており、
  (1) `recentText()` が `queue.sync` で**最大 512KB を同期読み**＝`.task` は `@MainActor` なので
  メインスレッドが読み終わるまで止まる（追記中なら `sync` がキューの排出も待つ）、
  (2) 読んだ全文を**1 つの `Text`** に流し込んでいた。SwiftUI は 1 つの `Text` の
  レイアウトを分割できないため、数十万文字は測るだけで数秒かかる。**表示のしかたが性能そのもの**だった。
- 決定: `recentLines(maxLines:)`（`async`・行分割済み・末尾から上限件数）へ置き換え、
  画面は `LazyVStack` で 1 行 1 ビューにする（見えている行だけレイアウトされる）。
  読み込み中は `BusySpinner`（[[ADR-96]]）を出す。全文が要るときは共有ボタン（`url`）を使う。
- 結果: 画面を開いてもメインが止まらない。トレードオフ: 行をまたぐテキスト選択ができなくなる
  （全文は共有で取れるので実害は小さい）。既定 3000 行より前は画面に出ない。
- 関連: `MosaicSupport/Diagnostics.swift` / `MosaicPhotos/Settings/DiagnosticsLogView.swift`。
  テスト: `DiagnosticsLogReadTests`。
- 教訓: **診断のための画面が診断対象を悪化させていた**。ログが長い夜ほど開くのが重くなる＝
  一番知りたいときに一番固まる。開発者向け画面でも「大きなデータをどう出すか」は本番と同じ基準で見る。
- 未解決（計測を入れた）: 同じログの末尾で、前面の AI アルバム評価によりメインが 9.9 秒／10.6 秒
  ブロックしていた（`aialbum: ... scored=51129 result=46460`）。採点自体は `Task.detached` で
  オフメインだが、その手前で 86k 件規模の台帳（タグ・OCR・顔数・人物名）を集めている。
  どこが支配的か読めないため、段ごとの計測（`aialbum.faceCounts` / `.peopleMap` / `.tagsLedger` /
  `.ocrLedger` / `.score`）を入れた。次のログで内訳を見る（性能原則 5）。
  併せて `result=46460`（ライブラリの 54%）という結果自体も要検討＝タグ 2 語で 46,458 件ヒット。

## ADR-98 中断は「1 単位」の内側まで届かせる／手を付けなかった分は完了として記録しない
- 状態: 採用（[[ADR-95]]「止めた意思を効かせる」の続き。単位の**粒度**の問題）
- 文脈: 実フィードバック「他のアプリを使って戻ったときに固まる」。実機 diagnostics-44 で
  **本物の固まり**を確認した（34 分でハング 2 件・うち 1 件が復帰直後の 10469ms）。
  ```
  11:28:29 PROCESS SUSPENDED ~156s
  11:28:30 tags: start — 13880 photos to tag   ← 復帰の瞬間にタグ付け開始
  11:28:30 scene: active / faces: stopScan (foreground return)
  11:28:41 tags: finished — 4 tagged           ← キャンセル後も 11 秒動いた
  ```
  `BackgroundTrickle` の不変条件は「停止判定は 1 単位ごと」だが、**その 1 単位が大きすぎた**。
  タグ付けの 1 単位はミニバッチ 4 枚で、クラウド写真だとサムネ DL 約 0.8〜1.0 秒＋Vision が入り、
  実測 `tags.photoMs=1(Σ2665.8ms)`＝1 枚 2.7 秒 → **1 単位で 11 秒**。単位の内側に中断点が
  無かったため、キャンセルしても丸ごと走り切っていた。
  なお「復帰の瞬間に重い処理が始まる」こと自体は避けられない（ゲートは背面で開き、
  ユーザーの復帰は予測できない）。効くのは**中断の速さ**だけ。
- 決定:
  - 単位の**内側**（`VisionTagAdapter.senseInfo` の 1 枚ごと／`AIPerceptionAdapters.perceive` の
    画像ロード後・推論前）で `Task.isCancelled` を見る。
  - ⚠️ 中断した写真は**結果の辞書に載せない**。空の結果を載せると「処理済み」として保存され、
    その写真は次の窓で二度と拾われない（[[ADR-92]] と同じ罠）。取得**不能**な写真は従来どおり
    空を載せる＝無限リトライを避けるため。この 3 分岐（成功／取得不能／中断）を崩さない。
  - 呼び手（`TagTagger` / `PhotoTagger` の `processUnit`）は、辞書に無いキーを
    `?? 空` で埋めない。CLIP 側は**中断時のみ**埋めない（未同梱時の無限ループ防止を壊さないため）。
- 結果: 復帰時に走り切る時間が「1 単位（最大 11 秒）」から「1 枚（最大 2.7 秒）」になる。
  トレードオフ: 中断で捨てた写真は次の窓でやり直し＝わずかに重複作業が出るが、
  「完了として記録して永久に取りこぼす」よりはるかに安い。
- 関連: `MobileCLIPKit/VisionTagAdapter.swift` / `AIPerceptionAdapters.swift` /
  `AutoAlbumCore/Tags/TagTagger.swift` / `Perception/PhotoTagger.swift`。
  テスト: `TrickleCancellationTests`（3 分岐の固定）。
- 教訓: 「1 単位ごとに停止判定」という不変条件は、**単位の実所要を測っていないと意味がない**。
  単位を大きくする最適化（ミニバッチ＝ANE 償却・ADR-83）を入れたときに、
  停止応答性の前提が壊れていた。粒度を変えたら応答性の前提も見直す。

## ADR-97 「前面のハング」は前面区間に収まる観測だけを数える（復帰をまたいだ ping を捨てる）
- 状態: 採用（[[ADR-82]]「背面の停止を前面の数字に混ぜない」の穴を塞ぐ）
- 文脈: diagnostics-40〜43 で毎回「復帰直後に 9〜11 秒のフォアグラウンドハング」が記録され、
  そのたびに原因を追ってきた。ところが **diagnostics-43 では重い処理が一つも走っていない**のに
  `hang main=11041ms` が出た——`labeler: prewarm cancelled at 0/314`（0 語で中断）、
  `model load skipped (cancelled) — face model` ×4、`faces.detect: ... load=0ms infer=0ms`。
  つまりこの数字は**アプリの停止ではなく計測の汚染**だった。
  穴は 2 つ: (1) `record` は「**返答時**に前面か」しか見ておらず、背面で送った ping が復帰の瞬間に
  返ると前面ハングとして数えられる。(2) `ProcessSuspension` は**正式な中断**しか捉えられないので、
  復帰前後の throttle はすり抜ける（この制約はコード内に既に書かれていたのに、判定条件へ
  反映されていなかった）。
- 決定: 前面へ復帰した時刻（`lastBecameActiveNs`）を持ち、**それより前に送られた ping は
  前面ハングに数えない**（`bgStalls` として参考値には残す）。`hang.begin` の即時報告にも同じ判定を掛ける。
- 結果: 「復帰直後の 10 秒ハング」という偽の症状が消え、前面の数字が体感を表すようになる。
  トレードオフ: 復帰の**直後に実際に**起きた停止も、その ping が復帰前に送られていれば 1 サンプル
  捨てる（次の ping からは正しく拾う＝0.2 秒間隔なので取りこぼしは軽微）。
- 関連: `MosaicSupport/MainThreadWatchdog.swift`、テスト `MainThreadWatchdogTests`。
- 教訓: **計測が体感を表しているか確かめる**（CLAUDE.md 性能原則 5）。この 1 件で 4 回のログ分析を
  誤った方向に費やした。センサーの既知の制約（「正式な中断しか捉えられない」）をコメントに書いた
  だけで満足せず、**判定条件に落とす**こと。

## ADR-96 待たせるなら「考え中」を見せる／人物一覧の N+1 クエリを畳む
- 状態: 採用
- 文脈: ユーザー要望「UI が固まるのでもいいが、その場合は考え中のクルクルを出してほしい。
  固まっているから出せないのか？」。これは**半分正しい**——切り分けが要る。
  - メインスレッドが実際にブロックされている間、SwiftUI は 1 フレームも描けない。
    フラグを立てた直後に重い処理へ入ると「何も出ないまま固まる」になる。
  - 一方、`UIActivityIndicatorView` の回転は `CAAnimation` で、いったんレンダーサーバへ
    コミットされれば**別プロセス側で進む**。つまり**ブロックが始まる前に画面へ出ていれば、
    メインが止まっている間も回り続ける**。SwiftUI の `ProgressView` はフレーム駆動になり得るため、
    「止まっている間こそ見せたい」用途では保証にならない。
  - 実際、レビュー画面には既に `ProgressView` があったのに「固まる」と報告された＝
    出していなかったのではなく、**描画されていなかった**。
  併せて、計測（ADR-95 追記3 で入れた `people.load.*`）が原因を確定させた。実機 diagnostics-42 で
  `people.load.clusters` が 725〜3585ms、**同じ長さのフォアグラウンドハングと 1 対 1 に対応**
  （671/725・896/997・2028/2188）。`peopleClusters` はクラスタごとに `faces(inCluster:)` を
  呼んでおり、936 クラスタなら **936 回の fetch**＋全 `DetectedFace` の materialize。
  表示に一切使わない `embedding`（512 次元・約1KB/顔）まで毎回読み出していた。
- 決定:
  - **見せる**: `PhotoSourceKit` に `BusySpinner`（`UIActivityIndicatorView` の
    `UIViewRepresentable`）と `busyOverlay(_:text:)` を新設し、待ちのある画面に付ける
    （レビューの候補抽出・カード準備、一括統合の適用中、人物アルバムのメンバー取得）。
    重い処理の**前**に出す場合は `runShowingBusy(_:settleFrames:)` で 1〜2 フレーム譲って
    表示を確定させてから始める（これを飛ばすと何も出ないまま固まる）。
  - **待たせない**: `peopleClusters` を**1 回の射影クエリ**（`propertiesToFetch` で
    `embedding` を除外）＋メモリ上のクラスタ ID 束ねに変える。`peopleEligibleClusters` も同様。
    ADR-88 が `memberRefKeys(inCluster:)` に入れた射影を、人物一覧の本体へ横展開する。
- 結果: 待ちが残る場面でも「処理中」だと伝わる。人物一覧の再読込は 936 回のクエリが 1 回になり、
  読み出すデータ量も顔あたり約1KB 減る。トレードオフ: (a) `runShowingBusy` は表示確定のため
  約 33ms の遅延を足す（体感より「無反応に見えない」ことを優先）。(b) 射影から漏れたプロパティに
  触れると SwiftData がフォールトで個別ロードするため、`peopleClusters` が使う項目を増やすときは
  `propertiesToFetch` の更新を忘れないこと（漏れても正しく動くが遅くなる）。
- 追記（一括レビューのボタン配置・実フィードバック）: 「全て選択／次の人物へが小さすぎる。
  配置的に『N 件をまとめる』を間違えて押しそう」。原因は 2 つで、(1) 補助操作が文字だけのボタンで
  タップ領域が文字の高さしかなかった（HIG の最小 44pt を大きく下回る）、(2) 確定操作との間隔が
  8pt しか無かった。→ 補助操作を **44pt の枠付きボタン**にし、確定操作とは**区切り線＋上下 18pt**
  で離す。原則: **取り返しのつく操作と、つきにくい操作を隣に置かない**。統合はやり直しに
  顔の移動と修正ジャーナルの巻き戻しが要るので、「押すつもりが無いのに押せる」状態を作らない。
  ※ 1 対 1 のレビュー（`FaceReviewView`）の No/Skip/Yes は既に 50pt 相当あり、変更不要だった。
- 追記2（起動直後・実フィードバック「起動直後であっても固まるのはまずい」）: 起動区間は
  ストア構築・68,200 件の読み込み・`pathAlbum.fast`（68,200 件の計算）・顔スキャン開始・
  サムネのバッチ取得が重なり、メインが 1 秒級で止まる（diagnostics-43 の `hang main=1340ms`）。
  ホーム画面には既に読み込み表示があったが、いずれも SwiftUI の `ProgressView`＝**メインが
  止まると一緒に止まる**ため「固まった」ように見えていた（レビュー画面と同じ構図）。
  → **アプリ側のユーザー向け `ProgressView` を全廃**し `BusySpinner` に統一する:
  起動画面（`RootView`「Now loading…」）／セクションヘッダの実行中表示／`LoadingRow`／
  ピープルのスキャン中表示／実行ボタン共通の `BusyLabel`／代表写真ピッカー。
  この置き換えは起動区間で特に効く——**スピナーはブロック前に画面へ出ている**ので、
  以後メインが止まっても回り続けるという条件を自然に満たす。
  ※ Developer Options / 設定の一部は対象外（英語のまま・デバッグ用途）。
- 関連: `PhotoSourceKit/Views/BusySpinner.swift` / `FaceCore/Faces/FaceStore+People.swift` /
  `MosaicPhotos/Home/FaceReviewView.swift` / `FaceBatchReviewView.swift` / `PersonAlbumView.swift`。
  [[ADR-88]]（射影クエリの初出）・[[ADR-95]]（計測を入れて原因を確定させた経緯）。

## ADR-95 「待つ側が資源を握らない」— 眠るならフラグを手放す／変わっていないものを取り直さない／一覧発行はまとめる
- 状態: 採用
- 文脈: 実機ログ diagnostics-38 で 5 つの症状が出た。掘ると**3 つの原則違反**に集約された。
  1. フォアグラウンドのハングが **105 回**（最大 3490ms）。分あたりのハング数が
     `faces: people=` の発行回数と**完全に一致**（05:03 は 30 回対 31 回、05:11 は 27 対 27、
     05:07 は 14 対 14）。人物リストの再発行 1 回につきメインが 600〜1000ms 止まっていた。
     発行元は顔スキャンの `onBatch`（バッチごと）とレビューの回答ごとで、2 秒に 1 回走っていた。
     さらに `PersonInfo` が `memberRefKeys`（そのクラスタの全写真キー）を積んでおり、
     数百人ぶんが毎回 MainActor を通り、`PersonAlbumView.init`（SwiftUI は再評価のたびに呼ぶ）が
     その全件を `PhotoRef.decode` し直していた。
  2. キャプションが**ゼロ**。`caption window (face scan skipped, pending=796)` は初めて出たのに、
     次の行が `bgfill: skip — already tagging/embedding`。04:47 に前面で始まった bgfill が
     ゲート閉（控えめ設定＋前面）のまま `waitWhilePaused` で眠り、**実行中フラグを 14 分間**
     握っていた（14 分で 208 枚しかタグ付けしていない＝ほぼ眠っていた）。窓 77 秒は丸ごと空転。
     加えて窓は「新しい顔スキャンを起こさない」だけで実行中のスキャンは止めないため、
     `isScanningFaces` が立ったままでキャプションは見送られる。キャプションは埋め込みの**後ろ**
     にしか置かれておらず、短い窓では順番が回ってこない。
  3. 埋め込みが 40181 件滞留したまま 0 枚。`bgfill: embed loop entry (unembedded=40181)` →
     `embed: finished — 0 photos in 0.0s`。タスクは既にキャンセル済み（前面復帰）だったが、
     **フェーズの切れ目でキャンセルを見ていない**ため、死んだタスクのまま次フェーズへ進み、
     実態と食い違うログだけを残していた。
  4. 起動直後の最大ハング 2 件（3490ms / 2791ms）は `cachedItems() → 68200 items` の直後。
     変化のない同期ポーリングでも毎回 68,200 行を fetch し、値型 68,200 個を作って署名を比べ、
     大半を捨てていた（`cache.fetchItems` 993ms / 1165ms が 3 秒間に 2 回）。
- 決定: 症状ごとの対症ではなく、3 つの原則として直す。
  - **眠るならフラグを手放す**: `BackgroundTrickle.waitWhilePaused` に上限（既定 60 秒）を設け、
    超えたら `true` を返して `run` がループを畳む（部分結果は commit してから）。呼び手の
    「実行中」フラグが解放され、次の窓が入れる。各処理は差分ベースなので取りこぼさない。
    さらに BGTask 窓の先頭では `restartBackgroundFill` で滞留した実行を**明け渡させる**
    （世代ガード `fillGeneration` で旧タスクの末尾処理が新タスクを踏まないようにする）。
    キャプション窓では実行中の顔スキャンも `stopScan()` で止め、その回だけ
    `captionsFirst` でキャプションを埋め込みより先に回す。フェーズの切れ目で `Task.isCancelled` を見る。
  - **変わっていないものを取り直さない**: `DropboxCacheStore` に変更リビジョン `itemsRevision`
    を持たせ（実際に増減・更新があったときだけ進む）、`reflectCachedItems` は札が同じなら
    **fetch すらしない**。CLAUDE.md 性能原則 3（無いものを繰り返し探さない）の同型。
  - **一覧の発行はまとめる／重い荷物を積まない**: `peopleClusters(includeMembers:)` を足し、
    一覧は `false`（`count` は正しいまま・メンバーは `memberRefKeys(forPerson:)` で
    開いた画面だけが取りに来る）。`loadPeople` は結果が同じなら**代入しない**
    （`@Observable` は代入だけで購読ビューを無効化する）。連続する変更は
    `setNeedsPeopleReload()` で 700ms 静止まで待って 1 回だけ発行する。
    `PersonAlbumView` はメンバーを `.task` で 1 回だけ取る（`init` から decode を外す）。
- 結果: ハングの原因（発行回数 × 1 回あたりのコスト）を両側から削った。キャプションは短い窓でも
  必ず順番が回る。BGTask 窓が居座りで空転しない。起動時の 68,200 件フェッチが変化時のみになる。
  トレードオフ: (a) 譲り待ち 60 秒で畳むため、ゲートが 61 秒後に開く場合は次の呼び出しまで待つ
  （窓の再入トリガはシーン遷移・BGTask・アイドルタイマーがあるので実害は小さい）。
  (b) 人物一覧の反映が最大 700ms 遅れる（スキャン進捗・レビュー結果の表示のみ）。
  (c) BGTask 窓の明け渡しで、前面由来の実行が進行中でも中断される（バッチ単位で保存済み）。
- 追記（diagnostics-39 で検証）: ハングは **105 回 → 9 回**、人物リストの発行は **101 回 → 2 回**、
  `cachedItems()` は起動 3 秒で 2 回 → 17 分で 2 回になった。残った 9 回を追うと**共通の形**が出た。
  - **やる気が無いのに準備だけしていた**: `bgfill: begin (pause=true)` から `tags: start` まで
    5〜8 秒あり、その間にメインが 2.0〜3.2 秒ブロックし、直後に `tags: finished — 0 tagged` で
    捨てられていた。ゲート判定より**先に**準備（お気に入り再取得・約 86k キーの取得・
    `AnalysisOrder.ordered` の安定ソート）が走っていたため。→ **準備の前にゲートを待つ**
    （60 秒で打ち切り）。並べ替えは `Task.detached` へ降ろす（性能原則 4）。
  - **check-then-act の競合**: `loadItems()` の呼び手はいずれも `items.isEmpty` で守っているが、
    起動直後はまだ空なので全員がガードを通り抜け、68,200 行の fetch が多重に走っていた
    （915ms + 1062ms）。リビジョンの札も反映前は更新されないので札だけでは防げない。
    → `loadItems()` を単一フライト化し、実行中なら同じタスクを待つ。
- 追記2（diagnostics-40 で検証）: **キャプションが初めて生成された**（`bgfill: captions 12 done
  (pending 796→784)`）。BGTask 窓は 4 分 45 秒で `bgtask: end (completed)`＝期限切れせず完走し、
  窓の中でキャプションが埋め込みより先に回った（`captionsFirst` が効いた）。人物リストの発行は
  レビュー連続回答の 19 回でハング 0（footprint 約 160MB）＝発行あたりのコストも解消。
  一方で**新しい違反**が 1 つ見つかった: **止めたつもりが止まっていなかった**。
  復帰時の `stopBackgroundWork()` は `prewarmTask` を cancel するが、表示ラベラは二重構築を防ぐため
  **共有 Task に合流**する作りで、外側の cancel はそこへ伝播しない。しかも約300語のループに
  中断点が無かったため、`CLIP text tower loaded in 15456ms` が復帰後も走り切り、その間
  ANE ゲートを占有していた（同時刻にメインが 11.6 秒ブロック）。
  → seam に `LabelProvider.cancelPrewarm()`（既定は no-op）を足し、共有 Task を直接 cancel する。
  ループは 1 語ごとに `Task.isCancelled` を見て、途中結果は**確定させない**（`isReady` は false の
  ままなので insight は Vision タグだけで即返る＝表示は壊れない）。
  併せて **ANE ゲートの 1 秒以上の待ちを記録**する（`ANE gate: waited Nms`）。占有側と待たされた側は
  無関係な経路のことがあり、待たされた側のログだけでは犯人が分からないため（性能原則 5）。
  ※ メインが 11.6 秒止まった機序そのものは本ログからは断定できていない（モデルロードは
  `MLModel.load` の async でメイン外、`LoadOnce` は await 前にロックを手放している）。
  上記の計測を入れた次のログで切り分ける。
- 追記3（diagnostics-41）: 「止めた直後に一番高価なことを始める」が**顔モデルでも**起きた
  （`faces: stopScan` の 1 秒後に `face model loaded in 10883ms`・メイン 10.5 秒ブロック）。
  `BackgroundTrickle` は単位の**前**にしかキャンセルを見ないので、`processUnit` に入った後に
  始まる遅延ロードは止められない。→ 重いモデルの遅延ロード入口（顔／CLIP テキスト塔／CLIP 画像塔）で
  **「キャンセル済みかつ未ロードなら始めない」**を共通判定にする（`LoadOnce` の外で判定＝
  中で nil を返すと恒久的な失敗としてキャッシュされ機能が死ぬ）。中断した 1 枚は結果に載せない。
  未解決: レビュー連続回答で 1 回あたり 540〜645ms のハングが残る。`loadPeople` の各段は全て
  off-main のはずで機序が読めないため、**推測で直さず段ごとの計測**（`people.load.tuning` /
  `.favorites` / `.clusters`）を入れた。次のログで内訳を見る（性能原則 5）。
- 関連: `PerceptionCore/BackgroundTrickle.swift` / `AutoAlbumCore/AIAlbum/AutoAlbumEngine+Recognition.swift`
  / `AutoAlbumCore/AutoAlbumEngine.swift` / `AutoAlbumCore/Perception/PhotoTagger.swift`
  / `FaceCore/Faces/FaceStore+People.swift` / `FaceCore/Faces/PeopleEngine.swift`
  / `DropboxCore/Cache/DropboxCacheStore.swift` / `DropboxCore/Store/DropboxPhotoStore.swift`
  / `MosaicPhotos/HeavyWorkScheduler.swift` / `MosaicPhotos/Home/PersonAlbumView.swift`。
  テスト: `BackgroundTrickleTests`（譲り上限）/ `PersonNamePriorityTests`（一覧はメンバー非搭載・
  遅延取得の一致）/ `DropboxCacheStoreMetadataTests`（リビジョン）。ADR-79/80/85/86/87/93 の続き。

## ADR-94 人物の名前は「付けた側」を必ず残す（束ね・統合とも）／衝突は必ず尋ねる
- 状態: 採用
- 文脈: 実フィードバック「複数のグループを束ねたら、折角つけた名前が消えることがある」。
  調べると**束ね（`personGroupID`）の主クラスタ選択が非決定的**だった:
  `clustersInGroup.first { 名前つき }` としていたが、`allClusters()` の取得順は SwiftData 任せで
  不定。名前つきが複数あると**表示のたびに違う名前**が採られ、ユーザーには「名前が消えた／
  勝手に変わった」と見える。統合（`mergeClusters`）側は名前つきを残していたが、
  **代表写真は引き継いでいなかった**ため「名前だけ移って顔は別人のまま」になり得た。
- 決定:
  1. **主クラスタの選択を決定的にする**: 名前つき → 写真の多い順 → clusterID 昇順。
     同じデータなら常に同じ人物名・代表が出る。
  2. **統合では名前と代表写真をセットで引き継ぐ**。統合先が未命名なら、名前と一緒に
     統合元の `coverFaceID` も移す。
  3. **束ねで名前が衝突したら必ず尋ねる**（`conflictingNames` / `unifyName`）。
     ダイアログは「どちらの名前を残すか」の選択肢に加え、**「束ねない（別人でした）」を
     destructive で**出す。別々の名前が付いている＝ユーザーが既に別人と表明している状態なので、
     そもそも操作が誤りである可能性が高い（統合側は同じ理由で以前から拒否＝`.differentNames`）。
     選んだ名前は束ねる**前に**全クラスタへ揃えるので、主クラスタの選び方に関わらず表示が安定する。
  4. 並び順は**写真の多い順**（既にそうなっていた＝`members.count` 降順・通し番号は並べ替え後）。
     回帰テストで固定した。
- 結果: 付けた名前が消えない。衝突時は誤操作を止める選択肢が出る。
- 関連: `FaceStore+People.peopleClusters/conflictingNames/unifyName` / `FaceStore+Edit.mergeClusters` /
  `PeopleEngine.linkPeople(_:keepingName:)` / `PersonPhotosView`（確認ダイアログ）/
  `PersonNamePriorityTests`（回帰 5 件）。
- 教訓: **ユーザーが与えた情報（名前）は、自動処理より常に優先**する。加えて、表示に使う
  代表要素の選択は**必ず決定的な順序**にする——「最初に見つかったもの」は順序が保証されない限りバグ。

## ADR-93 有界な作業を無限の作業の後ろに並べない（キャプションの窓配分）＋判断の可観測性
- 状態: 採用
- 文脈: ADR-92 の効果は確認できた（実機 diag-37: 前面ハングが 19 件・最大 10.3 秒 →
  **2 件・最大 3.2 秒**。`faceAnalysis.yield` も計上され、譲りが働いている）。
  一方、ADR-86 で入れたはずの **VLM キャプションが依然 1 枚も生成されていない**。
  利用者が Developer Options から夜間ルーチンを明示実行しても動かなかった。
  原因は 2 つ。
  1. **`maxCaptionDeferrals = 3` が実態に合っていない**。夜間窓は数分で、しかも毎回来るとは
     限らない。「3 回譲ってから 4 回目」では順番が回るまでに何日もかかる
     （diag-35 の `captions(pending=612 idle=9d)`＝9 日間ゼロ）。
  2. **取らなかったときに何も記録していなかった**ため、「残作業がゼロなのか順番待ちなのか」が
     ログから判別できなかった。ADR-86 で「沈黙は正常の証拠にならない」と書いておきながら、
     同じ穴を新しい判定に作っていた。
- 決定:
  1. **`maxCaptionDeferrals` を 3 → 1**（1 窓おきにキャプションへ回す）。根拠は
     **作業量の性質の違い**: キャプションの母数は**有界**（お気に入り約 1,600 枚で打ち止め・
     終われば `hasWork=false` になり判定自体が無効化される）。顔スキャンはクラウド 68,200 枚で
     **事実上終わらない**。有界な作業を無限の作業の後ろに並べる意味は薄い。
  2. **取らなかった理由も必ずログに残す**（`bgtask: caption deferred N/1 (pending=…)`）。
     残作業ゼロと順番待ちを区別できるようにする。
- 結果: キャプションが 1 窓おきに進み、有界な backlog を先に片付けられる。判断の理由がログに残る。
- 関連: `HeavyWorkScheduler.runHeavyWork/maxCaptionDeferrals`。[[ADR-86]]（同型の飢餓）・[[ADR-87]]。
- 教訓: **順番回しの上限は「作業量の性質」で決める**。有界な作業と無限の作業を同じ重みで
  並べてはいけない。加えて、**分岐は「やった」だけでなく「やらなかった」も記録する**。

## ADR-92 顔解析の 1024px 取得は閲覧中に譲る／取得できない写真は「走査済み」にしない
- 状態: 採用
- 文脈: ADR-90（1024px 化）は狙いどおり効いた——移行後の実測（diag-36）で**検出 21 顔中
  12 顔が埋め込みに成功（57%）**。移行前は同ログ内 1,878 枚でほぼ全滅だった。
  ただし**前面ハングが 2 件（最大 1.4 秒）→ 19 件（最大 10.3 秒）に悪化**した。
  原因は 2 つあり、どちらも ADR-90 の実装漏れ。
  1. **優先度制御を迂回していた**。`faceAnalysisThumbnails` は `apiClient.rpc` を直接叩き、
     表示用の `DropboxThumbnailBatcher`（可視セル優先・先読みは低優先・上限つき）を通らない。
     結果、閲覧中のサムネと同じ回線を無制御で奪い合い、`thumb.missWaitMs` が 1 件 8.6 秒に達した。
     ADR-90 以前は顔スキャンがキャッシュ済みサムネしか使わず、この競合は存在しなかった。
  2. **取れなかった写真を「顔ゼロで走査済み」と記録していた**。1024px が取れないとき
     256px へフォールバックする経路があり、低解像度のまま解析して確定させていた。
     `FaceTagger` も `one[refKey] ?? []` で「辞書にキーが無い（＝未解析）」と
     「解析したが顔ゼロ」を同一視していた。**スキャン版を上げるまで二度と見直されない**ため、
     一時的な取得失敗が恒久的な取りこぼしになる。
- 決定:
  1. **閲覧中は解析用の取得を行わない**。判定は既存の `BackgroundYield.uiBusy`（写真ビュー表示中・
     フル画像取得中・表示サムネ取得中・メモリ圧迫）に一元化する。譲った回は空を返すだけで、
     顔スキャンは次のバッチで拾い直す。譲りの回数は `faceAnalysis.yield` に計上して可視化する。
  2. **256px へのフォールバックを廃止**する。解析取得が設定されている構成では、1024px が
     取れなければ**その写真は今回見送る**（低解像度で確定させない）。
  3. **未解析は記録しない**。`detectFaces` は解析できた refKey だけを辞書に入れる（元からそう）。
     `FaceTagger` 側を `guard let faces = one[refKey] else { return nil }` に改め、
     解析できた写真だけ `recordScans` する。顔ゼロ（空配列）は従来どおり記録＝再スキャンしない。
     不変条件は回帰テスト（`FaceTaggerRecordingTests`）で固定した。
- 結果: 閲覧中に解析の通信が割り込まなくなる。一時的な取得失敗が恒久的な取りこぼしにならない。
  トレードオフ: 閲覧が続く間は顔スキャンが進まない（夜間が主戦場なので実害は小さい）。
- 関連: `AutoAlbumAdapters`（uiBusy ゲート）/ `FacePerceptionAdapter.detectFaces` /
  `FaceTagger.processUnit/commitBatch` / `FaceTaggerRecordingTests`。[[ADR-90]]。
- 教訓: **別経路で同じ資源を取りに行くと、既存の優先度制御が効かない**。共有資源
  （ここでは Dropbox の帯域）に新しい入口を作るときは、既存の入口が持つ制御
  （優先度・上限・譲り）を必ず引き継ぐか、通さない理由を明示する。

## ADR-91 人物レビューに「写真全体」表示、情報パネルの整理（意味のない行を消す・保存場所を出す）
- 状態: 採用
- 文脈: 利用者からの 3 点の指摘。
  1. 人物レビュー（「同じ人物ですか？」）で**顔だけでは判断できない**ことがある。
     後ろ姿・小さい顔・似た兄弟は、服・場所・一緒に写っている人といった**状況**が決め手になる。
  2. フル画面の情報パネルに「AI 解析」という行があるが、**横にも下にも何も出ていない**。
     実装を見ると `.ready` かつ結果ありのとき「AI analysis」という**見出しだけ**を出しており、
     直下にタグ・人物・説明が各自の見出しで並ぶため情報がゼロだった。
  3. その写真が**端末内なのか Dropbox なのか**が分からない。
- 決定:
  1. **顔 ⇄ 写真全体のトグル**をレビュー画面に置く。`loadFaceAvatar` は `box == nil` で
     「切り抜かず全体を返す」ようにし（従来は nil で早期 return＝何も出なかった）、
     トグルは `box` を nil に差し替えるだけにする。全体は横長が多いので丸ではなく角丸で見せる。
     状態は**カードをまたいで維持**する（毎回押し直さなくてよい）。
     ⚠️ 先読み（`avatarSpecs`）も同じ `box` を使う——キャッシュキーに box が入るので、
     表示と先読みでずれると先読みが丸ごと無駄になる（`avatarBox(_:)` に一元化）。
  2. **意味のない状態行を出さない**。残すのは「まだ解析していない／解析中／**解析したが
     何も検出されなかった**」の 3 つだけ。結果があるときは各セクションが語るので見出し行は出さない。
     `.ready` かつ結果なしは "AI analysis: done" では未解析と紛らわしいので
     "AI analysis: nothing detected"（検出なし）に改める。
  3. **保存場所の行を追加**（`PhotoInsight.source`）。「この iPhone 内」/「Dropbox 内」。
     混在一覧では入手経路で挙動（解析解像度・バックアップ対象か）が変わるため、区別できる価値がある。
     値はアプリ層の `photoInsight` クロージャが refKey から決める（既に localIdentifier を解決済み）。
- 結果: レビューで状況を見て判断できる。情報パネルから空振りの行が消え、代わりに実用的な
  2 情報（検出なし／保存場所）が出る。
- 関連: `FaceReviewView`（トグル・`avatarBox`）/ `PeopleSupport.loadFaceAvatar` /
  `PhotoInsight.source` / `PhotoInfoPanel.statusRow/sourceRow` / `SourceHostView`。

## ADR-90 クラウド顔解析は 1024px で取得し、顔ピクセル下限を 80px にする
- 状態: 採用（ADR-89 の計測に基づく決定）
- 文脈: ADR-89 のハーネスで実写真 571 枚（撮影日で層化抽出）を計測した結果:
  | サムネサイズ \ 顔ピクセル下限 | 32 | 48 | 64 | **80** | 96 | 112 |
  |---|---|---|---|---|---|---|
  | **256（現行）** | 13.2% | **3.8%** | 2.2% | 1.0% | 0.3% | 0.2% |
  | 640 | 47.3% | 30.8% | 20.7% | 13.2% | 8.3% | 5.3% |
  | **1024** | 55.0% | 49.3% | 38.8% | **29.7%** | 22.5% | 18.0% |
  現行（256px・48px 下限）の到達率は **3.8%** で、事実上機能していなかった
  （従来の推計 18% よりさらに低い）。256px は 32px まで緩めても 13.2% しか通らず、
  **サイズを上げない限り何をしても無駄**であることが確定した。
  副次的な事実: 571 枚中 281 枚（49%）で顔を検出、600 個中 338 個（56%）が信頼度 0.8 以上。
  通信量は 1 枚 91KB（想定 130KB より軽い）で全 68,200 枚でも 5.9GB。
- 決定:
  1. **顔解析だけ 1024px で取得**する（`faceAnalysisAPISize = w1024h768`）。表示・CLIP・シーンタグは
     256px のまま（CLIP 入力は 224px、シーンタグも 256px で足りる＝上げる理由がない）。
  2. **顔ピクセル下限を 48 → 80** にする。モデル入力 112px に対し拡大 1.4 倍に収まり品質を保てる。
     48px（49.3%）は魅力的だが 2.3 倍の拡大で、埋め込み品質を実測せずに採るのは危険。
     112px（18.0%）は堅いが取りこぼしが多い。**29.7%＝現行の 8 倍**で釣り合う点として 80px を採る。
  3. `minFaceSide(isCloud:)` の 0.15 特別扱いを**撤廃**する。あれは「サムネが 256px しかない」ことの
     代理指標で、解像度が足りるなら絶対ピクセル下限が本来の条件を担う。
  4. **1024px はディスクに保存しない**。68,200 枚 × 91KB＝5.9GB になる。`CloudAnalysisImageCache`
     （actor・上限 64 枚）に 1 バッチぶんだけ持ち、`take` で取り出すと同時に破棄する。
     表示用 256px キャッシュ（`DropboxCacheStore`）は従来どおり別経路で、**構造を一切変えない**。
  5. **バッチ取得必須**。単発だと 1 枚 0.97 秒（計測値）＝62,744 枚で実働 17 時間になる。
     表示用と同じ `get_thumbnail_batch`（25 枚/リクエスト）に相乗りし、ADR-83 の `warmUp` フックから
     バッチ取得して `analysisCache` へ積む。取りこぼしは単発→表示用サムネへ段階フォールバック。
  6. **クラウド分だけ測り直す**（`cloudAnalysisVersion` / `FaceStore.resetCloudScans`）。
     ローカルは元から 1024px で処理済みなので、全再スキャン（`reset()`）だと 17,953 枚が無駄になる。
     refKey の接頭辞（"C-"）で選択的に破棄し、クラスタと命名は残す（再スキャン分は既存人物へ合流）。
- 結果: クラウドの顔がローカルと同等の水準で拾えるようになる見込み（3.8% → 29.7%）。
  トレードオフ: 一巡の通信量が 5.9GB（Wi-Fi 夜間限定）。ディスク常駐は増えない。
- 関連: `FaceQualityGate.minFacePixels/minFaceSide` / `DropboxPhotoStore.faceAnalysisThumbnails` /
  `CloudAnalysisImageCache` / `FacePerceptionAdapter.warmUp/detectFaces` /
  `PeopleEngine.migrateCloudAnalysisIfNeeded`。[[ADR-89]]（計測）・[[ADR-83]]（warmUp の土台）。
- 残課題: 埋め込み品質の下限（80 が妥当か）は未実測。大きく写った顔を 48/80/112px へ縮小して
  埋め込みのコサイン類似度を測れば確定できる（ネットワーク不要）。二段階スクリーニング
  （256px で顔ゼロを選別して 1024px 取得を半減）も未実施＝256px の検出漏れを測ってから。

## ADR-89 クラウド顔スキャンのサイズ設計は「実写真で歩留まりを測ってから」決める
- 状態: 計測中（設計判断は計測後）
- 文脈: クラウド顔スキャンの埋め込み到達率がローカル約 50% に対し **18% 以下**（diag-32）〜
  0%（diag-34 の 32 枚）。算術上の原因は明白に見える——サムネ 256px に対し顔の下限が
  **48px 絶対**なので、顔が画面比 **18.75%** 以上を占めないと通らない（家族写真は通常 5〜12%）。
  ログの `rawFaces=4 embedded=0` が示すとおり **Vision は 256px でも顔を検出できている**ので、
  問題は「検出」ではなく「埋め込みに使うには小さすぎる」。さらに顔モデルの入力は 112×112 なので、
  48px の顔は 2.3 倍に引き伸ばされ、**通った 18% すら品質が疑わしい**。
  ただしこれは全部**算術からの推定**であり、実データではない。CLAUDE.md の性能原則
  「まず 1 単位あたりの内訳を実測」に従い、**サイズを決める前に実写真で測る**。
- 決定（計測の設計）:
  1. **サイズごとに再ダウンロードしない**。計測サイズ（w1024h768）で 1 回だけ検出し、
     各顔の正規化矩形から「サイズ S での顔ピクセル = 正規化辺 × S」を算術で出す。
     1 枚の取得から 256/640/1024 の全歩留まりが得られる（`FaceYieldMeasurement.yields`）。
  2. **撮影日で層化抽出**する（`stratifiedSample`）。対象データは「子どもの誕生前から 10 歳頃まで」が
     順不同に混在しており、**時期で顔の写り方が変わる**。先頭 N 枚では偏るため、撮影日順に
     20 層へ等分して均等に抜く。撮影日不明も実データの割合ぶん含める（除外も偏り）。
  3. 判定は本番と**同じ Vision 設定**で行う（`FacePerceptionAdapter.observeFacesForMeasurement`）。
     品質ゲートは通さず生の観測を返し、採否はサイズ別に算術で評価する
     ＝「どの下限なら何 % 通るか」の曲線が出る（`pixelFloors` = 32/48/64/80/96/112）。
  4. 実行は Developer Options の「クラウド顔スキャンの歩留まりを計測」。標本 1,000〜20,000 枚。
     結果は診断ログと `Caches/face-yield-report.txt` に残し、**通信量の見積り**
     （全クラウド写真を同サイズで取った場合の GB）も併記する。
- 計測後に決めること: (a) 採用サムネサイズ、(b) 顔ピクセル下限（モデル入力 112px に対して
  どこまで拡大を許すか）、(c) 二段階スクリーニング（安価な 256px で顔の有無を選別し、
  顔がある写真だけ高解像度で取り直す）を採るか、(d) 高解像度は**保存せず**解析後に 256px へ
  縮小して既存キャッシュへ入れる案（ディスク常駐を増やさない・既存のキャッシュ構造を壊さない）。
- 関連: `FaceYieldMeasurement`（純・テスト済み）/ `FaceYieldMeasurementRunner` /
  `DropboxPhotoStore.measurementThumbnailData` / `FacePerceptionAdapter.observeFacesForMeasurement`。
  [[ADR-88]]（残課題として提起）。計測結果は `records/face-accuracy.md` に追記する。

## ADR-88 ピープルが「固まる・顔が出ない」の 3 原因（アバター取得・レビュー生成・起動時代入）
- 状態: 採用
- 文脈: 実機ログ diag-34 と利用者報告（「人物を確認しに行くと考えているようで動かない」
  「ピープルの後半の顔が出ない」「代表写真の選択で顔が出ない」）を突き合わせて 3 原因を特定した。
  いずれも**同じ「巨大コレクション／ネットワークを待つ」系統**。
- 決定:
  1. **顔アバターはキャッシュ済みサムネだけを使う**。`loadFaceAvatar` はコメントに
     「追加DL無し」と書きながら `thumbnail(for:)`（＝未キャッシュならバッチャ経由で
     **ダウンロードを待つ**）を呼んでいた。All Photos（86,153 枚）を開いた直後は
     サムネ行列が飽和し、実測で `thumb.missWaitMs` が**平均 10.3 秒**（Σ3,839,346ms/373件）。
     アバターがこの行列の後ろに並ぶため、顔が延々出なかった。
     `DropboxPhotoStore.cachedThumbnail(for:)`（メモリ/ディスクのみ・通信なし）を新設し、
     無ければ即 nil で諦めて `prefetch` で温めるだけにする（アバターは「出なくてもよい」情報）。
  2. **レビュー候補の生成を軽量クエリにする**。`reviewItems` は**全クラスタ**について
     `faces(inCluster:)` で全メンバーを materialize していた。母数が育つと 1 回 1.2〜1.4 秒
     （実測 `people.reviewItems` 1383/1198/1217ms・同時に前面ハング）。必要なのは
     「refKey の集合」と「代表顔 1 件」だけなので、`memberRefKeys`（`propertiesToFetch` の
     射影）と `bestCoverFace(inCluster:coverFaceID:)`（1 件引き／品質上位 16 件）に置き換える。
  3. **起動時の 68,200 件の刻印をオフメインへ**。`reflectCachedItems` は署名計算だけ
     detached で、`stampFavorites(raw)`（68,200 件の map）は @MainActor のまま実行しており、
     起動のたびにメインが**2.5〜3.4 秒**止まっていた（実測 diag-34・`cache.fetchItems` 直後）。
     署名と刻印を同じ detached でまとめ、メインは完成配列の代入だけにする。
     ADR-82 で `analysisOrderedRefKeys` を直したが、**同じ起動経路の別の場所**が残っていた。
- 結果: アバターは即座に出るか出ないかが決まり、待たされない。レビューのカード生成が軽くなる。
  起動時のメイン停止が消える。トレードオフ: 未キャッシュのクラウド顔はアバターが出ない
  （次回以降 prefetch で温まる）。代表顔は品質上位 16 件からの選択になる（実質同じ結果）。
- 関連: `PeopleSupport.loadFaceAvatar` / `DropboxPhotoStore.cachedThumbnail` /
  `FaceStore.memberRefKeys/bestCoverFace(inCluster:)` / `DropboxPhotoStore.reflectCachedItems`。
  [[ADR-82]]（同じ起動経路の先行修正）・[[ADR-85]]（クラウド顔が候補に入るようになった契機）。
- 残課題: **クラウド顔スキャンの費用対効果**。歩留まりはローカル約 50% に対し
  クラウドは約 18%（diag-32）〜0%（diag-34 の 32 枚）。256px サムネに対し顔の絶対 48px 下限が
  効きすぎている可能性が高い。62,770 枚を処理する価値があるか別途判断する。

## ADR-87 飢餓を「テストで固定」し「沈黙を検出」する（同型バグ 3 回への構造的対処）
- 状態: 採用
- 文脈: 同じ飢餓バグを 3 度踏んだ（ADR-72 バックアップ / ADR-85 埋め込み / ADR-86 キャプション）。
  いずれも「A の間は B をやらない」と書き、A（母数が数万枚）が終わらないため B が永久に
  動かなかった。3 回とも**発見はユーザーの指摘か手作業のログ突き合わせ**で、テストは 1 件も
  落ちていない。原因は 2 つ: (1) 判定ロジックが各所にインラインで散っていてテストできない、
  (2) 動いていないことが**沈黙**として現れ、ログを見ても異常に見えない。
- 決定: 「順番回しの方針」と「停滞の検出」を**純ロジックとして切り出し、テストで固定**する。
  1. **`TurnTaking`（MosaicSupport・純）**: 「残作業があり、連続 `maxDeferrals` 回譲ったら
     **必ず**順番を取る」判定に集約。バックアップとキャプションの両方がこれを使う。
     テストで固定した不変条件: **相手が永久に終わらなくても `maxDeferrals + 1` 窓以内に
     必ず順番が来る**（`neverStarves`）。100 窓シミュレーションで公平な配分（4 窓に 1 回）も固定。
     過去バグの形（上限なし＝永久に取れない）も対比テストにした。
  2. **`AnalysisStallCheck`（PerceptionCore・純）**: 「残作業あり × 最終処理から N 日超」の
     パスを洗い出す。既存の `AnalysisActivity`（各パスの最終処理時刻・実装済み）を入力にする。
     夜間 BGTask の各窓で評価し、**停滞があるときだけ** `analysis STALLED — embeddings(pending=…
     idle=21d)` を診断ログへ出す（健全ならログを汚さない）。一度も動いていないパスは
     初回起動時刻からの経過で判定し、新規インストール直後の誤検知を避ける。
     テストは**過去 3 件の実バグをシナリオとして再現**（ADR-85 の「タグは進むが埋め込みだけ停滞」・
     ADR-86 の「VLM 同梱済みなのに未実行」）。
- 結果: 同型の飢餓はテストで検出できる。実機では停滞が自動でログに出るので、次は沈黙のまま
  数週間放置されない。トレードオフ: 停滞判定は「3 日」という経験則（設定不可）。
- 関連: `TurnTaking` / `AnalysisStallCheck` / `HeavyWorkScheduler.runHeavyWork/logStalledPasses`。
  [[ADR-72]]・[[ADR-85]]・[[ADR-86]]（対処した 3 件）。
- 教訓: **同じ形のバグを 2 度踏んだら、個別に直さず「その形」をテストで固定する**。
  3 度目でようやくそうした。加えて、**沈黙は自動で検出しにいく**——「ログに出ていない」を
  人間が気づける前提にしない。

## ADR-86 VLM キャプションの飢餓解消と、バックアップ実行の可観測性
- 状態: 採用
- 文脈: ADR-85 の修正後、**背景処理を全部列挙して過去ログ 5 本と突き合わせる**棚卸しを行った。
  | 処理 | 実行痕跡 | 判定 |
  |---|---|---|
  | Vision シーンタグ | 全ログに `tags:` | ○ |
  | CLIP 埋め込み | `embed: batch` が diag-28 以降 0 | ✗ → ADR-85 で修正 |
  | **VLM キャプション** | **`captions:` が全ログで 0** | **✗（本 ADR）** |
  | 顔スキャン | `faces.detect` 多数 | ○ |
  | アルバム生成 / 同期 / BGTask | `generate:` `longpoll` `bgtask:` | ○ |
  | 地名の Apple 補正 | ログ無し（収束後は無出力＝正常） | ○ |
  | **バックアップ** | **`bgtask: backup turn` はあるが実行の記録が無い** | **判定不能（本 ADR）** |
- 決定:
  1. **キャプションの飢餓を解消**。VLM は同梱済み（`VLMDecoder.mlpackage` 等）なのに一度も
     動いていなかった。原因は bgfill の `if !isScanningFaces` ガード——VLM(≈877MB)＋顔モデルの
     同時常駐を避けるための排他だが、顔スキャンの母数は数万枚（クラウド 67,377 枚）あるため
     **数週間ずっと `isScanningFaces == true`** で、キャプションは永久に順番が来なかった。
     対処は ADR-72/85 と同じ「順番を必ず回す」: 連続 3 窓譲ったら、その窓は**顔スキャンを
     起こさず**キャプションに明け渡す（`maxCaptionDeferrals`）。顔スキャンは差分なので次窓で続く。
     排他の意図（モデル同時常駐の回避）は保たれる。
  2. **バックアップの実行結果を診断ログへ**。`BackupEngine.addLog` はアプリ内バッファ
     （Developer Options 表示用）にしか入らず、`bgtask: backup turn` が出ていても
     「実際に走った / 宛先未設定でスキップ」が診断ログから**区別できなかった**。
     開始・スキップ理由・完了（uploaded/skipped 件数）を `Diagnostics.mark` に残す。
- 結果: キャプションが動き出す。バックアップの実行有無がログで追える。
- 関連: `HeavyWorkScheduler.runHeavyWork` / `AutoAlbumEngine.pendingCaptionCount` /
  `BackupEngine.startNightlyIfEnabled` / `BackupRunner`。[[ADR-72]]・[[ADR-85]]（同型の飢餓）。
- 教訓: **排他は飢餓とセットで設計する**。「A の間は B をやらない」と書いたら、必ず
  「A が終わらないとき B はどうなるか」を問う。本プロジェクトはこれを 3 度繰り返した
  （ADR-72 バックアップ / ADR-85 埋め込み / ADR-86 キャプション）。
  また**動いていないことは沈黙として現れる**——「ログが出ていない」は正常の証拠にならない。
  定期的に「動くべき処理の一覧」と実ログを突き合わせる棚卸しが要る（本 ADR の発見手順）。

## ADR-85 背景解析は各フェーズに必ず順番を回す（タグが埋め込みを飢餓させていた）
- 状態: 採用
- 文脈: 実機ログ diag-33 の検証で 3 件の欠陥が見つかった。うち 1 件は**重大**:
  **CLIP 埋め込みが数週間 1 枚も処理されていなかった**。
  | ログ | 未タグ | 未埋め込み | `embed: batch` |
  |---|---|---|---|
  | diag-31 | 33,662 | 43,611 | 0 回 |
  | diag-32 | 33,610 | 43,611 | 0 回 |
  | diag-33 | 24,505 | **43,626** | 0 回 |
  タグは 9,000 枚以上進む一方、未埋め込みは**減るどころか増えて**いた（新規写真ぶん）。
  原因は `scheduleBackgroundFill` の構造で、`tagUnprocessed` が**上限なし**（todo 全量）のため、
  タグが 24,505 枚すべて終わるまで下の埋め込みループに到達しない。夜間の窓は数分〜数十分で
  必ずタグの途中で expire するので、埋め込みは永久に順番が来なかった。
  **ADR-72 で直した「バックアップが埋め込みに飢餓する」とまったく同じ構造**（対処も同じ）。
- 決定:
  1. **タグに 1 回あたりの上限を設ける**（`tagBatchesPerRun = 40`＝8 枚×40 ≒ 320 枚）。
     超えたら打ち切って埋め込み・キャプションへ順番を回す。次の実行で続きから進むので
     総量は変わらず「どれも少しずつ進む」状態になる。ログに `remaining=` を出し、
     上限で打ち切ったのか本当に終わったのかを**ログだけで区別**できるようにする。
  2. **顔スキャンの候補にクラウドが入らない問題を修正**。`analysisOrderedRefKeys` は
     `dropboxStore.items` を読むが、起動直後はまだロードされておらず（2 秒後にロードされる）
     クラウド 6.7 万枚が丸ごと候補から漏れていた（diag-33: candidates=6699＝ローカルのみ）。
     `DropboxCloudPhotoProvider.cloudPhotos()` が既に持っている「空ならロード」ガードを揃える。
  3. **ベストショット判定をオフメインへ**。`beautifulPhotoKeys` が @MainActor で 62,330 件の
     filter・集合演算・ソートを回しており、フィルタ ON のたびメインが 1.4 秒止まっていた
     （diag-33 の唯一の前面ハング）。`CLAUDE.md` の性能原則「巨大コレクションを MainActor に
     通さない」の違反を自分のコードでやっていた。
- 結果: 埋め込みが動き出す。起動時からクラウドも解析対象に入る。ベストショットの引っかかりが消える。
- 関連: `AutoAlbumEngine.scheduleBackgroundFill/beautifulPhotoKeys` / `TagTagger.tagUnprocessed` /
  `PeopleSupport.analysisOrderedRefKeys`。[[ADR-72]]（同型の飢餓）・[[ADR-78]]・[[ADR-83]]。
- 教訓: **逐次フェーズは、前段が終わらない限り後段が始まらない**。窓が有限（夜間 BGTask）なら
  「前段が終わる」こと自体が起きない。フェーズを並べたら必ず**各段に上限を設けて順番を回す**。
  同じ罠を ADR-72 で踏んでいたのに、別の場所で繰り返した。

## ADR-84 バックアップは「次の 1 枚の読み込み」をアップロード中に走らせる
- 状態: 採用
- 文脈: ADR-83 の反省から `CLAUDE.md` に加えた性能原則「I/O と計算（通信）は重ねる」で
  コード全体を点検したところ、`BackupRunner` のアップロードループが同じ構造だった:
  `for asset in pending { read(asset)（ディスク I/O）→ upload(data)（HTTP）}` が完全な直列で、
  読み込み時間がまるごと待ち時間になっていた。写真 1 枚は数 MB なのでアップロードが支配的
  （読み:アップロード ≒ 1:3〜1:10 と推定）だが、その 1 割強は隠せる。
- 決定: **先読みはちょうど 1 枚**。現在の 1 枚をアップロードしている間に、次の 1 枚を読む。
  - メモリ増を**写真 1 枚分**に抑える（2 枚同時保持まで）。夜間 BGTask は jetsam 上限が厳しく、
    過去に generate のピークでアプリごと kill された（ADR-72）ため、深い先読みは採らない。
  - **ゲートが開いているときだけ先読みする**。電源/回線待ちで止まる間に読み込んだデータを
    抱え続けない（待ちは数時間になり得る）。
  - キャンセル・fatal では先読みハンドルを捨てる。検証・記録（ADR-40 の hash 照合と
    「済み」記録）の順序と条件は**一切変えない**——ここは失敗すると写真喪失に直結するため、
    読み込みの出所を差し替えるだけに留める。
  - 併せて `backup.readMs` / `backup.uploadMs` を計測に追加。**推定でなく実測**で
    「どちらが支配的か」を次回のログから判断できるようにする（ADR-83 の教訓）。
- 結果: 読み込みがアップロードの裏に隠れる。トレードオフ: 常駐メモリが写真 1 枚分増える。
- 関連: `BackupRunner.uploadAll/readAsset/uploadOne`。[[ADR-83]]（同じ原則の適用）・[[ADR-72]]（BG メモリ）。

## ADR-83 クラウド解析はサムネを「バッチ単位で先行取得」する（1 枚ずつ取らない）
- 状態: 採用
- 文脈: AI 解析を速くしたい。まず**実測の内訳**を過去ログから集計した（下表）。
  1 枚あたりの中央値:
  | 機能 | 端末写真 | クラウド写真 |
  |---|---|---|
  | 顔認識（検出＋埋め込み） | 235ms | **950ms** |
  | Vision シーンタグ（＋OCR/美的） | 434ms | 650ms |
  | CLIP 埋め込み | 342ms | — |
  | VLM キャプション | 計測なし（想定 1〜2 秒） | — |
  顔認識だけは画像ロードと推論が分離記録されており、内訳がはっきりした:
  | | 画像ロード | 推論 |
  |---|---|---|
  | 端末（diag-28/29） | 25〜55ms | 87〜122ms |
  | **クラウド（diag-31/32）** | **643〜829ms** | 69〜96ms |
  **推論の速度は端末・クラウドで変わらない**（70〜120ms）。差は全部サムネの入手コストで、
  クラウド解析の 85〜90% がダウンロード待ちだった。推論を最適化しても意味がない。
  真因は、解析が 1 枚ずつ `thumbnail(for:)` を呼んでいたこと。Dropbox のサムネ取得は
  25 枚/リクエスト・並列のバッチ API（`DropboxThumbnailBatcher`）なのに、1 枚ずつ待つと
  **バッチの利点がまったく効かず**、往復が推論と直列に並んでいた。
- 決定: **バッチ単位でサムネをまとめて先行取得する**。
  - `BackgroundTrickle.run` に `warmBatch` フック（バッチ取得直後・**推論の前**に 1 回）を追加。
  - 3 つの知覚 seam（`FacePerceptionProvider` / `TagPerceptionProvider` /
    `PhotoPerceptionProvider`）に `warmUp(refKeys:)` を追加（既定 no-op＝ローカル専用実装は無変更）。
    **即座に返る**契約にし、`await` しない＝1 枚目の処理とダウンロードを重ねる。
  - 実体は `DropboxPhotoStore.prefetch`（低優先 LIFO プール・25 枚/リクエスト・並列）。
    回線ポリシーは prefetch 側（`speculativeFetchAllowed`＝ADR-81）がそのまま守る。
  - **さらに 1 バッチ先読みする（追記）**。当初の実装は「現バッチをまとめて取る」だけで、
    バッチの先頭で毎回ダウンロード待ちが露出していた（バッチ間が重ならない）。
    各タガーは自分のキューを持っているので、`nextBatch` の中で**次バッチ分**も
    `warmUp` する。推論は ANE ゲートで直列＝1 バッチ約 1.6 秒（16×100ms）かかるため、
    その裏で次バッチのダウンロード（約 0.8 秒）を**完全に隠せる**。
    ダウンロードは URLSession＝ANE ゲートの対象外なので、推論と真に並行して進む。
    定常状態ではダウンロード待ちがゼロになる（初回バッチのみ露出）。
- 結果: クラウド 16 枚バッチが「16×800ms の直列待ち（12.8 秒）」から「1 回の並列ウェーブ
  ＋16×100ms の推論」へ。残り 67,377 枚のクラウド顔スキャンで効く。
  トレードオフ: 先行取得したサムネはキャッシュを消費する（LRU 管理下なので上限は不変）。
- 関連: `BackgroundTrickle.warmBatch` / 各 `warmUp` / `warmCloudPaths` /
  `makeCloudThumbnailWarmer`。計測の詳細は [background-behavior.md](background-behavior.md)。
  [[ADR-81]]（先読みの回線ポリシー）。
- 残課題: 効果は次の実機ログで検証する（`face.photoMs` のクラウド中央値が 950ms から
  どこまで下がるか）。VLM キャプションは未計測のまま。

## ADR-82 起動時のメインスレッド専有を解消し、計測を前面/背面で分ける
- 状態: 採用
- 文脈: ADR-79/80 の後、実機ログ（diagnostics-32）で復帰時の暴走は解消できた
  （`generate: aborted` が 4 回・復帰の 0〜1 秒後）。しかしログには 39 件のハングが残り、
  最大 28.5 秒だった。分類したところ**性質の違う 3 つ**が混ざっていた。
- 決定:
  1. **背面の「ハング」は計測ノイズなので分離する**。39 件のうち **31 件（最大 28.5 秒）が
     アプリ背面時**のものだった。iOS は背面でメインランループを絞り、必要なら中断する。
     `ProcessSuspension` は**正式な中断**しか捉えられないため、単なる throttle が
     「メインが 28 秒ブロック」として記録され、実機ログを読み誤らせていた（実際に誤読した）。
     `MainThreadWatchdog` に前面/背面の状態を持たせ（`BackgroundYield.isAppActive` の
     `didSet` から自動同期＝伝え忘れが起きない）、背面のサンプルは `pings/max` に混ぜず
     `bgStalls=N` として別枠で数える。即時ログ（`hang`/`hang.begin`）も前面のみに出す。
     前面のログには `(foreground)` を付けて出典を明示する。
  2. **解析候補の並べ替えをオフメインへ**。起動時の残り 2.7〜3.2 秒の主犯は
     `analysisOrderedRefKeys`（@MainActor）だった: `cloudImageRefKeys` が 6.8 万件の
     ソート＋map、`AnalysisOrder.ordered` が 8.6 万件のソート（比較ごとに接頭辞判定と
     Set 参照）を**メインで**実行していた。`dropboxStore.items` のスナップショット（COW＝
     取得は安価）を渡して全体を `Task.detached` へ移す。夜間 BGTask も同じ経路を通るため、
     背面での長い停止（09:40 の 28.5 秒＝クラウド 6.7 万件の候補構築）も同時に軽くなる。
  3. **存在しないバックアップメタデータを毎起動探しに行かない**。バックアップ未使用の
     ユーザーは v1/カタログの 4 パスとも存在せず、`get_metadata` が 409 を返すまで
     1 回 2.8〜3.3 秒かかっていた（毎起動 4 回）。「不在」を記録して TTL(24h) 内は
     再問い合わせしない。無効化は (a) 明示更新（バックアップ画面を開く＝`force: true`）、
     (b) **バックアップがメタデータを書いた直後**（`invalidateBackupMetadataAbsence()`）。
     (b) が無いと初回バックアップ後も最大 24 時間バッジが出ない＝必須。
- 結果: 前面のハングは最大 3.2 秒 → 起動経路の主因を除去。ログは前面の数値だけを示すので、
  以後の実測が読み違えようがなくなる。トレードオフ: 他端末が書いたバックアップメタデータの
  検出が最大 24 時間遅れる（アプリ内の操作・バックアップでは即時）。
- 関連: `MainThreadWatchdog` / `BackgroundYield.isAppActive` / `PeopleSupport.analysisOrderedRefKeys` /
  `DropboxPhotoStore+BackupMetadata` / `BackupRunner`。[[ADR-71]]（UI を軽く保つ）・[[ADR-79]]。
- 教訓: **計測が信用できないと対策が的を外す**（前回は背面のノイズを体感と誤読しかけた）。
  センサーには「その値が体感に効く文脈か」を持たせる。

## ADR-81 クラウド画像の「先読み」を回線ポリシーの対象にする（閲覧中の取得は対象外）
- 状態: 採用
- 文脈: ADR-80 で動作の早見表を作る過程で、**クラウドサムネ/フル画像の先読み（prefetch）が
  電源・回線ポリシーを一切見ていない**ことが分かった。`DropboxPhotoStore.prefetch` /
  `prefetchFullImage` はゲートなしでバッチャ・API を叩いていた。
  これは設計意図に対するバグ: `BackgroundDataPolicy.off` の定義は「背景通信を一切行わない
  （**閲覧時の手動取得のみ**）」であり、投機的な先読みは「閲覧時の手動取得」ではない。
  実害は「Wi-Fi のみ」設定でも、クラウドのグリッドをスクロールしただけでモバイル通信を
  使ってしまうこと（サムネは 1 枚が小さくても数万件規模のライブラリでは無視できない）。
- 決定: 通信を**性質で 2 つに分け**、先読みだけをゲートする。
  - **閲覧中の取得**（見えているセルのサムネ・開いた写真のフル画像）＝前景要求。**ゲートしない**。
    ユーザーが今見たいものを止めるとアプリが壊れて見える。`.off` の定義もこの経路は許している。
  - **先読み**（スクロール先のサムネ・次ページのフル画像）＝投機的な自動通信。
    **回線ポリシーに従う**（`NetworkStateMonitor.speculativeFetchAllowed()`）。
    止まっても表示は続く（オンデマンド取得に degrade するだけ）＝安全に絞れる。
  - 先読みに**電源ポリシー（充電中のみ）は課さない**。先読みはユーザーがスクロールしている
    最中にだけ起きる前景動作で、電源条件で止めると「バッテリー駆動では常にサムネが出遅れる」＝
    通常利用が壊れる。電源ポリシーが守るのは背面での消耗であり、画面を見ている間の消費は
    ディスプレイが支配的なため絞る利得が小さい。ただし**低電力モード中は先読みしない**
    （iOS の慣習＝明示的な省電力要求のときは投機的処理を止める）。
- 結果: 「Wi-Fi のみ」「オフ」設定が先読みにも効くようになり、意図しないモバイル通信が消えた。
  トレードオフ: モバイル回線での閲覧はサムネ表示がやや遅くなる（オンデマンド取得になるため）。
- 関連: `NetworkStateMonitor.speculativeFetchAllowed` / `DropboxPhotoStore+Images`。
  早見表は [background-behavior.md](background-behavior.md)。[[ADR-80]]（4 軸の設定）。

## ADR-80 重い処理の実行条件を「5 段階の梯子」から「4 軸の独立設定」へ
- 状態: 採用 → 改定（→ ADR-195: 「控えめ」の軸を廃止）→ **復活（→ ADR-197）**: 肯定形の「アプリを開いている間も解析する」として戻した（既定 ON）。軸を混ぜない原則は一貫して不変
- 文脈: ADR-25 で入れた `HeavyWorkTiming` の 5 段階（paused / nightly / chargeActive / battery /
  unlimited）は、**3 つの異なる軸を 1 本の梯子に混ぜて**いた: (1) 前面アイドル時に動かすか、
  (2) バッテリーでも動かすか、(3) モバイル回線でも動かすか。実害が 2 つ出た:
  - **組み合わせが選べない**: 「電源接続は必須のままモバイル回線だけ許す」が表現できない
    （`unlimited` にすると同時にバッテリー実行も解禁される）。
  - **電源・回線が二重設定**: 既存の `PowerStateMonitor.policy` / `NetworkStateMonitor.policy`
    （設定 → Background & Battery）と条件が重複し、どちらか厳しい方が効く＝**片方を緩めても
    動かない**という分かりにくさ。どの条件で動いた/動かなかったのか追跡も困難だった。
  - さらに ADR-79 追記のとおり、`nightly` 以外を選ぶと**復帰直後に前面で generate が走る**
    事故につながっていた（アイドル判定が復帰を無視していたため）。
- 決定: 梯子を解体し、**4 軸を独立した設定**にする。
  | 軸 | 設定 | 既定 |
  |---|---|---|
  | 自動処理する/しない | `HeavyWorkTiming`（enabled / paused） | enabled |
  | **前面でも動かすか** | **`conservativeKey`「画像分析を控えめに動かす」（新設）** | **ON＝前面では動かさない** |
  | 電源 | `PowerStateMonitor.policy`（充電中のみ / 常に / オフ） | 充電中のみ |
  | 回線 | `NetworkStateMonitor.policy`（Wi-Fi のみ / セルラーも / …） | Wi-Fi のみ |
  - `HeavyWorkTiming.allows` は電源・回線の**状態判定を持たず**、呼び出し側（`BackgroundYield`）が
    各モニタの `backgroundAllowed()` / `networkAllowed()` の結果を渡す純関数になる。
    これで判定が一意になり、二重設定が消える。
  - 「控えめ」ON は旧 `nightly` と同じ厳しさ（前面では一切動かさない）。OFF のときだけ
    `foregroundIdleSeconds`(20 秒) 放置で前面でも動く。**既定 ON**＝アプリの趣旨（閲覧を邪魔しない）。
  - 低電力モードは全設定で常時ブロック（安全弁）。電源ポリシーが `always` でも尊重する。
  - デバッグ経路（`debugForceHeavyWork` / 「今すぐ処理」の `manualBoostUntil`）は
    `heavyWorkAllowed` の冒頭で早期 return するため、控えめ設定の影響を受けない
    （＝Developer Options から学習・画像分析をその場で呼べる）。
  - BGTask の `requiresExternalPower` も電源ポリシーに従う（旧: 段階 < battery）。
  - **移行は 1 度だけ**（`migrationKey`）。旧段階から意思を読み取って各軸へ写す（純関数
    `migrationPlan`・テスト済み）: paused→自動処理オフ / nightly→既定のまま /
    chargeActive→控えめ OFF / battery→＋電源「常に」 / unlimited→＋回線「セルラーも」。
    電源・回線は**緩める方向にだけ**書く（ユーザーが個別に絞っていた設定を上書きしない）。
- 併せて CLIP の起動コストを 2 点修正:
  - **表示ラベラの事前ウォームをゲート内へ**。約300語の text encode と CLIP テキストタワーの
    ロードが、`scheduleBackgroundFill` の中で**ゲート判定の外**に置かれており、起動直後
    （pause=true）でも走っていた（新規インストール直後は実測 23 秒）。ゲートが閉じていれば
    起動しない・復帰時は `stopBackgroundWork` でキャンセルする。未ウォームでも実害はない
    （`isReady` が false なら insight は CLIP ラベルを飛ばし Vision タグだけで即返す）。
  - **ロード計測の中断汚染を除去**。`loadStamp` は壁時計 `Date()` 差分なので、バックグラウンド
    遷移をまたぐと背面にいた時間まで含む（23,438ms がまさにこれ）。`ProcessSuspension.epoch` を
    控え、中断をまたいだサンプルは数値を出さず "spans suspend — duration unreliable" と記す。
- 結果: 設定の意味が一意になり、「どの条件で動くのか」が説明可能になった（下表）。既定では
  アプリ使用中に重い処理が一切走らない。トレードオフ: 旧 5 段階に慣れたユーザーには UI が変わる
  （移行で挙動は保たれる）。
- 関連: `HeavyWorkTiming` / `BackgroundYield` / `PowerStateMonitor` / `NetworkStateMonitor` /
  `AutoAlbumSettingsView` / `CoreMLModelSupport.loadStamp`。
  [[ADR-25]]（実行方針の原典）・[[ADR-79]]（復帰時の停止）。
  **動作の早見表は `docs/architecture-note/records/background-behavior.md`**（正本）。

## ADR-79 フォアグラウンド復帰で夜間処理を「譲る」から「止める」へ
- 状態: 採用
- 文脈: 他アプリから戻ると数秒〜数十秒固まることがある（ユーザー報告）。原因はモデルの
  ロードそのものではなく、**復帰しても夜間処理（BGTask ルーチン）が生き続ける**こと。
  従来の設計は「ゲート（`BackgroundYield`）が閉じる → 各トリクルが `waitWhilePaused` で眠る」
  だけで、以下が残っていた:
  1. **実行中の 1 単位は完走する**。停止判定は単位ごと・推論の前なので、復帰の瞬間に走っていた
     単位は最後まで走る。VLM キャプションは 1 枚数十秒級で、その間 ANE/CPU が飽和する。
  2. **眠っている間もモデルを抱える**。VLM（≈877MB）が常駐したままで、復帰直後の高メモリ →
     圧迫 → サムネキャッシュ縮小 → ディスク再デコードの連鎖を招く。
  3. **`generate` は単位分割が無く途中で譲らない**（18 秒超・ピーク 550〜880MB の一枚岩）。
  「充電中にアプリを切り替えたときだけ BG 窓が開く」ため、症状が出たり出なかったりする。
- 決定: 復帰時は**譲る（pause）ではなく止める（cancel）**。
  - `HeavyWorkScheduler.stopForForeground()` を `scenePhase == .active` で呼ぶ。BGTask 本体の
    Task を cancel し、加えて**fire-and-forget で起こしたエンジン側のタスク**（`PeopleEngine.stopScan`
    / `AutoAlbumEngine.stopBackgroundWork`）にも停止を伝える（構造化されていないため cancel が
    伝播しない）。完了は待たない（復帰時にメインを塞がないため）。
  - トリクルは 1 単位ごとに `Task.isCancelled` を見るので、実行中の 1 枚が終わり次第すぐ降りる。
    各処理は**差分ベース**なので次の夜間窓で続きから再開する（取りこぼしなし）。
  - `BackgroundTrickle.waitWhilePaused` に **`onPauseBegin`**（譲り開始時に 1 回だけ）を追加し、
    キャプションは譲りに入った瞬間に VLM を解放する。
  - `generate` はステップ境界（enrich 後 / prune 後 / 保存前）で `Task.isCancelled` を確認して降りる。
    あわせて **86k 件の台帳取得を detached の中へ移す**（従来は @MainActor でいったん受けており、
    実測でメインが ~10 秒止まっていた＝diagnostics-30 の `hang main=10342ms` が step4 と一致）。
  - `PeopleEngine` はスキャン**世代**（`scanGeneration`）を導入。停止直後に始まった新スキャンの
    ハンドル/進捗フラグを、遅れて終わる旧タスクの末尾処理が踏まないようにする（二重起動の防止）。
  - D: `scenePhase` 遷移ごとに実行中の作業を診断ログへ 1 行残す（`scene: active — running=[…]`）。
    「復帰の瞬間に何が走っていたか」をログだけで特定できるようにする。
- 結果: 復帰後に残るのは実行中 1 単位ぶんだけになり、モデルは即解放される。トレードオフ:
  夜間処理の進みは（復帰のたびに中断されるぶん）わずかに遅くなるが、差分再開なので総量は変わらない。
- **追記（実機ログ diagnostics-31 で判明・上記だけでは不十分だった）**: 復帰時のカクつきは
  BGTask ではなく**前面の定期ループ**が主因だった。3 点を追加で修正:
  1. **復帰そのものを「操作」として記録する**（`noteUserInteraction()` を `.active` で呼ぶ）。
     `idleSeconds` は離席前の最終タッチからの経過なので、戻った瞬間は「20 秒以上アイドル」と
     判定され、`HomeView` の定期ループ →`refreshIfNeeded` がその場で `generate` を起動していた
     （実測: 復帰と同時に generate が 22.8 秒・メインが 2.4s/1.7s/5.9s/3.9s ブロック）。
     ※ `nightly` 設定なら前面実行は元から禁止だが、それ以外の段階では起き得る。
  2. **generate を世代（`workEpoch`）で中断可能に**。generate は**呼び出し側のタスク上で走る**
     （前面ループから呼ばれる）ため `Task.isCancelled` では止められない。`stopBackgroundWork()`
     が世代を進め、実行中の generate はステップ境界で自ら降りる。
  3. **地名補正（ADR-76）の空振りを弾く**。`refinePlaceNames` は台帳 86k 件を丸ごと読むのに
     定期ティックのたびに実行され、**単独で 10.5 秒のメインハング**を起こしていた
     （diagnostics-31 の `hang main=10581ms`）。「写真件数＋補正世代」の署名が前回と同じなら
     読まずに帰る（デバッグボタンは `force: true` で従来どおり実行）。
- 関連: `HeavyWorkScheduler.stopForForeground/noteScenePhase` / `MosaicPhotosApp` /
  `AutoAlbumEngine.stopBackgroundWork/generate` / `PeopleEngine.stopScan` /
  `BackgroundTrickle.onPauseBegin` / `TagTagger.captionUnprocessed`。[[ADR-25]]（実行方針）・
  [[ADR-71]]（UI を軽く保つ）・[[ADR-74]]（モデル解放）。

## ADR-78 ベストショット（きれいな写真）フィルタ＝Vision 美的スコアのしきい値判定
- 状態: 採用
- 文脈: 「よく撮れている写真だけ見たい」。候補は NIMA 等の外部モデル・LAION 線形ヘッド
  （CLIP 埋め込みの内積）・OS 内蔵の Vision 美的スコア。本アプリは夜間タグ付けで
  **`VNCalculateImageAestheticsScoresRequest`（-1〜1）を全写真の台帳（TagsV1）に永続化済み**
  （従来はカバー選定の加点のみに使用）。
- 決定: **既存の Vision スコアをしきい値判定**して「ベストショット」集合を作る（外部モデル追加なし）。
  - 判定＝`aesthetic >= PhotoQuality.beautifulThreshold`（0.5・一元管理）**かつ**スクショでない
    （台帳の isScreenshot）。スコア未付与（解析待ち）は含めない＝夜間解析が進むほど増える。
  - 供給経路: `AutoAlbumEngine.beautifulPhotoKeys()`（refKey 集合）→ アプリの
    `photoQualityProvider` 環境クロージャ（**集合をキャプチャした同期判定クロージャ**を返す・
    id は `canonicalRefKey` で正規化）→ PhotoSourceKit の `PhotoFilter.beautifulOnly`。
    フィルタ ON のときだけ台帳を読む。判定 nil（読み込み中/未注入）は**素通し**（空表示で
    固まって見せない）。プロバイダ未注入ならフィルタ欄自体を出さない（レイヤー分離維持）。
  - しきい値の校正・技術シグナル合成（ブレ/目つぶり等）・isUtility 除外・LAION 第二スコアは
    将来の拡張（お気に入りを正解データにした AUC 校正で判断）。
- **追記（同日）: 固定しきい値 0.5 は実機で全滅**（「ベストショットはまだありません」）。Vision の
  overallScore は日常写真だと 0 前後に集まり 0.5 超えは稀（実障害）。**分布適応**へ変更:
  しきい値＝「そのライブラリのスコア上位 `topFraction`(20%) の境界」を [floor 0.2, ceiling 0.6] に
  クランプ（`PhotoQuality.adaptiveThreshold`・純・テスト付き）。`beautifulPhotoKeys` は実測ログ
  （scored/p50/p90/max/thr/best 件数）を diagnostics に残し、以後の校正の判断材料にする。
- 結果: 追加の解析コストゼロ（記録済みスコアの fetch 1 回）で全ソースのグリッドに
  「ベストショットのみ」フィルタが付く。しきい値はライブラリの分布に自動で追従する。
- 関連: `PhotoQuality` / `TagStore.refKeys(aestheticAtLeast:)` / `AutoAlbumStore.screenshotRefKeys` /
  `AutoAlbumEngine.beautifulPhotoKeys` / `PhotoFilter` / `photoQualityProvider`。[[ADR-24]]（タグ台帳）。

## ADR-77 フル画面写真のピンチ拡大（UIScrollView ベース・1x でのジェスチャー透過）
- 状態: 採用
- 文脈: フル画面ビューに Apple 写真アプリ同等のピンチ拡大が欲しい。ただしフル画面には既に
  3 つのドラッグ系ジェスチャーが同居している（横＝TabView ページング／縦＝情報パネルへのスクロール／
  最上部の下引っ張り＝閉じる）。拡大のパンがこれらと衝突すると、拡大中に誤ってページが送られたり
  画面が閉じたりして使い物にならない。SwiftUI の MagnificationGesture 自作はラバーバンド・慣性パン・
  ズームアンカーの品質が出しにくい。
- 決定: **UIScrollView をズームコンテナにする**（`ZoomableImageView`・UIViewRepresentable）。
  調停の核心は「**1x のときはパンを消費しない**」: contentSize ≦ bounds かつ alwaysBounce=false だと
  UIScrollView の pan は不成立になり、外側（ページング・縦スクロール・下引っ張り）へ素通しされる。
  拡大中は pan を消費（端はラバーバンド）＝誤操作が構造的に起きない。加えて:
  - ズーム状態は `onZoomChanged` で親へ報告し、`FullPhotoView` が縦スクロールを `scrollDisabled` で停止
    （下引っ張り閉じも連動して止まる）。
  - シングルタップ＝**没入モード**（上下バー＋ステータスバーをフェード・ページ送りしても維持）。
    ダブルタップ失敗待ちで共存（約 0.3s 遅延は Apple 写真と同じトレードオフ）。
  - ダブルタップ＝1x ⇄ 2.5x（タップ位置中心）。最大倍率＝**画像ピクセル 1:1**（最低 3x 保証・
    それ以上はボケるだけなので止める）。1x 未満で指を離すと 1x へスプリング（bouncesZoom）。
  - ページが画面外へ出たらズームを 1x へリセット（Apple 写真と同じ）。顔ハイライト枠は
    ズームコンテンツ内のオーバーレイなので拡大に追従する。
  - 数値計算（フィットサイズ・最大倍率・中央寄せ）は `ZoomMath`（純）に分離して macOS テスト。
- 結果: ピンチ/ダブルタップ/パン/没入モードが Apple 写真同等の操作感で動き、既存ジェスチャーとは
  ズーム状態で排他になる。トレードオフ: 拡大中はページ送り不可（ズームを戻せば可・誤送り防止を優先）。
- 関連: `ZoomableImageView` / `ZoomMath` / `FullPhotoView` / `PhotoPageView`。[[ADR-71]]（UI 応答性）。

## ADR-76 地名補正を「写真のあるセル」へ拡張し台帳へ伝播（＋オフライン距離格下げ）
- 状態: 採用
- 文脈: ADR-68 の Apple 補正は実装後も体感精度が上がらなかった。調査で 2 つの構造欠陥が判明
  （事例「Apple の地名補正が既存写真に届かなかった」参照）: (1) 補正対象がトリップ代表座標＝
  メンバー座標の**単純平均**で、複数都市の旅行では無意味な地点になり写真の属するセルに届かない。
  (2) `EnrichedPhoto.placeName` はエンリッチ時に台帳へ固定され、補正後の `generate()` も古い名前で
  組み立てるため**補正が既存写真に伝播しない**。また一次バックエンドのオフライン都市 DB
  （cities15000 最近傍）は、遠い最近傍にも自信満々に**隣の大都市名**を付けていた。
- 決定: 3 点セット。
  1. **補正対象＝実際に写真があるグリッドセル**（`PlaceRefinement.cellCentroids`）。台帳の座標付き
     写真をセル集約し、**枚数の多い順**に重心座標を CLGeocoder へ渡す（1 晩 300 件・1.2s 間隔は
     ADR-68 のまま）。数千セルでも数晩で収束し、以後は refined スキップ＝無コスト。
     トリップ代表座標の補正は撤去。
  2. **台帳伝播**（`PlaceRefinement.ledgerChanges` → `AutoAlbumStore.updatePlaces`）。補正後、resolver
     キャッシュの現在値と台帳を突き合わせて placeName/country の差分だけ更新し、trips を再生成。
     伝播は**毎晩必ず**行う（差分計算は安価）＝過去の中断があっても自己修復。地名選択規則は
     `cityName` と同一（locality→admin→country）。未解決セルは触らない（nil 上書きしない）。
     場所アルバム（Places）は `PlaceNameResolver.refinementGeneration`（補正世代）を
     `PlaceScanner.refreshIfNeeded` が見て再スキャン（起動をまたぐ分は毎起動の初回スキャンが拾う）。
  3. **オフラインの距離格下げ**（`OfflinePlaceDB.demoted`）: 最近傍都市が **25km 超なら市名を捨てて
     県名**、**150km 超なら国名のみ**（500km 超 nil は従来どおり）。「誤った市名」を「粗いが正しい
     名前」に変える。格下げ導入で意味が変わるため、オフライン由来（refined でない）キャッシュだけ
     版管理（`offlineLogicVersion`）で破棄し再解決させる（Apple 解決済みは維持）。
- 結果: 補正が写真の実在セルに当たり、既存写真・トリップ名・訪問地・AI 検索（LexicalSearch は台帳の
  placeName を見る）のすべてに反映される。オフラインのみの期間も誤市名が減る。トレードオフ:
  収束まで数晩かかる（枚数の多いセル優先で体感は早い）／夜間の通信は ADR-68 と同量のまま。
- 関連: `PlaceRefinement` / `AutoAlbumEngine.refinePlaceNames` / `AutoAlbumStore.updatePlaces` /
  `OfflinePlaceDB.demoted` / `PlaceNameResolver.refinementGeneration`。[[ADR-68]]（土台）。

## ADR-68 地名の高精度化: オフライン先行＋Apple(CLGeocoder)で背景補正（トリップ）
- 状態: 採用 → **ADR-76 で拡張**（対象をトリップ代表座標→写真のあるセルへ・台帳伝播を追加。
  「成功のみキャッシュ・失敗はリトライ・1.2s スロットル」の核心は不変）
- 文脈: 時間と場所アルバムの地名がずれる。原因はオフライン都市 DB（`OfflinePlaceDB`・人口1.5万以上へ
  **最寄りスナップ**）の粗さ（小さな町・郊外は最寄り大都市名に丸まる／区・町名 subLocality を出せない）。
  一方、旧 `CLGeocoder`（オンライン）は**レート制限**と**失敗の恒久キャッシュ**で「Trip」固定になり撤去
  された経緯がある。Apple の精度を、旧実装の失敗を避けて取り込みたい。
- 決定: **ハイブリッド**。(1) 表示は従来どおり**オフラインで即時**（ネット不要）、(2) **背景（夜間・
  電源＋Wi-Fi＋ロック中＝ADR-25 ゲート）**で `CLGeocoder.reverseGeocodeLocation(_:preferredLocale:)` により
  高精度化。旧失敗を避ける核心:
  - **成功だけキャッシュ上書き**（`PlaceComponents.refined=true`＋`subLocality` 追加）、**失敗はキャッシュ
    しない**（次回リトライ＝「Trip 固定」の根絶）。
  - **1 件/1.2s** のスロットル・対象は**トリップ代表座標に限定**（少数）＝レート制限に当たらない。
  - 既に refined 済みグリッドセルはスキップ＝**収束後は無コスト**。`shouldContinue`（heavy ゲート）で中断。
  - 高精度化で名前が変わったら trips を**作り直して**表示更新。`refined>0` のときだけ（通常は 1 回）。
  - `PlaceComponents` の新フィールドは optional＝旧キャッシュ JSON はキー欠落で nil に decode され互換。
- 結果: 正確な市区町村・区名になり、トリップは地点数が少ないのですぐ収束。オフラインは残るので圏外/未接続
  でも動く（粗い名前→夜間に高精度化）。トレードオフ: 夜間に少量の通信（Wi-Fi 時のみ）。場所アルバム
  （Places グリッド）は今回対象外＝トリップ（ユーザー指摘）に限定（必要なら同様に拡張可）。
- 関連: `PlaceNameResolver.refineWithAppleGeocoder`・`PlaceComponents`(subLocality/refined)・
  `AutoAlbumEngine.refreshIfNeeded/refinePlaceNames`。[[ADR-25]]（実行方針）。旧オフライン化の再検討。

## ADR-69 重い処理のゲートを「ローカル＝通信不要／クラウド＝回線ゲート」に分離
- 状態: 採用（[[ADR-25]] の精緻化）
- 文脈: [[ADR-25]] で「重い処理は電源＋Wi-Fi＋ロック中のみ」に単純化した際、**回線条件を全作業に一律**で
  課していた（`HeavyWorkTiming.allows` が常に networkOK を要求）。しかし**端末内写真の顔スキャン・CLIP
  埋め込み・Vision タグは通信不要**。そのため夜間に Wi-Fi が未接続/未検出（例: BG 起動直後は
  `NWPathMonitor` の初回コールバック前で `isOnWiFi=false`／モバイル運用）だと、**ローカル解析まで丸ごと
  停止**していた（実機ログで顔スキャンが一晩でほとんど進まず＝[[case: 夜間のオンデバイス解析が進まない]]）。
  皮肉なことに CLIP 埋め込みの内側ループは「Wi-Fi 無ければローカルのみ進める」実装を既に持っていたが、
  外側の共通ゲート（`heavyShouldPause`）が先に止めるため**死にコード**だった。
- 決定: 共通ゲートを 2 系統に分ける。
  - `HeavyWorkTiming.allows(..., requiresNetwork:)` に引数追加。`false` なら回線条件を課さない（電源・
    低電力・使用中の安全弁は不変）。
  - `BackgroundYield.heavyWorkAllowedLocal`（回線不要）を新設。**`heavyShouldPause()` はローカル版**を見る
    ようにし、顔スキャン・埋め込み・タグはこれを使う＝Wi-Fi 無しでも端末内写真は進む。
  - `heavyWorkAllowed`（回線あり）は据え置き。**generate/地名補正**（クラウドメタ・通信を伴う）はこちら。
  - **クラウド分は各処理が個別ゲート**: 埋め込みは `PhotoTagger` が `networkAllowed()` でクラウドのサムネDL
    をスキップ（既存）、顔スキャンは `FaceTagger.scan` が候補を **ローカル先・クラウドは後回し**に並べ替え、
    回線NG なら今回はクラウドを対象から外す、Vision タグも回線NG ならローカルのみ。
- 結果: 夜間に Wi-Fi が無くても端末内写真の People/タグ/埋め込みが埋まる。クラウド分（通信を要する）は
  従来どおり Wi-Fi 待ち。トレードオフ: 「Wi-Fi のみ」の 5 段階説明は**クラウド通信の話**に限定される
  （ローカル計算は段階に関わらず電源＋非使用で走る）＝説明を UI に補足。
- 関連: `HeavyWorkTiming.allows(requiresNetwork:)`・`BackgroundYield.heavyWorkAllowedLocal/heavyShouldPause`・
  `FaceTagger.scan`(local-first/cloud-defer)・`AutoAlbumEngine+Recognition`(tag local-only)・
  `PeopleEngine.startScan`(networkAllowed 結線)。`HeavyWorkTimingTests`。

## ADR-70 オンデバイス AI モデルの常駐制御（遅延・直列ロード・使用後解放）
- 状態: 採用
- 文脈: Core ML モデル（CLIP 画像/テキスト塔・facenet・VLM=SmolVLM≈877MB）は各ランタイムが
  `LoadOnce` で遅延ロードするが、**一度読むとプロセス終了まで常駐**し、**モデル横断のロード制御が無い**。
  夜間 1 窓の `runHeavyWork` が顔スキャン(facenet)と CLIP 埋め込み/VLM キャプションを**並行**起動する
  ため、facenet＋CLIP2塔＋VLM(877MB) が**同時常駐**してメモリが跳ね jetsam を誘発していた。さらに
  facenet だけ `.shared` アクセスで **eager ロード**し、`isFaceModelAvailable` を評価するホーム描画
  （メインスレッド）で起動時に読まれていた（[[case: 夜間のオンデバイス解析が進まない]] の続き）。
- 決定:
  - **1-a｜顔モデルを遅延化**: `FaceModelRuntime` を `LoadOnce` 化し、`isAvailable`/`FacePerceptionAdapter`
    は**同梱判定(バンドルURL)のみ**に（`.shared` に触れてもロードしない）。実ロードは初回 `embed`。
  - **1-c｜ロードの直列化ゲート**: `CoreMLModelLoader.serializedLoad`（グローバル NSLock）を全モデルの
    `MLModel(contentsOf:)` に通し、**同時に 1 つだけ**ロード（ロード時の一時メモリ確保ピークを重ねない。
    推論自体はロック外＝並列のまま）。
  - **1-b｜フェーズの相互排他**: 夜間パイプラインで **VLM キャプションは顔スキャンが動いていない間だけ**
    実行（`isScanningFaces` を見て見送り）。facenet と VLM(877MB) を同時常駐させない。
  - **1-d｜使用後/圧迫時に解放**: `LoadOnce.reset()` を追加。VLM は**キャプションフェーズ完了後**に解放
    （`TagPerceptionProvider.releaseCaptionModelIfLoaded`）＋**メモリ圧迫時**に最優先解放
    （`MemoryPressureMonitor.register` に登録）。CLIP/facenet は小さいので常駐維持。
- 結果: 同時常駐するモデルを実質「1 つ＋作業セット」に抑え、起動時のメイン同期ロードも消えた。
  トレードオフ: VLM は解放後の再キャプションで再ロード（お気に入り限定・低頻度なので許容）。
- 関連: `CoreMLModelSupport`(serializedLoad/LoadOnce.reset/isLoaded)・`FaceModelRuntime`(遅延化)・
  `FacePerceptionAdapter.isAvailable`・`VLMRuntime`(release/pressure 登録)・`VisionTagAdapter`・
  `TagTagger`/`AutoAlbumEngine+Recognition`(フェーズ相互排他・解放)。[[ADR-25]]/[[ADR-69]]。

## ADR-71 UI を軽く保つ（メインアクタの大規模コレクション処理を off-main／間引き）
- 状態: 採用
- 文脈: 実機ログで、Dropbox 同期中に**メインアクタで 67k 件級の処理が 0.4 秒ごと**に走っていた
  （UI がもたつく原因）。調査で複数箇所を特定。
- 決定:
  - **2-a｜MergedPhotoStore 再構築のデバウンス**: Observation の変化ごとに 68k の merge+sort を投げず、
    0.4 秒トレーリングで 1 回に集約。初回 `start()` のみ即時。
  - **2-b｜Dropbox items の比較を off-main 署名化**: `cached != items`（67k 全比較・メイン）を、
    off-main の `Hasher` 署名比較に置換。変化時だけ `stampFavorites`＋代入。`items=[]` の各所で署名リセット。
  - **2-c｜LocalPhotoStore の map を off-main**: `assets` の didSet（~18k の LocalPhotoItem 生成・メイン）を
    廃し、fetch と同じ detached タスクで (assets, items) を作って一括代入（`setLoaded`）。
  - **2-d｜メンバーアルバムの sort/map を off-main**: `LocalAssetIndex.assets(for:)` はソートしない
    （辞書引き＋追いフェッチのみ）。整列＋map は `LocalPhotoStore(preloadedAssets:)` の detached へ一本化
    （準備中は一瞬ロード表示）。
  - **2-e｜PlaceGrouping 逐次再構築の間引き**: 初回スキャンの段階表示を**8 セルごと**に（O(都市数²) の
    メイン再構築を削減）。最後に必ず 1 回。
- 結果: 同期中の UI 引っかかりを解消（メインの大規模処理を除去/間引き）。トレードオフ: items 反映は
  最大 0.4 秒デバウンス分・大アルバムのオープンは一瞬ロード表示（体感許容）。
- 関連: `MergedPhotoStore`(debounce)・`DropboxPhotoStore`(itemsSignature/reflectCachedItems)・
  `LocalPhotoStore`(setLoaded・preloaded 非同期)・`LocalAssetIndex.assets(for:)`・`PlaceScanner`。

## ADR-72 夜間バッチのメモリ/実行制御（generate 省メモリ・バックアップと AI の非同時実行）
- 状態: 採用（[[ADR-25]]/[[case: 夜間のオンデバイス解析が進まない]] の続き）
- 文脈: `generate` は 86k 件を扱いメモリピークが大きく、`runHeavyWork` はバックアップと AI を**同一窓で
  同時**に起こしていた（どちらもメモリ/IO 重）。
- 決定:
  - **3-a｜generate の返り値を件数に**: detached 計算が `photos`（86k の EnrichedPhoto 配列）を**メインへ
    返して .count だけ**に使っていたのを `Int` に変更（86k×文字列複数の配列をメインで握らない）。
  - **3-b｜バックアップと AI を非同時実行**: `runHeavyWork` は **CLIP 埋め込みの残作業が 0 の窓でだけ**
    バックアップを開始（`pendingEmbedCount()`）。AI が一巡するまでバックアップは見送り（差分再開で取りこぼし無し）。
  - **3-c｜generate を軽い処理の後に**（[[case]] 側で実施済み）＋空きメモリ >900MB の窓だけ実行を維持。
- 結果: generate のメイン常駐ピーク減・重い処理の同時実行を回避。トレードオフ: バックアップは AI 完了まで
  後ろ倒し（夜間の別窓で回る）。
- 関連: `AutoAlbumEngine.generate`(photoCount)/`pendingEmbedCount`・`HeavyWorkScheduler.runHeavyWork`。

## ADR-73 ANE 直列化ゲートを「呼び出し側」から「ANE に触れる場所」へ移す
- 状態: 採用（[[ADR-70]] で導入した `MLInferenceGate` の掛け位置を変更。ゲート自体は継続）
- 文脈: [[ADR-70]] は diagnostics-19（Vision `perform` と Core ML 推論が ANE を同時使用 → Vision が永久に
  返らない）対策として `MLInferenceGate` を導入したが、**掛ける場所を呼び出し側（夜間タガー）**にしていた
  （`FaceTagger` / `TagTagger` / `PhotoTagger` が provider 呼び出し全体を包む）。この方式には構造的な穴が
  あった:
  - **漏れる**: ゲートは「包んだ経路」しか守らない。**前景の CLIP テキスト塔**（`QueryEmbedder` →
    `TextEmbedder.embed`、および `AIAlbumService` の `prewarm`）は誰も包んでおらず素通りしていた。
    AI アルバム作成・検索は**ユーザーが起きている時間帯**に走るため、夜間の顔スキャンと重なれば
    diagnostics-19 と同じ「Vision × Core ML 同時」が成立する状態が残っていた。
  - **内側で不変条件を破れる**: `TagTagger` はゲートを取った内側で `VisionTagAdapter.senseInfo` を呼び、
    その中で `boundedConcurrentResults(maxConcurrent: 3)` が **3 本の `VNImageRequestHandler.perform` を
    同時実行**していた。「ANE は同時に 1 つ」をゲートの内側で自ら破っていた。
  - **占有時間が伸びる**: provider 呼び出し全体を包むと、画像ロード（PHImageManager デコード／Dropbox
    サムネ取得＝**ANE を使わない I/O**）までゲート内に入り、その間ほかの推論が全部止まる。
- 決定: ゲートは **ANE に実際に触れる場所＝`MobileCLIPKit` の各ランタイム/アダプタの内側**でのみ取る。
  呼び出し側（タガー・検索）は一切包まない。
  - `MobileCLIPRuntime.encodeText/encodeImage/encodeImages`・`FaceModelRuntime.embed`・
    `VLMRuntime.caption`・`VisionTagAdapter.sense`・`FacePerceptionAdapter` の Vision 2 箇所
    （顔観測／クロップ再検証）が各自ゲートを取る。内部実装は `unsafe*`（保持済み前提）に分ける。
  - **入れ子禁止**。`MLInferenceGate` は非再入なので、`run` の中で `run` を呼ぶと固まる。保険として
    `@TaskLocal isHeld` で同一タスクの再入を検出し素通し＋`assertionFailure`（`Task.detached` には
    伝播しないので、ゲート内で detached を作らないこと）。
  - 粒度は**段ごと**にする。顔 1 枚を 1 回のゲートで包まず「顔観測／各クロップ再検証／埋め込みバッチ」で
    分ける。表示ラベラの概念埋め込み（約300語）も **1 語ずつ**ゲートを取り直す（まとめて握ると
    その数秒〜十数秒、顔スキャンとタグ付けが完全に止まる）。
- 結果: ゲートを通らない ANE 経路が消え、ロードは並列のまま推論だけ直列になった（`senseInfo` の 3 並列は
  **ロードの並列**として意味を保ち、推論はゲートで 1 つずつ）。トレードオフ: 前景の検索が夜間処理 1 単位
  ぶん待つ可能性がある（単位を小さく保つことで緩和）。ゲートの掛け忘れは今後も起こり得るので、
  「ANE に触れるコードを足したら、その場でゲートを取る」を規約として `CLAUDE.md` にも書いた。
- 関連: `PerceptionCore/MLInferenceGate.swift`・`MobileCLIPKit/{MobileCLIPRuntime,FaceModelRuntime,
  VLMRuntime,VisionTagAdapter,FacePerceptionAdapter,AIPerceptionAdapters,CLIPDisplayLabeler}.swift`・
  `FaceCore/FaceTagger.swift`・`AutoAlbumCore/{TagTagger,PhotoTagger,AutoAlbumEngine+Recognition}.swift`。
  [[ADR-70]]・事例「ANE 直列化ゲートに穴があった」。

## ADR-74 Core ML モデルロードの async 化とメモリ圧迫時の解放（CLIP/facenet）
- 状態: 採用（[[ADR-70]] の常駐制御を補強）
- 文脈: `LoadOnce` は **NSLock を握ったまま同期 `MLModel(contentsOf:)`** を実行していた。CLIP 画像塔は
  実機 16〜35 秒かかるため、その間 Swift 並行の協調スレッドを 1 本まるごとブロックしていた。
  また圧迫時にモデルを手放すのは VLM だけで、CLIP 画像塔と facenet は一度ロードすると常駐し続けていた
  （`MemoryPressureMonitor.register` の仕組みは既にあるのに未登録）。
- 決定:
  - `LoadOnce` を **actor ＋ async ロード**（`MLModel.load(contentsOf:configuration:)`）にする。actor は
    再入するので、実行中の `Task` を保持して後続の `get` を**合流**させ二重ロードを防ぐ。ロード中に
    `reset()` が割り込んだ場合は世代カウンタで検出し、その結果を確定させない。
  - `MobileCLIPRuntime` / `FaceModelRuntime` も `MemoryPressureMonitor.register` する。ただし
    **critical のみ**解放する（warning で手放すと再ロード 16〜35 秒を頻繁に払う。VLM は 877MB と桁が
    違うので従来どおり全レベルで解放）。実行中の推論は自分のハンドルを強参照しているので途中で消えない。
  - 遅延ロードはゲートの内側で起きるため、ロード中に他の ANE 処理は走らない（[[ADR-73]] の性質を維持）。
- 結果: 長時間のロードが協調スレッドを占有しなくなり、圧迫時に CLIP/facenet も解放されるようになった。
  トレードオフ: 推論 API（`encodeText`/`embed`/`debugAnalyze` 等）が軒並み async になり、顔精度計測ハーネス
  （`FaceAccuracyEvalTests`）も async 化が必要になった（`autoreleasepool` は await を跨げないため、
  画像デコードだけを包む形へ変更）。
- 関連: `MobileCLIPKit/CoreMLModelSupport.swift`（`LoadOnce` actor 化・`loadBundledModel` async）・
  各ランタイム・`MosaicPhotosTests/FaceAccuracyEvalTests.swift`。[[ADR-70]]・[[ADR-73]]。

## ADR-75 ANE ゲートの優先度分離とキャンセル時の繰り上げ／グローバル可変状態の排除
- 状態: 採用（[[ADR-73]] の副作用への対処＋Swift 並行処理レビューの反映）
- 文脈: [[ADR-73]] でゲートを ANE 接触点へ移した結果、**前景（検索・AI アルバム作成）の CLIP テキスト
  埋め込みもゲートを通るようになった**。安全にはなったが、厳密 FIFO のため夜間バッチの待ち行列
  （`senseInfo` は同時 3 件が並ぶ）の後ろに前景が並び、体感が悪化し得る状態になっていた。
  併せて `swift-concurrency` スキルによるレビューで、待機列がキャンセルに応答しないこと、
  `MosaicSupport` に保護なしのグローバル可変状態が残っていることが分かった。
- 決定:
  - **優先度付きゲート**: `MLInferencePriority`（`.interactive` / `.background`）を導入し、待機列を
    2 本に分けて解放時は interactive を先に起こす。`encodeText(_:priority:)` の既定は `.background`
    で、`MobileCLIPTextEmbedder.embed`（＝ユーザーが待っている検索）だけが `.interactive` を渡す。
    interactive は低頻度なので background の飢餓は実際上起きない。
  - **キャンセル時は列の先頭へ繰り上げ**: `MLInferenceGate` / `AsyncSemaphore` / `HeavyImageLane.Gate`
    の 3 つが同じ形で `withCheckedContinuation` を使いながらキャンセルを無視していた。
    `withTaskCancellationHandler` を足し、キャンセルされた待機者を**自分の優先度クラスの先頭へ繰り上げる**。
    「待たずに抜けさせる」は採らない——`acquire`/`release` の 1:1 対応（`run` の戻り値も非 Optional）を
    壊さないため。enqueue より先にキャンセルが届く競合は `earlyCancels` で吸収する。
  - **BGTask ハンドラの MainActor 越境を解消**: `HeavyWorkScheduler.taskID` を `nonisolated let` に、
    `getPendingTaskRequests` 完了ハンドラ（任意スレッド）からの `submit()` は `Task { @MainActor in }` 経由に。
  - **グローバル可変状態をロック箱へ**: `PerfTrace`（`isEnabled`/`counters`/`flushTimer`/`pendingScreens`）・
    `Diagnostics`（`formatter`/`memorySource`）・`MemoryBudget.override` を、それぞれ NSLock を内包する
    `@unchecked Sendable` の小さなクラスに閉じ込めた（`nonisolated(unsafe)` は使わない）。
- 結果: `-strict-concurrency=complete` の警告が `MosaicSupport`/`PerceptionCore` で **0 件**、アプリ自作
  コードで 8 件 → 3 件（残りはカバー画像ローダの非 Sendable クロージャ＝別途）。自動生成の Core ML
  ラッパー（未使用・20 件）は対象外。
  **残る最悪ケース**: 優先度は待機列の順序を変えるだけで**実行中の保持者は横取りできない**。モデル
  ロード（CLIP 画像塔は実機 16〜35 秒）がゲート内で走るため、その最中の前景要求はロード完了まで待つ。
  ロードをゲート外へ出せば解消するが、「ロードと推論の同時実行が ANE で安全か」は**実機未検証**
  （確認済みなのは*ロード同士*の並列が安全なことだけ＝`CoreMLModelSupport` の記述）。危険側に倒れると
  再び永久ハングなので、実機計測するまではロードもゲート内に置く。
- 関連: `PerceptionCore/MLInferenceGate.swift`・`ImageCacheKit/AsyncSemaphore.swift`・
  `MosaicSupport/{HeavyImageLane,PerfTrace,Diagnostics,MemoryBudget}.swift`・`HeavyWorkScheduler.swift`・
  `MobileCLIPKit/{MobileCLIPRuntime,AIPerceptionAdapters}.swift`。[[ADR-73]]・[[ADR-74]]。
- 残課題: `SWIFT_VERSION = 5.0` のまま（`SWIFT_APPROACHABLE_CONCURRENCY`・`SWIFT_DEFAULT_ACTOR_ISOLATION`
  は有効なのに言語モードは Swift 5）＝**コンパイラはデータ競合を検査していない**。各 `Package.swift` にも
  `swiftSettings` が無くアプリと既定分離が非対称。まず `SWIFT_STRICT_CONCURRENCY=complete` を警告のまま
  常設し、自動生成ラッパーの扱いを決めてから Swift 6 言語モードへ、が安全な順序。
- 追補（ADR-75 の自己レビューで判明した不具合と是正）:
  - **`earlyCancels` の無制限増加（混入バグ）**: `promote` は「どちらの列にも居ない＝まだ enqueue 前」と
    決めつけていたが、実際には (b)**もう払い出し済み**（onCancel の `Task` が actor に届く前に
    `release()` が resume して列から外した）というケースがある。(b) を (a) と誤認して `earlyCancels` に
    書き込むと、その ID は二度と enqueue されないので**永久に残り Set が無制限に増える**。
    スクロール中の先読み破棄で高頻度に起きるため `AsyncSemaphore` / `HeavyImageLane` で特に効く。
    → `pendingIDs`（払い出し前だけ真）で 2 つを区別する。3 ファイルとも同修正。
  - **メモリ圧迫時の解放が非同期化していた退行**: [[ADR-74]] の `LoadOnce` actor 化により
    `reset()` / `isLoaded` が async になり、圧迫ハンドラ（`MemoryPressureMonitor.handle` は同期呼び出し）
    から `Task { await box.reset() }` となっていた。877MB の VLM を jetsam との競争で後回しにするため、
    **NSLock ＋ async ロードのハイブリッド**へ戻す（ロードは async のまま＝協調スレッドを塞がない、
    `reset()`/`isLoaded` は同期に復帰）。ロックは `await` を跨いで保持しない。
  - **ゲートのテストを新設**: `MLInferenceGateTests`（`PerceptionCore`・macOS `swift test`）。優先度順・
    キャンセル繰り上げ・二重 resume なし・内部状態の後始末を検証する。上記の混入バグは
    **意図的にバグを再導入して失敗することを確認済み**（`testStalePromoteIsNotRecorded`）。
    競合そのものはスケジューラ依存で決定的に再現できないため、`debugPromoteStaleWaiter` seam で
    契約を直接検証している。
  - 軽微 3 件も同時に解消: `finiteFloats` の `NSNumber` 箱詰め（連続領域なら型付きバッファで一括読み・
    非連続は従来経路へフォールバック）、`CoreMLModelHandle` の `.keys.first` 非決定性（名前ソート＋
    複数入出力なら診断ログ）、バッチ結果の範囲外アクセス（`min(out.count, indexMap.count)`）。

## ADR-70 顔モデル換装: facenet → AuraFace-v1（5点整列・チューニングプロファイル）
- 状態: 採用
- 文脈: 識別の主弱点は年齢不変性（FG-NET TAR@FAR1% 48.1% vs LFW 98.6%）で、ADR-56〜69 で
  クラスタリング側は絞り切った＝モデルが天井。P5（ADR-56）は「ArcFace 級で許諾的ライセンスの
  学習済み重みは存在しない」として換装を保留していたが、**AuraFace-v1**（Apache 2.0・
  **学習データも商用可**・ArcFace 系 ResNet100・AGEDB 96.10）の公開で前提が変わった。
- 決定:
  - **換装**: `scripts/build_auraface.sh`（HF から ONNX → onnx2torch → Core ML fp16・
    onnxruntime との照合検証つき）。ファイル名は FaceEmbedder.mlpackage 据え置き。45MB→125MB。
  - **5 点整列**: ArcFace 系は目2＋鼻＋口角2 を 112×112 テンプレートへ相似変換した入力で
    学習されている。相似変換は複素数回帰の閉形式（最小二乗・決定的・SVD 不要）で実装し、
    退化・過大回転は従来の両目整列へフォールバック。
  - **チューニングプロファイル（FaceTuning）**: 計測で、しきい値・第2パス・統合候補帯域・監査・
    校正可動域の**全定数がモデルの類似度分布に張り付いている**ことが判明（AuraFace は同一人物
    平均が 0.550→0.434 と約 0.1 低く、facenet 定数のままだと F1 0.557 と*悪化して見えた*）。
    散らばった static を FaceTuning に集約し、**同梱モデルの宣言**（face_config.json の
    tuning/alignment/pipelineVersion）で選ぶ。モデルと定数・整列・版数がずれる事故を構造的に防ぐ。
  - **v5 プロファイル**（掃引で決定）: thr 0.35・margin 0.04・sizeMax 0.08・第2パス 0.40・
    候補下限 0.25・校正 0.25〜0.40・負例同一 0.45・監査 0.20/0.40。
- 結果: FG-NET TAR@FAR1% 48.1→**62.0%**・クラスタ F1 0.664→**0.790**・分裂 3.7→**2.8**。
  LFW も退行なしどころか改善（TAR@FAR0.1% 94.0→97.3%・家族シナリオ F1 **1.000**・分裂 1.0）。
  数値は `records/face-accuracy.md`（2026-08-06）。移行は pipelineVersion 5 の全再スキャン
  （命名持ち越し・修正ジャーナル保持）。トレードオフ: モデル +80MB・推論は R100 で重め
  （夜間処理なので許容・実機 ANE で要確認）。facenet へ戻すには build_facenet.sh（プロファイルも
  自動で戻る）。
- 追補（2026-08-07・初回実機ログ）: **修正ジャーナルのスケール汚染**を発見・修正。
  `FaceCorrection` の類似度・埋め込みは記録時のモデル空間に張り付いているのに世代タグが無く、
  facenet 世代の正例（0.5-0.7）が AuraFace の校正を可動域上限 0.40 まで押し上げていた
  （実機 thr=0.40・最適は 0.35）。負例エグゼンプラも旧空間の埋め込みで照合不能だった。
  → `FaceCorrection.profile`（FaceTuning.name・既存行 nil＝facenet）を追加し、校正・負例・
  notSame/sameGroup 照合を**現行プロファイルの行に限定**（モデルを戻せば旧行が再び有効）。
  ADR-51 の「修正ジャーナルはモデル不変のため再スキャンを跨いで有効」は**同一モデルの
  再スキャン**にのみ成り立つ前提だったことを明確化。
- 関連: `FaceTuning`・`FaceAlignment.arcFaceTransform/similarityTransform`・`FaceModelConfig`・
  `FacePerceptionProvider.pipelineVersion/tuning`・`scripts/build_auraface.sh`／`convert_auraface.py`。
  ADR-56（P5 保留）/ ADR-51（アライメント）/ ADR-68・69（定数の由来）。

## ADR-69 クラスタ事後監査（統合後に「実は別人」を見つける）
- 状態: 採用
- 文脈: ユーザーの着想 —「写真が増えると精度が上がる。今まで気づけなかったが実は兄弟で微妙に似ていて
  同じ人と認識していた、というケースを後から見つけられないか」。統合はユーザー確認を経ているが、
  **確認した時点では材料が足りない**ことがある（兄弟は数枚では区別できない）。既存の夜間再クラスタは
  「割り当て直し」であって、統合済みクラスタの中身を疑う仕組みは無かった。
- 決定: `FaceClusterAudit`（純ロジック）を新設し、クラスタを 2-means で分割して
  **凝集度と分離度**を測る。ADR-59（外れ値除去・不採用）が「重心から遠い顔＝混入」として
  1 顔ずつ抜き、成長データで失敗したのに対し、本監査は**クラスタ全体の形**を見る:
  成長は連続した 1 つの帯・別人の混入は離れた 2 つの塊。加えて**同じ写真に両群の顔が居る**なら
  同一人物は 1 枚に 1 回しか写れないので決定的証拠として扱う。
  採用値は margin 0.25 / separation 0.35（データセット計測。FG-NET 誤検出 8%・検出 67%／
  LFW 誤検出 0%・検出 99%。検出したものは全件正しく 2 分割できた）。
  **自動分割はしない**（成長データで誤検出 8% が残る）。1 対 1 のレビューカードで尋ね、
  いいえ → 2 分割＋負例、はい → `sameGroup` 記録で二度と尋ねない（正例として校正にも効く）。
  混入は分裂より害が大きいので、レビューでは最優先に出す。
- 結果: 「統合したら終わり」だった学習ループが閉じ、**写真が増えるほど過去の誤りも直る**ようになった。
  トレードオフ: FG-NET で 8% の誤検出（＝たまに見当違いの質問が出る）。自動化しないので実害は
  「1 回の余計な回答」に留まる。しきい値を上げれば誤検出は減るが検出も落ちる（0.30/0.30 で 4%/44%）。
- 関連: `FaceClusterAudit`・`FaceStore.auditSplitItems/splitCluster/confirmSameGroup`、
  `FaceReviewItem.splitCluster`、`FaceAccuracyEvalTests`（H スイープ）、
  `records/face-accuracy.md`（2026-08-06）。ADR-59（不採用の先例）/ ADR-46（レビュー）/ ADR-68。

## ADR-68 ピープルの過分割対策: 分裂率の計測・競合を見るマージン・一括レビュー
- 状態: 採用
- 文脈: 0〜10 歳の子供を含む実ライブラリ（実質 3 人）で **ピープルが 2,000 人超**になった。
  原因は 3 層ある。
  (1) **アルゴリズム**: マージンゲート（ADR-57）とサイズ適応マージン（ADR-58）は「似た**別人**
  （兄弟）が近くにいるとき紛らわしい顔を取り込まない」ための仕掛けで、弾いた顔は**新クラスタ**に
  なる。ところが人物が数人しかいないライブラリでは、近くにいるのは*同じ人の別時期のクラスタ*
  なので、**正しい合流を止めて分裂を量産**する。しかも count<11 の小クラスタは実効しきい値が
  最大 0.60 に上がるため、生まれた小クラスタは育たないまま固定される。
  (2) **計測**: 指標が B-Cubed F1 と純度だけで、**分裂（1 人が何個に割れるか）を直接見ていなかった**。
  さらに FG-NET（82人/1,002枚）・LFW（901人/4,862枚）はどちらも「多数の人物を少しずつ」で、
  家族アルバムの「**数人を何万枚も**」を一度も測っていない。だから出荷まで気付けなかった。
  (3) **修復手段**: レビュー（ADR-46 A1）は **1 回答＝1 統合**。2,000 個を 3 人に畳むには
  2,000 回答が必要で、原理的に追いつかない。
- 決定:
  - **計測（D）**: `FaceEvalMetrics` に**分裂率（1人あたりクラスタ数）**と最悪値を追加し一級指標にする。
    ハーネスに **F 家族シナリオ**（写真数上位 K 人だけを取り出す＝少数ID×大量写真）を追加。
    以後、クラスタリング変更は全体指標と家族シナリオの**両方**で採否を決める。
  - **アルゴリズム（B）**: **サイズ適応マージンの免除**（`FaceClustering.rivalAwareSizeMargin`）。
    近くに「別人らしい競合」がいないときだけ、小クラスタへの上乗せ（ADR-58）を免除する。
    「似ている」の判定バーは `threshold + 0.20`（緩いと別人まで免除して純度が落ちる）。
    **ただし確立した人物が 10 人未満のライブラリに限る**（`rivalAwareSizeMarginMaxPeople`）。
    計測で**人数によって正解が反転する**ことが分かったため: 小クラスタが「同じ人の断片」なのか
    「本当に別人」なのかが家族アルバムと大人数ライブラリで逆になる。無制限に効かせると
    LFW（901人）で F1 0.895→0.843 と退行するが、10 人未満に限れば 0.889（−0.006）に収まる。
    マージンゲート側の免除は**不採用**（サイズ免除だけで利得が出揃い、FG-NET 全体では悪化）。
    ⚠️ ADR-56 で不採用にした**自動連鎖統合とは別物**（あれは類似だけを根拠に推移的に繋いで
    雪崩式の誤統合を起こした。こちらは「合流の可否を判断する場面で、競合の正体を見る」だけ）。
    ⚠️ 比較の基準は「prod＋第2パス（ADR-66）」。第2パス無しと比べると改善を過大評価する。
  - **レビュー（A）**: **一括レビュー**（`FaceBatchReviewItem` / `FaceBatchReviewView`）。基準の人物に
    似たクラスタをグリッドに並べ、同じ人だけ選んで一度に統合する。統合でアンカーが育つので、
    次の回はさらに遠い時期のクラスタが候補に入る＝**人間の確認を種にした連鎖**。
    既定は未選択＋「すべて選ぶ」1 タップ（統合は取り消せないので、破壊的操作は明示でのみ起こす）。
    未選択は「別人と答えた」ではないので**負例は記録しない**（明示の「いいえ」は 1 対 1 レビューで受ける）。
  - **UI（C）**: カルーセルは上位 30 人＋「すべて表示」。`AllPeopleView`（検索つきグリッド）を新設し、
    分裂が多いときは「まとめて確認」へ誘導する。
  - **実機の品質可視化（E）**: `FaceQualityReport`。データセット計測と違い実機には正解ラベルが
    無いので純度は出せないが、**正解なしでも測れる代理指標**を出す:
    (1) **統合候補ペア数**＝レビュー帯域にあるのに別々のまま残っているクラスタ対＝「まだ畳めて
    いない分裂」の実測、(2) **同一写真違反**＝1 枚の写真に同じ人物が 2 回。
    ⚠️ 同一写真 cannot-link は**割り当て時にしか効いていない**（`recordScan` / `rebuildClusters`）。
    **統合（レビュー・一括レビュー・手動）は検査していない**ので、事後にここで検出する。
    共起 notSame（3 回以上で別人扱い）は統合*提案*の抑制であって、1〜2 回の共起がある対は
    ユーザーが統合できてしまう。Developer Options に表示し、夜間再クラスタ後は診断ログへ
    1 行残す（実機で Mac 無しに追える）。
- 結果: FG-NET 家族シナリオ（上位3人）で **分裂 4.7→2.0・純度 0.862→0.970・F1 0.558→0.895** と
  全指標で改善（トレードオフではない）。全体（82人）でも F1 0.640→0.664・分裂 4.1→3.7。
  LFW（901人）は F1 0.895→0.889（−0.006）で ADR-57 が許容した水準に収まる。
  数値は `records/face-accuracy.md`（2026-08-05）に記載。
  分裂率が下がってもゼロにはならない（家族3人でも 2.0）ので、残りは一括レビューで畳む前提の設計。
  既存ユーザーへは `PeopleEngine.clusterRuleVersion` を上げることで次の夜間処理で
  自動的に畳み直される（顔の再スキャンは不要＝既存の埋め込みを再利用）。
- 追補（2026-08-05・実機検証後）: **校正の暴走のほうが支配的だった**。実機ログ（9,446 枚・修正 130 件）で
  しきい値が校正により **0.60**（可動域上限 0.70）まで上がり、サイズ適応マージン（+0.10）と積み上がって
  **実効 0.70** で動いていた。単発クラスタ 1,170 個・最大クラスタ 15 顔という実測はこれで説明がつく。
  - **校正の可動域上限を 0.70 → 0.55**（`FaceCalibration.clampRange`）。校正自体は維持する
    （ユーザー修正は反映され続ける）が分裂側へ振り切らせない。家族3人で分裂 5.0→2.3・F1 0.503→0.791、
    LFW は 0.904→0.906 で中立。
  - **実効しきい値の頭打ち**（`FaceStore.capEffectiveThresholdWhenFewPeople`）。しきい値は校正で動くため、
    そこへサイズ加算が乗ると跳ね上がる。少人数ライブラリでは加算しない（家族5人で分裂 3.6→2.4）。
    ⚠️ 人数ゲート必須: 無制限だと LFW で F1 0.906→0.873 と退行する（サイズ免除と同じ反転）。
  - 教訓: **可動域は「動ける範囲」ではなく「壊れない範囲」で決める**。上限 0.70 は
    「ユーザーが望むなら高くできる」つもりだったが、実際には校正が容易に到達し、
    そこはデータセット上「ほぼ何も集まらない」領域だった。
- 追補2（2026-08-05・2 回目の実機検証後）: **統合候補の帯域が狭すぎた**。対策後も人物 368 のまま
  （大半が同一家族の別の顔）で `mergeCandidates` は 264→14 に枯れていたが、これは「統合し尽くした」
  のではなく**候補帯域の外に出ていた**ため。帯域は「しきい値 − 0.10」＝ 0.45 だったが、台帳の実測では
  **同一人物でも年齢差 11-20 年の平均類似度は 0.440・21 年+ は 0.400**＝成長期の子供こそ候補に出ない。
  - **統合候補の下限を 0.35 に**（`FaceStore.mergeCandidateFloor`）。21 年差の平均 0.400 を拾い、
    別人（両者 12 歳以下＝兄弟の代理）の平均 0.294 は下回らない。統合は必ずユーザー確認を経るので
    自動誤統合は起きない（候補は類似度降順・誤りは「いいえ」で負例として学習）。
  - **検出の取りこぼしは計測を先に入れた**（`FaceDetectionStats`）。9,566 枚から顔 3,078 個
    （0.32 個/枚）・`secondPass=1494`（全顔の 48% が品質フロア未満）と怪しいが、棄却理由は
    `debugAnalyze`（1 枚ずつの検証用）でしか見えなかった。**本番経路のまま理由別に数える**
    仕組みを入れ、Developer Options と診断ログ（`faces/detect:` 行）に出す。しきい値は測ってから触る。
  - 教訓: **「候補が尽きた」は「もう無い」ではない**。`mergeCandidates` が 264→14 に減ったのを
    最初は成果と読んだが、実際は帯域の外に出ただけだった。指標が良くなった理由を、
    その指標の定義に立ち返って確認する。
- 追補3（2026-08-06・3 回目の実機検証後）: **レビューの母数が UI の人物と違っていた**（本丸）。
  UI は「実際の写真枚数 ≥ 3」で 370 人を出す一方、レビュー・一括レビュー・統合候補数は
  `PersonCluster.count`（＝**重心に寄与した顔数**）≥ 3 で母数を取っていた。実機は全顔の 48% が
  品質フロア未満＝第2パス（ADR-66）で membership だけ付くため、「UI には人物として出るが
  count は 1〜2」が大多数になり、**370 人の大半が統合候補として検討すらされていなかった**。
  帯域を 0.45→0.35 に広げても候補が増えなかった（14→55 止まり）のはこのため。
  - **判定を `FaceStore.peopleEligibleClusters(minFaces:)` に一本化**し、UI とレビューが
    必ず同じ土俵（実際の写真枚数）を使うようにした。
  - `FaceDetectionStats` はプロセス内カウンタでスキャン完了後の端末では常に空だったため、
    保存済みデータから出せる `photosWithNoFace` を品質レポートに追加。
  - 教訓: **同じ言葉（「人物」）が層ごとに違う定義になっていないか**を疑う。UI・レビュー・
    診断で 3 通りの「人物」があり、数字が噛み合わない原因を 2 回続けて見落とした。
- 追補4（2026-08-06・4 回目の実機検証後）: 追補3 で統合が実際に動き出した
  （`mergeCandidates` 55→37,457・人物 370→290・最大クラスタ 95→299）。実運用で 2 点が判明:
  - **一括レビューの基準が固定される**: 統合のたびに同じ基準で次を探すため、その人の一致を
    出し切ると**候補が全部別人になり機能が死ぬ**。未選択候補も覚えないので同じ顔が出続ける。
    → 「次の人へ」（基準を送る）＋出題済み候補の除外を追加。
  - **一括統合で同一写真違反が発生**（実測 2 件）: 候補の共起判定が「3 回以上で別人」
    （統合サジェストの基準）だったため、共起 1〜2 回の対を統合できてしまった。
    → **候補は共起が 1 回でもあれば出さない**。`coOccurrenceNotSame=3` は「別人として学習する」
    基準として別に維持する（同じ数字を 2 つの目的に使い回していたのが誤り）。
- 追補5（2026-08-06・5 回目の実機検証後）: **2,000 人超 → 47 人**（最大クラスタ 611 枚）に到達。
  一方で誤統合が 2 種類あることが実運用で判明（ユーザー指摘）:
  - **別々の名前が付いた人物どうしの統合**は誤りの可能性が高い（ユーザーは名前を付けた時点で
    「別人」と表明している）。実際その操作をしていないのに `named` が 8→7 に減っていた。
  - **同一写真違反が 2→9 に増加**。候補フィルタ（追補4）だけでは手動統合を防げない。
    違反が生じる統合は**その統合自体が誤り**なので、拒否したうえで別人として学習すべき。
  - → `mergeClusters` に**実行地点のガード**を 2 つ入れ、拒否理由（`MergeRejection`）を返す。
    同一写真の共起で拒否したときは `notSame` を記録して再クラスタに反映する。
    候補側でも別名どうしを除外。既存の違反は `repairSamePhotoViolations()` で修復
    （最良の 1 顔だけ残し、外した顔は負例として記録＝再発しない）。
  - **ホームのピープル列は 6 枚以上の人だけ**にする（`PeopleEngine.minFacesForCarousel`＝「5 枚以内は出さない」）。
    数枚の断片はトップに並べる価値が薄く列を埋める。レビュー母数の 3 枚は据え置き（統合対象には残す）。
  - 教訓: **候補を絞るのと実行を止めるのは別**。不変条件は「提案しない」ではなく
    「実行させない」で守る。手動操作の経路が常に残っている。
- 追補6（2026-08-06・確度の重み付け）: **回答の確度が学習に反映されていなかった**（ユーザー指摘）。
  1 対 1 の確認（2 枚を並べて尋ねる）は両方を見て判断するので確度が高いが、まとめて確認は
  候補の小さなアバターだけを見るので確度が低い。にもかかわらず `FaceCorrection` は kind しか持たず、
  しきい値校正は全サンプルを等重みで数えていた。実機では修正 216→611 件の増分の大半が一括レビュー
  由来で、**校正が低確度の回答一色に染まる**構造だった。
  - `FaceCorrection.confidence` を追加（既存行は nil ＝ 1.0 の加算的マイグレーション）。
    `FaceCalibration.calibratedThreshold` を重み付きにし、件数ではなく**重みの合計**で境界を選ぶ。
    最小サンプル数の足切りも重み合計で見る。重み: 1 対 1 の確認・手動 = 1.0／まとめて確認 = 0.4。
  - 同一写真違反の修復は手動ボタンをやめ、**一括レビューの回答時に自動実行**する
    （ユーザーに毎回選ばせる操作ではない・実フィードバック）。
  - 教訓: **ユーザーの回答は「同じ重みの真実」ではない**。UI の作りが確度を決めるので、
    入力経路ごとに重みを持たせる。件数で殴られる指標（校正・多数決）は特に危険。
- 関連: `FaceStore.peopleEligibleClusters`、`FaceQualityReport.photosWithNoFace`、
  `FaceStore.AnswerConfidence`、`FaceCorrection.confidence`、
  `FaceStore.MergeRejection`/`repairSamePhotoViolations`、`PeopleEngine.minFacesForCarousel`、
  `FaceClustering.rivalAwareSizeMargin/rivalAwareSizeMarginMaxPeople/rivalAlikeMargin/
  effectiveThresholdCap/ambiguousPolicy`、`FaceCalibration.clampRange`、
  `FaceStore.mergeCandidateFloor`、`FaceDetectionStats`、
  `PeopleEngine.clusterRuleVersion`、
  `FaceEvalMetrics.clustersPerIdentity`、`FaceStore.batchReviewItem`、`PeopleEngine.answerBatch`、
  `FaceBatchReviewView` / `AllPeopleView`。ADR-46（レビュー）/ ADR-56（自動連鎖の不採用）/
  ADR-57・58（マージン）/ ADR-66（第2パス）。

## ADR-67 Dropbox 写真のお気に入り（アプリ側）＋解析の処理順をお気に入り優先に
- 状態: 採用
- 文脈: (1) お気に入りはローカル（`PHAsset.isFavorite`）専用で、Dropbox 写真にはハートも出せなかった
  （Dropbox に favorite 概念が無い）。(2) 解析（顔スキャン・CLIP 埋め込み・シーンタグ）の処理順が
  「ローカル全部→クラウド全部（各新→古）」で、お気に入りが優先されていなかった。ユーザーは
  **お気に入り（大事な写真）から先に**解析して People 等へ反映したい。
- 決定:
  - **クラウドお気に入り（アプリ側）**: Dropbox は cloudPath 単位でアプリが管理する。`DropboxFileItem` に
    `isFavorite`（stored・既定 false）を持たせ、`DropboxPhotoStore` が UserDefaults 永続集合
    （`cloudFavoritePaths`）から items 構築時に**刻印**する。`DropboxFileItem: PhotoItem` で
    `supportsFavorite = true`＝フル画面/グリッドのハートが出る。`DropboxPhotoStore.setFavorite` で
    付け外し（永続＋表示 items へ即刻印）。`MergedPhotoItem`/`MergedPhotoStore` は既存の forward で自動対応。
  - **解析の処理順（`AnalysisOrder`・純ロジック）**: **お気に入り(ローカル→クラウド)→その他(ローカル→
    クラウド)、各群は新→古**。refKey の "L-"/"C-" と favorites 集合で 4 群に**安定分割**（群内の新→古は
    維持）。favorites はローカル（PHAsset）＋クラウド（`favoriteCloudPaths`）を合流（`favoriteImageRefKeys`）。
    適用: 顔スキャン（`analysisOrderedRefKeys`）・シーンタグ（候補を並べ替え）・CLIP 埋め込み
    （store paging のため**各ページを並べ替え**＝撮影日窓ごとにお気に入り先行の近似）。
- 結果: Dropbox 写真もお気に入りにでき、その集合が「代表写真の自動選択」「解析の処理順」に効く。
  お気に入りから先に People/検索へ反映される。トレードオフ: CLIP はページ単位の近似（全 85k を
  メモリに載せない paging と両立）。クラウドお気に入りは path 依存＝別アカウントへ切替時は別集合になる。
- 関連: `DropboxFileItem.isFavorite/withFavorite`・`DropboxPhotoStore+Favorites`（永続・刻印・setFavorite）・
  `DropboxFileItem+PhotoItem`（supportsFavorite）・`AnalysisOrder`＋`AnalysisOrderTests`・
  `PeopleSupport.favoriteImageRefKeys/analysisOrderedRefKeys`・`AutoAlbumEngine+Recognition`（タグ/CLIP 順）・
  `PhotoTagger.embedUnprocessed`（favorites）。

## ADR-66 顔の recall 回復: 品質フロア未満の顔を重心を汚さず membership 割当（第2パス）
- 状態: 採用
- 文脈: 「人が写っているのに People に出ない」写真がある。混入対策（ADR-48/52/53/56-58）で precision を
  上げた副作用で、品質フロア（FaceStore 0.40）未満の顔を**クラスタへ割り当てない**（記録のみ）ため、
  横顔（yaw≥30°で品質 0.2 キャップ）・ぶれ・小さめの顔が表示から漏れる。**テストデータセットで確認**:
  FG-NET/LFW（顔中心クロップ）の検出率は 99.6%/100%＝ゲート棄却はほぼ無い（データセットが集合写真・
  横顔・遠距離を含まないため実問題を再現しない）が、**検出・埋め込み済みの顔の 8.5%（FG-NET）/15.8%
  （LFW）が品質フロア未満で捨てられていた**（min 品質 0.20＝横顔/ぶれの 0.2 キャップ）。実写真ではこの
  割合はさらに大きいはず。
- 決定: **第2パス割当**（`FaceClustering.assignMembershipOnly`）。主クラスタリング後、フロア未満で未割当
  だった顔（埋め込みはある）を、最寄りクラスタへ**membership だけ**割り当てる。**sum/count/centroid/
  prototype は変えず faceIDs にだけ加える＝既存クラスタの純度は不変**。閾値は**データセット計測で 0.55**
  に決定（キャッシュ埋め込み＋ラベルで直接評価）:

  | データ | 捨てていた顔 | 0.55 で回復 | 正解率 | 混入 |
  |---|---|---|---|---|
  | FG-NET(子供) | 85(8.5%) | 76(89%) | 88.2% | 11.8% |
  | LFW(大人) | 769(15.8%) | 756(98%) | 98.5% | 1.5% |

  同一写真 cannot-link（除外クラスタ）も尊重。nightly rebuild と増分割当の両方に組み込む（増分は
  クラスタ形成前なら未割当のまま＝nightly が拾う）。
- 結果: いままで「0% 表示」だった顔を「89%/98% 表示・88%/98.5% 正解」に変える＝recall 回復。重心不変
  なので既存クラスタの純度・混入率は増えない（回復顔が稀に別アルバムへ入るが、元々どこにも出ていない
  写真なので実害は小さい）。フロア未満のままなので**代表顔（cover）には選ばれない**（bestCoverFace は
  高品質優先）＝アバターは正面顔のまま。検出自体に失敗した顔（size/confidence/verify 棄却＝埋め込み無し）は
  対象外（それらのゲート緩和は別途 precision 計測が要る＝残課題）。
- 関連: `FaceClustering.assignMembershipOnly`/`secondPassThreshold`・`FaceStore+Rebuild`（第2パス）・
  `FaceStore.recordScan`（増分の第2パス）・`FaceClusteringTests`（純度不変の担保）・
  `FaceInfoExpansionTests`（新挙動）・face-accuracy 台帳（計測）。ADR-48/52/53/56-58 の続き（precision↔recall）。

## ADR-65 重い画像処理を UI から隔離する（低優先レーン・QoS 付け直し・GPU 回避・UI 譲り）
- 状態: 採用
- 文脈: 「別プロセスに移して nice で優先度を下げる」を iOS でやりたい（不可）が、前景でたまに UI が
  重くなる。AI 本体（CLIP/顔/VLM 埋め込み）は既に電源＋ロック中のみ（ADR-25）＝前景では走らないが、
  **画像表示の機構**（サムネデコード・顔アバター生成・カバー生成・PHAsset 列挙・オンデマンド CLIP）が
  各所で `.userInitiated`（UI 近傍の高 QoS）で走り、コアを UI と奪い合っていた（Task 優先度分布は
  `.userInitiated` 26／`.utility` 29／`.background` 2）。サブシステムごとに個別リミッタはあるが、全体を
  束ねる低優先レーンが無かった。
- 決定（提案1〜5）:
  - **1 低優先レーンに集約**: `MosaicSupport.HeavyImageLane`（actor で**同時数=コア数−2**に制限＋
    body は呼び出し側コンテキストで実行）。急ぎでないバルク画像処理（顔アバター・カバー生成・先読み系）を
    通す＝バーストで全コアを飽和させ UI/レンダーサーバを飢餓させない。
  - **2 QoS 付け直し（nice 相当）**: ユーザーが今待っている画像だけ `.userInitiated`、非緊急は `.utility`/
    `.background` へ。降格: 顔アバター生成/デコード・カバー生成・FaceStore 初期化・バックアップメタデータ
    デコード（画面を開く操作＝`loadAssets` の 67k 列挙は据え置き）。
  - **3 2レーン化（緊急 vs バルク）**: 可視セル・タップしたフル画像＝レーンを通さず直接・高 QoS。
    バルク（一覧の顔アバター/カバー）＝ HeavyImageLane。Dropbox サムネは既存の2段バッチャが該当。
  - **4 GPU 競合の回避**: 実機の Core ML を `.all`→**`.cpuAndNeuralEngine`**（ANE 主体・GPU 回避）。
    GPU は UI 合成（Metal）と食い合うため、前景推論（検索の CLIP テキスト埋め込み）が UI 描画とコアを
    奪わないようにする。同梱モデルは INT8/fp16 で ANE 向け＝速度低下は小さい見込み。
  - **5 UI 中は譲る**: `BackgroundActivityMonitor` に nonisolated な **UI ビジー・スナップショット**
    （写真表示・**スクロール中**・クラウド/フル取得中の OR）を追加。`HeavyImageLane.waitWhileUIBusy` が
    これを見てバルク処理を一時停止・アイドルで再開。グリッドのスクロール状態を `isScrollingGrid` で報告。
- 結果: バルク画像処理が「同時数制限＋低 QoS＋スクロール中は譲る」の1本のレーンに乗り、UI とのコア/
  GPU 競合が構造的に減る（＝別プロセス低優先ワーカの近似）。可視セル・フル画像の即応性は据え置き。
  効果の定量は実機 `MainThreadWatchdog`（>83ms ヒッチ）＋ `PerfTrace` で before/after 計測して確認する。
  トレードオフ: (4) は夜間（ロック中）に GPU を使わない＝GPU 有利なモデルは僅かに遅くなり得る（前景の
  滑らかさ優先・要実機確認）。
- 関連: `HeavyImageLane`・`BackgroundActivityMonitor`(isUIBusySnapshot/isScrollingGrid)・`FaceAvatarImage`・
  `HomeRows.loadLocalCover`・`PeopleEngine`(FaceStore init)・`DropboxPhotoStore+BackupMetadata`・
  `CoreMLModelSupport.makeConfiguration`・`PhotoCollectionView`(スクロール報告)。[[ADR-25]]（実行方針）。

## ADR-64 UI/描画の向きテスト手法（四隅オラクル＋4層戦略）
- 状態: 採用
- 文脈: グリッドサムネ横倒し（ADR-62 系・真因＝小 targetSize で PHImageManager が向きの狂ったサムネを
  返す実行時挙動）は**純ロジックのユニットテストでは捕まらず**、UI/描画の実挙動を対象にするテストが
  不足していた。再発防止の手法を整備する。
- 決定: 共通の武器として**四隅オラクル**（四象限に赤/緑/青/黄を塗った非対称マーカー画像→向きを尊重して
  レンダリングした四隅の色で向きの崩れを検出＝`OrientationOracle`）を作り、4 層で使う。
  - **Layer 1 純描画ロジック**: `PHAssetImageLoader.normalizedUp`/`resizedUp`/`orientationSafeSize` を
    オラクルで検証（.up 焼き込み・縮小しても見た目維持・小要求は 640 下限）。iOS Sim・高速・決定的。
  - **Layer 2 seam でバグ本丸を再現**（★中核）: 向き安全サムネの契約を純関数 `orientationSafeThumbnail(
    cellSize:fetch:)` に切り出し、`LocalPhotoStore.requestThumbnail` はこれ経由に（実フェッチ=rawThumbnail）。
    テストは「**小要求だと向きの狂った画像を返す偽装フェッチャ**」を注入し、出力が正立になること＋
    fetch が 640 下限で呼ばれることをアサート。**実機/写真ライブラリ無しでバグ級の退行を自動検出**できる。
  - **Layer 3 実 PHImageManager 経路（フィクスチャ PHAsset・opt-in）**: 縦長マーカーを写真ライブラリへ
    一時保存→`LocalPhotoStore.thumbnail` を小/大で呼び正立を確認→**必ず削除**。`RUN_PHOTO_FIXTURE_TESTS=1`
    のときだけ実行（写真ライブラリ書き込みのため既定スキップ）。
  - **Layer 4 XCUITest 視覚回帰**（opt-in）: 主要画面（ホーム/グリッド/フル写真/人物）へ遷移して名前付き
    スクショを添付＝手動手順の formalize。`RUN_VISUAL_REGRESSION=1` のときだけ（個人写真が写り得るため
    既定スキップ・添付はローカル限定）。将来 perceptual-hash でゴールデン比較すれば完全自動化。
- 結果: Layer 1/2 は常時実行され、向き系の退行を CI で自動検出（Layer 2 は契約が小要求へ退行すると失敗）。
  Layer 3/4 は opt-in の忠実度・視覚回帰。テスト自身の妥当性も担保（`testFakeModelsTheBug` で偽装が実際に
  向きを崩すことを確認）。トレードオフ: 実行時挙動（PHImageManager の版差）そのものは seam の偽装で近似。
- 関連: `MosaicPhotosTests/OrientationOracle`・`ImageOrientationTests`(L1/L2)・`PhotoFixtureOrientationTests`(L3)・
  `MosaicPhotosUITests/VisualRegressionUITests`(L4)・`PHAssetImageLoader.orientationSafeThumbnail`・
  `LocalPhotoStore.requestThumbnail`/`rawThumbnail`。ADR-62 の続き（横倒し修正の再発防止）。

## ADR-63 AI 学習パイプラインの高速化（並列ロード・顔埋め込みバッチ・タグ並列・適応スロットル）
- 状態: 採用
- 文脈: 夜間バックログ（タグ/CLIP 埋め込み/顔/キャプション）の消化を速くしたい。実コード精査で判明した
  ボトルネック: (1) **画像ロードが逐次**（CLIP/タグ/顔の各知覚が `for refKey { await load }`＝ANE の
  バッチ推論に対しロードが直列の律速）、(2) **顔埋め込みが非バッチ**（顔ごと×マルチクロップ3で単発推論・
  CLIP はバッチ済みなのに facenet は1枚ずつ）、(3) タグの**画像間並列なし**＋`assetKindTags` の PHAsset
  再フェッチ、(4) スロットルが固定（発熱時の緩和なし）。既に最適化済み: CLIP 8枚バッチ推論・Vision 1回
  perform・タワー別遅延ロード・実行は電源＋アイドル＋ロック中のみ（ADR-25）。
- 決定（候補A〜D）:
  - **A: 画像ロードの並列化**。`loadRefImages`/`boundedConcurrentResults`（同時数制限つき TaskGroup）を
    追加し、CLIP 知覚のミニバッチ内ロードを並列化（256px・同時4）。ロード律速を解消。
  - **B: 顔埋め込みのバッチ化**。`FaceModelRuntime.embed([CGImage])`（`MLArrayBatchProvider`）を追加し、
    `analyzeFaces` を「全顔の全クロップを平坦配列に集約→1回バッチ→顔ごとに平均」へ再構成。
    **クロップ内容・平均・品質ゲートは不変＝精度は変えない**（速度だけ）。
  - **C: タグの並列化＋再フェッチ削減**。`VisionTagAdapter.senseInfo` をロード＋Vision 一括パスの
    並列（同時3）に。ローカルは PHAsset を**1回だけ**フェッチして画像取得と種別タグで共用。
    `TagTagger` の「単位」を 4 枚ミニバッチに（内部並列なので譲り応答性はほぼ維持）。
  - **D: 適応スロットル**。`BackgroundTrickle` のバッチ間スリープに**サーマル係数**（nominal 1x /
    fair 1.5x / serious 3x / critical 8x）。ユーザー選択のプリセットより速くはせず（1x が上限）、
    逼迫時のみ緩めて発熱/サーマルスロットリングを回避＝連続処理の実効速度を守る。全タガー共通。
  - CLIP 埋め込み・顔検出の**入力サイズ/内容は不変**（知覚バージョンは据え置き＝再埋め込み不要）。
- 結果: 各パスのロード律速が外れ、顔は単発推論の償却が効く見込み（実測は実機・PerfTrace の
  `clip.embedMs`/`face.photoMs`/`tags.photoMs` で確認）。精度・保存内容は不変。トレードオフ: 並列ロードで
  瞬間メモリが「同時数×入力px」ぶん増える（256px CLIP は軽微・同時数で有界）。タグの譲り粒度が 1→4 枚に
  なるが内部並列で実時間は同等。**シミュレータでは CLIP/顔/VLM は元々スキップ**のため、効果検証は実機。
- 関連: `AIImageLoading`(loadRefImages/boundedConcurrentResults)・`AIPerceptionAdapters`(perceive 並列化)・
  `FaceModelRuntime.embed([...])`・`FacePerceptionAdapter.analyzeFaces`(3パス化)・`VisionTagAdapter.senseInfo`・
  `TagTagger`(ミニバッチ)・`BackgroundTrickle`(thermalPauseMultiplier)。[[ADR-25]]（実行方針）。

## ADR-62 コードリファクタリング一式（PHAsset 画像ローダ統一・日付/Store 構築の集約・分割）
- 状態: 採用
- 文脈: 重複と分散が保守性・バグの温床になっていた。特に (a) PHAsset からの画像取得が 8 箇所以上で
  個別に手書きされ、`PHImageRequestOptions`（deliveryMode/resizeMode/network）と**向き正規化の有無**が
  バラバラ（グリッドサムネ・顔クロップの回転ズレの温床）、(b) アルバム日付の `dates.min()/max() ?? .distantPast`
  が各戦略に散在（1980 混入の温床）、(c) メンバー限定 `MergedPhotoStore` 構築が 4 ビューに複写、
  (d) 二重 resume 防止（lock+guard）の定型重複、(e) 未使用ビュー・肥大ファイル。
- 決定:
  - **画像ローダ統一**: `MosaicSupport.PHAssetImageLoader` に集約（全画像コンシューマが共通依存するのは
    MosaicSupport のみ＝新規依存エッジ不要）。`makeOptions`/`Quality` は Photos のみで成立させ macOS テストを
    維持、UIImage 系は `#if canImport(UIKit)`。向き正規化（`normalizedUp`/`normalizedUpCGImage`）・
    二重 resume 防止を内包。`loadLocalCover`/`requestAspectCGImage`/`loadLocalCGImage`/グリッドサムネ/
    キャプション等を委譲化。`orientationNormalizedCGImage` は互換名を残し委譲（呼び出し側不変）。
  - **日付集約**: `AlbumDates`（純・`CaptureDate.meaningful` で 1980 等を除外）に一本化。TimePlace/Path/AI が利用。
  - **Store 構築**: `MergedPhotoStore.forMembers(localIDs:cloudPaths:dropboxStore:assetIndex:)` に集約（4 ビュー）。
  - **分割/削除**: 未使用 `LocalThumbnailView`/`LocalPhotoPageView` を削除。`DropboxPhotoStore` の
    バックアップメタデータ節を `+BackupMetadata.swift` へ抽出（`backupMetadata` を internal(set) に）。
- 結果: 画像経路の向きが一元管理になり、重複が大幅減り、今後の画像取得は 1 経路に集約。**残っていた
  グリッド横倒しは後続で真因（小さい targetSize）を特定し解決**（`orientationSafePixel`=640 下限で取得→
  縮小・case-studies「グリッド…横倒し」項）。トレードオフ: MosaicSupport が Photos/UIKit ユーティリティを
  持つ（既に PowerStateMonitor で UIKit 使用の前例あり）。
- 関連: `PHAssetImageLoader`・`AlbumDates`・`MergedPhotoStore+Members`・`DropboxPhotoStore+BackupMetadata`／
  コミット 2d2c3d59・95be4abc・d258c009。回転の未解決分は case-studies「代表顔/グリッド回転」項に継続。

## ADR-61 2 階層人物モデル（人物=複数クラスタの束）の検証: 識別では融合に優位なし・UI 保留
- 状態: 保留（純ロジック＋評価のみ実装・本番データモデル/UI 未着手）
- 文脈: 「1 人=1 クラスタ」の前提を捨て、同一人物が成長帯別に複数クラスタになることを許容し、上位の「人物（Person）」で束ねる案。融合（物理的に 1 重心へ潰す＝現行の統合）だと成長期に重心が壊れる懸念があり、クラスタを別々に保ったまま束ねれば純度を保てるのではという仮説。**大きな UI/データモデル変更に進む前に、検証可能な核（純ロジック＋評価）で優位を確かめる方針**。
- 決定: `FacePersonGrouping`（人物=複数クラスタ代表・帰属は全クラスタ代表との最大類似度・純ロジック）を実装し、ハーネスで「融合 vs 2 階層」の人物識別精度を FG-NET で計測（登録80%/クエリ20%・年齢混在分割と時系列分割の 2 通り）。**結果: 2 階層は融合を上回らない**（混在 93.5% < 融合 95.9%／時系列 88.1% < 89.0%）。→ **UI/データモデル実装は保留**。
- 理由: (1) 当初仮説「融合すると重心が壊れる」が実データで成立せず——成長差程度なら平均重心が十分カバーし融合は頑健。(2) 2 階層は人物あたりの顔が限られると時期別クラスタが小さくノイジーになり、最大類似度帰属が別人の小クラスタへの誤帰属を増やす（識別率がむしろ低下）。(3) 分裂クラスタは現行の「統合（融合）」で束ねれば識別は落ちないため、2 階層の複雑なデータモデルを入れる技術的メリットが（識別の観点では）薄い。
- 結果: ユーザー直感（複数グループ許容で成長問題を解決）を、UI 大改修の前に検証可能な形で評価し、優位が無いことを確認して無駄な実装を回避（「検証可能な実装方式」の価値そのもの）。純ロジック `FacePersonGrouping` とハーネス評価は将来の再検証資産として残置（本番未接続）。表示上の分裂解消という別価値は残るが、それは融合統合で足りる。
- 関連: `FacePersonGrouping.swift`（純・本番未接続）・`FacePersonGroupingTests`・`FaceAccuracyEvalTests`（識別評価・混在/時系列分割）・`records/face-accuracy.md`。ADR-60 の続き。
- 追補（分け方の再検証・結論を一部修正）: 「2 階層は優位なし」は**自動クラスタリングで平均 2.7 個に細分割した場合**の結論だった。分け方を**少数の年齢帯固定分割**に変えると逆転する。人物識別（FG-NET）で、成長を跨ぐ時系列分割: 融合 89.0% に対し 帯2 91.1%/帯3 **92.8%**/帯4 92.4%/帯5 90.3%＝**少数帯（2〜3）は融合を上回る**（帯3 が +3.8pt 最良）。通常時（実運用分割）は融合 95.9% と同等。＝細分割が悪く、少数の時期帯分割は「成長を跨ぐと勝ち・通常は同等」でほぼ上位互換。細かすぎ（帯5）はノイジー化で落ち最適は 3 帯前後。**年齢推定 API は不要で撮影日で代用可能**（子供は撮影日≒年齢・EXIF 確実。年齢帯の数値が撮影日帯分割の効果を示す）。→ 撮影日帯で人物内をグループ化する 2 階層の実装を次段で検討（保留から前進）。
- 追補2（適応的 2 階層の裏取り・設計確定）: 「大人分割は害」の懸念を LFW（成人 423 人）で検証＝融合 99.1% ＝ 自動 2 階層 99.1%・成人の自動クラスタ数は平均 **1.2**（成人は元々 1 グループにまとまり分割されない＝分割の害は撮影日機械分割時のみ）。→ **設計確定: 適応的 2 階層**＝クラスタリングは現状維持、分裂した**子供クラスタのみ**を人物として時期グループで束ね、**大人は融合**。子供/大人の判定は**代表顔に VLM で「子供か否か」を問う**（全顔不要・粗い 2 分類・数枚＝コスト最小＝ユーザー案）。分割軸は撮影日（子供は撮影日≒年齢・EXIF 確実・年齢推定モデル不要）。帰属は全グループ代表の最大類似度。実証: 子供は時期分割で成長を跨ぐ識別 +3.8pt（FG-NET）・大人は融合で 99.1% 維持（LFW）＝両立。次段はデータモデル（人物→複数グループ）＋VLM 子供判定＋撮影日束ねの実装。
- 実装（A→B 完了・ADR-61 の結論を反映）: **適応的 2 階層を本番実装**した。設計を簡素化＝**束ね＝子供の代理**（大人は分裂せず束ねる機会がないので VLM 子供判定は不要）。(A) `FacePersonGrouping.personReps`（撮影日で子供は時期分割・大人は融合の代表を作る純ロジック・テスト）。(B-1) `DetectedFace.captureDate`／`PersonCluster.personGroupID`／`DetectedFaceSignal.captureDate` を optional 追加（FacesV1 据え置きの軽量マイグレーション）＋`FacePerceptionAdapter` が PHAsset.creationDate を載せる。(B-2) `peopleClusters` を personGroupID 束ね対応（融合せず純度保持・全 nil で旧挙動一致）＋`linkClusters`/`unlinkCluster`/`ungroupPerson`。UI は「別の人物へ統合（融合・不可逆）」→「同じ人物として束ねる（純度保持・解除可能）」に置換＋束ね解除メニュー（`PersonInfo.isGrouped`）。(B-3) 名前解決（`peopleNames`/`peopleNamesByRefKey`）と顔ハイライト（`faceBoxes`）を束ねグループ対応＝検索・表示・枠が束ねた人物を 1 人として一貫。クラウド撮影日 seam と「新規顔の時期グループ帰属」（personReps を assign に使う）は将来拡張として残す。効果は ADR-61 本文の計測どおり（子供は時期分割で成長を跨ぐ識別 +3.8pt・大人は融合維持）。

## ADR-60 年齢を使った顔クラスタリング: 計測の結果 不採用
- 状態: 不採用（評価はハーネス内のみ・本番コード変更なし）
- 文脈: 「年齢を推定して年齢ごとに分類すれば成長期の分裂が解決するのでは」という案。公開の年齢推定 API は無い（内部の年齢層分類はあるが呼べない・サードパーティ同梱はライセンス/サイズ/精度の問題）が、FG-NET の正解年齢ラベルを「推定が完璧だった場合の入力」として使えば**上限効果**をモデル同梱なしで測れる。
- 決定: 2 つの使い方を FG-NET で計測して採否。**両方とも不採用**。(X-1) 年齢層で顔を分割→各層内クラスタリング＝純度は現行同（0.879）だが**網羅が 0.443→0.252 に半減**（同一人物が年齢帯をまたいで分裂＝求める方向の逆）。(X-2) 通常クラスタ後に年齢範囲が連続するクラスタ対を統合＝**純度が 0.10 に崩壊**（同年代の別人を大量誤統合・年齢制約付きでも推移的統合は雪崩。ADR-56 の P3 と同型）。
- 理由: 年齢は「同一人物か別人か」を分離する力をほぼ持たない——同年齢の別人は無数におり、同一人物は年齢をまたぐため。**完璧な年齢でこの結果なので、誤差のある年齢推定モデルを同梱する費用対効果は無い**と確定。資料の示唆（人物クラスタリングは faceprint 距離のみで行い、年齢層は関係推論・メモリー・検索に使う＝人物同定の軸にしない）とも一致。年齢差を乗り越えるのはクラスタリング工夫ではなくモデルの質（ADR-55 の TAR@FAR が年齢差で劣化＝埋め込み自体の限界）の問題。
- 結果: ユーザー直感（年齢分類で解決）を実装前に否定でき、モデル同梱の無駄を回避。年齢情報の残る可能性は「人物同定でなく別人排除の弱い制約／関係推論（親子・兄弟）／検索絞り込み」だが、いずれも公開 API では年齢が取れず現状は保留。
- 関連: `FaceAccuracyEvalTests`（年齢クラスタリング計測・FG-NET 年齢ラベル使用）・`records/face-accuracy.md`。ADR-55/56 の続き。

## ADR-59 クラスタ純度の事後検査（外れ値除去）: 計測の結果 不採用
- 状態: 不採用（評価実装は残置）
- 文脈: 一般的なクラスタリングに「クラスタ代表からの距離の重み付き中央値で外れ値を検出し純度を保つ」考え方がある。我々は入口ゲート（品質フロア）だけで事後検査を持たないため、中央値ベースの外れ値除去を後段に足せば混入をさらに減らせる見込みで評価した（中央値重心バリアントが LFW 最良だった傍証もあり）。
- 決定: `FaceClusteringVariants.pruneOutliers`（各クラスタでロバスト重心＝要素中央値からの距離が「中央値距離×dropFactor」かつ「絶対 minAbsDist=0.30」を超える顔を外れ値として単独化）を評価実装し、現行本番構成の後段で FG-NET/LFW を計測。**結果: 不採用**。純度は上がるが FG-NET（成長・主眼）で総合 F1 が全設定 0.566〜0.584 と prod 0.589 を下回った。LFW（同年代）は 0.785→0.789 の微改善。
- 理由: 外れ値除去は「クラスタ重心から遠い顔＝別人の混入」を前提とするが、成長データでは「遠い顔＝同一人物の別年齢の顔」であることが多く、正当な顔を弾いて網羅（再現率）が落ちる。まさに主眼（成長期の子供・兄弟）で逆効果。難所を優先する方針（ADR-56）に従い既定不採用。
- 結果: 「純度を上げれば良い」という直感が、成長データでは網羅を犠牲にして総合を下げることを実装前でなく**計測で**確認できた（計測駆動の価値）。評価実装は将来の参照（同年代中心ライブラリ向けオプション等）に残す。
- 関連: `FaceClusteringVariants.pruneOutliers`（本番未接続）・`FaceAccuracyEvalTests`（E スイープ）・`FaceClusteringVariantsTests`・`records/face-accuracy.md`。ADR-58 の続き。

## ADR-58 サイズ適応マージン（小/新クラスタの合流を厳しく）
- 状態: 採用
- 文脈: ADR-57 のマージンゲートに続く精緻化。固定しきい値・固定マージンは全クラスタ一律だが、一般的な貪欲クラスタリングには「**確立の浅いクラスタは合流を厳しく、多数の顔が集まって確立した人物には寛容にする**」適応マージンの考え方がある。成長期に分岐した年齢帯クラスタは最初小さいため、兄弟を吸い込む誤併合が起きやすい——ここに直接効く可能性。
- 決定: `FaceClustering` に**サイズ適応マージン**を追加。合流の実効しきい値を `threshold ＋ sizeMargin(count)` とし、`sizeMargin` は count=1 で最大（`sizeAdaptiveMarginMax`）・成熟サイズ（既定 11）で 0 へ線形減衰。評価バリアントとして FG-NET/LFW でスイープし採否決定。本番採用構成は `clusterThreshold=0.50`＋`assignMargin=0.05`＋`sizeAdaptiveMarginMax=0.10`（逐次スキャン・夜間再クラスタ共通）。
- 結果: 両データセットで純度・F1 とも改善（FG-NET F1 0.585→0.589・純度 0.849→0.879／LFW F1 0.775→0.785・純度 0.867→0.902）。初期状態（0.45・F1 0.444）からの累積で **FG-NET F1 +33%・純度 0.42→0.88**。トレードオフ: (1) 上乗せ最大 0.15 は網羅が崩れる（0.10 が最良）。(2) 固定マージンとの併用が必須（サイズ適応のみは純度 0.69 に劣化＝小クラスタが乱立）。(3) 紛らわしい顔・小クラスタは単独に留まりやすく、レビュー A1 が受け皿。実機は夜間再クラスタ（しきい値変更トリガ）で移行。
- 関連: `FaceClustering.sizeAdaptiveMarginMax/sizeAdaptiveMatureCount/sizeMargin`・`FaceStore`（3 定数・2 構築点）・`FaceAccuracyEvalTests`（D スイープ）・`FacePhase1Tests`・`records/face-accuracy.md`。ADR-56/57 の続き。

## ADR-57 マージンゲート付きクラスタリングの採用（系統1 計測）＋ LFW ベンチマーク追加
- 状態: 採用
- 文脈: ADR-56 でしきい値 0.60 を採用した続き。P3（推移的連鎖統合）の失敗を踏まえ、**推移性を持たない**局所判定で再現率を取り返す 3 手法（A マージンゲート付き貪欲 / B 二段階＋比率テスト / C 中央値重心）を評価実装し、FG-NET（難所）＋新規追加の LFW（易しいケース・4,862 枚/901 人）で計測して採否を決めた。
- 決定: **A を採用**＝`assign` に「1 位と 2 位のクラスタ類似度差が margin(0.05) 未満の紛らわしい顔は合流させない」ゲートを追加し、しきい値は 0.55 に変更（`FaceStore.clusterThreshold=0.55`/`assignMargin=0.05`・逐次スキャンと夜間再クラスタの両方に適用・cannot-link されたクラスタとの紛らわしさも競合として扱う）。FG-NET F1 **0.585**（ADR-56 の 0.542 比 +8%・純度 0.849）・LFW 0.775（旧 0.45 比 +36%）。margin=0.10 は網羅崩壊、B は劣後、C（中央値重心）は LFW 最良 0.807 で**次点として将来の併用候補**に記録。LFW 検証で TAR@FAR1%=98.6%＝facenet の弱点が年齢変化に集中していることも確認（モデル換装の期待値の根拠）。あわせて `FaceClustering.dot` を vDSP 化（照合が夜間再クラスタの支配項）。
- 結果: クラスタリング F1 は初期状態（0.45・0.444）から 2 段階の計測駆動改善で **0.585（+32%）**・純度 0.42→0.85 に到達。トレードオフ: 紛らわしい顔が単独クラスタに留まりやすくなる（クラスタ数増）が、これは「間違えて統合するより保留して人間に聞く」方針（ADR-56）と整合し、レビュー A1 が受け皿になる。実機は夜間再クラスタ（しきい値変更トリガ）で移行。
- 関連: `FaceClustering.assignMargin`/`dot(vDSP)`・`FaceStore.clusterThreshold/assignMargin`・`FaceClusteringVariants`（評価用）・`FaceAccuracyEvalTests`（LFW/縮小グリッド/FACE_EVAL_ONLY・SKIP_VERIFY）・`scripts/fetch_face_eval_datasets.sh`（LFW 追加）・`records/face-accuracy.md`。ADR-55/56 の続き。

## ADR-56 データセット計測に基づく顔認識改善の採否（しきい値 0.60 採用・自動統合系は不採用）
- 状態: 採用
- 文脈: 識別精度（TAR@FAR1%=48.1%・B-Cubed F1=0.444）が主課題。改善候補 P1（しきい値引き上げ）/P2（自動マルチプロトタイプ）/P3（クラスタ連鎖統合）/P4（偽陽性計測）/P5（モデル換装再調査）を、ADR-55 のハーネスで**データセット計測して採否を決める**方針（自前写真は使わない）。
- 決定と計測結果（FG-NET・詳細は records/face-accuracy.md）:
  - **P1 採用**: 既定しきい値 0.45→**0.60**（F1 0.444→0.542・純度 0.42→0.72・再現率 0.47→0.435）。混入側の自動回復手段は無いが分裂側には統合サジェスト・名前持ち越しがあるため、純度優先が有利。`FaceStore.clusterThreshold=0.60`・**しきい値変更を夜間再クラスタの発火条件に追加**（版上げ不要で全体反映）。
  - **P2 不採用**: オンライン自動プロトタイプ（K=5・多様性 0.75）は全しきい値で悪化（thr0.60: F1 0.523 vs 0.542）。クラスタ内の単結合的連鎖で混入増。実装は評価用に残置（本番は従来どおり確認アンカーのみ）。
  - **P3 不採用**: 推移的連鎖統合は精度崩壊（純度 0.10〜0.34）。数十万ペアの裾では別人の子供同士にも高類似ペアがあり雪崩統合になる。**自動統合はせず、統合は人間の確認（A1 レビュー）に限定**が結論。実装（chainMergePlan）は評価用に残置。
  - **P4 完了**: 顔なし 18 枚（風景/物/猫犬・偽陽性の典型源）で**誤採用 0（0.0%）**＝ゲート群が偽陽性を抑止できていることを初めて数値で確認。
  - **P5 保留継続**: ArcFace 級で許諾的ライセンスの重みは現存せず（コード MIT でも重みは非商用が通例）。商用可の dlib 系は現行 facenet 同等精度で妙味なし。
- 結果: 「候補→計測→数値で採否」の運用が初めて一巡し、直感に反する結果（P2/P3 の悪化）を実装前に検出できた。クラスタリング F1 は 0.444→0.542（+22%）。トレードオフ: しきい値 0.60 で分裂は微増（統合サジェスト帯域 [0.50,1.0] とレビューが受け皿）。実機は次回の夜間再クラスタで新しきい値に移行する。
- 関連: `FaceStore.clusterThreshold`・`PeopleEngine.rebuildClustersIfNeeded`（threshold marker）・`FaceClustering.autoPrototypeLimit/selectPrototypes/chainMergePlan`（評価用・本番未接続）・`FaceAccuracyEvalTests`（バリアント計測・negatives）・`records/face-accuracy.md`（計測ログ）。ADR-54/55 の続き。

## ADR-55 顔認識の精度計測ハーネス（正解ラベル付きデータセット＋B-Cubed/TAR@FAR）
- 状態: 採用
- 文脈: 顔認識の改善（ADR-45〜54）を重ねてきたが、精度を**数値で**測る仕組みがなく、しきい値・ゲート変更の効果は実機の体感に頼っていた（変更→端末で大量スキャン→目視、のループは遅すぎる）。難所（兄弟・0〜15 歳の成長）を含む計測基盤が要る。
- 決定: (1) **データセット**: `scripts/fetch_face_eval_datasets.sh` が `~/DEV/tmp/face-eval/` に収集。`fgnet/`＝FG-NET Aging Database（82 人・1,002 枚・0〜69 歳の同一人物成長写真・ファイル名から人物 ID/年齢の labels.csv を自動生成。学術用途公開のため**内部評価のみ・同梱/再配布禁止**）＋ `own/`＝自前写真テンプレート（images/＋labels.csv。実ライブラリの兄弟・成長期はここが最も実態に合う）。(2) **評価 API**: `FacePerceptionAdapter.debugAnalyzeWithEmbeddings`＝本番同一経路（検出→全ゲート→アライメント→マルチクロップ平均）で埋め込みまで返す。(3) **指標（純ロジック `FaceEvalMetrics`・テスト付き）**: クラスタリング品質＝**B-Cubed P/R/F1**＋ペア一致 P/R/F1（precision=混入・recall=分裂と対応し、変更がどちらを動かしたか分離して見える）。検証＝同一/別人ペア類似度から **TAR@FAR 1%/0.1%**・最良 F1 しきい値。(4) **ハーネス**: `FaceAccuracyEvalTests`（シミュレータ・実機不要）が全データセットを評価し、年齢帯別の顔採用率・年齢差バケット別の同一人物類似度・「別人×両者12歳以下」（兄弟の代理）・**しきい値スイープ表（0.35〜0.70）**を `FACEEVAL:` 行に出力。埋め込みはデータセット内に `embeddings-v{faceScanVersion}.json` でキャッシュ（2 回目以降は指標計算のみ・版上げで自動無効化）。
- 結果: 「変更→1 コマンド→数値」の計測ループが成立し、しきい値 0.58 論争などを実データで決着できる。トレードオフ: (1) シミュレータは CoreML CPU 実行のため絶対値は実機と微差（相対比較には十分）。(2) FG-NET は白黒スキャン混在の研究データで、実ライブラリの分布とは異なる＝**own/ の自前ラベル写真が本命**（FG-NET はモデル間・変更前後の相対比較用）。(3) 並列テスト実行では print が出ないため `-parallel-testing-enabled NO` で実行する。
- 関連: `FaceEvalMetrics.swift`・`FaceAccuracyEvalTests.swift`・`scripts/fetch_face_eval_datasets.sh`・`FacePerceptionAdapter.debugAnalyzeWithEmbeddings`・`FaceEvalMetricsTests`。事例: Swift の `split(separator: "\n")` は CRLF（"\r\n"＝1 書記素）にマッチせず CSV が 0 件パースになる罠（`components(separatedBy: .newlines)` で解消）。
- 初回計測（FG-NET・v4 パイプライン・シミュレータ）: 顔採用 998/1002（99.6%・0-2歳でも 98%＝ゲートの過剰棄却なし）。検証=同一ペア平均 0.550/別人 0.151・TAR@FAR1%=48.1%・最良 F1 しきい値 0.64。**年齢差別の同一人物類似度: 0-2年=0.718 → 6-10年=0.558 → 11-20年=0.440 → 21年+=0.400**（年齢差 10 年超で現行しきい値 0.45 を平均が下回る＝成長期分裂の定量確認）。別人×両者12歳以下（兄弟代理）=0.294。**しきい値スイープ: B-Cubed F1 は 0.55〜0.65 で最大（0.60 で 0.542・precision 0.42→0.72）**、現行 0.45 は F1=0.444＝precision 側に大きな伸び代。既定しきい値の引き上げ（0.55〜0.60）は FG-NET 上ではデータ支持あり——ただし分布が異なるため **own/（自前ラベル写真）での計測を経てから判断**する。

## ADR-54 顔認識フェーズ1（cannot-link・共起 notSame・マルチクロップ埋め込み・サジェスト帯域拡張ほか）
- 状態: 採用
- 文脈: 外部レビュー由来の改善タスク 6 件を検証したところ、採用可 3・修正のうえ採用 2・誤り 1（ArcFace「MIT」→実際は非商用限定）だった。加えて「モデル不要でデータだけで効く」制約 2 件（同一写真 cannot-link・共起 notSame）を追加し、フェーズ 1 として一括実装（再スキャンは v4 の 1 回に集約）。
- 決定:
  - **同一写真 cannot-link**: 1 枚の写真に同じ人物は 1 回しか写れない。`assign(excludedClusterIDs:)` を追加し、逐次スキャン（recordScan・写真内で使用済みクラスタを除外）と制約付き再クラスタ（品質降順ループ・確認顔の占有を事前登録）の両方に適用。兄弟・家族写真の混入への直接対策。
  - **共起 notSame**: 2 クラスタが同じ写真に **3 回以上**一緒に写っていたら別人とみなし、統合サジェストを出さない（`coOccurrenceNotSame`・ユーザー操作ゼロで兄弟の誤統合を防ぐ）。
  - **統合サジェストの帯域上限撤廃**（旧⑤の 1 行版）: A1 の帯域を [thr−0.10, thr) → **[thr−0.10, 1.0]**。制約付き再クラスタは命名クラスタを別々の種として保持するため、しきい値以上の高類似ペアが統合されずに残る実ギャップがあった（新 UI 不要・既存レビューに乗る）。
  - **マルチクロップ埋め込み平均**（旧④）: 主クロップ＋水平反転＋（アライメント時は）bbox 切り抜きの 3 埋め込みを要素平均→再正規化（純関数 `averagedEmbedding`）。切り抜きゆらぎへの安定性向上。埋め込みが変わるため **faceScanVersion v4** に採番（名前持ち越しで全再スキャン）。
  - **定数引き上げ**（旧③）: `minFacePixels` 32→**48**（facenet の 160px 入力に対し 32px は拡大しても品質が出ない）・`blurHardFloor` 15→**25**。
  - **再クラスタ発火条件の拡張**: 「修正が増えた」に加え「**新規スキャン 500 写真以上**」「**30 日経過**」でも夜間再クラスタ（順序依存の誤りを定期自己修復）。
  - **校正レンジ拡張**（旧②の修正版）: `clampRange` 0.35...0.60 → **0.35...0.70**。既定 0.45 は据え置き（0.58 への固定引き上げは、実ライブラリの症状が「分裂过多」でありモデルの公表しきい値とも整合しないため不採用。v4 再スキャン後にハーネスで実測してから判断）。
  - **clusterAll 衛生修正**（旧①）: qualityFloor/qualities 引数を追加（本番経路は未使用でテスト専用だったため実害なし・誤用防止）。
- 結果: 混入（cannot-link・共起・定数）と分裂（帯域撤廃・マルチクロップ・定期再クラスタ）の双方に、モデル変更なしで対処。トレードオフ: (1) v4 全再スキャン 1 巡（埋め込み 3 倍推論込み・夜間）。(2) 共起 3 回は誤検出の偶発重複でも達し得るが、抑制対象は「サジェストを出さない」だけで統合自体は手動で可能。(3) 年齢/性別推定（VLM 利用のフェーズ 2 案）とクラウド高解像度補完（フェーズ 3 案）はユーザー判断で不採用。ArcFace はライセンス問題（非商用限定）が解決するまで保留のまま。
- 関連: `FaceClustering`（excludedClusterIDs/clusterAll/averagedEmbedding）・`FaceStore`（cannot-link×2・coOccurrenceNotSame・帯域）・`FacePerceptionAdapter`（マルチクロップ・horizontallyFlipped）・`PeopleEngine`（faceScanVersion=4・発火条件）・`FaceCalibration.clampRange`・`FacePhase1Tests`。ADR-45/46/48/51〜53 の続き。

## ADR-53 顔の偽陽性対策（検出信頼度ゲート＋クロップ再検証）と検証ハーネス
- 状態: 採用
- 文脈: ぼけて顔ですらない写真（模様・物体）が顔として登場する実障害。原因は 2 系統: (1) ADR-48/52 のゲートは**スキャン時**に効くため、v2 全再スキャン（ADR-51）が完了するまで旧来のゲート前データが残る。(2) 検出信頼度（`VNFaceObservation.confidence`）を見ておらず、「切り抜いた結果が本当に顔か」の再確認も無かったため、偽陽性がぼけ/露出ゲートをすり抜け得た（模様は鮮明・適正露出のことがある）。また、しきい値調整のたびに端末へ入れて大量スキャンする検証ループが遅すぎる問題があった。
- 決定: (1) **検出信頼度ゲート**: confidence < 0.8 の顔は埋め込み自体を行わない。(2) **クロップ再検証（二段検出）**: 顔中心に切り抜いた画像内でもう一度 `VNDetectFaceRectanglesRequest` を実行し、クロップ幅の 25% 以上を占める顔が検出できなければ「顔でない」と棄却（模様・物体の誤検出はここで落ちる）。Vision 不能環境（シミュレータの一部）は判定不能＝棄却しない。(3) **検証ハーネス**: adapter の判定経路を `analyzeFaces`（1 顔ごとの `FaceGateReport`＝通過/棄却理由・信頼度・品質・ぼけ・露出・寸法）に再構成し、公開 `debugAnalyze` を新設。`FaceGateHarnessTests`（アプリターゲット・XCTest）が **`~/DEV/tmp/face-samples/` の問題写真**を本番同一経路（1024px ロード込み）で解析して `FACEGATE:` 行に判定表を出す。**端末投入・ライブラリスキャン不要**で「写真を置く→1 コマンド→数値を見て `FaceQualityGate` を調整→再実行」を数十秒で回せる（フォルダ/モデル無しは skip・最終確認は実機）。
- 結果: 偽陽性は「低信頼度」「クロップ内に顔なし」の 2 段で落ち、ぼけ/露出/サイズと合わせ 6 種のゲートになる。しきい値調整の反復が実機大量スキャンからハーネス実行へ短縮。トレードオフ: 顔 1 つあたり Vision 矩形検出が 1 回増える（クロップは小さく数 ms 級・夜間のみ）。既存の汚染データは v2 再スキャン完了までは残る（Developer Options の Force heavy work + Run BG routine now で前倒し可能）。
- 関連: `FaceSeams.swift`（minDetectionConfidence/cropVerifyMinSide）・`FacePerceptionAdapter`（analyzeFaces/FaceGateReport/debugAnalyze/verifyFaceInCrop）・`MosaicPhotosTests/FaceGateHarnessTests.swift`。ADR-48/51/52 の続き。
- 追記2（実フィードバック「IMG_020432.HEIC などで顔の位置がズレる」）: 真因は **EXIF 回転の不統一**。`UIImage.cgImage` は回転未適用の生ビットマップのことがあり（HEIC 顕著）、PHImageManager はターゲットサイズにより回転適用済み/未適用のどちらも返し得る。検出（640/1024px）とアバター（400/600px）で別表現を掴んだ写真だけ顔矩形がズレていた。対処: **全 CGImage 化を `orientationNormalizedCGImage`（.up へ再描画・MobileCLIPKit 公開）に統一**（検出 loadLocalCGImage・クラウド seam×2・アバター requestAspectCGImage/クラウド）。検出画像の画素が変わるため `faceScanVersion` を v3 に採番（旧ズレデータは再スキャンで作り直し・名前は持ち越し）。
- 追記（実フィードバック「レビューに顔じゃないものが出る」）: 境界レビュー（A2）は「クラスタ内で最も疑わしい顔」を選ぶ設計のため、**旧データに残る偽陽性がまさに最優先で出題**されていた（切り抜き座標は検出・アバターとも PHImageManager 同経路で一貫＝位置バグではない）。対処として `reviewItems` の境界候補から**品質フロア（0.40）未満の顔を除外**（判断を求める価値がない・再スキャンで浄化されるまで表示にも出さない）。

## ADR-52 顔ごとのぼけ・露出ゲート＋絶対ピクセルの最小顔サイズ
- 状態: 採用
- 文脈: ADR-48 のゲート（顔向き・目閉じ・正規化サイズ比）後も、ぼけた顔・極端な露出（暗部/白飛び）の顔が faceCaptureQuality だけでは十分に弾けず、クラスタ精度を下げる不確かな埋め込みが混入し得た。また最小顔サイズが正規化比率のみのため、低解像度画像では比率を満たしても実ピクセルが小さすぎる顔が通っていた。顔単位のぼけ・露出を直接返す公開 API は無いため、公開手段（画素統計）で同等のゲートを作る。
- 決定: (1) **ぼけスコア**＝顔クロップを 64px 正方グレースケールへ縮小し**ラプラシアン分散**を計算（純ロジック `FaceImageMetrics.compute`・固定サイズへの縮小でクロップ寸法に依らない同一スケール）。分散 <15 は強ぼけ→品質を 0.2 へキャップ（フロア 0.40 未満＝クラスタ不参加）、<60 は軽ぼけ→品質 ×0.7。(2) **露出スコア**＝同じ縮小輝度の平均。<20 / >235 は極端→キャップ、<40 / >215 は暗め/明るめ→品質 ×0.6。(3) **絶対ピクセルゲート**＝顔 bbox の実ピクセル辺 <32px は埋め込み自体を行わず除外（正規化比率ゲートとの二段構え）。(4) しきい値・減衰規則は `FaceQualityGate` に集約（未計測 nil は減衰なし＝フォールバック検出でも安全）。指標計算はシミュレータの CIDetector フォールバック時にも効く（品質 1 固定の穴を補完）。
- 結果: ぼけ顔・露出破綻顔・極小顔の不確かな埋め込みがクラスタへ入らず、重心汚染と誤マージが減る。記録自体（DetectedFace）は残るため顔数・枠表示は不変。埋め込みの作り方は変わらないため**再スキャン不要**（版上げなし・ADR-51 の v2 再スキャンで既存顔にも自然適用）。トレードオフ: 顔 1 つあたり 64×64 の縮小描画＋2 パス走査が増えるが数 ms 級（夜間のみ）。しきい値は一般的な経験値ベース＝実機の分布を見て `FaceQualityGate` で調整する。
- 関連: `FaceSeams.swift`（FaceQualityGate 拡張）・`FaceAlignment.swift`（FaceImageMetrics）・`FacePerceptionAdapter`（faceMetrics/絶対ピクセルゲート）・`FaceInfoExpansionTests`（ぼけ/露出/指標/ピクセル各テスト）。ADR-48/51 の続き。

## ADR-51 顔アライメント（目の位置正規化）＋処理解像度 1024px（スキャン版数機構つき）
- 状態: 採用
- 文脈: facenet は「目が水平・標準位置に揃った顔」で学習されているが、現行は bbox＋30% マージンの矩形切り抜きのみで、顔の傾き・位置が写真ごとにバラバラのまま埋め込みに渡っていた（同一人物でも角度違いで埋め込みが離れ、分裂の一因）。また端末写真の処理解像度 640px では集合写真の端の顔が 30px 程度になり埋め込み品質が劣化していた。
- 決定: (1) **顔アライメント**: 両目ランドマーク（ADR-48 で取得済み）から「回転角＋切り抜き位置」を計画する純ロジック `FaceAlignment.plan`（目線を水平に回転・両目中点を上から 35%・横中央へ・辺は bbox 長辺×1.6＝従来と同画角。目が近すぎ/傾き>45°/顔<16px は nil）を新設し、adapter が CGContext の回転描画で切り抜く。**計画不能な顔は従来の bbox 切り抜きへフォールバック**（シミュレータ・ランドマーク欠落でも動く）。(2) **処理解像度**: 端末写真を 640→**1024px**（メモリ増 ~2.6 倍/枚は夜間・1 枚ずつ＋メモリ圧迫ゲートで吸収。クラウドはキャッシュサムネのまま）。(3) **スキャン版数機構（今回の肝）**: 埋め込みの作り方が変わると新旧の埋め込みでコサイン類似度が壊れるため、`PeopleEngine.faceScanVersion`（v2 採番）を新設。版上げ検知で**命名クラスタのスナップショット**（名前＋メンバー refKey・最大500）を Application Support に永続化 → 全消去 → 通常の夜間スキャンで再構築 → セッション末尾ごとに**写真の重なり（≥ max(2, 旧メンバーの20%)）で名前を新クラスタへ段階的に再適用**（数晩に跨る再スキャンでも名前が戻る・90 日で失効）。修正ジャーナル（FaceCorrection）は残す（同一モデルのため負例・校正は引き続き有効。アライメントで埋め込みは多少ずれるが照合しきい値内と判断）。
- 結果: 同一人物の埋め込みが近づき（アライメント）、小さい顔の品質が上がる（1024px）＝分裂・混入の双方に効く。版数機構により今後のパイプライン変更（ArcFace 換装等）も「版を上げるだけ」で命名を失わずに移行できる。トレードオフ: (1) 全写真の再スキャン 1 巡（夜間・数晩）。(2) 再スキャン中は人物アルバムが一時的に未命名で再構築されていく（名前は進行に応じて自動復帰）。(3) 確認済みアンカー（confirmedAt）は顔行の作り直しで失われる（faceID の index が変わり得るため持ち越さない・レビューで再蓄積）。
- 保留（本資料の未実施案・値と論点の控え）: **ArcFace 換装**＝標準 ArcFace は入力 112×112・5 点アライメント前提（本 ADR のアライメントが前提条件）。insightface のコードは許諾的だが**学習済み重みは非商用限定が多く配布前にライセンス確認必須**（EdgeFace 等の代替も検討）。変換は PyTorch→coremltools 経路。**クラウド重要写真の高解像度補完**＝お気に入り＋命名クラスタのメンバーに限り w640h640 を夜間取得して再スキャン（サイズ指定サムネ取得と refKey 単位の再スキャン API が必要）。**連絡先ピッカー**＝既存の名前変更フローに CNContactPicker を追加（権限はピッカー表示まで遅延）。**再クラスタ発火条件の拡張＋same-photo cannot-link**＝「修正増」に加え「新規顔 500 件/30 日」でも発火・同一写真内の顔は同一クラスタへ入れない制約（フル DBSCAN は O(n²) で端末夜間には過大のため不採用）。
- 関連: `FaceAlignment.swift`（純ロジック）・`FacePerceptionAdapter`（alignedCrop/1024px/regionCenter）・`FaceStore`（namedClusterEntries/reapplyNames → ADR-232 で assertedClusterEntries/reapplyAssertions に改名）・`PeopleEngine`（faceScanVersion/migrateScanVersionIfNeeded/名前持ち越し永続化）・`FaceAlignmentTests`。ADR-45/46/48 の続き。

## ADR-50 認識・検索パラメーターのチューニング（タグ 2 段化・OCR 足切り・カバーの利用シグナル）
- 状態: 採用
- 文脈: 各種しきい値の総点検を実施。顔クラスタリング系（品質フロア 0.40・顔向きゲート・クラウド顔サイズ 0.15 等＝ADR-48）と付帯情報系（OCR/人数/美的スコア＝ADR-47）は点検の結果いずれも適正で変更なし。残る改善は (a) シーンタグの精度しきい値が検索用途には狭すぎる、(b) OCR に信頼度の足切りがない、(c) カバー選定が利用シグナル（共有/閲覧＝ADR-49 の台帳）を使っていない、の 3 点だった。
- 決定: (1) **シーンタグの 2 段化**: 台帳（検索インデックス）は precision 0.9→**0.75**・最大 10→**25 個**に広げ再現率を上げる。**表示は従来どおり insight 側で上位 10** に絞る（役割別の 2 段構成）。`TagStore.currentVersion` を 3 に採番し夜間に全再タグ。(2) **OCR の信頼度足切り**: 認識候補の confidence **0.3 未満は捨てる**（低信頼の誤読が字句検索を汚すのを防ぐ）。(3) **カバー選定に利用シグナルを追加**: `pickCoverRef` に共有済み **+15**・閲覧回数 **+min(view,10)** を加点（`UsageStore` を AIAlbumService へ結線）。序列は お気に入り(100) > 美的(±40) > 共有(15) > 横長(10)/閲覧(≤10) > GPS(5) を維持＝明示操作ほど強い。(4) しきい値レス方針（ADR-24）の semanticMargin 0.06・RRF k=60・顔系の各値は点検のうえ据え置き。
- 結果: タグ一致の検索再現率が上がり（曖昧なシーンも 25 個枠で拾える）、除外条件のタグ照合も広がる。カバーは「共有した・よく見返す」写真に寄る。トレードオフ: (1) 再タグ 1 巡（v3）。(2) precision 0.75 のタグはノイズも増えるが、検索は RRF 融合＋証拠ゲート＋LLM 審査の積層が刈る（表示は 10 個のまま体感不変）。
- 保留（新機能・別判断）: イベント検出レシピとホリデー検出の加重和は「閾値の見直し」ではなく生成戦略の新設になるため今回は見送り。採用候補値を残す —— イベント: Wedding=3.5h/10人/シーン5枚(2種以上・wedding,bride,groom,bridesmaid)、Celebration=3h/4人/10枚(graduation,ceremony,wedding)、Concert=5分/2枚(concert,singer,deejay,microphone)、Hiking=1枚(hiking,mountain,canyon,waterfall,forest)、Beaching=2枚(beach,surfing,shore,sand,swimsuit)、NightOut=3h/2枚(nightclub,bar,cocktail,wine)、Museum=1h/12枚(illustrations,painting,museum)、Dinner=40分/2枚(restaurant,food)。ホリデー: 加重和=Scene 0.6＋Location 0.25＋Date 0.15・判定しきい値 0.8・シーン重要度 0.3/0.4/0.6/1.0。
- 関連: `VisionTagAdapter.sense`（0.75/25・OCR 足切り）・`TagStore.currentVersion=3`・`CoverSelection.pickCoverRef(usage:)`・`AIAlbumService.usageCounts`（結線）・`PhotoSenseInfoTests`。ADR-47/48/49 の続き。

## ADR-50 UI 操作性の統一（操作は下部・メニューは「…＋長押し」・画面内メニュー）
- 状態: 採用
- 文脈: 操作導線がバラバラで迷いを生んでいた。(a) フル画面ビューは操作を**上部**に置く一方、ホーム/グリッドは**下部バー**、(b) ホームカードのサブメニューが AI アルバム＝「…」ボタン、人物＝長押し、と入口が不統一、(c) アルバムを開いた画面内から編集/削除に辿れない、(d) フル画面下部バーの操作アイコンを透過の丸ボタン（`.ultraThinMaterial`）にしたら**明るい写真に溶けて視認できない**、戻るが左上のまま。
- 決定:
  - **操作は下部に統一**: `PhotoPageView` のお気に入り/共有/顔ハイライトと**戻るボタン**を下部へ集約。下部バーはグリッド（`PhotoSourceContentView`）と同じ**全幅・不透明 `.bar` 背景（高さ49）**にして写真に溶けないようにする（丸ガラスボタンは廃止）。戻るは**左下**、アクションは右側。上部は日付＋地名ラベルのみ。
  - **メニューは「…＋長押し」の両方**: ホームの自動アルバムカード（`AlbumCarousel`）に長押し `contextMenu` を追加、人物カード（`HomeRows.PersonCard`）に「…」ボタンを追加。アイコンは `ellipsis.circle(.fill)` に統一、破壊的操作は `role: .destructive`。
  - **画面内メニュー seam**: `PhotoSourceContentView` に環境キー `sourceMenuContent`（`(@MainActor () -> AnyView)?`）＋右上ツールバー項目を追加（`PhotoSourceKit` は `AutoAlbumCore` 非依存のまま）。中身は呼び出し側が決める＝AI アルバム画面＝削除 `Menu`、人物アルバム画面＝既存 `peopleActions`（改名/代表/顔管理/束ね）を「…」→確認ダイアログで開く。
  - 作業中に **ADR-49 の共有ボタンがコード未実装だった退行**を発見（README/ADR には記載）。`ActivityShareSheet`（UIActivityViewController ラッパー・completed 時のみ `photoUsageEvent(.share)`）として実装し直した。
- 結果: 全画面で「操作は下」に統一、サブメニューは「…」でも長押しでも開け、アルバム画面内からも編集/削除できる。下部バーは不透明背景でどの写真でも視認可能。ビジュアル確認はシミュレータのスクリーンショット（UI テストで座標タップ→フル画面を撮影・確認後にテスト削除）で実施。
- 関連: `PhotoPageView`（topControls/bottomControls・`.bar` 背景・戻る左下・ActivityShareSheet）・`PhotoSourceContentView`/`PhotoSourceEnvironment`（`sourceMenuContent`）・`AlbumCarousel`（contextMenu）・`HomeRows`（PersonCard の「…」）・`AutoAlbumPhotosView`（onDelete→Menu）・`PersonAlbumView`（menuTarget＋peopleActions）・`HomeView`（結線）。ADR-49 の続き。

## ADR-49 写真の利用カウンタ（閲覧・再生・共有）と共有機能
- 状態: 採用
- 文脈: 写真ごとの利用状況（見た/再生した/共有した回数）を持ちたい（お気に入りは PHAsset.favorite で既存）。共有機能自体が未実装だったため、共有カウントには共有ボタンの新設が前提になる。
- 決定: (1) **別コンテナ "UsageV1"**（TagsV1/FacesV1 と同じパターン）に `PhotoUsageRecord`（refKey・view/play/shareCount・lastViewedAt）を置き、`UsageStore`（@ModelActor・オフメイン生成）で加算・照会。(2) **閲覧カウント**＝フル画面ページに **0.7 秒以上とどまった**ときだけ 1 回（`task(id: currentID)` のキャンセルが番人＝高速スワイプの通過は数えない）。(3) **共有**＝`PhotoPageView` 右上に共有ボタンを新設（フル画像をロードして UIActivityViewController へ）。`completionWithItemsHandler` の completed のときだけカウント（キャンセルは数えない）。(4) **再生**＝動画/Live Photo 未対応のためスキーマとイベント経路だけ用意（常に 0・表示は >0 のときのみ）。(5) イベントは PhotoSourceKit の環境クロージャ `photoUsageEvent`（AutoAlbumCore 非依存の enum）→ アプリ層（SourceHostView）→ `AutoAlbumEngine.recordUsage`（`canonicalRefKey` で "L-…"/"C-…" へ正規化）の seam 構成（photoInsight と同型）。(6) 表示は情報パネルの「アクティビティ」行（目/共有アイコン＋回数・insight に合成）。
- 結果: 全ソース（All/Photos/Cloud/アルバム/場所/人物）のフル画面で閲覧・共有が記録され、情報パネルで確認できる。将来「よく見る写真」系の AI アルバム条件にも使える台帳になる。トレードオフ: (1) 閲覧のたびに 1 upsert（actor・軽量）。(2) 共有はフル画像ロード後にシート表示（クラウドは取得待ちでスピナー）。(3) play は現状ダーク（動画対応時に配線）。
- 関連: `AutoAlbumCore/Usage/PhotoUsage.swift`・`AutoAlbumEngine.recordUsage`/`canonicalRefKey`・`PhotoSourceEnvironment.photoUsageEvent`・`PhotoPageView`（滞在カウント・共有ボタン・ActivityShareSheet）・`PhotoInfoPanel`（アクティビティ行）・`SourceHostView`（橋渡し）・`PhotoUsageTests`。

## ADR-48 顔情報の拡充（顔向きゲート・品質フロア引き上げ・目閉じ/笑顔・サイズ閾値）
- 状態: 採用
- 文脈: 顔クラスタの混入（別人が混ざる）の主要因は、横顔・ぼけ顔・目閉じ・小さすぎる顔の埋め込みが信頼できないのに正面顔と同じ重みでクラスタへ入ることだった（face-info-expansion）。ArcFace への換装（最大効果・フェーズ C）とは独立に、公開 API だけで取れる信号でゲートを強化できる。
- 決定: 優先度 1〜5 を実装（6=ArcFace 換装はモデル変換・同梱が必要な別作業のため対象外のまま）。
  - **顔向きゲート**: `VNDetectFaceLandmarksRequest` を品質リクエストと**同一 perform** に追加し、yaw/roll を取得（観測の対応づけは bbox IoU>0.3）。**|yaw|≥30° または |roll|≥45° は品質を 0.2 へキャップ**＝フロア未満で未割当（横顔の誤クラスタ混入の主因を遮断）。
  - **品質フロア 0.15 → 0.40**: ぼけ顔の重心汚染を削減。フロア未満も `DetectedFace` として記録は残す（顔数・枠表示用）。
  - **目閉じ**: ランドマークの目領域の縦横比（<0.15）で近似判定し品質 ×0.6。
  - **顔サイズ閾値**: ローカル 0.05 / **クラウド 0.15**（低解像度サムネの誤検出削減・従来は両方 0.05）。
  - **笑顔フラグ**: CIDetector（`CIDetectorSmile`）を 1 パス追加し bbox で照合、`DetectedFace.hasSmile`（optional 追加＝FacesV1 据え置きの軽量マイグレーション）へ保存。**代表顔の自動選択**（coverFaceID 未指定時）を「品質＋笑顔 +0.3＋顔サイズ +0.2」のスコア選択に変更。
  - 品質調整は純ロジック `FaceQualityGate`（AutoAlbumCore・テスト対象）に集約し、adapter は取得と委譲のみ。シミュレータ/フォールバック検出（yaw 等が nil）は減衰なし＝従来挙動。
- 結果: 新規スキャン分から横顔・目閉じ・微小顔がクラスタへ入らなくなり、混入と「同じ写真に複数の枠」が減る。代表顔は笑顔・高品質に寄る。既存の顔行は保存済み品質のままだが、**夜間の制約付き再クラスタ（ADR-46 B2）が新フロア 0.40 で全体を割り当て直す**ため段階的に浄化される（顔向き減衰の完全適用は再スキャン＝Developer Options の Reset people で前倒し可）。トレードオフ: (1) ランドマーク＋CIDetector ぶん 1 枚あたり数十 ms 増（夜間のみ）。(2) フロア引き上げでクラウド（低解像度）顔の採用数が減る＝クラウド中心の人物アルバムは小さくなる（誤りより取りこぼしを選ぶ・安全側）。
- 関連: `FaceSeams.swift`（FaceQualityGate/DetectedFaceSignal.hasSmile）・`FaceModels.swift`（DetectedFace.hasSmile）・`FaceStore.swift`（フロア 0.40・bestCoverFace）・`FacePerceptionAdapter.swift`（一括 perform・IoU 対応づけ・CIDetector 笑顔）・`FaceInfoExpansionTests`。ADR-45/46 の続き。指示書 `docs/architecture-note/features/face-info-expansion.md` は実装完了につき削除（ArcFace 換装＝フェーズ C は ADR-46 残課題として継続）。

## ADR-47 写真付帯情報の拡充（Vision 一括パス：OCR・動物・人数・美的スコア・アセット種別）
- 状態: 採用
- 文脈: 写真 1 枚から取得する情報がシーン分類（VNClassifyImageRequest）のみで、看板・書類・スクショ内の文字、ペット種別、人数、見栄えなどの信号を捨てていた。Vision は同一画像への複数リクエストを 1 回の `perform([...])` に載せると前処理・ロードを共有できる。
- 決定: (1) **知覚 seam を `senseInfo`（`PhotoSenseInfo`）へ拡張**し、`VisionTagAdapter` が 1 回の Vision パスで シーン分類＋**OCR**（`VNRecognizeTextRequest` accurate・言語自動判定・最大300字）＋**動物種別**（`VNRecognizeAnimalsRequest`・信頼度0.7・タグへ合流）＋**人数**（`VNDetectHumanRectanglesRequest` 上半身）＋**美的スコア**（`VNCalculateImageAestheticsScores`・iOS 18+・-1〜1）を取得。ローカル写真は **PHAsset 種別タグ**（screenshot/panorama/live photo/hdr/portrait/burst）も合流。OCR 解像度のためローカル読み込みを 384→1024px に引き上げ（分類は内部縮小のためコスト増は OCR ぶんのみ）。(2) **台帳（PhotoTagRecord）に optional 追加**（ocrText/humanCount/aesthetic＝軽量マイグレーション・コンテナ名 TagsV1 据え置き）し、`currentVersion` を 2 に採番→既存写真も夜間に再タグ（キャプションは保持）。(3) **AI アルバム**: OCR 台帳を字句検索（`LexicalSearch.rank(ocrTexts:)`）の新チャネルに（看板/固有名詞の検索という新次元）。動物・種別タグは既存のタグ一致ランキング/離散除外にそのまま乗る。美的スコアは `pickCoverRef` の加点（±40・お気に入り100は超えない）でカバー選択を改善。(4) **フル写真ビュー**: 情報パネルに「写真内のテキスト」欄（検出時のみ・4行まで）。
- 結果: 検索が「見た目（CLIP）＋シーン（タグ）＋文字（OCR）」の3系統になり、スクショ・書類・看板系のクエリに強くなる。タグ拡充・カバー改善は追加コストほぼゼロ（同一パス）。トレードオフ: (1) 夜間再タグ 1 巡が発生（版上げ・OCR accurate で 1 枚あたり従来比+数百ms）。(2) クラウド写真の OCR は 256px サムネ品質のため大きな文字のみ。(3) 美的スコアは iOS 18 未満の実行環境では nil（挙動不変）。
- 関連: `TagTagger.swift`（PhotoSenseInfo/seam）・`TagStore.swift`（v2）・`VisionTagAdapter.swift`（一括パス）・`LexicalSearch`/`AIAlbumSearcher`/`AIAlbumService`（OCR 結線）・`CoverSelection`（美的加点）・`PhotoInsight`/`PhotoInfoPanel`（表示）・`PhotoSenseInfoTests`/`LexicalSearchTests`。指示書 `docs/architecture-note/features/photo-info-expansion.md` は実装完了につき削除。

## ADR-46 人物レビュー（アクティブラーニング）としきい値校正・制約付き再クラスタリング・マルチプロトタイプ
- 状態: 採用
- 文脈: ADR-45（負例＋品質ゲート）後も 2 症状が残る: (1) 同一人物が複数アルバムに割れる（人物内ばらつき＞固定しきい値・単一重心の限界）、(2) 1 アルバムに複数人が混入。またユーザーが「アルバムから間違いを探す」のは非効率で、iPhone 写真アプリの「この人は◯◯さんですか？」（**判断が割れるケースだけ尋ねるアクティブラーニング**）の方が操作性・学習効率とも高い。逐次クラスタリングは追加順に依存し、早い段階の誤りを後から自己修正できない構造問題もある。
- 決定: A 全部＋B 全部を実装（A3 は独立フローにせず同一レビュー UI に統合・B2 は端末現実的な「品質順・制約つき全体再割り当て」方式・B3 のプロトタイプは確認済みの顔から生成）。
  - **A1 統合サジェスト**: 重心類似が「合流の一歩手前」（校正しきい値−0.10〜しきい値）のクラスタ対を「同じ人物ですか？」カードで提示。はい→統合（正例）。いいえ→「別人」記録（`notSame`・重心埋め込みで永続化＝ID の揺れに強い・以後提案しない＋**双方向の負例**として合流拒否）。
  - **A2 境界の顔の確認**: クラスタ内で重心類似が最も低い未確認の顔（しきい値+0.10 未満＝混入が最も出やすい位置）を「この写真は「◯◯」さんですか？」で確認。はい→`confirmedAt` 付与＝**アンカー**（B3）＋正例。いいえ→分離（負例）。既存の「付け替え先を選んだ」操作も確認（正例＋アンカー）として記録。
  - **A3**: レビュー UI（`FaceReviewView`・カード式 はい/いいえ/スキップ）をピープルのヘッダー（checkmark.seal）から常時起動＝初回一括確認も同じ画面。
  - **B1 しきい値校正**: 修正ジャーナルに記録時の**ペア類似度**を保存（reassign=顔×誤り重心・merge=重心×重心・confirm=顔×所属重心・notSame=重心×重心）。`FaceCalibration`（純関数）が正例/負例を最もよく分離する境界を選ぶ（正負各 8 サンプル未満は既定 0.45・0.35〜0.60 にクランプ）。
  - **B2 制約付き再クラスタリング**: 夜間スキャン完了後、修正が増えていたら全体を割り当て直す。命名済み/確認済みクラスタは **ID・名前・代表を保持する種**とし、確認顔（アンカー）を must-link で先に固定 → 残りを**品質降順**（高品質の顔が核を作る＝順序依存をほぼ解消）に校正しきい値＋負例つきで割り当て。新規クラスタ ID は既存全 ID より先（`minimumNextID`）。
  - **B3 マルチプロトタイプ**: クラスタ類似度を「重心 or アンカー群との最大」に拡張。年代・眼鏡・角度で重心から遠い顔も、確認済みアンカーに近ければ合流する（A2 の回答がそのまま認識力になる相互強化）。
- 結果: 「間違い探し」から「確認カードに答える」へ操作性が転換し、回答が統合・アンカー・負例・しきい値校正・再クラスタの 5 経路で精度に還流する。ジャーナルは埋め込みキーなので再スキャン・将来のモデル入れ替えを跨いで有効。トレードオフ: (1) レビュー生成はクラスタ対 O(k²)＋境界顔の全メンバー走査（数万顔で数秒・シート表示時のみ・ローディング表示）。(2) 再クラスタは O(顔数×クラスタ数)（数万顔で数秒・夜間のみ・修正が増えたときだけ）。(3) 校正の最低サンプル 8 未満は挙動不変。残: 埋め込みモデル更新（facenet→ArcFace 系・実測比較）は本 ADR の土台の上に後続（フェーズ C）。
- 関連: `FaceCalibration.swift`・`FaceReview.swift`・`FaceClustering`（prototypes/similarity/minimumNextID）・`FaceStore`（reviewItems/confirmFace/markNotSamePerson/rebuildClusters/校正キャッシュ）・`PeopleEngine`（reviewItems/answer*/rebuildClustersIfNeeded）・`FaceReviewView`・`FaceCalibrationTests`/`FaceStoreLearningTests`/`FaceClusteringTests`。ADR-45 の続き。
- 追記（2026-07・実機フィードバック 3 点）: (1) **未命名クラスタのカードは名前で尋ねない**＝「Person 1 さんですか？」は誰のことか分からず答えられない。命名済みだけ名前カード、未命名は**代表の顔と境界の顔を並べた「同じ人物ですか？」**（見た目だけで判断可能）に変更（`FaceReviewItem.isThisPerson` に `name: String?`＋`coverFace` を追加・境界顔＝代表顔のときはカード自体を出さない）。(2) **レビュー導線が小さな ✓ アイコンでは気付かれない**→ ピープルのヘッダーをテキスト付きの色付きカプセルボタン（checkmark.seal.fill＋「Review／人物を確認」）に変更。(3) **複数人が写る写真でどの顔をこの人物と認識したか分からない**→ 人物アルバムの全画面表示で該当クラスタの顔を黄枠でハイライト（`FaceStore.faceBoxes(refKey:clusterID:)`→`PeopleEngine.faceHighlights`→PhotoSourceKit の新環境クロージャ `faceHighlightProvider`。PhotoSourceKit は AutoAlbumCore 非依存のまま＝`photoInsight` と同じ seam パターン。Vision 正規化座標を scaledToFit 画像上へ y 反転でマッピング）。
- 追記2（2026-07・実機フィードバック）: (1) **同じ質問の繰り返しを抑制**＝レビューカードは決定的に生成されるため、スキップ（未回答）のカードが毎回再登場していた。表示したカード ID の回数を記録（`PeopleEngine.noteReviewShown`・UserDefaults）し、**3 回見せても答えられなかったカードは以後出題せず次点候補で埋める**（`FaceStore.reviewItems(excluding:)`・除外は生成段階で行い limit 枠を次点に譲る）。(2) **顔ハイライトは常時→オンデマンドに**＝黄枠の常時表示は鑑賞の邪魔（たまに確認したい機能）。人物アルバムの全画面表示の右下に `person.and.viewfinder` ボタンを置き、押したときだけ枠を表示/非表示（既定 OFF・矩形の取得もボタン押下時まで遅延）。
- 追記3（2026-07）: **顔ハイライトをサムネイルグリッドにも**（実フィードバック「サムネイル画面で使いたい」）。人物アルバムの下部バーに `person.and.viewfinder` トグルを追加し、ON 中は各サムネセルに認識した顔の黄枠（CAShapeLayer）を重ねる。ポイントは座標変換：グリッドは aspect-fill の**中央正方形トリミング**表示のため、Vision 正規化矩形（元画像基準・原点左下）を元画像のアスペクト比で正方形クロップ座標へ変換する純ロジック `FaceBoxMapping.squareCropUnitRects`（PhotoSourceKit・テスト付き）を新設。クロップで半分以上欠ける顔は描かない。アスペクト比は端末写真＝PHAsset のピクセル寸法（`LocalAssetIndex.asset(for:)`）、クラウド写真＝キャッシュ済みサムネ（Dropbox はアスペクト保持）から取得し、注入側（`PersonAlbumView`）で変換して渡す（PhotoSourceKit は AutoAlbumCore 非依存を維持・環境キー `faceHighlightGridProvider`）。トグルは既定 OFF・画面ごとの一時状態で、通常ソースではボタン自体が出ない。ボタン位置・アイコンは実フィードバックで 2 度再調整：最終形は**グリッド＝下部バー（Home/歯車と同列）**・**フル画面＝左上・戻るボタンの隣（マテリアル丸ボタン・状態は PhotoPageView が持ちページ送りで維持）**で、アイコンは `person.and.viewfinder`（機能が伝わらない）をやめて**顔マーク（`face.smiling`・ON は `face.smiling.inverse`＋黄色）**に統一。さらに用途（「この人物として選ばれた顔のチェック」）に合わせ、同じ写真の複数の顔が同一クラスタに入っていても（混入の疑い）`faceBoxes` は**重心に最も近い 1 顔だけ**を返す（全顔に枠が付くとどれがこの人物か分からない＝実フィードバック）。残課題: 混入自体の抑制として「同一写真内の顔は同一クラスタに入れない」cannot-link 制約の導入を検討。

## ADR-45 顔認識をユーザー修正から学習する（品質ゲート＋負例エグゼンプラ＋修正ジャーナル）
- 状態: 採用
- 文脈: 顔認識の精度が低く、ユーザーが修正（顔の付け替え・人物の統合）しても効果が限定的だった。分析: (1) 修正は重心へ即反映されるが、**負の記憶がない**＝次に来る似た顔がまた同じ誤りクラスタに入る（「直しても再発する」）。(2) しきい値 0.45 固定・全クラスタ共通。(3) 修正は使い捨て（再スキャン/モデル更新で消える）。(4) 品質を全く使っておらず（`quality` は常に 1）、ぼけ顔・横顔・微小顔が同じ重みで重心に混入。CNN 本体のオンデバイス再学習は非現実的（ラベル量・基盤・電池）と判断し不採用。
- 決定: 修正を「教師データ」として活かす 2 本柱＋品質の土台を実装。(a) **品質ゲート**＝`VNDetectFaceCaptureQualityRequest` で実際の顔品質（0…1）を取得し、クラスタリングで **品質重み付き重心**（ぼけ顔ほど寄与小）＋**品質フロア**（0.15 未満は未割当＝重心を汚さない・顔数カウントには残す）。(b) **負例エグゼンプラ**（ADR の肝）＝付け替え時（付け替え元に他の顔がいた＝誤って一緒にされていたときのみ）「この顔埋め込み ≠ この誤りクラスタ重心」を **`FaceCorrection` ジャーナルに埋め込みで永続化**。`assign` は候補クラスタが負例に該当（入力顔が負例顔に近い かつ 候補重心が誤り重心に近い）なら合流を拒否し次点/新規へ。**埋め込みキーなので clusterID の揺れ・再スキャン・将来のモデル入れ替えを跨いで効く**。(c) 統合も記録（merge・将来の replay 材料）。`reset()`（再スキャン）はジャーナルを**残す**ので負例が自動再適用され既知の誤りが再発しない。Developer Options に「学習もリセット」（`resetIncludingCorrections`）を追加。
- 結果: 「直しても再発する」が負例で止まり、品質ゲートでぼけ顔由来の偽クラスタ・重心汚染が減る。修正が再スキャンを跨いで生き残る。純ロジック（品質重み・負例判定）は `FaceClustering` に集約しテスト（品質フロア/重み/負例拒否/過剰拒否なし）で固定。トレードオフ: (1) 負例判定は O(修正数)/顔だが上限 400・実測は数十件で無視可。(2) しきい値の自動校正（フェーズ 2）・距離学習（フェーズ 3）・埋め込みモデル更新（フェーズ 4 後半＝facenet→ArcFace 系の実測比較）は本 ADR の土台（ジャーナル）の上に後続。(3) 負例の過剰拒否リスクは sameThreshold 0.55 / wrongThreshold 0.88 で抑制（同一人物の正当な合流は残す＝テストで担保）。
- 関連: `FaceClustering`（品質重み・負例・NegativePair）・`FaceModels.FaceCorrection`・`FaceStore`（負例ロード/記録・品質フロア・reset は残す）・`FacePerceptionAdapter`（faceCaptureQuality）・`PeopleEngine.reset(includingCorrections:)`・`FaceClusteringTests`。ADR-33（クラウド顔スキャン）・ADR-29（人物統合）の続き。

## ADR-44 Dropbox の読み込み対象フォルダ（マルチルート同期・バックアップフォルダは常時含む）
- 状態: 採用
- 文脈: Dropbox はアカウント全体を読み込んでいたが、特定フォルダ以下に限定したい（家族共有アカウントで他人のフォルダを見たくない等）。事前分析: 使用 API（list_folder/continue/longpoll/get_thumbnail_batch/download）はすべてパスまたはカーソル基準でフォルダ限定にそのまま対応し、対象が減るぶん軽くなる一方。唯一の設計論点は「バックアップフォルダがスコープ外になると、オフロードのクラウド代替・バックアップ済み写真の表示・クラウド顔スキャンが壊れる」こと。
- 決定: (1) 設定「読み込み対象フォルダ」（既定 "/"＝全体・`DropboxSourceSettings`・正規化純ロジック＋テスト）。(2) **マルチルート同期**＝「選択フォルダ＋バックアップフォルダ」を常に同期対象にする（`normalizedRoots` が重複・包含を畳む："" があれば 1 本、子孫は親に吸収）。(3) エンジンはルートごとに独立ループ（TaskGroup 並行・各ルートに baseline cursor→初回スキャン→longpoll）。カーソル保存はルート "" が従来の素の accountId 行（既存インストールの全再同期を回避）、フォルダルートは "accountId|/path" の複合キー行（アイテムはグローバル共有のため cache 変更なし）。UI 状態はプライマリ（選択フォルダ）のみが駆動し、追加ルートは静かに同期。(4) フォルダ指定の初回同期は単一再帰スキャン（全体のみ従来の浅掘り→トップフォルダ方式）。stale prune は**自ルート配下のみ**（他ルートのアイテムを消さない）。(5) スコープ変更は UserDefaults マーカーで検知し、キャッシュ全消去→再スキャン（カーソルはパスに紐づくため）。設定 UI（DropboxSettingsView・en/ja ローカライズ）から適用。
- 結果: フォルダ限定で同期・キャッシュ・AI 索引が軽くなり、バックアップ関連機能はスコープ設定に左右されない。トレードオフ: (1) フォルダ指定時は longpoll 接続が最大 2 本（専用セッション・許容）。(2) スコープ変更時は一回の全再スキャン（接続し直しと同等・既存の自己修復パターン）。(3) clearAll は全 SyncState 行を削除する形に変更（複合キー行の残留防止・単一アカウント運用前提）。
- 関連: `DropboxCore/Sync/DropboxSourceSettings.swift`・`DropboxSyncEngine`（マルチルート）・`DropboxPhotoStore.syncRootsProvider/applySourceFolderChange`・`DropboxSettingsView`・`RootView`（結線）・`DropboxSourceSettingsTests`。ADR-39/41（バックアップフォルダ依存機能）との整合。

## ADR-43 バックアップ系のオフメイン化と横断リファクタリング（体感チューニング一式）
- 状態: 採用
- 文脈: リファクタリング＋パフォーマンス候補の棚卸しで、体感に効く問題を複数特定した。(A1) BackupEngine が plain ModelContext を直接使い、起動時の全記録 fetch×2・毎枚 save・照合の全件 fetch が**メインスレッド**で走る（AutoAlbumStore の実測 14.5s ハングと同型の罠）。(A2) バックアップメタデータを毎起動フルダウンロード（v1 最大 15〜25MB＋全シャード・逐次）し MainActor でデコード。(A3) バックアップ画面を開くたび全ライブラリ列挙。(A4) フル画像の情報パネルが 1 枚ごとに直列 4 fetch。(A5) コンポーザーのスナップショットが 5 分で無条件失効（再訪のたび 85k 再フェッチ）。構造面では (B1) BackupEngine の肥大、(B2) uploader 5 エンドポイントのリクエスト組み立てコピペ、(B3) シャード書き込みの 2 実装、(B4) JSON 取得の 3 実装、(B6) Runner 330 行の一本道。
- 決定: (A1/B1) SwiftData を **`BackupStore`（@ModelActor・`makeDetached()` でオフメイン生成）**へ全面移管し、境界は Sendable 値（`BackupRecordLite`）。Engine はキャッシュ＋コーディネータに縮退、起動時キャッシュウォームは非同期化。`isBackedUp` はウォーム前 nil（誤バッジ防止）。(A2) メタデータ読み込みを **rev ベースのローカルキャッシュ**（get_metadata で版数確認・一致なら Caches から・不一致のみ DL）＋**並列取得**＋**オフメインデコード/マージ**に。(A3) 状況集計をキャッシュし記録変更で無効化。(A4) insight のタグ/キャプション/CLIP ラベル照会を並列化。(A5) スナップショット失効を「時間」から「**件数変化**」（enrichment/embedded の COUNT 比較）に変更＝夜間バッチが進まない限り再構築しない。(B2) contentRequest/rpcRequest/rpcJSON へ一元化。(B3) `MetadataShardWriter` に download→merge→upload を集約。(B4) `fetchJSON` に一元化。(B6) Runner をフェーズメソッド（権限→索引→差分→アップロード→メタデータ）へ分割（挙動不変）。
- 結果: 起動時のメインスレッド SwiftData がゼロに・メタデータの起動時通信は「rev 確認のみ」（変更が無ければ本文 DL ゼロ）・情報パネルとコンポーザー再訪が高速化。トレードオフ: (1) rev 確認の RPC は JSON ファイル数ぶん発生（数百バイト×N・並列）。(2) バッジはキャッシュウォーム完了まで非表示（数百 ms 級）。(3) OffloadService の台帳コールバックが async 化（「記録が先」の不変条件は await で担保・テスト更新済み）。B5（QuerySpec の DNF 簡素化）は検索品質ハーネスでの検証つきで別途。
- 関連: `BackupKit/BackupStore.swift`・`MetadataShardWriter.swift`・`BackupRunner`（フェーズ分割）・`DropboxBackupUploader`（共通ビルダー）・`DropboxPhotoStore.fetchCachedJSON`・`AutoAlbumEngine+Recognition.insight`・`AutoAlbumEngine+Suggestions`。ADR-26（横断リファクタリング）・事例「@ModelActor がメインで実行」の続き。

## ADR-42 夜間の自動バックアップ（重い処理ウィンドウへの相乗り）＋状況表示
- 状態: 採用
- 文脈: バックアップは手動「Back Up Now」のみで、アプリを開いたまま完走を待つ必要があった。一方、AI 索引（タグ/埋め込み/キャプション/顔）は夜間の BGProcessingTask（電源＋Wi-Fi＋非使用・ADR-25）で自動進行しており、バックアップも同じ条件で動くのが自然。また「どこまでバックアップされているか」が実行中の進捗（N/M）以外に見えなかった。
- 決定: (1) `HeavyWorkScheduler.runHeavyWork` の夜間シーケンスに `BackupEngine.startNightlyIfEnabled()` を追加。宛先が Dropbox のときだけ、**手動と完全に同じ経路**（1 回の上限設定・電源/回線ポーズ・ADR-40 の検証つきアップロード・ADR-41 の端末フォルダ）で実行する。BGTask の完了待ちループにバックアップ実行中を加え、**期限切れ時は明示キャンセル**（「済み」記録は hash 検証後にしか付かないため、中断しても次回差分から安全に再開）。(2) `BackupEngine.backupStatus()`（対象総数・完了数。ライブラリ全列挙はオフメイン・完了数は現存写真との積集合＝削除済みを数えない）を追加し、バックアップ画面の先頭に「バックアップ済み X / Y・残り N 枚は夜間に自動で進みます」を常時表示。
- 結果: 放っておけば夜間に少しずつ全量バックアップされる（既定上限 10 枚/回のため、全量を急ぐ場合は上限を無制限へ）。状況が数字で見えるため「動いているのか」の不安が消える。トレードオフ: 夜間ウィンドウを AI 索引と分け合う（どちらもトリクル・譲り合いは既存ゲートに従う）。
- 関連: `HeavyWorkScheduler.swift`・`BackupEngine.startNightlyIfEnabled/backupStatus`・`BackupSettingsView`（状況行）。ADR-25/40/41 の続き。

## ADR-41 バックアップの端末フォルダ分離（家族での Dropbox アカウント共有対応）
- 状態: 採用
- 文脈: 1 つの Dropbox アカウントを家族で共有し、複数端末が本アプリでバックアップする想定。ファイル名衝突による喪失は ADR-40（409 の hash 照合＋autorename）で解消済みだが、**`.mosaic` メタデータ（download→merge→upload 方式）は端末間で後勝ち競合し、相手のエントリを黙って消す**（ADR-38 で「単一端末運用前提」と明記していた制約）。人物名・アルバム所属・オフロード台帳の再構築材料が失われ得る。
- 決定: バックアップの実保存先を `<ルート>/<端末フォルダ>/` に分離する（ファイルも `.mosaic` も端末ごと）。端末フォルダ名は `<表示名>-<短ID>`（例 "iPhone-3F2A8C"）。**短 ID は初回生成して Keychain に永続化**（`BackupDeviceIdentity`・再インストール後も同一。identifierForVendor は再インストールで変わるため不採用・端末名は iOS 16+ で汎用名のみ＋ユーザー変更でフォルダが割れるため不採用）。表示名（UIDevice.model）と ID は catalog.json（deviceID / deviceName）にも記録し、機種変更時の「既存フォルダの引き継ぎ」UI の材料にする。**既存のフラットな旧ファイルは移動しない**（記録はフルパス基準なのでそのまま整合）。メタデータ読み込みは「ルート（旧）＋端末フォルダ（新）」の複数ルート統合（`loadBackupMetadata(from: [String])`）。
- 結果: ファイル衝突とメタデータ競合が構造的に消え、家族共有で安全になった。再インストールは Keychain ID で同一フォルダに向き、アップロード済み記録の消失も 409→hash 照合で自己修復する（ADR-40 との相性）。トレードオフ: (1) Cloud タブでは家族全員の写真が見える（アカウント共有の本質・必要なら将来フォルダフィルタ）。(2) 同一人物の複数端末は独立バックアップ＝共通写真は二重容量。(3) 機種変更時の引き継ぎ UI（フォルダ一覧から選択）は未実装（catalog に材料のみ）。
- 関連: `BackupKit/BackupDeviceIdentity.swift`・`BackupEngine.deviceBackupRoot`・`BackupCatalog.deviceID/deviceName`・`DropboxPhotoStore.loadBackupMetadata(from:[String])`・`BackupDeviceIdentityTests`。ADR-38/40 の続き。

## ADR-40 オフロードの多層防御と検証つきアップロード（content_hash）
- 状態: 採用
- 文脈: オフロード（バックアップ済みローカル写真の削除）はバグが即「写真の永久喪失」になり得る。テスト方法と確実性の設計を先に固めた。調査で現行アップロードに**具体的欠陥**を発見: (1) HTTP 200 を無条件に成功扱い（応答の content_hash 未検証＝壊れた保存を検出できない）。(2) **409（同パス既存）を無確認で「済み」扱い**——同名の別写真（IMG_0001.jpg は普通に衝突する）が「バックアップ済み」と誤記録され、オフロードで消すと永久喪失する。
- 決定: **「削除は証明の後」**を不変条件に多層防御で実装。(a) **検証つきアップロード**＝ローカルで Dropbox content_hash（4MB ブロック SHA-256 連結→SHA-256・`DropboxContentHash` 純関数・独立計算のテストベクタで検証）を計算し、`files/upload` 応答の hash と**一致して初めて「済み」**（不一致は `.hashMismatch`＝絶対に済み記録しない）。409 は `files/get_metadata` で同一性確認→不一致なら **autorename** で別名アップロード。(b) **オフロードの判定**は純関数 `OffloadPlanning.verdict` に集約: Live Photo除外・バックアップ後編集除外・データ取得不能除外・**その場での hash/サイズ完全一致必須**（記録でなくリモート実測と照合）。(c) **実行順序**＝直前再検証 → 台帳記録（先）→ PhotoKit 削除（OS 確認ダイアログ必須・「最近削除した項目」30 日）→ キャンセルなら台帳ロールバック → metadata へ offloadedAt/verifiedAt マーカー。(d) **段階導入**＝ドライラン既定（何も消さない検証一覧）・実削除は Developer Options ゲート＋1 回の上限（既定 10）。(e) **テスト戦略**＝層1: hash/判定の純関数（Python で独立計算した期待値）・層2: 偽 Dropbox（HTTPClient スタブ）＋削除モック（`PhotoDeleter` seam）で「消してはいけない全ケースで削除要求ゼロ」を機械的に保証（クラウド不在/hash不一致/サイズ不一致/読込不能/編集済み/Live/キャンセル→ロールバック/上限/ドライラン）。
- 結果: 削除の前提がすべて実測（今この瞬間の一致）になり、部分アップロード・同名衝突・編集消失の各事故経路がテストで封じられた。トレードオフ: (1) アップロードごとに hash 計算（数 MB で ~10ms・無視可）と 409 時の get_metadata 1 往復。(2) 実機での実削除はテスト用写真での段階運用が前提（ドライラン → 少数 → 拡大）。(3) 動画・Live Photo・iCloud 最適化写真はオフロード対象外（スキップ理由を UI に明示）。
- 関連: `BackupKit/DropboxContentHash.swift`・`DropboxBackupUploader`（検証つき upload / get_metadata）・`BackupRunner`（409 の hash 照合＋autorename）・`OffloadService` / `OffloadPlanning` / `PhotoDeleter` / `PhotoKitDeleter`・`OffloadSettingsView`（ドライラン UI）・`BackupDebugSection`（実削除ゲート）・`DropboxContentHashTests` / `OffloadSafetyTests`。ADR-38/39 の続き。

## ADR-39 オフロード台帳と端末アルバムの合成表示（クラウド代替）
- 状態: 採用
- 文脈: 将来のオフロード（バックアップ済みローカル写真の検証つき削除）後も、端末アルバムを「何事もなかったかのように」表示したい。単純に「metadata の albums 逆引きで、端末に無い写真を全部クラウドから補完」すると、**ユーザーが写真アプリで意図的に削除した写真まで蘇ってしまう**——補完してよいのは「アプリ自身がオフロードした写真」だけであり、その区別には削除の主体を記録する台帳が必要。
- 決定: (1) **オフロード台帳** `OffloadRecord`（@Model・BackupKit ストアに追加）＝アプリが削除した写真の localIdentifier / dropboxPath / 所属アルバム / 撮影日を記録。エンジンがメモリキャッシュ（アルバム名→パス・撮影日昇順）を持ち同期参照できる。再インストール時は metadata v2 の **`offloadedAt` マーカー**（Entry に追加・オフロード実行時にのみ付く）から台帳を再構築（`rebuildOffloadLedgerIfEmpty`・ユーザー削除の写真は対象外）。(2) **端末アルバムの合成表示** `DeviceAlbumPhotosView`＝PHAssetCollection 現存メンバー（ローカル）＋台帳のクラウド代替を、メンバー限定 `MergedPhotoStore` で混在表示（PersonAlbumView / AutoAlbumPhotosView と同型・撮影日ソートで元の位置に混ざる）。台帳が空なら cloudPathFilter が空集合＝従来表示と完全に同一。(3) **アルバム改名対策**＝カタログに `albumIDs`（アルバム名→PHAssetCollection.localIdentifier・マージ保持で旧名対応も残る）を先行追加。
- 結果: オフロード実装時に必要な「削除の主体の区別」「アルバム表示の穴埋め」「改名追跡」の 3 点が先に揃い、削除機能そのもの（contentHash 照合→削除→記録）だけを足せばよい状態になった。現時点では台帳が常に空のため動作は不変。トレードオフ: (1) 台帳キャッシュはアルバム数×オフロード枚数に比例（数万枚規模でも数 MB 級・許容）。(2) アルバム内の手動並び順は撮影日ソートで代替（忠実な再現は将来課題）。(3) 復元（端末へ再取り込み）時の台帳削除 API（`removeOffloads`）は用意済みだが呼び出し元（復元機能）は未実装。
- 関連: `BackupKit/OffloadRecord.swift`・`BackupEngine`（ledger API＋キャッシュ）・`BackupMetadataPlanning.offloadCandidates`・`BackupIndexing.buildAlbumIDIndex`・`DropboxCore/BackupMetadataV2.swift`（albumIDs）・`DropboxBackupMetadata.Entry.offloadedAt`・`MosaicPhotos/Home/DeviceAlbumPhotosView.swift`・`HomeView`（結線）。ADR-38（メタデータ v2）の続き。

## ADR-38 バックアップメタデータ v2（カタログ＋撮影月シャード・再生成不能情報の保全）
- 状態: 採用
- 文脈: 将来「バックアップ済みのローカル写真を端末から削除（オフロード）してストレージを空ける」機能を計画している。削除後もアルバム表示・人物・場所・検索が成立するには、**端末を削除すると再生成できない情報**を Dropbox 側に保全する必要がある。調査の結果、既存 v1（単一 `.mosaic/metadata.json`）には 2 つの問題があった: (1) **欠落** — アルバム/お気に入り/撮影日は保存済みだが、人物名（顔クラスタのユーザー命名＝v1 では常に空）・GPS（EXIF に無い写真は PHAsset が唯一の出典）・localIdentifier（ローカル⇔クラウド対応表）・スクリーンショット判定・VLM キャプションが未保存。(2) **スケール** — 全エントリを毎バックアップで丸ごと書き直すため、6 万枚規模で 15〜25MB/回になる。
- 決定: **v2 = カタログ＋撮影月シャード**へ移行する。`.mosaic/catalog.json`（スキーマ版・シャード一覧・アルバム/人物カタログ・数 KB）＋ `.mosaic/meta/<YYYY-MM>.json`（撮影月ごとのエントリ集・**触った月だけ**ダウンロード→マージ→アップロード）。Entry に Optional フィールドを追加（localIdentifier / latitude / longitude / isScreenshot / caption / verifiedAt）＝ v1 JSON と相互に読める。人物名は `PeopleEngine.peopleNamesByRefKey()`、キャプションは `TagStore` から seam（`peopleNamesProvider` / `captionsProvider`・Composition Root 結線）で取得。**v1 は凍結**（新規書き込みは v2 のみ）とし、読み込み（`DropboxPhotoStore.loadBackupMetadata`）は「v1 ベース → カタログのシャードを上書きマージ」で統合する。シャード決定は UTC の撮影月（端末 TZ 依存だと同じ写真が別シャードに入るため）・日付不明は `undated`。`verifiedAt` はオフロード実装時の「contentHash 照合済み」記録用に先行定義。
- 結果: 再生成不能情報が漏れなく Dropbox に残り、オフロード・機種変更・再インストール後の復元材料が揃う。メタデータ通信は「触った月＋カタログ」に有界化。純ロジック（シャード分割/マージ/カタログ更新）は `BackupMetadataPlanning` に分離し macOS テストで固定。トレードオフ: (1) シャード更新は download→merge→upload の 2 往復/月（同時実行や他端末との競合はラストライター勝ち＝バックアップは単一端末運用前提）。(2) v1 ファイルは残置（読み込み統合でカバー・全量 v2 移行ツールは未実装）。(3) オフロード本体（検証つき削除・refKey 移行・アルバム表示の合成）は次段。
- 関連: `DropboxCore/Models/BackupMetadataV2.swift`（BackupCatalog）・`DropboxBackupMetadata.Entry`（v2 フィールド）・`DropboxPhotoStore.loadBackupMetadata`・`BackupKit/BackupMetadataPlanning.swift`・`BackupRunner`（収集と書き込み）・`DropboxBackupUploader.download/uploadJSON`・`AutoAlbumAdapters`（結線）・`BackupMetadataPlanningTests`。ADR-34（キャプション）・ADR-33（顔クラスタ）。

## ADR-37 AI アルバム作成の入力支援（サジェストチップ＋接地プレビュー＋ヒット件数）
- 状態: 採用
- 文脈: AI アルバムの検索文は自由入力のみで、(1) ライブラリに何があるか（命名済み人物・地名・頻出被写体）を思い出しながら書く必要があり、(2) 書いた文がどう解釈されるか（人物に接地したか・場所が効くか）が作ってみるまで分からなかった。
- 決定: コンポーザーに 3 つの入力支援を追加する（すべて既存の決定的レイヤーの流用＝表示と実検索が乖離しない）。**(1) サジェストチップ** — ライブラリから観測された語をタップで挿入。**確実にヒットする語だけ**を出す: 人物=命名済み顔クラスタ（`namedPeopleProvider`）/ 場所=カタログ実在の地名（`AIAlbumCatalog` 頻度順）/ よく写るもの=頻出タグ（`TagStore.topTags` 新設）∩レキシコン（`japaneseLabel(forTag:)` 新設で日本語表示・表示も接地も保証）/ 日付=パーサ対応の定型（去年・今年・一昨年）。**(2) 接地プレビュー** — 入力の解釈のされ方を色付きチップで表示（人物=ピンク「山田太郎」・場所=緑・視覚語=青「海 → sea」・日付=橙「期間」）。インライン色付けでなくチップ方式＝「太郎 → 山田太郎」の正規化先まで見せられ、`previewInterpretation`/`PersonNameGrounder`/`groundedPairs`（新設）/`RelativeDateParser` の流用で実装が薄い。**(3) ハード条件のヒット件数** — 人物/場所/日付条件があるとき「条件に合致: N 枚」をライブ表示（`QueryEvaluator.hardFilter` をオフメインで・0 件は橙で警告）。基盤は `AutoAlbumEngine+Suggestions`（ライブラリスナップショット 5 分キャッシュ＝タイプごとの 85k 再フェッチ回避・接地プレビューは 350ms debounce）。
- 結果: 空振りしないアルバムをチップから組み立てられ、入力中に「誰・どこ・何が効くか」と概算ヒット数が見える。トレードオフ: (1) スナップショット初回構築（85k lite＋カタログ＋タグ集計）はコンポーザー初回表示時の一拍（以後 5 分キャッシュ）。(2) 件数はハード条件のみ（意味検索込みは重いので出さない・夜間に精緻化の注記を維持）。(3) インライン色付け（AttributedString TextEditor）は Phase 2 候補として見送り。
- 関連: `AutoAlbumEngine+Suggestions`（AIAlbumSuggestions/GroundingPreview/スナップショット）・`TagStore.topTags`・`JapaneseVisualLexicon.groundedPairs`/`japaneseLabel(forTag:)`（public 化）・`AIAlbumComposerView`（チップ UI・debounce）・`LexiconSuggestionTests`。ADR-23（解釈の決定的プレビュー）・ADR-29（人物接地）。

## ADR-36 画像解析はすべて「新しい写真から先に」処理する
- 状態: 採用
- 文脈: 各解析パスの処理順が不定（CLIP 埋め込み＝SQLite の既定順・シーンタグ＝Set 列挙順・キャプション＝refKey 昇順・顔＝PHFetch 既定順）で、撮りたての写真の解析がいつ終わるか運任せだった。ユーザーは新しい写真ほど早く検索・アルバムに反映されてほしい。
- 決定: **全解析パスの供給順を撮影日降順（新しい順・日付なしは最後）に統一**する。(1) CLIP 埋め込み＝`unembeddedRefKeys` に `SortDescriptor(\.captureDate, .reverse)`、(2) シーンタグ＝候補列挙を `enrichedRefKeysNewestFirst()` に、(3) VLM キャプション＝お気に入りを `newestFirst(refKeys:)` で並べ静的キューで処理（TagStore は日付を持たないため AutoAlbumStore 側で整列・動的クエリ→静的キューに変更・実行中の新規は次回巡回）、(4) 顔スキャン＝`localImageRefKeys` に PHFetch の creationDate 降順＋`cloudImageRefKeys` を captureDate 降順ソート（FaceTagger/TagTagger は候補順を保存するため列挙側の整列だけで効く）。
- 結果: 撮りたて・同期したての写真から解析され、検索/アルバム/ピープルへの反映が最速になる。コスト: ソート付き fetch の増分は「512枚ドレインごとに ~数十ms」級で推論（1枚百ms〜秒）に対し無視できる。PHFetch は creationDate 索引済み。トレードオフ: キャプションが動的クエリ→実行開始時の静的キューになり、実行中に増えた対象は次回巡回まで待つ（従来と同じ巡回粒度）。
- 関連: `AutoAlbumStore.unembeddedRefKeys`/`enrichedRefKeysNewestFirst`/`newestFirst(refKeys:)`・`TagStore.captionPendingSet`・`TagTagger.captionUnprocessed(favoritesNewestFirst:)`・`AutoAlbumEngine+Recognition.scheduleBackgroundFill`・`PeopleSupport.localImageRefKeys`/`cloudImageRefKeys`。ADR-30（インターリーブ）・ADR-34（お気に入り限定キャプション）。

## ADR-35 自然文検索の強化: マルチプローブ採点＋候補へのオンデマンドキャプション
- 状態: 採用
- 文脈: 自然文検索を「もっと柔軟に」する構想（ReACT 2フェーズのエージェント検索）の第一歩として、まず**評価ハーネス**（`SearchQualityTests`・Imagenette 200枚×28クエリ・Recall@k）で現行パイプラインを計測した。結果、**精度はほぼ完璧（memberP 0.99）だが言い換え表現の再現率が弱い**（paraphrase-en 0.61 / ja-free 0.68）＝改善ターゲットは「取りこぼしの回収」と数値で確定（model-evaluations §4）。
- 決定: 2 つの決定的強化を導入する。**(A) マルチプローブ採点** — 解釈時（夜間・FM）に `expandProbes`（空振り Refine と同じ単語リスト生成＝小型 LLM が壊れにくい形）で言い換えプローブ（英語・最大4）を生成し `SavedInterpretation.probes` へ永続化（ADR-23 の「解釈は 1 回」を維持・版 v6 採番で既存アルバムも次回夜間に再解釈）。意味採点は主フレーズ＋プローブの **max-over-probes**（どれかの言い回しに近ければ拾う）。採点規則は `QueryEmbedder.semanticScore` に一元化し、フル評価（`searchWithPool`）と増分評価（`refreshIncremental`）が必ず同一規則を使う。**(B) 候補へのオンデマンドキャプション** — LLM 審査（`AlbumVerifier`）の直前に、候補上位のキャプション未生成分へ**その場で VLM を回して証拠を濃くする**（予算 40 枚/審査・生成分は TagStore へ永続化＝再生成しない・シミュレータ/VLM 未同梱は無効）。キャプションのお気に入り限定（ADR-34）の**意図された例外**＝「候補だけに重いモデルを注ぐ」。
- 結果: ハーネス A/B で**言い換え再現率 +17〜18pt**（paraphrase-en 0.61→0.79 / ja-free 0.68→0.85・全体 memberR 0.76→0.86）。精度は 0.99→0.96 と微減だが、本番はこの後段の LLM 審査（B で証拠も濃くなる）が刈る。トレードオフ: (1) 解釈時に FM 呼び出しが 1 回増える（プローブ生成・夜間）。(2) 審査時に VLM 最大40枚 ≈ 3 分弱（夜間・電源＋ロック中ゲート内）。(3) ハーネスのプローブはクエリ集の固定値＝FM 生成品質そのものは実機で別途確認。
- 残課題（ReACT 構想の残り）: Phase 1 の **FM 駆動の絞り込みループ**（観測＝プール集計→行動選択の反復）は未実装。今回の決定的強化で再現率ギャップの大半を回収できたため、残る価値（複雑な複合クエリの反復精化）は実機で A/B してから判断する。実装する場合も「閉じた行動空間・回数上限・決定的フォールバック」の設計（本 ADR 検討時の議論）に従う。
- 関連: `AIAlbum/QueryEmbedder`（QueryVectors.positives・semanticScore 一元化）・`AIAlbumSearch`（probes・public化）・`AIAlbumService`（queryVectors/rankedSearch/refreshIncremental・captionOnDemand 転送）・`AIAlbumInterpretationStore`（probes・v6）・`AIAlbumInterpreter`（プローブ生成・public化）・`AIAlbumVerificationCoordinator`（captionOnDemand・予算40）・`AutoAlbumEngine`（結線）・`MultiProbeTests`・`SearchQualityTests`／`scripts/gen_eval_fixture.py`・`eval_queries.json`。ADR-23・ADR-24・ADR-34・[[model-evaluations]] §4。

## ADR-34 VLM を SmolVLM-500M に格上げ＋キャプションはお気に入り限定にする
- 状態: 採用
- 文脈: SmolVLM-256M のキャプションは曖昧（物体誤認・「文字がある板」止まり）で、テキストベースの意味検索強化の頭打ち要因。より良い VLM を実測比較（`bench_vlm_quality.py`）: **SmolVLM-500M**（Apache・decoder-only＝ANE安全確実）は物体を正しく特定し看板の文字（"Please Prepay"）まで読む＝明確に高品質。SmolVLM2-500M も同等（画像は v1 が素直）。**FastVLM-0.5B は最速・高品質だが `apple-amlr`（研究用途限定）で製品同梱不可**＝不採用。500M は Core ML で **877MB**（デコーダ 691MB fp16・視覚 INT8 94MB・埋込 90MB＝256M の 402MB の約2.2倍）でメモリが律速。実機で自己検証（next-token torch=coreml 一致）・footprint 確認の上で採用。
- 決定: **VLM を SmolVLM-500M へ格上げ**（`convert_smolvlm.py` は `SMOLVLM_MODEL` で差替・視覚のみ INT8 は据え置き・Swift ランタイムは config 駆動で hidden 960 等を自動追従）。さらに **キャプション（重い文章生成）はお気に入り（PHAsset favorite）限定**にする＝タグ/CLIP埋め込み/顔は全写真のまま、VLM だけ絞る。実装: `AutoAlbumEngine.favoriteRefKeysProvider`＋`favoritesCache`（Composition Root が `favoriteImageRefKeys` を注入）／`TagStore.captionPending(favorites:)`・`captionPendingCount(favorites:)`／`captionUnprocessed(favorites:)`／`scheduleBackgroundFill` のインターリーブでお気に入り集合を渡す。進捗分母は**お気に入り数**（`AnalysisProgress.captionableTotal`）、フル画像の「生成中」プレースホルダも**お気に入りのみ**表示。`captionModelVersion` を 6 に採番し旧キャプション破棄＆付け直し。
- 結果: favorite にした写真だけ高品質な一文説明が付く（重い VLM を少数に絞って現実的に）。AI アルバム生成側は**キャプションを元々「任意の補助証拠」扱い**（証拠ゲートは tag か caption・FM 審査の任意コンテキスト・ランキングには不使用）なので**構造変更は不要**（favorite 限定でも破綻しない）。トレードオフ: (1) 500M は 877MB・実機 footprint 増＝メモリ律速（要実機監視）。(2) 非お気に入り・クラウド（favorite 概念なし）はキャプション無し＝テキスト検索の網羅は限定的。(3) お気に入り変更は次回夜間巡回で追従。
- 関連: `scripts/convert_smolvlm.py`(SMOLVLM_MODEL)・`build_smolvlm.sh`・`bench_vlm_quality.py`・`AutoAlbumEngine`(favoriteRefKeysProvider/favoritesCache)・`TagStore`/`TagTagger`(favorites)・`AutoAlbumEngine+Recognition`(scheduleBackgroundFill/insight/analysisProgress)・`AnalysisProgress.captionableTotal`・`AIAnalysisStatusView`・`AutoAlbumAdapters`。ADR-31（CLIP INT8）・ADR-32（Florence撤回）・[[model-evaluations]]。

## ADR-33 ピープル（顔クラスタ）をクラウド写真にも広げる（128px サムネ・追加DL無し・option B）
- 状態: 採用
- 文脈: 顔認識（Vision 検出＋顔モデル埋め込み→クラスタ）は端末写真のみ（候補は PHAsset 列挙・画像は 640px ロード）だった。クラウド（Dropbox）写真でも人物を出したい。ただし顔検出は最も解像度が要る（小さい顔を拾うため端末は 640px）一方、クラウド分析は通信を増やさない方針で**キャッシュ済み 128px サムネ**を再利用している（ADR-24 系）。「品質 vs 通信」で 3 案を提示し、ユーザーが **B（128px で動かす・追加DL無し・低品質許容）** を選択。
- 決定: **クラウド写真も 128px の Dropbox キャッシュサムネで顔検出**する（追加ダウンロード無し・CLIP/タグと同じ経路）。実装: `FacePerceptionAdapter` に `cloudImage` を注入し cloud refKey を処理／候補列挙を `allImageRefKeys`（PHAsset ＋ `dropboxStore.items` の "C-…"）に拡張（4 呼び出し箇所）／人物アルバムを `PersonAlbumView`（メンバー限定 MergedPhotoStore・local+cloud）で表示／代表顔アバター・顔タイルの `loadFaceAvatar` を `HeavyWorkScheduler.stores.dropboxStore` 経由でクラウド対応。顔スキャンの進捗分母もクラウド件数を合算。
- 結果: クラウド写真でもピープルが動き、人物アルバムに端末＋クラウド両方のメンバーが出る。トレードオフ（割り切り）: **128px では小さい/遠い顔を取りこぼし、顔埋め込みも粗く別人誤判定が増える＝大きく写った顔（ポートレート/自撮り）中心**。代表顔アバターも 128px からの切り抜きでボケる。クラウド同期が未完なら候補が空になり得るが、夜間 BGTask/再起動の次回スキャン（増分）で拾う。良い品質が要るなら将来 option C（顔用だけ w256〜640 を 1 枚 1 回取得・通信方針の見直し要）。
- 関連: `MobileCLIPKit/FacePerceptionAdapter`(cloudImage)・`MosaicPhotos/Home/PeopleSupport`(allImageRefKeys/cloudPaths/loadFaceAvatar)・`PersonAlbumView`(新規)・`AutoAlbumAdapters.makePeopleEngine`(dropboxStore)・`HomeView`/`HeavyWorkScheduler`/`AIAnalysisStatusView`(候補列挙)。ADR-29（人物統合・人物名検索）・ADR-24（クラウド分析の 128px 再利用）。

## ADR-32 VLM キャプションを Florence-2-base へ置き換え → **撤回して SmolVLM に戻す**
- 状態: **撤回**（採用 → 実機で破綻し撤回。SmolVLM-256M を継続採用）
- 撤回理由（2026-07・実機検証の結論）: Florence-2-base は Mac の Core ML では全 compute unit で正しく動く（~0.4秒/枚）が、**実機の ANE でも GPU でも fp16 演算が Mac と食い違い、全写真同一の無関係テキスト**（言語モデルの地の文）を吐く。段階切り分けで encoder 出力は有限（fp16 NaN でない）・mask も正常と確認でき、**デコーダの fp16 数値が実機で乖離**（近接 logits の argmax 反転）と判明。fp32 化しても実機 GPU では直らず、`computeUnits=.cpuOnly` なら Mac と一致して正しくなる見込みだが、**CPU 固定では 3〜5秒/枚（Mac 実測 1.3秒）と遅く、Florence を選んだ最大の理由（ANE で 3〜5倍速）が消える**。「CPU 固定なら ANE で速い SmolVLM の方が良い」との判断で撤回。※ SmolVLM が遅くて行き渡らない問題の真因はパイプライン順序バグ（ADR-30）で、それは修正済みのため SmolVLM でも夜間に行き渡る。
- 撤回で戻したもの: `VLMRuntime`（SmolVLM 実行系・視覚埋め込み差し込み＋固定長デコード）・`MosaicPhotos/VLM/`（SmolVLM 資産・`build_smolvlm.sh` で再生成）・`CoreMLModelSupport.makeConfiguration`（cpuOnly 分岐撤去）・`LicensesView`（SmolVLM Apache-2.0）・`CLAUDE.md`。`captionModelVersion` を 5 に上げ Florence の誤キャプションを全消去＆SmolVLM で付け直し。Florence の変換/評価スクリプト（`convert_florence*.py`/`build_florence.sh`/`bench_vlm.py`）は参考として残置。
- 教訓: **Core ML の正しさは Mac だけでは検証しきれない**。encoder-decoder の cross-attention は実機 ANE/GPU の fp16 で Mac と乖離し得る（decoder-only の SmolVLM では顕在化しなかった）。新モデル採用前に**実機での正しさ**を確認する手段（今回の生成ID/有限性ログ）が必須。関連事例は case-studies 参照。
- 以下は撤回前の当初決定（記録として保持）。
- 文脈: 同梱 VLM（SmolVLM-256M・Apache-2.0）は 1枚 1〜2秒（実機）と遅く、夜間バッチでキャプションが行き渡らない一因だった（ADR-30）。「VLM でしか取れない情報は残す」前提で、より軽量・高速なモバイル向け VLM を検討。候補比較（`scripts/bench_vlm.py`・Mac PyTorch MPS）で **Florence-2-base（MIT・約231M）** が SmolVLM（256M）比で自然文キャプションが約5倍速く、内容は同等以上（OCR も滲む）と判明。さらに **Core ML 変換 PoC**（`scripts/convert_florence_poc.py`）で、実機出荷経路の Core ML/ANE でも動作し **~0.4秒/枚**（エンコーダ150ms＋デコーダ6〜7ms/token）＝SmolVLM の 1〜2秒に対し 3〜5倍速が保たれることを確認（※この PoC は Mac の CPU_AND_NE で、実機 ANE の破綻は見抜けなかった）。
- 文脈: 同梱 VLM（SmolVLM-256M・Apache-2.0）は 1枚 1〜2秒（実機）と遅く、夜間バッチでキャプションが行き渡らない一因だった（ADR-30）。「VLM でしか取れない情報は残す」前提で、より軽量・高速なモバイル向け VLM を検討。候補比較（`scripts/bench_vlm.py`・Mac PyTorch MPS）で **Florence-2-base（MIT・約231M）** が SmolVLM（256M）比で自然文キャプションが約5倍速く、内容は同等以上（OCR も滲む）と判明。さらに **Core ML 変換 PoC**（`scripts/convert_florence_poc.py`）で、実機出荷経路の Core ML/ANE でも動作し **~0.4秒/枚**（エンコーダ150ms＋デコーダ6〜7ms/token）＝SmolVLM の 1〜2秒に対し 3〜5倍速が保たれることを確認。
- 決定: **Florence-2-base を採用**（タスク `<DETAILED_CAPTION>` を焼き込み・1文〜数文の自然文説明）。実行方式は encoder-decoder 型: **VLMVision**（画像→encoder隠れ状態＋mask・画像正規化 mean/std とタスクをエンコーダに内包・ImageType scale=1/255）＋ **VLMDecoder**（decoder_input_ids[1,MAXLEN] を固定長で全系列 forward し現在位置の logits を貪欲選択・KVキャッシュ無し・動的長は ANE 非対応のため固定長＝SmolVLM 時代と同方式）。SmolVLM と違い**トークン埋め込み表は不要**（デコーダがトークン ID を直接受ける）＝Swift は復号のみ（`GPT2Tokenizer`・byte-level BPE は BART も同一写像）。encoder/decoder 間は fp16 で直結（キャスト copy 回避）。ビルドは `scripts/build_florence.sh`＋`convert_florence.py`（transformers 4.49 固定＝remote code の都合・ランタイムは Core ML なので無関係）。`captionModelVersion`(1→2) を採番し、起動時 `TagStore.resetCaptions()` で旧 SmolVLM キャプションを 1回クリア→新モデルで付け直す（`captionPending` は `caption==nil` のみ対象＝クリアしないと旧が残るギャップを塞ぐ）。ライセンスは HF モデルカードで MIT を確認。
- 結果: キャプションが 3〜5倍速くなり、内容は同等以上＋看板文字（OCR）まで拾える。同梱サイズは 442MB（VLMVision 258＋VLMDecoder 184）で SmolVLM 491MB より僅かに小。トレードオフ: (1) 全写真の再キャプション（夜間・段階的）。(2) 固定長デコーダは O(L²) 再計算だが 〜48 token で ~0.3秒＝許容。(3) Florence の remote code は transformers 4.49 でしか変換できない（ビルド環境の固定で対応）。(4) タスク展開の落とし穴＝task_ids は processor 経由で作る必要（literal トークン化はタスク名をエコー・変換時に踏んで修正）。(5) 検証は Mac Core ML(CPU_AND_NE)＋Python 復号一致まで（実機 ANE の最終確認は要・シミュレータは VLM スキップ設計）。phase 2 で Florence の INT8 化（~230MB）や OCR/領域タスクの活用余地。
- 関連: `MobileCLIPKit/VLMRuntime`(Florence 化)・`GPT2Tokenizer`(復号流用)・`VisionTagAdapter`(seam 不変)・`scripts/convert_florence.py`/`build_florence.sh`/`convert_florence_poc.py`/`bench_vlm.py`・`AutoAlbumEngine.captionModelVersion`(1→2)・`TagStore.resetCaptions`・`AutoAlbumSettingsKeys.captionModelVersion`・`LicensesView`(MIT 表記)。[[ADR-30]]（インターリーブ）・[[ADR-24]]（多層 AI）。

## ADR-31 CLIP を INT8 重み量子化して容量を半減する（精度ほぼ不変）
- 状態: 採用
- 文脈: 同梱 CLIP（OpenCLIP ViT-B-32/DataComp・fp16）は 289MB（画像168＋テキスト121）と大きい。「似た精度で軽く」を目標に、現行fp16／INT8量子化／TinyCLIP-40M の3構成を Core ML で実測比較した（`scripts/bench_clip.py`・Imagenette×1000クラス zero-shot・Mac CPU）。結果: 現行 289MB/75.0%、**INT8 145MB/76.0%**、TinyCLIP 161MB/61.0%（fp32では67%だが fp16変換で低下）。TinyCLIP は −14pt と精度低下が大きく、テキスト側の語彙埋め込み表で 161MB と INT8 より重い＝不採用。
- 決定: **現行 ViT-B-32 を INT8 重み量子化（`linear_quantize_weights`・linear_symmetric・weight_threshold=512）**して据え置く。モデル・埋め込み次元(512)・入力経路すべて不変。容量 **289→145MB**、精度は 75→76%（誤差・量子化ノイズ）、メモリも微減。CPU では速度同等（重みのみ量子化＝計算時 fp16 に復元）、実機 ANE ではメモリ帯域減で微有利の見込み。`scripts/convert_clip.py` に `QUANTIZE=int8` パス（`maybe_quantize`）を追加、`build_mobileclip.sh` は既定 ON。`perceptionVersion` を 7→8 に採番し、起動時 `resetSceneTagged()` で全写真を**再埋め込み**（埋め込み値が僅かに動くため。数晩・夜間トリクル）。
- 結果: 同梱容量が半減し、精度・機能は不変。トレードオフ: (1) 再埋め込みが 85k 枚ぶん走る（夜間・段階的）。移行中は旧fp16と新INT8の埋め込みが混在するが、同一モデルの量子化差はごく小さく検索への実害は軽微（別モデル差し替えと違い空間がほぼ動かない）。(2) テキスト側量子化で inf を含む const 1つが「量子化スキップ」警告（fp16 のまま残り無害）。(3) 実機 ANE での fp16/INT8 NaN はランタイムの有限性チェックが nil 落としで保険。
- 関連: `scripts/convert_clip.py`（maybe_quantize）・`scripts/build_mobileclip.sh`（QUANTIZE 既定int8）・`scripts/bench_clip.py`（3構成比較）・`AutoAlbumEngine.perceptionVersion`(7→8)・`AutoAlbumStore.resetSceneTagged`。[[ADR-11]]（fp16 image encoder）。

## ADR-30 CLIP 埋め込みと VLM キャプションを夜間バッチでインターリーブする
- 状態: 採用
- 文脈: 実機で VLM キャプションが 1 枚も出力されない不具合。原因は `scheduleBackgroundFill()` のパス順が **タグ → CLIP 埋め込み（全量）→ キャプション** の逐次で、キャプションが**埋め込みの完了を待つ**構造だったこと。実機は 85,418 枚中 17,981 枚（約 21%）しか埋め込めておらず（毎晩の電源＋アイドル窓は短く、1 枚 ANE でも枚数が多い）、埋め込みが 100% になるまでキャプションが 1 枚も始まらない＝事実上キャプションが永遠に未着手だった。
- 決定: 埋め込みとキャプションを**少量ずつ交互（ラウンドロビン）に回す**。タグ（数十 ms/枚・検索の一次ランキング）は従来どおり先に全量へ行き渡らせ、その後 `while !heavyShouldPause()` で「埋め込み 12 バッチ → キャプション 3 バッチ」を繰り返す。各ランタイムに `maxBatches` を渡してバッチ数で切り上げられるようにし（`PhotoTagger.embedUnprocessed` は既存、`TagTagger.captionUnprocessed` に追加）、`AutoAlbumStore.unembeddedCount()` と `TagStore.captionPendingCount()`（新設）で **1 ラウンドの前後差** を見て、両方とも 1 枚も進まなければ終了する。この終了判定はシミュレータ（埋め込み・キャプション両方 `#if targetEnvironment(simulator)` でスキップ）でも安全に即終了する。譲り条件は従来と同じ（埋め込み＝操作中も譲る＋`heavyShouldPause`、キャプション＝`heavyShouldPause`）。
- 結果: 埋め込みが未完でもキャプションが並行して進み始める。停止判定は 1 枚単位のまま（ロック解除直後の操作は即譲る）。トレードオフ: (1) 埋め込みの全量完了は交互ぶん僅かに遅くなる（キャプションに窓を分けるため）が、両者とも「数晩がかり」の性質なので体感差は小さい。(2) ラウンドの前後差カウント（`fetchCount` 2 回/ラウンド）が増えるが、12+3 バッチ＝60 枚超に 1 回なので無視できる。(3) バッチ比（12:3）は実機の 1 枚所要（埋め込み速い・キャプション遅い）に合わせた暫定値で、`BackgroundProcessing` プリセット化は将来課題。
- 関連: `AIAlbum/AutoAlbumEngine+Recognition.scheduleBackgroundFill`・`Perception/PhotoTagger.embedUnprocessed`（maxBatches）・`Tags/TagTagger.captionUnprocessed`（maxBatches 追加）・`Tags/TagStore.captionPendingCount`（新設）・`AutoAlbumStore.unembeddedCount`・`BackgroundTrickle`。ADR-24（多層 AI・夜間トリクル）・ADR-25（電源＋Wi-Fi＋ロック中）。

## ADR-29 ピープルの人物統合と、AI アルバムの人物名検索（決定的接地＋LLM 補強）
- 状態: 採用
- 文脈: (1) 同一人物が顔クラスタリングで 2 人物に割れることがあり、まとめる手段が無かった（1 顔ずつの付け替えのみ）。(2) 人物アルバム名はフルネーム（「木村太郎」）が多いのに、AI アルバムのクエリで名だけ（「太郎」）や複数人物（「太郎と花子」）を指してもヒットしなかった。原因は評価（`QueryEvaluator` の people 部分一致）ではなく、**接地**（サニタイズがカタログ完全一致 or 原文出現しか通さない・"Person N" 混じりカタログ）と**解釈器の出力**（人名抽出は夜間 LLM のみ・作成時プレビューでは立たない）にあった。
- 決定: **(A) 人物統合** — `FaceClustering.merging`（生合計の加算＝加重平均重心・1 顔ずつ adding と等価）＋`FaceStore.mergeClusters(from:into:)`（顔の一括付け替え・sum/count マージ・統合元削除・clusteringCache 無効化）＋`PeopleEngine.mergePerson`。UI は長押しメニュー「別の人物へ統合…」→ ピッカー → 確認。名前・代表写真は統合先を優先。**(B) 人物名検索** — 決定的な純ロジック `PersonNameGrounder` を主軸に据える。名前付きクラスタのフルネーム一覧（`PeopleEngine.namedClusterNames`・"Person N" 除外）をカタログに、クエリ原文から各フルネームの「全体＋前方（姓）＋後方（名）の部分文字列（長さ2以上・中間片は作らず誤爆抑制）」で照合し、該当フルネームを `.people(...)` 条件（部分一致 OR）に載せる。**作成時プレビューと夜間解釈の両方で接地**するので、LLM 非依存で即ヒットする（夜間 LLM はあだ名・ローマ字ゆれの補強）。カタログは `AutoAlbumEngine.setNamedPeopleProvider` で Composition Root から注入。視覚語抽出は人名部分を除いた残りで行う（「花子」の「花」を flower と誤抽出しないよう `strippingNames`）。
- 結果: 割れた人物をまとめられ、「太郎」「太郎と花子」「木村（姓で同姓全員）」がヒットする。純ロジックはテスト済み（merging の加算等価・接地の姓名部分一致・中間片非マッチ・視覚語の誤抽出回避）。トレードオフ: (1) 統合は自動 undo なし（確認アラートで担保）。(2) 接地は `EnrichedPhoto.people`（索引時に焼き込み）を評価するため、リネーム/統合の直後は再エンリッチまで旧名が残り得る（既存の版管理＝criteria 変更/version bump で追従）。(3) 複数人物は OR（「両方写る」AND 生成経路は今回入れていない）。
- 関連: `Faces/FaceClustering.merging`・`FaceStore.mergeClusters`・`PeopleEngine.mergePerson`/`namedClusterNames`、`MosaicPhotos/Home/PersonPhotosView`（PersonMergePickerView）・`PeopleActions`、`AIAlbum/PersonNameGrounder`・`QuerySpecSanitizer.addingPeople`・`AIAlbumInterpreter`（preview/interpretation）・`AIAlbumService`・`AutoAlbumEngine.setNamedPeopleProvider`・`AutoAlbumAdapters`。ADR-23（解釈の永続化）・ADR-24（people 部分一致）。

## ADR-28 フル画像の情報パネルに抽出情報を漏れなく出す（顔数・スクショ。CLIP はラベル化済み）
- 状態: 採用
- 文脈: 「画像から抽出した情報は全てフル画面に出したい」との要望。監査の結果、写真 1 枚から永続化している情報のうち **顔の数**（`ScannedPhoto.faceCount`・実測）と **スクリーンショット判定**（`PhotoEnrichment.isScreenshot`）が `PhotoInsight` 型に載っておらず、`insight()` が構築時に捨てていて画面に出ていなかった。CLIP 512 次元ベクトル（`PhotoEmbedding.vector`）は生では意味不明だが、既に `CLIPDisplayLabeler`（約300語ゼロショット・最大6語）で語化され Vision シーンタグと統合して「Detected」欄に表示済み（＝ベクトルのキーワード化は実装済み・眠っていない）。
- 決定: `PhotoInsight` に `faceCount: Int?`（未スキャン=nil で「0 と未計測」を区別）と `isScreenshot: Bool` を追加。顔数は `FaceStore.faceCount(refKey:)`（単一・軽い fetch）→ `PeopleEngine.faceCount(forItemID:)` を新設し、人物名と同じく `SourceHostView` の photoInsight クロージャで合成（顔スキャンは端末のみ＝クラウドは nil）。isScreenshot は `insight()` が `rec.photo` から詰める。パネルは、人物名があれば「名前 · N faces」、名前が無くても顔スキャン済みなら「N faces（Detected faces）」を出し、スクショは「Screenshot」バッジを出す。CLIP ラベルは現状の統合表示を維持（生ベクトルは出さない）。
- 結果: 抽出済みの情報がフル画面に出揃った（タグ＝Vision＋CLIP語化・キャプション・人物名・顔数・スクショ・場所・日付・EXIF・地図・解析状態）。未表示のまま残すのは内部値（aspect・contentHash・linkKey・顔 bbox/embedding）と、AI 抽出でないユーザーフラグ（isFavorite＝別途ハートで表示）。
- 関連: `PhotoInsight.swift`、`PhotoInfoPanel.insightSection`、`AutoAlbumEngine.insight`、`PeopleEngine.faceCount(forItemID:)` / `FaceStore.faceCount(refKey:)`、`SourceHostView`。CLIP 語化は `CLIPDisplayLabeler`。

## ADR-27 AI 解析の進捗をユーザー向けに可視化（各パスの最終実行時刻を永続化）
- 状態: 採用
- 文脈: オンデバイス AI（意味検索の CLIP 埋め込み・シーンタグ・キャプション・顔スキャン）は既定でフォアグラウンド停止＝夜間トリクル（ADR-25）のため、ユーザーからは「動いているのか・どこまで済んだのか」が全く見えなかった。進捗の数値は Developer Options や AutoAlbumSettings に断片的に出ていたが、デバッグ用で分散し、最終実行時刻はBGタスク全体（`bgTaskLastRun`）しか無かった。
- 決定: 設定「Albums & Search」に**ユーザー向け専用画面「AI Analysis（AI 解析の状況）」**を新設。(1) 各パスの「完了数／総数（進捗バー＋%）」を表示（分母は取り込み済み写真数＝`enrichmentCount`。CLIP/タグ/キャプションで共有、顔は端末写真数）。(2) **各パスの最終実行時刻を新規に永続化**＝`AnalysisActivity`（UserDefaults・パス別キー）を各タガーの**バッチ確定点で記録**（実際に 1 枚以上処理したときだけ＝空振りで更新しない＝「止まっている」を正しく反映）。(3) 「解析中」は `BackgroundActivityMonitor`（ライブ）を body 直読みで即時反映。(4) 集約取得は `AutoAlbumEngine.analysisProgress()`（1 回で total/embedded/sceneTagged/captioned）と `PeopleEngine.scanStats()` を新設（従来 internal だった TagStore/FaceStore の件数を public 委譲で薄く露出）。キャプション/顔はモデル未同梱ならセクション非表示。デバッグ用の詳細（Developer Options）は従来どおり別に残す。
- 結果: 「動いていない／完了したか分からない」が解消。文字列は英語 base＋日本語訳を String Catalog に追加（補間キーは位置指定 `%2$lld…%1$lld` で語順対応）。トレードオフは公開 API と UserDefaults キーが増える点、及び分母を `enrichmentCount` で近似する点（取り込み前の写真は母数に入らない＝生成前は 0/0 と出る）。
- 関連: `MosaicPhotos/Settings/AIAnalysisStatusView.swift`、`AutoAlbumCore/AnalysisActivity.swift`（`AnalysisProgress` 含む）、`AutoAlbumEngine+Recognition.analysisProgress()`、`PeopleEngine.scanStats()`、各タガーのバッチ確定点。ADR-25（夜間トリクル）。

## ADR-1 検索を語彙ゼロのオープン語彙 CLIP に一本化
- 状態: 採用
- 文脈: 固定タグ方式は語彙外検索に弱く、辞書の保守も負担。
- 決定: 固定語彙・OCR を捨て、CLIP のオープン語彙＋構造化条件＋字句の RRF 融合にする。
- 結果: 自由なクエリに強い。代わりに全写真の画像埋め込み（背景処理）が必要。表示タグだけは別途ゼロショットで出す。
- 関連: `AutoAlbumCore/AIAlbum/`、`MobileCLIPKit/`。

## ADR-2 写真ソースを共通プロトコルに統一
- 状態: 採用
- 文脈: 端末・Dropbox・統合で同じ閲覧体験を出したい。
- 決定: `PhotoStore` / `PhotoItem` / `PhotoLoadState` に揃え、共通ビューを共有。
- 結果: 新ソースを足しても表示側は不変。各ソース固有の init パラメータは維持。
- 関連: `PhotoSourceKit/Interface/`。

## ADR-3 ロジック層 / UI 層の分離（Core/UI）
- 状態: 採用
- 文脈: 副作用（PhotosKit/URLSession/SwiftData）を UI から切り離してテストしたい。
- 決定: `*Core`（SwiftUI 非依存）と `*Kit`（UI・`@_exported import`）に分割。
- 結果: 純ロジックを macOS で高速テスト。import は 1 つで済む。パッケージ数は増える。

## ADR-4 グリッドを UICollectionView に置換
- 状態: 採用
- 文脈: 6 万件で `LazyVGrid` + `scrollTo` のスクラバーが不安定。
- 決定: `UIViewRepresentable` で UICollectionView（diffable・プリフェッチ・contentOffset スクラバー）を採用。
- 結果: 大ジャンプが安定。UIKit のボイラープレートが増える。
- 関連: コミット 9897653、事例「スクラバー不具合」。

## ADR-5 単一 fullScreenCover + enum で遷移
- 状態: 採用
- 文脈: 複数の `.fullScreenCover(item:)` 併用で提示競合し、別アルバムの中身が出る不具合。
- 決定: 遷移先を単一の `HomeDestination` enum にまとめ、`.fullScreenCover` は 1 つに。`.sheet` も同様に統合。
- 結果: 競合解消。分岐は `switch` に集約。
- 関連: `MosaicPhotos/HomeView.swift`、事例「アルバムが無関係な写真になる」。

## ADR-6 CLIP 埋め込みを別テーブル + Float16
- 状態: 採用
- 文脈: 埋め込みを `PhotoEnrichment` に inline 格納していたため、全件 fetch のたびに 138MB 級 blob を展開し、写真の多い実機で起動クラッシュ。
- 決定: 埋め込みを `PhotoEmbedding` 別テーブルへ分離し、Float16 で保存。メタデータ fetch は blob に触れない。
- 結果: 常駐が「ページ 1 枚分」に。スキーマ再構築（`AutoAlbumV10`）が必要。
- 関連: コミット f84529c、事例「メモリ枯渇」。

## ADR-7 ModelContainer を自己修復で構築
- 状態: 採用
- 文脈: SwiftData は破損・不整合で起動時 trap し、実機で原因不明クラッシュ。
- 決定: `makeResilientContainer` で「削除→再試行→インメモリ」とフォールバック。
- 結果: 起動を止めない。最悪データは失うが回復する。
- 関連: コミット 7e53db3。

## ADR-8 起動の重いストア構築を非同期化
- 状態: 採用
- 文脈: `HomeView.init` が同期で `ModelContainer` を作り、最初の描画をブロック。
- 決定: `RootView` が `HomeStores.build()` で非同期構築し、1 秒超でローディング表示。
- 結果: 体感起動が改善。
- 関連: コミット 8bc97dd、事例「起動の高速化」。

## ADR-26 横断リファクタリング＝共通足場の抽出と肥大型の責務分割（挙動不変）
- 状態: 採用
- 文脈: 機能追加が続き、同じ骨格のコピーが層をまたいで蓄積（背景トリクル 4 ループ / 自己修復 ModelContainer 3 実装 / Core ML ランタイム 3 種のロード・推論 / BPE トークナイザ 2 種の足場 / UI 小物パターン約 25 箇所）。また最大級の型（AIAlbumService 462 行・BackupEngine 346 行・DropboxThumbnailBatcher 329 行）に複数責務が同居し、変更の影響範囲が読みにくくなっていた。
- 決定: **挙動不変（ログ文言・PerfTrace ラベル・public API・永続化フォーマット・見た目を維持）**を絶対条件に、(1) 共通足場の抽出＝`BackgroundTrickle`（1 枚単位の譲り骨格・AutoAlbumCore）/ `makeResilientModelContainer`（MosaicSupport）/ `CoreMLModelSupport`・`BPESupport`（MobileCLIPKit。bpe() 本体は CLIP=最左 1 箇所マージと GPT2=全出現一括マージでアルゴリズムが異なるため共通化しない）/ `BusyLabel`・`LoadingRow`・`sectionHeader`・`loadCover`（アプリ層）、(2) 責務分割＝AIAlbumService→`AIAlbumInterpreter`/`AIAlbumVerificationCoordinator`/`QueryEmbedder`（フル評価と増分評価の「同じ規則」不変条件をコピペ同期から型共有に）、BackupEngine→`BackupRunner`/`BackupProgressStore`、Batcher→`DropboxThumbnailBatchRequest`(純)/`DropboxThumbnailChunkFetcher`。あわせて死線（旧行ベースのホーム行 3 型）と誤情報（Minimum iOS 表示）・既定値リテラルの散在を整理。
- 結果: 譲りポリシー・コンテナ復旧・モデルロードの方針変更が各 1 箇所で済む。トークナイザは同梱語彙での並置比較でビット等価を実測確認。キュー戦略・審査規則が独立テスト可能に。トレードオフはファイル数の増加と、共通骨格（BackgroundTrickle 等）への間接参照が 1 段増えること。
- 関連: コミット a07e38b8〜dcea41af（7 コミット）。ADR-25（1 枚単位の停止判定）・ADR-23/24（同じ規則の不変条件）。

## ADR-25 重い処理は「電源＋Wi-Fi＋画面ロック中」のみ実行（フォアグラウンド完全停止）
- 状態: 採用（ADR-20/ヘビーワークゲートの後継）→ **改定（→ ADR-79/195）**: 「前面完全停止」は「20 秒触っていなければ動く・触れば 1 単位で譲る」へ。一枚岩は非アクティブ限定のまま（ADR-107）
- 文脈: 旧ゲートは「電源＋低電力OFF＋アイドル60秒」。充電しながら閲覧中に60秒触らないと重い処理（顔認識・タグ付け・再評価）が走り出し、操作再開時にバッチ途中（クラウド写真のタグ付けは1バッチ最大約27秒）が譲れず操作が固まった。「操作あり」の検出も狭く（ホームのスクロール等はアイドル扱い）、誤発火した。ユーザー方針:「操作中は一切動かさない。電源＋Wi-Fi＋ロック中に動かす。AIアルバム作成もなるべくそのタイミングに」。
- 決定: (1) 中央ゲート `BackgroundYield.heavyWorkAllowed` を「電源＋低電力OFF＋**アプリ非アクティブ（scenePhase）**＋**Wi-Fi**」へ変更しアイドル判定を廃止。手動ブースト（今すぐ処理）だけ非アクティブ/Wi-Fi を免除。BGTask 起動時は scenePhase 変化が来ないため `runHeavyWork` 冒頭で非アクティブを明示（初期値 true のままゲートが開かない罠）。(2) 停止判定を **1 枚単位**に統一（タグ/顔/キャプション。CLIP は既に1枚単位）。(3) AI アルバム作成は**2段階**＝作成時は決定的プレビュー（レキシコン＋日付＋タグ照合・LLMなし・1〜2秒・`pendingFinalization`）→ 夜間に `finalizePending`（FM解釈＋証拠ゲート＋LLM審査＋Refine）で本番化。(4) 場所の定期再スキャンにも同ゲート適用。(5) 表示ラベラの概念埋め込み（約300語）を夜間パイプライン先頭で prewarm（写真初回オープンの数秒負荷を排除）。(6) 作成画面に動作タイミングの説明（プレビュー→夜間本番化）を明記。
- 結果: フォアグラウンドでは重い処理が一切走らず操作が軽くなる。索引の進行は「充電＋Wi-Fi＋ロック」時間帯に限定され初回全量は日数を要する（エスケープハッチ=今すぐ処理）。BGTask は OS 裁量のため毎晩の実行保証はない。
- 関連: `BackgroundYield` / `MosaicPhotosApp`（scenePhase）/ `HeavyWorkScheduler` / `TagTagger`・`FaceTagger`（1枚粒度）/ `AIAlbumService.previewInterpretation`・`finalizePending` / `AIAlbumComposerView`。ADR-20・ADR-23・ADR-24。
- 追記（2026-07-08・5段階のユーザー設定化）: 固定ゲートをやめ、**実行タイミングをユーザーが 5 段階で選択**できるようにした（`HeavyWorkTiming`・既定は従来どおり「おまかせ＝夜間のみ」）。段階は単調に条件を緩める: 一時停止 → おまかせ（電源+Wi-Fi+非使用）→ 充電中はアプリ使用中の合間も（アプリ内全タッチを `TouchActivityTracker`（UIWindow 上の認識器）で捕捉し 20 秒アイドルで開始・タッチ即停止）→ バッテリーでも（残量 20% 以上）→ 制限なし（モバイル回線も）。低電力モード・メモリ圧迫は全段階でブロック（安全弁）。battery 以上では BGTask の `requiresExternalPower` も外す。判定は純関数 `HeavyWorkTiming.allows`（単調性を総当たりテストで固定）。設定 UI は AutoAlbumSettingsView「処理のタイミング」（各段の条件と影響を footer に明記）。

## ADR-24 AI アルバム検索を「タグ台帳＋LLM審査」へ再設計（閾値レス・エージェント型）
- 状態: 採用（ADR-23 の検索段を拡張）
- 文脈: 生 CLIP コサイン＋絶対閾値（floor 0.20 / 除外 0.22）の検索は、ユーザーのライブラリ分布が事前に不明なため閾値を調整できず、実測で「除外が全写真の 97% を落とす」「意味採用 0 件」が発生。小型オンデバイス LLM の構造化出力も 3 回連続で異なる形に壊れた（例語オウム返し・"any" プレースホルダ・「ここ2年」→year 2026）。ユーザー方針: 精度優先・夜間バッチは数日かかってよい・辞書は 300 語では不足。
- 決定: 4 層の再設計。(1) **P0 接地**＝日付は RelativeDateParser を唯一の出典・place/people はカタログ/原文出現のみ・翻訳失敗の非キャッシュ（解釈 v4）。(2) **P1 タグ台帳**＝OS 内蔵 Vision 分類（約1,300クラス・`hasMinimumRecall(forPrecision:)` の校正済み足切り＝自前閾値が消える）を夜間トリクルで全量付与（TagsV1 別コンテナ）し、タグ一致（離散・閾値レス）を第 3 のランキングとして RRF 融合、除外語はタグの離散一致でもハード除外。(3) **P2 LLM 審査**＝候補上位 60 件の証拠行（日付・場所・顔数・タグ・キャプション）を FM が keep/drop/unsure 判定し、unsure は最大 2 回の再判定で**多数決**（同数は keep＝安全側）。空振り時は LLM がプローブ語を生成して 1 回だけ再検索（Refine）。(4) **P3 VLM キャプション**＝SmolVLM-256M（Apache-2.0・約 500MB 同梱）で写真ごとに英語短文を夜間生成（1〜2 秒/枚・数晩がかり）し、審査の「目」にする。語彙制約が構造的に消える。
- 結果: 閾値依存がほぼ消滅（タグ=写真内順位・審査=言語判定）。判定根拠がタグ/キャプションで可視化されデバッグ可能。トレードオフ: バンドル +約500MB（VLM）・索引はカバレッジ依存（未索引写真は検索に弱い＝優先埋め込みで緩和予定）・審査は FM 搭載端末のみ（無ければ素通し）。
- 関連: `TagStore/TagTagger/VisionTagAdapter` / `AlbumVerifier` / `VLMRuntime/GPT2Tokenizer` / `scripts/build_smolvlm.sh`。ADR-19（権利フリーモデル）・ADR-23。事例「否定条件が二重に不発」「LLM 構造化出力の連続破綻」。

## ADR-23 AI アルバムの解釈は「作成時に 1 回・永続化」（カタログ追従の再解釈を廃止）
- 状態: 採用（旧方式＝カタログ署名によるキャッシュ全破棄を置換）
- 文脈: 旧方式は「解釈の正しさはライブラリの語彙（カタログ＝地名/人物一覧）に依存する」前提で、カタログ署名（件数ベース）が変わるたび解釈キャッシュを全破棄し LLM で再解釈していた。署名はメモリのみのため**毎起動で必ず全アルバム再解釈**（実測 9.4s のメインハング）となり、写真が増え続ける・アルバムを多数作る運用では原理的に破綻する。
- 決定: **解釈（検索文 → QuerySpec・英訳）は検索文の性質**と再定義し、作成/編集時に 1 回だけ LLM 実行して `AIAlbumInterpretationStore`（JSONFileStore・SwiftData スキーマ変更なし）へ永続化。存在しない地名・人名が解釈に残ることを許容する（照合は部分一致なので、該当写真が索引され次第自動的に当たる）。相対日付は相対形のまま保存し評価時に解決。LLM 呼び出し（解釈・翻訳）は Task.detached で完全オフメイン化。再評価は (1) 増分＝新規埋め込み分だけ採点してスコアプール（上位300・永続）へマージ、(2) ドリフト検知＝評価済み枚数の差が閾値（500）超でアイドル時にフル再評価、の 2 経路。起動時の再評価は廃止（保存済みメンバーを即表示）。
- 結果: 起動時の LLM 実行ゼロ（ハング根治）。写真追加のコストが O(全体)→O(新規)。アルバム数が増えても LLM は作成時の 1 回のみ。トレードオフ: 増分評価では lexical（地名/人物の字句）変化は反映されずフル再評価待ち。カタログ由来の表記寄せは作成時のヒントに限定される。
- 関連: `AIAlbumInterpretationStore.swift` / `AIAlbumService.swift` / `AIAlbumSearch.swift`（searchWithPool / mergePool / memberKeys＋テスト）/ `PhotoTagger.swift`（onBatch が新規 refKeys を通知）/ `AutoAlbumEngine.swift`（ドリフト検知）。事例「起動時に設定画面が固まる（FM 再解釈 9.4s）」。

## ADR-22 撮影日時のサニタイズは表示層でなくデータ入口で行う
- 状態: 採用
- 文脈: EXIF 欠落・0 値・カメラ既定値（1970/1980 等）の写真が「1980-01-01」等として表示・整列される。当初は表示3箇所（フル画面ラベル・アルバム日付・グリッド見出し）だけで `meaningful` 判定して「日時不明」化したが、ソート（`PhotoItemSorting`）・自動アルバム生成（`PhotoEnrichment` の保存値）・場所スキャンは生値のままで、無意味な日付が並び順や旅行アルバムの日付に混ざり続けた。
- 決定: 判定の核を `MosaicSupport.CaptureDate.meaningful`（有効窓 1990-01-01〜現在+2日）に置き、**データの入口**でサニタイズする：`LocalPhotoItem.captureDate`（PHAsset）・`DropboxFileItem.init`（同期パース／キャッシュ復元の両生成点を一括）・`PhotoEnricher`（エンリッチ保存値）。表示層の `DisplayDate.meaningful` は委譲にして防御としては残す。
- 結果: ソート・生成・グルーピングにも無意味な日付が入らない。入口が1関数に集約され閾値変更が1箇所。トレードオフ: 1990 年より前の正当なスキャン写真も「日時不明」になる（スマホ写真アプリの用途では許容）。既存の生成済みアルバムは再生成するまで旧日付が残る。
- 関連: `MosaicSupport/CaptureDate.swift`（+テスト）、`DropboxFileItem.swift` / `PhotoEnricher.swift` / `LocalPhotoItem.swift`。

## ADR-21 逆ジオコーディングを同梱DBで完全オフライン化（GeoNames cities15000）
- 状態: 採用
- 文脈: 座標→地名の解決を `CLGeocoder`（オンライン・レート制限・要ネットワーク）で行っていた。一括生成時にスロットリングで失敗し、`PlaceNameResolver` が**失敗（空）を恒久キャッシュ**するため旅行アルバムが「Trip」で固定化、Places も地名にならない不具合が出ていた（[[ADR-1]] 系の自動アルバム品質に影響）。
- 決定: GeoNames `cities15000`（約3.4万都市・CC BY 4.0）から生成したコンパクトなバイナリ `cities15000.bin`（約0.8MB）を `PhotoSourceKit` に同梱し、`OfflinePlaceDB` が**最近傍検索**で市区町村/都道府県/国を返す（`scripts/build_places.py` で生成、リトルエンディアン：lat/lon の f32 配列＋行政区/国プール＋都市名）。`PlaceNameResolver` のバックエンドをこれに置換し `CLGeocoder` を廃止。結果は決定的なのでグリッドキャッシュ（GeoGridKey）はそのまま安全に永続化できる。地名は**英語(ローマ字)と日本語の両方**を bin に保持し、アプリの表示言語（`AppLocale.isJapanese`＝上書き言語または端末言語）で切り替える（日本語が無い都市は英語へフォールバック）。英語＝GeoNames `name`、日本語＝言語別別名 `alternateNamesV2`（isolanguage=ja）。例 日本語: 千代田区/東京都/日本・パリ/フランス、英語: Chiyoda/Tokyo/Japan・Paris/France。
- 結果: ネット問い合わせ**ゼロ**・即時・無制限・失敗なし。アルバム命名と Places の両方が安定（「Trip」固定が解消）、命名が同期化して非同期スロットリングも不要に。**UI 言語に追従**して日/英で表示（`PlaceNameResolver` のキャッシュも言語別キー）。トレードオフ＝精度は「最も近い既知都市」（用途的に十分。都道府県・国は堅牢）、日本語名の無い一部の海外マイナー都市はローマ字。bin は日英両方で約1.2MB（CLIP 約60MB に比べ微小）。CC BY 4.0 を NOTICE/アプリ内 Licenses に表記。
- 関連: `PhotoSourceKit/Places/OfflinePlaceDB.swift`・`PlaceNameResolver.swift` / `scripts/build_places.py` / `Packages/PhotoSourceKit/Package.swift`(resource) / `NOTICE`・`Licenses`。

## ADR-20 メモリ圧迫対応を中枢へ集約（解放ハンドラ登録＋詳細ログ＋履歴）
- 状態: 採用
- 文脈: メモリ節約後も実機で起動直後クラッシュ（jetsam）が起きていた。圧迫検知は `DispatchSource` にあったが「フラグ＋ログ」止まりで、画像キャッシュの解放は別系統（UIKit の `didReceiveMemoryWarning`）頼みのため、DispatchSource の critical では解放まで繋がらず、warning でも"上限半減"止まりだった。「圧迫時にログを出し・メモリを解放し・クラッシュを避ける」を一本化したい。
- 決定: `MosaicSupport.MemoryPressureMonitor` を中枢化する。(1) `register(_:) -> token` で**解放ハンドラ**を登録できるようにし、(2) `handle(level:)` が圧迫フラグ設定・履歴記録・**全ハンドラ呼び出し**・診断ログ追記（レベル／footprint／端末RAM／ハンドラ数）を一括実行。`Diagnostics` の DispatchSource は `handle(.warning/.critical)` を流すだけにする。`MemoryImageCache` は `register` で **warning=上限半減（LRU 縮小）／critical=即時全消去（`removeAllObjects`）** を解放する（ImageCacheKit が MosaicSupport に依存）。背景 CLIP 埋め込みは従来どおり `isUnderPressure` で一時停止。Developer Options（`MemoryDebugSection`）に**累計回数＋直近イベント履歴**（時刻・レベル・footprint）を表示。CLIP モデルのアンロードと footprint 常時監視は今回スコープ外（再ロード遅延・常駐コスト）。
- 結果: 圧迫イベントが単一経路で「ログ→解放」に直結。critical で素早くメモリを返し jetsam 余地を減らす。履歴で実機の前兆を切り分け可能。トレードオフ＝critical 全消去後はサムネ再デコードが一時的に増える（30秒で上限復帰）。
- 関連: `MosaicSupport/Diagnostics.swift`（MemoryPressureMonitor）/ `ImageCacheKit/MemoryImageCache.swift` / `MosaicPhotos/Settings/MemoryDebugSection.swift`。E1 段階縮小（[[ADR-15]] 電源ゲート・背景処理停止）と併用。

## ADR-19 同梱 CLIP モデルを OpenCLIP ViT-B-32/DataComp（MIT）へ差し替え（権利フリー化）
- 状態: 採用
- 文脈: MobileCLIP-S2 の重みが Apple ML Research Model License（研究目的限定・商用不可）で AGPL／App Store 配布に使えない（[[ADR-18]] の残課題）。許容ライセンスのモデルへ差し替える。
- 決定: **OpenCLIP `ViT-B-32` / `datacomp_xl_s13b_b90k`（重み・コードとも MIT）** を採用。汎用変換 `scripts/convert_clip.py`（open_clip→Core ML、**CLIP mean/std をモデル内に内包**して ImageType は scale=1/255 のまま＝アプリ無改修、画像 fp16／テキスト fp32）で変換し、`build_mobileclip.sh` を更新。同梱ファイル名（`MobileCLIP*` / `MobileCLIPKit` / `mobileclip_config.json`）は互換のため**据え置き**（中身は OpenCLIP）。Swift は `MLImageConstraint` による自動リサイズ＋正規化内包のため**無改修**（imageSize 256→224 は config 経由）。CLIP BPE 語彙・`CLIPTokenizer` は同一で流用。`perceptionVersion` を 7 に採番し**全写真を再埋め込み**。
- 検証（認識率ハーネス・ImageNet-1k ゼロショット top-1・200枚・CPU／クエリ 10件）:
  - MobileCLIP-S2（研究限定）= **81.0%**、ViT-B-16/datacomp = 75.0%、**ViT-B-32/datacomp = 75.0%（採用）**、ViT-B-32/openai = 64.5%。クエリは全候補 10/10。
  - TinyCLIP は open_clip 非対応（独自アーキ・config 無し）でこのツールチェーンではロード不可のため候補から除外。
  - 採用は**軽量（patch32・画像 enc ~60MB）かつ 75%** の ViT-B-32/datacomp（オンデバイス速度/省電力との両立）。MobileCLIP 比で約 -6pt だがクエリ性能は同等。
- 結果: **MIT 化で AGPL＋App Store（デュアル）の最後の障害が解消**。NOTICE/README/アプリ内 Licenses を OpenCLIP へ更新し研究限定の警告を撤去。モデル本体は `.gitignore` 対象でローカル生成（`build_mobileclip.sh`）。
- 関連: `scripts/convert_clip.py` / `scripts/build_mobileclip.sh` / `scripts/eval_recognition.*` / `AutoAlbumEngine`(perceptionVersion) / `Licenses.swift`・`LicensesView.swift` / `NOTICE` / `README`。

## ADR-18 ライセンス：ソース AGPL-3.0＋デュアル配布（App Store は著作権者が Apple 条件で）／第三者はアプリ内表示
- 状態: 採用（経緯: 当初 AGPL 採用 → 一旦撤回 → **デュアル方式で再採用**）
- 文脈: 配布形態は「**アプリ＝App Store／ソース＝GitHub**」。本体ライセンスを定めつつ、GPL/AGPL × App Store の非互換を回避したい。
- 決定: ソースは **AGPL-3.0-or-later**（`LICENSE` に公式全文・原文）。**デュアル配布**＝著作権者（単独）は同一成果物を App Store では Apple 標準条件で配布できる（AGPL は他者ライセンシーを縛るが著作権者自身は別条件で配布可）。第三者依存は MIT/BSD で両立可。将来コントリビュートで縛られないよう **`CONTRIBUTING.md` に DCO＋再ライセンス許諾**、宣言を `NOTICE` に明記。アプリ内「Settings → Licenses」に本アプリ(AGPL/デュアル)と第三者資産を表示。§7 例外方式は不採用（例外が全員に及ぶため。自分だけが App Store 配布するデュアルが適切）。
- 結果: ソース公開（AGPL）と自分の App Store 配布が両立。**未解決の前提**: MobileCLIP の**重みは研究目的限定・商用不可で App Store 不可**のため、バンドル出荷前に**許容ライセンスのモデル（OpenCLIP の MIT/Apache 等）へ差し替え**が必要（別タスク）。例外条項では解けない。
- 関連: `LICENSE` / `NOTICE` / `CONTRIBUTING.md` / `MosaicPhotos/Settings/Licenses.swift`・`LicensesView.swift` / README。
- 文脈: 公開にあたり本アプリのライセンスを定め、使用ライブラリ/資産の必要な帰属表示を行いたい。
- 決定: 本体を **AGPL-3.0-or-later** とし、リポジトリ直下に公式全文の `LICENSE`（原文のまま・翻訳しない）を設置。アプリは第三者の Swift ライブラリ依存ゼロ（全ローカルパッケージ）。同梱/使用する第三者資産を **設定 → Licenses**（`LicensesView`＋データ `Licenses.swift`）で一覧表示：本アプリ(AGPL)/同梱(MobileCLIP=Apple・CLIP 語彙/トークナイザ=MIT)/Apple(SDK・SF Symbols)/ビルドツール(coremltools=BSD3・PyTorch=BSD3・open_clip=MIT・ml-mobileclip=Apple・Pillow=HPND・NumPy=BSD3)/ドキュメント(Mermaid=MIT)。MIT/BSD3/HPND は正確なテンプレートで生成、Apple/PyTorch は告知＋upstream リンク。**ライセンス本文は英語原文のまま**、画面の枠・用途説明のみ日本語化。
- 結果: 帰属を満たしつつアプリ内で確認可能。AGPL によりソース公開義務（GitHub 公開で充足）。第三者ライブラリ追加時は `Licenses.swift` に1項追加する運用。
- 関連: `LICENSE` / `MosaicPhotos/Settings/Licenses.swift` / `LicensesView.swift` / `SettingsView.swift`。

## ADR-17 UI 国際化を String Catalog で（base=英語・per-package・日本語追加・アプリ内言語切替）
- 状態: 採用（全 UI パッケージ＋アプリ本体・日英・言語切替つき）
- 追補（全パッケージ化＋言語切替）: `PhotoSourceKit`/`LocalPhotoKit`/`DropboxKit`/`BackupKit` をすべて per-package で日本語化（`PhotosFeatureKit` は UI 文字列なし）。各パッケージは `L(_:)=AppLocale.string(_:bundle:.module)` を使う。**アプリ内言語切替**は `MosaicSupport.AppLocale`（`overrideCode` と `string(_:bundle:)`）＋`AppLanguage`（system/ja/en）で実現：設定 → General の「Language」ピッカー（既定 **System**＝端末が日本語なら日本語・それ以外は英語）。`RootView` が `.environment(\.locale,)` でアプリ本体 `Text` を、`AppLocale.overrideCode` で各パッケージ `L()` を同時に切り替える（端末設定に依らず日英を即切替）。`MosaicPhotosApp.init` で `AppLocale.loadFromDefaults()`。検証: 端末ビルドで `ja.lproj` が **アプリ＋4 パッケージバンドル**すべてに生成、`swift test` 通過。残: Developer/Debug は英語のまま（対象外）、`PhotoLoadState`/エラー文等の動的 String（`Text(変数)`）は英語フォールバック。
- 追補2（可視部分の総ざらい）: アプリ本体で **String 変数として渡るため未訳だった値**（ソース行の「All Photos」「Cloud」「On-Device Photos」やサブタイトル・`navigationTitle(title)`・件数 `%lld photos`(複数形)・「No matches」等）をアプリ用 `L(_:)=AppLocale.string(_:bundle:.main)` で包んで日本語化。設定の**ヘルプ/フッター文**も翻訳（`Text("a" + "b")` の連結フッターは verbatim で未訳になるため単一リテラル化して翻訳）。エラーメッセージは対象外。アプリのカタログは ja 115 件をコンパイル確認。
- 文脈: UI 文字列は元々すべて英語ハードコード（base 言語が揃っている）。国際化したいが UI が多数のパッケージに分割されている。
- 決定: **String Catalog（`.xcstrings`）** を採用、base=英語。**案A（per-package）**：各 UI パッケージに `Localizable.xcstrings` を置き `Package.swift` に `defaultLocalization: "en"` ＋ `resources: [.process("Localizable.xcstrings")]`（**SwiftPM CLI は `.xcstrings` を自動認識しないため明示宣言が必須**＝これが無いと `Bundle.module` 生成されず `swift test` が落ちる）。パッケージ内は `Text("x")` が既定で `Bundle.main` を見る問題を避けるため、`String(localized:bundle:.module)` を包む小ヘルパー **`L(_:)`** で全 API（Text/Label/Button/Section/navigationTitle 等は String を verbatim 表示）を一様にローカライズ。アプリ本体は `Text("x")` リテラルが既定で `Bundle.main` を見るのでコード改変不要、`MosaicPhotos/Localizable.xcstrings` を置き、`project.pbxproj` の `knownRegions` に `ja` 追加。**Developer Options/Debug は対象外（英語のまま）**。翻訳は機械翻訳。キー不一致は英語にフォールバック（安全）。
- 結果: 端末ビルドで `ja.lproj`（アプリ本体＋`PhotoSourceKit_PhotoSourceKit.bundle` 双方）が生成され日本語化を確認。`swift test`（macOS）も通過。残：他 UI パッケージ（LocalPhotoKit/DropboxKit/PhotosFeatureKit/BackupKit）と動的 String（`PhotoLoadState` メッセージ等＝`Text(変数)` は verbatim で未翻訳）の対応は後続バッチ。日付/数値/地名はロケール対応（概ね自動）。
- 関連: `PhotoSourceKit`（`Localization.swift`＝`L()`・`Localizable.xcstrings`・`Package.swift`）/ `MosaicPhotos/Localizable.xcstrings` / `project.pbxproj`（knownRegions）。

## ADR-16 バックグラウンドの「通信」を回線種別でゲート（Wi-Fi 優先・段階設定）
- 状態: 採用
- 文脈: 電源ゲート（[[ADR-15]]）に加え、背景処理のうち**通信を伴うもの**（Dropbox 同期・バックアップ upload・クラウド写真の CLIP 埋め込み＝サムネDL・逆ジオコーディング）をデータ通信量の観点で制御したい。Wi-Fi のときだけにしたい＋何段階か選びたい。ユーザーが**閲覧中に行う取得**（サムネ/フル画像）は前景操作なので止めない。
- 決定: `MosaicSupport.NetworkStateMonitor`（`@MainActor @Observable`・`NWPathMonitor` 内包・電源/メモリ系モニタと同列・UIKit 非依存）を新設。`isReachable`/`isOnWiFi`(wifi||有線)/`isExpensive`/`isConstrained`(低データ) を監視し、ポリシー `BackgroundDataPolicy`（unrestricted / **wifiOnly=既定** / wifiNoLowData / off・キー `NetworkStateMonitor.policyKey`）と合わせ `networkAllowed()` を返す。通信を使う背景処理は **`backgroundAllowed()`（電源）かつ `networkAllowed()`（回線）** で実行。CLIP 埋め込みは**スマート方針**＝回線NG時は `unembeddedRefKeys(limit:localOnly:)` で**クラウド分をスキップしローカルだけ続行**、ローカルが尽きたら終了し、電源/回線の復帰（`onChange`）で `scheduleBackgroundFill()` を再起動してクラウド分を拾い直す。設定は General の `BackgroundSettingsView` に「Background Data」4段階を電源の隣へ。可視化はアクティビティバーに回線チップ（Wi-Fi 緑 / 保留 橙 / Off・圏外 灰）を電源チップの隣に追加（[[ADR-14]]）。
- 結果: 既定でセルラーの背景通信を避けつつ、ローカル AI 処理はセルラーでも進み、Wi-Fi でクラウド分が再開。閲覧時取得は常に行う。トレードオフ: 既定で全ユーザーが「Wi-Fi のみ」に変わる（セルラーでは同期・バックアップ・クラウド埋め込みが保留）。`localOnly` 絞り込みは `refKey.starts(with:"L-")` の SwiftData 述語に依存。
- 関連: `MosaicSupport/NetworkStateMonitor.swift` / `AutoAlbumCore`（`PhotoTagger.embedUnprocessed`・`AutoAlbumStore.unembeddedRefKeys(localOnly:)`・`scheduleBackgroundFill` を public 化）/ `HomeView.swift`（evaluateSync・resumeBackgroundWork・place ループ）/ `BackupKit/BackupEngine.swift` / `MosaicPhotos/Settings/BackgroundSettingsView.swift` / `DropboxKit/DropboxActivityBar.swift`。

## ADR-15 バックグラウンド処理を電源状態でゲート（電池節約）
- 状態: 採用
- 文脈: 継続的なバックグラウンド処理（特に CLIP 背景埋め込み＝ANE/GPU 推論＋クラウドサムネDL、Dropbox 差分同期、定期再生成/場所スキャン、バックアップ）が電池を消費する。電源接続時だけ走らせたい。
- 決定: 横断モニタ `MosaicSupport.PowerStateMonitor`（`@MainActor @Observable` シングルトン・`MemoryPressureMonitor` と同系列）を新設。`UIDevice.batteryState`＋`ProcessInfo.isLowPowerModeEnabled` と各通知を監視し、設定ポリシー（`BackgroundPowerPolicy`: whileCharging / always / off・キー `PowerStateMonitor.policyKey`・**既定 whileCharging**）と合わせて `backgroundAllowed()` を返す。whileCharging は「電源接続(charging/full) かつ 低電力モード OFF」。ゲート対象（スコープ最大）: CLIP 背景埋め込み（`shouldPause` に `!backgroundAllowed` を追加＝電源復帰で自動再開）/ 自動アルバム定期再生成（`refreshIfNeeded` の guard）/ 場所の定期再スキャン（Home のループで guard）/ Dropbox 差分同期（電源変化で `startSync`/`stopSync`）/ バックアップアップロード（ファイル間で一時停止→電源復帰で再開）。設定 UI は**アプリ横断のため設定ルートの General に独立**（`BackgroundSettingsView`「Background & Battery」3択）。当初 Albums 配下に置いたが、機能横断（同期/AI/スキャン）に効くので General へ移設。
- 結果: 電池中は重い背景処理が止まり、電源接続で自動再開。初回起動の一回限りの読み込み・生成・キャッシュ表示は妨げない（継続/定期処理のみゲート）。トレードオフ: 既定で全ユーザーが「充電中のみ」に変わる（クラウド一覧は電池中は更新されない）。手動バックアップも電池中はファイル間で一時停止する（ログに明示）。ポリシー変更は埋め込み/ループには即時、Dropbox 同期は次の電源変化/再起動で反映。電源状態と背景処理の稼働は ADR-14 の[[アクティビティバー]]に可視化（電源チップ＋背景ランプ）。なお `UIDevice.batteryState` はシミュレータや判定不能時に `.unknown` を返すため、`charging/full` のみで判定すると背景処理が全ゲートで止まる（シミュレータでランプがグレーのまま）。**`.unplugged` と確定したとき以外は電源扱い**（`.unknown` は電源扱い）にしてロックを避ける。
- 関連: `MosaicSupport/PowerStateMonitor.swift` / `AutoAlbumCore`（`AutoAlbumEngine+Recognition`・`AutoAlbumEngine.refreshIfNeeded`）/ `HomeView.swift`（evaluateSync・place ループ）/ `BackupKit/BackupEngine.swift` / `MosaicPhotos/Settings/BackgroundSettingsView.swift`。

## ADR-14 Dropbox 通信アクティビティの可視化（スロット LED インジケータ）
- 状態: 採用
- 文脈: Dropbox 通信が「今どれだけ並行して動いているか」が見えず、サムネイル先読みの遅延（事例「ポツポツ」）の切り分けが体感頼みだった。各並列スロットの稼働状況を端末上で直接観察したい。
- 決定: 横断的なライブ計測 `DropboxActivityMonitor`（`@MainActor @Observable` シングルトン・`Diagnostics`/`LogChannel` と同系列・**UIKit 非依存**＝macOS テスト維持）を新設し、各チャンネルが報告する: サムネイル（容量=`maxConcurrentRequests`／稼働スロット=in-flight バッチ本数／先読み待ち枚数）は `DropboxThumbnailBatcher`、同期は `DropboxPhotoStore.syncState` の `didSet`、フル画像 DL は `fullImage`/`originalImageData` を begin/end で計測、バックアップ upload は `BackupEngine.phase` の `didSet`。UI は `DropboxKit.DropboxActivityBar`（**形＝チャンネル／色＝状態（灰=待機・青=通信・緑=監視・赤=失敗）／数・塗り＝強度**）で、サムネイルは同時実行スロットをレーン（ピップ）表示。表示は **Dropbox 通常設定（`DropboxSettingsView`）の「Show activity bar」トグル（既定 ON・キー `DropboxActivitySettingsKeys.showBar`）** で制御し（デバッグ機能ではなく通常機能と位置づけ）、`View.dropboxActivityBar()` を `HomeView`/`SourceHostView` 最上部へ重ねる（`allowsHitTesting(false)` で素通し）。
- 結果: スロットの稼働本数・先読みキュー深さ・同期/DL/upload の同時状況が一目で分かり、並列化や並列数設定（`thumbnailConcurrency`）の効果を実機で確認できる。報告は MainActor 上の Int/enum 代入のみで軽量（表示 OFF でも常時更新）。fullScreenCover は別ビューツリーのため最上部表示は各ホストへ個別適用が必要。
- 追補（バー統合・電源/背景処理の可視化）: 同じ 1 行のバーに **電源ゲート（`PowerStateMonitor`）** と **バックグラウンド処理（`BackgroundActivityMonitor`）** を統合した。左端に電源チップ（稼働可=緑⚡／電池待ち=橙／Off=灰）、Dropbox クラスタの右に背景ランプ（AI 埋め込み＝残り枚数併記・自動アルバム生成・場所走査・アルバム走査、稼働中はパルス）。背景の状態は `AutoAlbumEngine` の `isTagging`/`isGenerating`/`isGeneratingPath` の `didSet` と埋め込み進捗コールバック、`PlaceScanner`/`LocalAlbumScanner` の `isScanning`（app 層で橋渡し）が `BackgroundActivityMonitor`（MosaicSupport・@Observable）へ報告する。これで [[ADR-15]] の電源ゲートにより「なぜ背景処理が止まっているか（電池待ち）」も一目で分かる。`DropboxKit` に `MosaicSupport` 依存を追加。
- 関連: `Support/DropboxActivityMonitor.swift` / `MosaicSupport/BackgroundActivityMonitor.swift` / `MosaicSupport/PowerStateMonitor.swift` / `Store/DropboxThumbnailBatcher.swift` / `Store/DropboxPhotoStore.swift` / `BackupKit/BackupEngine.swift` / `DropboxKit/DropboxActivityBar.swift`、事例「ポツポツ」（[case-studies]）。

## ADR-13 フォルダ名アルバムに日付抽出を追加（名前＋年でグループ）
- 状態: 採用
- 文脈: Dropbox フォルダ名アルバムは名前しか取り出せず、日付（多様な表記）を活かせていなかった。クラウド写真は EXIF 日付を欠くことも多い。
- 決定: `FolderDateParser`（純・テスト）で多様な表記を粒度つき期間 [start,end] に正規化（ISO/圧縮/和暦/月名/範囲、曖昧な数値日付は端末ロケールで解決、12超は日と確定）。`PathAlbumStrategy` はパスのフォルダ部から日付を取り、**名前＋年でグループ（年違いは別アルバム）**、アルバム期間にフォルダ日付を採用（無ければ EXIF）。表示は「名前 (年)」（名前に年が含まれれば付けない＝**名前から日付は除去しない**）。
- 結果: 「Hawaii (2023)」「Hawaii (2024)」のように年で分かれ、EXIF 日付が無くても期間が付き、日付ソート/AI 日付検索に効く。曖昧日付はロケール依存（プレビュー前提）。範囲のハイフン区切り等は今後拡張余地。
- 関連: `Strategies/FolderDateParser.swift` / `PathAlbumStrategy.swift`。

## ADR-12 AI アルバム検索を合成可能な QuerySpec（OR/NOT/多ファセット）へ拡張
- 状態: 採用（フラットな `AIAlbumQuery` の AND 専用を一般化。`AIAlbumQuery` は後方互換で残す）
- 文脈: 「ここ2年の子供」「京都か奈良の家族のお気に入り、スクショ除く」等、アプリが持つ多様な情報（日付/場所/人物/人数/向き/位置/ソース/内容）への複雑条件・OR・NOT を柔軟に組みたい。
- 決定: DNF（節の OR・節内 AND・各条件 NOT 可）の `QuerySpec`/`QueryClause`/`Condition` を新設。ハード条件（日付/場所/人物/人数/ソース/お気に入り/スクショ/向き/位置）は `QueryEvaluator` で評価、内容語(content)は CLIP でソフト採点（`AIAlbumSearcher.search(baseLite:spec:)`）。相対日付は `RelativeDateParser`（日英）で FM 非対応端末でも解釈。Foundation Models は `GeneratedSpec`（Generable・clauses=OR）で出力、RuleBased はフラット解釈を単一節へ橋渡し。`AIAlbumService` は `interpretSpec` 経由に配線。
- 結果: 相対日付・複数ファセット・**OR を再投入**。回帰（全滅）対策を二重化: (1) FM スキーマから人数(peopleAtLeast)・位置(hasLocation)を**廃止**し、人物の有無・概念は内容(content=ソフト)で扱う／日付は妥当範囲のみ採用（`sanitizedDate`）。(2) `AIAlbumSearcher` に**安全網**＝ハード条件で base が全滅しても意味の意図があれば内容のみへ緩和（ただし緩和時にヒット0なら空を返し全件は出さない）。これで「データで満たせないハード条件」での全滅を防ぐ。除外内容(not(content))の減点・多ソース日付（旅行アルバム由来）は後続。
- 関連: `AIAlbum/QuerySpec.swift` / `QueryEvaluator.swift` / `RelativeDateParser.swift` / `AIAlbumSearch.swift` / `FoundationModelsQueryUnderstanding.swift` / `AIAlbumService.swift`。

## ADR-11 CLIP 画像エンコーダを fp16 のみにする（実機 ANE 優先）
- 状態: 採用（旧「fp32（シミュレータ NaN 回避）」を置換）
- 文脈: 実機ログで画像埋め込みが 1 枚 0.5〜1.3 秒と遅い。原因は画像エンコーダを `compute_precision=FLOAT32` で変換していたため、Neural Engine(ANE) に載らず GPU/CPU フォールバックしていたこと。元の fp32 はシミュレータの NaN 回避が目的だったが、シミュレータ最適化は本末転倒。
- 決定: `convert_mobileclip.py` の画像エンコーダ変換を `FLOAT16` にする（実機 ANE 対応＝高速化）。fp16 はシミュレータで NaN 化し得るが、ランタイムの有限性チェックが nil に落として安全に無効化する。`ImageRecognitionTests` の画像タワー依存テストはシミュレータでスキップし実機検証に寄せる。
- 結果: 実機の埋め込みが ANE 実行で高速化する見込み（要・実機実測）。シミュレータでは画像 CLIP が無効化され得る（テキスト系は不変）。モデルは `build_mobileclip.sh` の再実行で再生成が必要。
- 関連: `scripts/convert_mobileclip.py`、`MobileCLIPRuntime`（simulator `.cpuOnly` / 実機 `.all`）、事例「CLIP 埋め込みが遅い」。

## ADR-10 GitHub を CI・公開・リリースに活用
- 状態: 採用
- 文脈: 個人開発でも回帰検知・設計資料の公開・タグ運用の自動化が欲しい。秘密の誤コミットも機械的に防ぎたい。
- 決定: GitHub Actions で CI（`scripts/test.sh fast` を gate、iOS sim は best-effort）/ Pages で `docs/architecture-note` を公開 / タグ push で Release 自動生成。リポジトリ設定で Secret scanning + Push Protection を有効化。CodeQL は **default setup の autobuild が Xcode+SPM 構成で失敗**するため、`build-mode: manual` の advanced ワークフロー（CI と同じ `xcodebuild -sdk iphonesimulator CODE_SIGNING_ALLOWED=NO`）に切替（`.github/workflows/codeql.yml`）。なお `DropboxSecrets.swift`（appKey・.gitignore 対象）が CI に無くアプリビルドが失敗するため、ワークフローで**ダミー DropboxSecrets を生成**してからビルドする（静的解析に実キーは不要）。
- 結果: push ごとに回帰検知、設計資料が公開URL化、リリースノート自動化、秘密混入の自動ブロック。ワークフロー push にはトークンの `workflow` スコープが必要。iOS 26 シミュレータはランナー事情に依存するため iOS テストは非ブロッキング。
- 関連: `.github/workflows/ci.yml` / `pages.yml` / `release.yml`。

## ADR-9 Diagnostics で端末上ログ
- 状態: 採用
- 文脈: 実機で Mac/Console なしに不具合を追えない。
- 決定: 未捕捉例外・メモリ圧迫・各ログを `Caches/diagnostics.log` に残し、Developer Options で閲覧/共有。
- 結果: 実機の原因追跡が可能に。`fatalError`/SwiftData trap は対象外（標準クラッシュログ側）。
- 関連: コミット cfc8223 前後、`MosaicSupport/Diagnostics.swift`。
