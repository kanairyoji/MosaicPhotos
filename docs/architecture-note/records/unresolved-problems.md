...
## 既存の同期カーソルは `include_media_info` 無しで作られている（撮影日・撮影地の取りこぼし）

- 箇所: `Packages/DropboxCore/Sources/DropboxCore/Sync/DropboxSyncEngine.swift`（`getLatestCursor`）
- 分類: dataQuality / 優先度 **P1**（初出 2026-09-20・レビューループ 16 周目）
- **コード側は 16 周目で直した**（カーソルを作る呼び出しにも `include_media_info` を付けた）。
  ここに残すのは**既に配られたカーソルの救済**が未解決だから。
- 何が起きていたか: `list_folder/continue` は「元の呼び出しの設定を引き継ぐ」が、その
  「元の呼び出し」は**カーソルを作った呼び出し**。初回スキャンの `list_folder`
  （`include_media_info=true`）が作るカーソルは捨てられ、生き残るのは
  `get_latest_cursor` が返したカーソルだけだった。そこに指定が無かったので、
  **最初の同期のあとに増えた・触られた写真すべて**で `media_info` が来ない。
  - 撮影日が `time_taken` でなく `client_modified`（≒アップロード時刻）になる
  - 撮影地（緯度経度）が付かない＝場所別に出てこない
  - さらに悪いことに、初回スキャンで正しく付いた撮影日も、その写真を Dropbox 側で
    移動・改名すると差分で上書きされて**失われる**（緯度経度は `if item.latitude != nil`
    で守られているが、撮影日は無条件に書かれる）
- 位置づけ: 「クラウドの写真が時系列に並ばない」に対して ADR-128（台帳で上書き）と
  ADR-199（共有は解析データで渡す）を入れてきたが、**どちらも症状側の手当て**だった。
  これが根。ただし台帳の無いクラウド写真（他端末のカメラアップロード・Dropbox 側での追加）
  には手当てが届いていなかったので、実害はそこに出ていたはず。
- 解決と言える条件: 既存の利用者の手元でも、撮影日と撮影地が `media_info` から付くようになる。
- 検討した方向:
  - (a) 版を上げ、初回同期をやり直す（`initialSyncCompletedAt` を nil に戻す）。
    正しいカーソルが作り直され、**既に狂っている撮影日・欠けている撮影地もまとめて直る**。
    代償は全件の再スキャン 1 回（進捗 UI のある経路なので利用者には見える）。
  - (b) 古いカーソルで差分を汲み切ってから、新しいカーソルを取り直す。再スキャンは避けられるが、
    既に狂っている撮影日は直らない。
  - (c) 何もしない（新規インストールだけ正しくなる）。
  ADR-199 で同じ形（受信能力の版を上げて 1 回だけ取り直す）を入れているので、
  (a) はその前例に沿う。**全件再スキャンの代償をどう見るかが判断のしどころ。**
