...
## 非ブロッキングの CI ジョブが赤くても、誰も気づかない

- 箇所: `.github/workflows/ci.yml` の `ios` ジョブ（`continue-on-error: true`）。
- 分類: 運用 / 優先度 P2（初出 2026-09-26）
- 症状: `ios` が failure でも **run 全体は success** になるので、`gh run list` でも
  GitHub のバッジでも緑に見える。実際に **09-23 から 3 日間**赤いまま気づかなかった
  （`case-studies.md`「CI が 3 日間赤かったのに気づけなかった」）。
- なぜ非ブロッキングなのか: ランナーの iOS SDK / シミュレータ事情で落ちることがあり、
  それでワークフロー全体を止めたくない（コミット `cc68180` 前後の判断）。**この判断自体は妥当**。
- 選択肢:
  1. **ブロッキングに戻す**。シミュレータ選択を UDID 化・暖機・リトライまで入れた今なら
     安定しているかもしれない（直近の失敗はランナー事情ではなくテストのレースだった）。
  2. **失敗を知らせる**。`ios` が失敗したら通知（Issue を立てる / Slack）。緑のままでよいが
     気づける。
  3. **2 回続けて落ちたらブロッキング扱い**にする（一時的なランナー事情と本物の回帰を分ける）。
  4. 入れない。ジョブ単位で見る運用（`gh api .../jobs`）を覚えておく。
...
- 症状: `PerfTrace.takeCounts()` のカウンタはプロセス全体で共有される。`CloudCaptureDateFillTests` の
  規模テストが、単体では通るのに全体実行でだけ「読み出し 48 回・170 回」と数えて落ちた。
- 対処（済）: 新しいテストは `FaceStore.fetchCountForTesting`（ストアごとの回数）で数える形にした。
- **残りも対処した（2026-09-23）**: 「今は通っているが並びしだいで落ち得る」と書いた翌日に実際に落ちた
  ——`scripts/test.sh fast` の全体実行で `ScaleRegressionTests` の 2 本が 7→9 回・32→62 回と数えた
...
利用者の指示で **ADR-202**（止めてから知らせる）を実装した。照合が実体の無いオフロード済み
写真を見つけたら、自動オフロードを 0 へ落とし、一覧の上に赤い帯を出し、設定画面で
「確認した」を押すまで自動オフロードを設定させない。確認しても自動オフロードは自動では戻さない。

⚠️ 実機で確かめること: `device-verification.md` の **Z5**（誤発動しないこと・帯が出ること）。
...
- **一部だけ片付いた（2026-09-24・常駐メモリの棚卸し）**: 消費側が
  `index.compactMapValues(\.localIdentifier)` と書いていたぶん（＝**撮影日を落とすためだけに
  索引を丸ごと複製する**・最大 7 万件で 12MB）を消した。`BackupCopyHiding.hiddenPaths` が
  `[String: BackupCopyInfo]` を直接受けるようにしたので、複製は要らない。
  ⚠️ **本体は未着手**——「呼ばれるたびに `backupCopyRecords()` の全件 fetch ＋
  受信撮影日の JSON 全デコードをやり直す」という、この項目の主題はそのまま。
  選択肢 1〜4 の判断はまだ要る。
...
- 症状:
  ...
  2. **消したはずの表示が出る**: `submitContinuedTask()` は `BGTaskScheduler.submit` の
     ...
     ロック画面と Dynamic Island のインジケータが出る（**推定・実機未確認**）。
     ⚠️ ただし前面のみモードではアプリを離れた時点でセッションが止まるので、
     漏れるのは**前面に居る間だけ**（18 周目の監査で範囲を訂正）。
...
## （解決済み）再クラスタ後の「重心へ寄与したか」の記録が実際の計算と食い違う — 2026-09-21 に対処（ADR-210）

- 対処: 「寄与したか」を**事実の記録**とし、実際に `sum` へ足した当人だけが書く形にした
  （`FaceClustering.place` が `contributed` を返し、`FaceSeedBuilder` が足した faceID を返す）。
  再クラスタの材料の選び方は最初の割り当てと同じ「品質フロア以上か」だけで決める＝自己参照を断った。
  併せて `isUserClaimed` に**確認顔**を含め（インスタンス版・最後の 1 顔の経路でだけ引く）、
  整合検査（`FaceCentroidAudit`）を再クラスタの前に挟んだ。
- 実害の確認: 回帰テストで、直す前は低品質の顔を 8 枚外した時点で `count` が **6 → 0** になった。
- 以下は当時の記録。

### （当時の記録）再クラスタ後の「重心へ寄与したか」の記録が実際の計算と食い違う
...
## 代表写真の指定だけは、再スキャンを跨げない

- 箇所: `Packages/FaceCore/Sources/FaceCore/Faces/CarriedAssertion.swift`
  （荷物に `coverFaceID` / 代表写真が無い）
- 分類: dataLoss / 優先度 P3（初出 2026-09-26・ADR-232 を書いている途中で見つけた）
- 症状: 無名の人物に**代表写真だけ**を選んでおくと、パイプライン版を上げた再スキャン・
  手動の再スキャンでその指定が消える（自動選択に戻る）。
- なぜ漏れたか: `FaceStore.isUserClaimed` は `coverFaceID` を「ユーザーの表明」に数えており、
  再クラスタでも `FaceSeedBuilder` がアンカーとして守っている。**守られているのは再クラスタだけ**で、
  控え（`CarriedAssertion`）の `isEmpty` は名前・束ね・グループ所属しか見ていない。
- 直し方の候補: `coverFaceID` そのものは再スキャンで消える（faceID は写真＋矩形から決まる）ので、
  **代表写真の refKey** を荷物に足し、戻すときはその写真の顔から代表を選び直す。
  ⚠️ 1 枚に複数の顔が写っているとどれを選ぶかが決まらない（`bestCoverFace` に委ねるか、
  矩形も一緒に控えるか）ので、そこを決める必要がある。
- なぜ P3 か: 失うのは「どの写真をアバターにするか」だけで、人物・名前・グループは残る。
  ただし**利用者が手で選んだもの**ではあるので、ADR-130/132 の原則からは守る側。

## （解決済み）再クラスタで無名クラスタの ID が毎晩変わり、ピープルグループが黙って欠ける — 2026-09-26 に対処（ADR-231）

⚠️ `FaceSeedBuilder.ClusterRef.inPeopleGroup` を種の条件に足した（**無名でもグループの
メンバーなら種にする**＝ADR-132 の適用範囲を広げる判断）。あわせて `PeopleGroupInfo.resolve`
に `unresolvedClusterIDs` を持たせ、落としたメンバーを診断ログへ出す（**無音をやめた**）。
「素直な修正は ADR-132 の範囲を広げることになる」という懸念はそのとおりで、
**広げるのが正しい**と判断した——グループに入れる行為そのものがユーザーの表明で、
名前を付けるのと同じ重み。以下は当時の記述。
...
## （解決済み）パイプライン版を上げると束ねとピープルグループが失われる — 2026-09-26 に対処（ADR-232）

⚠️ 持ち越しの荷物を `CarriedAssertion`（名前・束ね・グループ所属）にし、照合は名前と同じ
「写真の重なり」1 本にした。「束ねだけ別の持ち越し方にするか」という選択は、**同じ照合で足りる**
と判断した（束ねも所属も、結局は「どの新クラスタが旧人物か」が決まれば復元できる）。
あわせて `reset()` がグループのメンバーを空にし（意味を失った ID で別人が居座るのを防ぐ）、
メンバーの書き戻しは**足すだけ**にした（数晩に分かれるため）。以下は当時の記述。
...
## （解決済み）モデル世代の切り替えが `@Model` をアクターの外で読む — 2026-09-26 に対処（ADR-232）

⚠️ **副産物として消えた**。`promoteShadow` が `@Model` を外から読んでいたのは、
「名前でグループを組み直す」ために新旧の `allClusters()` を `reduce` していた 2 行だった。
照合を写真の重なり 1 本にしたので、この経路自体が無くなった
（`PeopleEngine` 全体で `allClusters()` の呼び出しが 0 になった）。以下は当時の記述。
...
- 直し方の候補: `assertedClusterEntries()`（既にある射影）と同型の Sendable 射影を
  ...
  ⚠️ **2026-09-26: この直し方は要らなくなった**（ADR-232）。`promoteShadow` の
  「名前でグループを組み直す」`reduce` 2 本が撤去され（照合は写真の重なり 1 本）、
  `PeopleEngine` から `allClusters()` の呼び出しが無くなった。

## （解決済み）クラウド一覧の反映が古いまま固定され、何も直しに来ない — 2026-09-26 に対処（ADR-230）

⚠️ 反映タスクの結果に「**反映できた版**」を添え、合流してよいかを
`canJoinReflect(reflected:wanted:)`（純ロジック）で決めるようにした
——「版を持たせる／目印に版を添える」のうち**前者**を採った（結果に載せれば、目印の消え方と
無関係に正しく判定できる）。同期ルート変更の枝には世代（`loadGeneration`）の前進を足した。
`clearAll` が `itemsRevision` を進めない件は 09-23 の ADR-222〜225 で既に対処済みだった
（この項の記述が古かった）。以下は当時の記述。
...
- ⚠️ **2026-09-21: 症状 1 は実機で確認された**（diagnostics-82）。受信側で共有フォルダが
  消えたあと、そのルートのキャッシュ 8,513 行が「開けない写真」として残り続けた。
  ルートごと消えた場合は ADR-205 で、ルートが生きたまま削除通知を取りこぼした場合は
  ADR-206 の週次の見直し（最長 7 日で回復）で直るようになった。
  **この項に残るのは症状 2（保存先フォルダを変えたときに古いフォルダの写真が出たまま）と、
  反映の合流が古いスナップショットを返す件**。
- 症状（1 は実機確認済み・2 は推定）:
...
- なぜ設計判断が要るか: 合流をやめれば（毎回やり直す）正しいが、
  反映は初回同期中 5 秒ごと・ポーリング中 0.4 秒ごとに走るので、6.8 万件の取り出しが重なる。
  「合流してよいのは、合流先が自分より新しい版を読んでいるときだけ」という条件を
  どう表現するか（版を持たせる／走行中の目印に版を添える）を決める必要がある。
  ⚠️ **この判断はレビュー 12〜14 周目と同じ形**（「誰が最後に書いたか」で決める）。
  同じ道具立てで解けるはず。

## ADR 番号が 4 組ダブっている（別々の判断に同じ番号）

- 箇所: `docs/architecture-note/records/decisions.md`
- 分類: recordsIntegrity / 優先度 P2（初出 2026-09-21・レビュー 4 周目）
- 症状: **同じ番号が 2 つの別々の判断に付いている**。番号で参照している側（コメント・
  CLAUDE.md・他の記録）は、どちらを指しているか読めない。

  | 番号 | 一方 | もう一方 | 番号での参照 |
  |---|---|---|---|
  | ADR-50 | 認識・検索パラメーターのチューニング | UI 操作性の統一 | 1 箇所 |
  | ADR-68 | 地名の高精度化（CLGeocoder 背景補正） | **ピープルの過分割対策** | 85 箇所 |
  | ADR-69 | 重い処理のゲートをローカル／クラウドに分離 | **クラスタ事後監査** | 24 箇所 |
  | ADR-70 | オンデバイス AI モデルの常駐制御 | **顔モデル換装（AuraFace）** | 52 箇所 |

- 分かっていること: 68/69/70 の参照を抜き取って見たところ、**どれも太字の側（顔まわり）を
  指していた**（`HomeView` のまとめて確認、`FaceAccuracyEvalTests` の事後監査、
  CLAUDE.md の AuraFace 換装）。もう一方は番号で参照されている形跡が薄い。
- なぜ設計判断が要るか: 直し方は「どちらかに新しい番号（207〜）を振り直す」だが、
  **どちらを動かすか**と**参照をどこまで追うか**を決める必要がある。
  - (a) 顔まわりでない方（地名・ゲート・常駐制御・UI 操作性）を振り直す。
    参照が薄いので書き換えは少ない。ただし ADR は「決めた順の通し番号」なので、
    古い方を残して新しい方を動かすのが筋という見方もある。
  - (b) 後から採番した方（ファイル上で行番号が小さい＝新しい方）を機械的に振り直す。
    規則は明快だが、(a) と結論が一致するとは限らない。
  - (c) 番号は動かさず、両方の見出しに枝番（ADR-68a / ADR-68b）を振る。
    既存の参照が一切壊れない代わりに、以後「どちらの 68 か」を常に書く必要が残る。
  - いずれの場合も、**撤回・変更時は項を消さない**規則（このファイル冒頭）に従い、
    動かした側に「状態: 採番し直し（旧 ADR-N）」を残す。
- 再発防止: 採番のたびに `grep -o "^## ADR-[0-9]*" … | sort | uniq -d` が空であることを
  見る（このレビューで見つけた方法）。

## 追加ルートが死んでも、利用者には何も出ない

- 箇所: `Packages/DropboxCore/Sources/DropboxCore/Sync/DropboxSyncEngine.swift`
  （`reportState(_:isPrimary:)` は**プライマリルートのぶんしか画面へ出さない**）
- 分類: silentFailure / 優先度 P2（初出 2026-09-21・ADR-205 のレビュー 2 周目）
- 症状: 家族フォルダ（受信側）やバックアップフォルダが Dropbox から消えても、
  画面には何も出ない。ADR-205 で「ルートが消えたらキャッシュを落として止まる」ように
  したが、その報告は `isPrimary` のルートにしか出ない。実機の diagnostics-82 が
  まさにこの形で、利用者は**写真が開けないこと**で初めて気づいた（18 日後）。
- なぜ設計判断が要るか: 「どこに出すか」を決める必要がある。候補は 3 つ。
  - (a) 共有の設定画面に「このフォルダが見つかりません」を出す（設定を開かないと気づけない）。
  - (b) オフロードの緊急停止（ADR-202）と同じ、一覧の上の帯（強いが、
    「相手が共有をやめた」だけでも出るので頻度が読めない）。
  - (c) 何も出さず、診断ログだけ（いまの状態）。
  頻度が読めないので、まず実機で (c) のログを見てから決めたい。
- 関連: ADR-205 / 事例「共有アルバムの写真をタップすると『ファイルがありません』」。

## （解決済み）クラウドのキャッシュに、定期的な照合が無い — 2026-09-21 に対処（ADR-206）

⚠️ 週 1 回、`initialSyncCompletedAt` が古くなったら初回同期をやり直して掃除する
（`CloudReconcilePolicy` / `DropboxSyncEngine.shouldReconcileNow`）。前面では走らせない。
以下は当時の記述。

- 箇所: `Packages/DropboxCore/Sources/DropboxCore/Sync/DropboxSyncEngine.swift`
  （一覧の取り直しと突合は `initialSync` の中だけ・`cachedPaths(withPrefix:)` − 取得済み）
- 分類: staleUI / 優先度 P1（初出 2026-09-21・diagnostics-82 の調査から）
- 症状: 差分（longpoll → `continue`）で「消えた」通知を**一度でも取りこぼす**と、
  その行はキャッシュに永久に残る。一覧には出るのに開けない写真になる
  （実機で確認＝「ファイルがありません」）。バックアップには週 1 回の照合があるが、
  **クラウドのキャッシュには同等のものが無い**。
- 取りこぼしが起きる道: カーソル失効（ADR-203 で手当て）・ルート消失（ADR-205 で手当て）・
  差分の適用中にプロセスが死ぬ・`applyDelta` の保存失敗。**前 2 つは塞いだが、
  残りは塞げていない**し、既に入ってしまった行を掃除する道が無い。
- なぜ設計判断が要るか: 照合は**全件の再帰一覧**なので、6.8 万枚では通信も時間もかかる。
  決めることは 3 つ。
  1. **頻度**（週 1 回／初回同期から N 日／手動のみ）。バックアップの照合と同じ枠に乗せるか。
  2. **走らせる条件**（`BackgroundYield` のどの `HeavyWork` か＝`cloudMonolith` 相当か）。
  3. **ルート単位か全体か**。ルートごとに `list_folder` を 1 本張れば差分は要らないが、
     ルートが多いと往復が増える。
- 直し方の候補: `initialSync` の Step 4（一覧 − 取得済み → 掃除）を切り出して
  `reconcile(root:)` にし、`markInitialSyncCompleted` と同じ場所に「最後に照合した日」を
  持たせて期限が来たら呼ぶ。掃除の判断は**一覧が全部取れた回だけ**に限る
  （途中で失敗した回に掃除すると、生きている写真を消す）。
- 関連: ADR-203 / ADR-205 / 事例「共有アルバムの写真をタップすると『ファイルがありません』」。
...
- 症状: スクラブ中に、画面外へ流れた写真のサムネを取りに行く。回線ポリシーは先読みの
  **入口では判定している**ので、Wi-Fi で積んだあとセルラーへ切り替わったぶんだけが
  セルラーで取られる（⚠️ 当初「セルラーでも取りに行く」と書いたが入口の判定を見落としていた・
  18 周目の監査で訂正）。
...
## （解決済み）オフロードの印と撮影日時の 4 件 — 2026-09-20 に対処

利用者の判断を受けて 4 件を片付けた。経緯は消さずにここへ残す（`decisions.md` の
**ADR-200 / ADR-201**、`case-studies.md` の該当項が正本）。

| 元の項目 | 判断 | 対処 |
|---|---|---|
| オフロードの印が誰も読まない場所へ書かれている【P0】 | 「オフロードは未使用。後方互換性を壊して、あるべき設計にしてよい」 | ADR-200。`BackupMetadataStore` を唯一の書き手にし、ルートを受け取る形へ。誤った場所の印の**救済は作らない**（未使用のため） |
| メタデータの控えが、背景アップロードの書き込みごと消される | 同上（同時に直す必要があった） | ADR-200。消費口を `takeAll` 1 つにし、追記と同じ錠の下へ |
| 控えの送信が二重になり、送りっぱなしで溜まり続ける | 同上 | ADR-200。同じく `takeAll` で解消（「送る写真が 0 枚」の回も空になる） |
| クラウド写真の撮影日は一覧からは取れない（API 仕様） | 「遅延取得したい」 | ADR-201。1 枚ずつ `get_metadata`・無かったことも記録・一覧の日付で上書きしない |

⚠️ **残っている後片付け**: 過去の試用で `<backupRoot>/<年>/<年-月>/.mosaic/` に
空のフォルダが作られている可能性がある。読まれないだけで害は無いので、
気になれば Dropbox 上で手で消してよい（アプリは作り直さない）。
...
**直す場所**（2026-09-22 の整理で絞れた）: 規則は `AIAlbumIncremental`（純ロジック・テストあり）、
経路は `AIAlbumService` の `incrementalHardOnly`（3 の「根拠ゲートと審査を飛ばす」枝）と
`incrementalSemantic`（2 のタグ除外・4 の上限はここに足す）。今の挙動は
`AIAlbumIncrementalTests` が固定しているので、直すときは期待値を変えるテストから書く。
...
1. **（解決済み・2026-09-21／ADR-206）週次の照合（ADR-166）がほぼ永久に走らない**
   - ⚠️ 期限の週だけ照合をバックアップの前へ出した（`NightlyPlan.Inputs.backupReconcileDue`）。
     併せて `missingOffloadedPaths` に `listedAt` 保護を足し、失敗した回は 1 日で再挑戦する。
     以下は当時の記述。
   ...
   - 同じ窓の 3 手前で `.startBackup` がエンジンを busy にする（`phase` は同期で立つ）ので、
     `.reconcileBackup` は空振りし `lastReconcileAt` も動かない
     （⚠️ 当初「必ず」と書いたが、積み残しが無くバックアップが早く終わった回は走り得る）。
...
2. **`expirationHandler` の設定が MainActor 1 ホップ遅れる**
   - 箇所: `HeavyWorkScheduler.register()`（登録のクロージャ）と `handle(_:)`（設定）
   - 背景起動では、その隙間に何が MainActor を握るかは**実機でしか分からない**。
     そこで窓が切れると誰も止めず、OS への完了通知も遅れる（推定）。
     ⚠️ 当初「その間に `HomeStores.build()`（20 以上のストア構築）が走る」と書いたが**誤り**
     ——期限切れの受け口は本体より**前**に設定されるので `build()` は隙間の原因ではなく、
     `HomeStores` の保持プロパティも 14 個。18 周目の監査で訂正。
...
5. **`background-behavior.md` と実装が食い違う行が 3 つ**
   ...
   - 併せて `gatherInputs` が回線モニタを直読みしており（ADR-196 の「モニタを直読みしない」に反する）、
     `requiresExternalPower` が電源ポリシー `.off` と `.always` で同じ（最も緩い）要求になる
     ——背景処理を切った人にも、常に許可した人と同じく「電源不要の窓」を要求しに行く。
...
- 症状: ADR-163 の「解析を起こしてから顔の残数を測る」修正（`analysisStep` / `remainingSteps` の分割）は、
  ...
  `store.scannedRefKeys()`（走査済み写真の全件 fetch）などの後。
  ⚠️ 当初ここに「7.5 万行」と書いていたが、その数字は**候補の件数**（diagnostics-45）の
  流用で、`ScannedPhoto` の行数を測ったものではない（18 周目の監査で訂正）。`gatherInputs` は `kick` の直後に走るので、
  新しい窓では常に 0 のまま。
...
## パッケージのテストが signal 11 で落ちることがある（再現せず・2026-09-18 / 09-20 / 09-21）
...
- ⚠️ **2026-09-21: `AutoAlbumCorePackageTests` でも同じ形が出た**（241 件の単体再実行も、
  直後の全体実行も通過）。**パッケージ固有ではない**——インメモリ `BackupStore` の
  並行生成という推定は、少なくとも唯一の原因ではない。`swiftpm-testing-helper` か
  並列実行の側を疑う方が筋が良さそう。次に出たら、落ちたパッケージと直前に足した
  テストを必ずここへ書き足すこと（3 点そろえば形が見える）。
- 再発: 2026-09-20、`ReconcileScenarioTests` を追加した直後の 1 回。**同じ形**
  （インメモリ `BackupStore` を作るテストが並行）。直後に同じフィルタで 3 回続けて走らせたが
  0 件＝再現せず。2 度目の観測なので、原因の推定（コンテナ生成の並行）は補強された。
...
## （解決済み）顔の再クラスタで「種を作れなかった人物」の顔が孤児になる — 2026-09-21 に対処（ADR-210）

- 対処: 種を作れなかったクラスタでは、そこへ入れた `pinned` も取り消すようにした
  （そのメンバーは通常の再割り当てへ回る）。孤児を作ってから `repairOrphanFaces` で直す形をやめた。
- 以下は当時の記録。

### （当時の記録）顔の再クラスタで「種を作れなかった人物」の顔が孤児になる（2026-09-19・ADR-198 で発見）
