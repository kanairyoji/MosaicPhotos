...
## オフロード済みの写真の実体が消えても、利用者に伝わらない

- 箇所: `Packages/BackupKit/Sources/BackupKit/BackupStore.swift`（`reconcile`）と
  `BackupEngine.reconcileWithDropbox`。台帳は `BackupAssetRecord` と `OffloadRecord` の 2 本立て
- 分類: dataLoss（告知なし）/ 優先度 P2（初出 2026-09-20・偽 Dropbox でシナリオを通して判明）
- 症状: オフロード済みの写真（端末の原本は削除済み＝**クラウドが唯一のコピー**）を、
  利用者が Dropbox の Web や他端末から消すと、その写真は**どこにも無くなる**。
  ところがアプリは何も言わない——週次の照合は `BackupAssetRecord` を外すだけで、
  **オフロード台帳（`OffloadRecord`）には触らない**。台帳が残るのでアルバムには出続け、
  開いたときに取得へ失敗する。「なぜか表示できない写真」として現れる。
  ⚠️ 現在の挙動は `ReconcileScenarioTests` で**固定済み**（変えるときは意図して書き換える）。
- なぜ設計判断が要るか: 素直な案は「照合でオフロード台帳も外す」だが、それは
  **アルバムから黙って消える**ことを意味する。利用者から見れば「アプリが写真を消した」に
  等しい。逆に残し続けると、壊れた写真が一覧に並ぶ。どちらも告知とセットでないと成立しない。
- 選択肢:
  1. **知らせる**。照合で「実体が消えたオフロード済みの写真が N 枚」を検出し、設定画面か
     診断ログに出す。台帳はそのまま残す（利用者が判断できる）。
  2. **印を付ける**。`OffloadRecord` に「実体が見つからない」印を足し、一覧では灰色や
     バッジで示す。消しはしない。
  3. **外す**。照合で台帳から外す。静かに消えるので、1 とセットでないと採れない。
  4. **何もしない**（現状）。オフロードを使い始める前なら実害は無い。
- 補足: オフロードを実際に使い始めるなら、**1 か 2 は入れたほうがよい**。
  「唯一のコピーを消すと何が起きるか」を利用者が知らないまま運用することになるため。
