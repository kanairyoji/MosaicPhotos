...
- 残課題: **12 時間 窓が来なかった**理由は未解明（充電中だったので電源条件ではない）。
  footprint が窓の中で 700MB 超に達しており、jetsam か DAS のレート制限が疑わしい。
  次のログでは `bgtask: submitted` の後に窓が来るか、Console の `dasd` で
  `FinalDecision: Must Not Proceed` の根拠を見る（`deep-dive/background-window.html` の手順）。
  メモリ削減（窓の中でバックアップ・共有・顔の再クラスタが重なる）も別途検討する。
