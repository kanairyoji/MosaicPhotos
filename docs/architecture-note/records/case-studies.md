...
## 症状を 2 回手当てして、根は 16 周目に出た

レビュー 16 周目。この作業の出発点は「共有フォルダが撮影時間順でない」という報告だった。
そこから ADR-199（共有の解析データに撮影日を載せる）を入れ、9 周かけて磨いた。
**根は別のところにあった。**

- 原因: Dropbox の差分取得は「元の呼び出しの設定を引き継ぐ」。その「元の呼び出し」は
  `list_folder` ではなく**カーソルを作った呼び出し**。初回スキャンの `list_folder`
  （`include_media_info=true`）が作るカーソルは捨てられ、生き残るのは
  `get_latest_cursor` が返したカーソルだけで、そこに指定が無かった。
- 帰結: **最初の同期のあとに増えた・触られたクラウド写真すべて**で `media_info` が来ない。
  撮影日が `client_modified`（≒アップロード時刻）に落ち、撮影地も付かない。
  初回スキャンで正しく付いた撮影日も、Dropbox 側で移動・改名すると差分で上書きされて失われる
  （緯度経度は nil チェックで守られているのに、撮影日は無条件に書かれる——同じブロック内の非対称）。
- これまでの手当てとの関係: ADR-128（台帳の撮影日で上書き）も ADR-199（共有は解析データで渡す）も
  **症状側**だった。どちらも台帳や解析データのある写真しか救えないので、
  他端末のカメラアップロードや Dropbox 側での追加には届いていなかった。
- 見つかり方: コメントが**正しいことを書いていた**——「continue 側は元の list_folder の
  設定を引き継ぐため指定不要」。その通りなのだが、`continue` に渡るカーソルが
  その `list_folder` のものではなかった。**前提が 1 つずれているだけの正しい文**は、
  読んでも気づけない。

### 教訓
**同じ症状に 2 回手当てをしたら、3 回目は上流を疑う。**
ADR-128 と ADR-199 は独立した現象に見えたが、どちらも「クラウド写真の撮影日が
アップロード時刻になる」だった。2 回目の手当てを入れる前に
「この症状が出る写真の集合は何か」を書き出していれば、
台帳にも解析データにも載らない写真が残ることに気づけた。

### 同じ周に直したもの
- **モデル更新の最中に意味検索が 1 ページで止まる。** 件数の上限が絞り込みの前に効くので、
  旧モデルの行が混ざるページは短くなる。呼び出し側は「短い＝表の終わり」で打ち切る。
  移行は新しい写真から進むので、1 ページ目でほぼ必ず短くなる。
- **増分評価が旧空間のベクトルを混ぜていた。** ページ走査の側は現行だけを返すのに、
  増分の入口には絞り込みが無かった（ADR-186）。

### 直さず記録したもの
AI アルバムの 6 件は**アルバムの中身が変わる**ので、ハーネスの計測で決める規則に従って
記録のみ（CLAUDE.md）。DropboxPhotoStore の 3 件（反映が古いまま固定される・
キャッシュを消しても予算が詰まる・先読みの取り消しが効かない）も設計判断が要る。

- 関連: `Packages/DropboxCore/Sources/DropboxCore/Sync/DropboxSyncEngine.swift` /
  `Packages/AutoAlbumCore/Sources/AutoAlbumCore/Store/AutoAlbumStore.swift`。
...
（欠陥密度が最も高く、結合 44＝最も多くのファイルから参照される）。6 件。
...
- **テストの置き場所の番号が折り返しで衝突する寸前だった。** 上限 32 に対して実際の配布数が
  ちょうど 32。テストを 1 本足した瞬間に 33 本目が `-0` を受け取り、**まだ使っている
  1 本目の中身を消す**。折り返しをやめた（1 回の実行では同じ名前を二度配らない）。
...
- 症状: レビュー 2 周目で発見（実機未確認）。ADR-199 の追補で入れた「受信能力の版が上がったら
  ...
  1 セットで 100MB 規模になり得る——`PhotoEmbedding` を inline に持っていた頃の
...
- 原因: **受信側には撮影日を知る手段が 1 つも無かった。**
  ...
  - 共有コピーは `copy_batch_v2` の**サーバーサイドコピー**で作るので EXIF 由来の `time_taken` が
    付かないことが多く、日付は「提供者が反映した時刻」に落ちる。
...
- 経緯: ADR-196/197/198（52 ファイル・+3,340/−1,702 行）は**一度もレビューを受けていなかった**。
...
- 拾えたもの（重い順・抜粋）:
  ...
  5. **ADR-163 のゲートが死んでいた**: `faceBacklog` を `NightlyPlan` の計算時に測っていたが、
