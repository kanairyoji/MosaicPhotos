...
enum HeavyWorkScheduler {
    ...
    /// ⚠️ ただし**そのモデルを使う処理が走っている間は手放さない**
    /// （`isCLIPBusy` / `isFaceModelBusy`）——走っている解析から取り上げると、その場で
    /// 10〜35 秒の再ロードが始まり、ゲートの中なのでほかの推論も止まる。
    /// ⚠️ 埋め込み・顔スキャン・生成が条件に入ったのは **ADR-228**（それ以前は窓とブースト
    /// だけで、前面のトリクルが埋め込み中に背面へ落ちるとモデルを取り上げていた）。
    /// ⚠️ **モデルごとに**見るようになったのもレビューの指摘（それ以前は「どちらかが
    /// 走っていれば両方残す」で、顔スキャン中に背面へ落ちると CLIP の塔まで残っていた）。
    @MainActor
    ...
    /// という前提が崩れない長さにしてある。
    ///
    /// ⚠️ 条件の**出どころ**は背面側と同じ（`isCLIPBusy` / `isFaceModelBusy`）。
    /// 解放点ごとに別々の述語を書き起こすと、どれが効いたのか実機ログから
    /// 切り分けられなくなる（ADR-196 の「11 述語」と同じ轍）。
    /// **前面も背面も同じ 2 つを、同じ粒度（モデルごと）で渡す。**
    @MainActor
