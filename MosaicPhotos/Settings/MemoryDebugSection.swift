...
struct MemoryDebugSection: View {
    ...
    private var deviceSection: some View {
        ...
        return Section {
            ...
            LabeledContent("画面状態", value: BackgroundYield.scenePhase.rawValue)
            LabeledContent("免除の段", value: "\(BackgroundYield.exemption)")
            LabeledContent("ローカル解析", value: BackgroundYield.verdict(for: .localTrickle).reason)
            LabeledContent("クラウド解析", value: BackgroundYield.verdict(for: .cloudTrickle).reason)
            LabeledContent("一枚岩（本番化）", value: BackgroundYield.verdict(for: .localMonolith).reason)
            LabeledContent("一枚岩（生成）", value: BackgroundYield.verdict(for: .cloudMonolith).reason)
